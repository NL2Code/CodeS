    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="dialog"):
            yield MarkdownViewer(
                self.key_bindings, classes="spaced", show_table_of_contents=False
            )
...
class InteractiveShellScreen(Screen):
    def __init__(
        ...
        super().__init__(name, id, classes)
        self.input_tarea = ExtendedTextArea(
            "", id="input", language="python", theme="vscode_dark"
        )
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(
            f"Python: {platform.python_version()}  Django: {django.__version__}"
        )
        yield Footer()
