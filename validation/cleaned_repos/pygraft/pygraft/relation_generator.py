...
class RelationGenerator:
    ...
    def generate_relations(self):
        ...
        self.pattern2rels = {
            tuple(sorted(p)): set() for p in self.one_rel_compatibilities
        }
    ...
    def add_property(self, property):
        ...
        if property == "owl:InverseFunctional":
            ...
            potential_relations = [
                ...
                if r not in self.inversefunctional_relations
            ]
            while (
                len(self.inversefunctional_relations)
        ...
        if property == "owl:Asymmetric":
            ...
            if sample_size > len(relation_pool):
                sample_size = len(relation_pool)
            self.asymmetric_relations = random.sample(
                list(relation_pool), k=sample_size
            )
        ...
        if property == "owl:Transitive":
            ...
            if sample_size > len(relation_pool):
                sample_size = len(relation_pool)
            self.transitive_relations = random.sample(
                list(relation_pool), k=sample_size
            )
        ...
        while running_inverseof_prop < self.prop_inverse_relations:
            ...
            try:
                ...
                compatible_patterns = set(
                    self.compat_inverseof[frozenset(first_pattern)]
                )
                possible_pattern = set(observed_patterns).intersection(
                    compatible_patterns
                )
                second_pattern = random.choice(list(possible_pattern))
                first_rel = random.choice(
                    list(self.pattern2rels[frozenset(first_pattern)])
                )
