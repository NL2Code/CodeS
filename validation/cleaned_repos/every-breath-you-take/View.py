...
class View(QChartView):
    def __init__(self, parent=None):
        ...
        self.chart_acc = self.create_chart(
            title="Breathing Acceleration", showTitle=False, showLegend=False
        )
        ...
        self.series_breath_cycle_marker = self.create_scatter_series(
            self.GRAY, self.DOTSIZE_SMALL
        )
        self.axis_acc_x = self.create_axis(
            title=None,
            tickCount=10,
            rangeMin=-self.BREATH_ACC_TIME_RANGE,
            rangeMax=0,
            labelSize=10,
            flip=False,
        )
        # self.axis_y_pacer = self.create_axis(title="Pacer", color=self.GOLD, rangeMin=-1, rangeMax=1)
        self.axis_y_breath_acc = self.create_axis(
            "Chest expansion (m/s2)", self.BLUE, rangeMin=-1, rangeMax=1, labelSize=10
        )

        # Heart rate chart
        self.chart_hr = self.create_chart(
            title="Heart rate", showTitle=False, showLegend=False
        )
        self.series_hr = self.create_scatter_series(self.RED, self.DOTSIZE_SMALL)
        self.axis_hr_y = self.create_axis(
            title="HR (bpm)", color=self.RED, rangeMin=50, rangeMax=80, labelSize=10
        )

        # Breathing rate
        self.series_br = self.create_spline_series(self.BLUE, self.LINEWIDTH)
        self.series_br_marker = self.create_scatter_series(
            self.BLUE, self.DOTSIZE_SMALL
        )
        self.series_br_marker.setMarkerShape(QScatterSeries.MarkerShapeTriangle)
        self.axis_br_y = self.create_axis(
            title="BR (bpm)", color=self.BLUE, rangeMin=0, rangeMax=20, labelSize=10
        )

        # Heart rate variability chart
        self.chart_hrv = self.create_chart(
            title="Heart rate variability", showTitle=False, showLegend=False
        )
        ...
        self.series_maxmin_marker = self.create_scatter_series(
            self.RED, self.DOTSIZE_SMALL
        )
        self.axis_hrv_x = self.create_axis(
            title=None,
            tickCount=10,
            rangeMin=-self.HRV_SERIES_TIME_RANGE,
            rangeMax=0,
            labelSize=10,
        )
        self.axis_hrv_y = self.create_axis(
            title="HRV (ms)", color=self.RED, rangeMin=0, rangeMax=250, labelSize=10
        )
        ...
        self.circles_widget = CirclesWidget(
            *self.model.pacer.update(self.pacer_rate), self.GOLD, self.BLUE, self.RED
        )
        ...
        circlesLayout.addWidget(
            self.circles_widget, alignment=Qt.AlignHCenter | Qt.AlignVCenter
        )
        ...
        self.pacer_times_hist_rel_s = np.full(
            self.PACER_HIST_SIZE, np.nan
        )  # relative seconds
    ...
    def update_acc_series(self):
        ...
        self.breath_acc_times_rel_s = (
            self.model.breath_acc_times - time.time_ns() / 1.0e9
        )
        series_breath_acc_new = []

        for i, value in enumerate(self.breath_acc_times_rel_s):
            if not np.isnan(value):
                series_breath_acc_new.append(
                    QPointF(value, self.model.breath_acc_hist[i])
                )
