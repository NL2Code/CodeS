...
class CircularBuffer2D:
    ...
    def enqueue(self, new_row):
        if len(new_row) != self.cols:
            raise ValueError(
                "New row must have the same number of columns as the buffer"
            )
    ...
    def dequeue(self):
        ...
        self.dequeued_row = np.array(
            self.buffer[self.tail]
        )  # Returns nan without np.array()
    ...
    def is_full(self):
        return (self.head == self.tail) and not np.isnan(
            np.array(self.buffer[self.tail])
        ).all()
...
class PolarH10:
    ...
    U1_CHAR2_UUID = (
        "6217ff4d-91bb-91d0-7e2a-7cd3bda8a1f3"  # write-without-response, indicate
    )
    ...
    PMD_CHAR2_UUID = (
        "fb005c82-02e7-f387-1cad-8acd2d8df0c8"  # notify – Start the notify stream?
    )

    # POLAR ELECTRO Oy SERIVCE
    ELECTRO_SERVICE_UUID = "0000feee-0000-1000-8000-00805f9b34fb"
    ELECTRO_CHAR1_UUID = (
        "fb005c51-02e7-f387-1cad-8acd2d8df0c8"  # write-without-response, write, notify
    )
    ELECTRO_CHAR2_UUID = "fb005c52-02e7-f387-1cad-8acd2d8df0c8"  # notify
    ELECTRO_CHAR3_UUID = (
        "fb005c53-02e7-f387-1cad-8acd2d8df0c8"  # write-without-response, write
    )
    ...
    def acc_data_conv(self, sender, data):
        ...
        if data[0] == 0x02:
            ...
            timestamp = (
                PolarH10.convert_to_unsigned_long(data, 1, 8) / 1.0e9
            ...
            record_duration = (
                n_samples - 1
            ) * time_step  # duration of the current record received in seconds
            ...
            sample_timestamp = (
                timestamp - record_duration + self.polar_to_epoch_s
            )  # timestamp of the first sample in the record in epoch seconds
            offset = 0
    ...
    def ecg_data_conv(self, sender, data):
        ...
        if data[0] == 0x00:
            ...
            sample_timestamp = (
                timestamp - recordDuration + self.polar_to_epoch_s
            )  # timestamp of the first sample in the record in epoch seconds
            while offset < len(samples):
    ...
    def convert_array_to_signed_int(data, offset, length):
        return int.from_bytes(
            bytearray(data[offset : offset + length]),
            byteorder="little",
            signed=True,
        )
