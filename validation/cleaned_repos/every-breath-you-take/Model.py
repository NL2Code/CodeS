...
class Model:
    def __init__(self):
        ...
        self.ACC_UPDATE_LOOP_PERIOD = (
            0.01  # s, time to sleep between accelerometer updates
        )
        self.IBI_UPDATE_LOOP_PERIOD = 0.01  # s, time to sleep between IBI updates
        self.ACC_HIST_SAMPLE_RATE = (
            20  # Hz, rate to subsample acceleration (raw data @ 200 Hz)
        )
        ...
        self.acc_principle_axis = np.array(
            [0, 0, 1]
        )  # Positive z-axis is the direction out of sensor unit (away from chest)
        ...
        self.hrv_values_hist = np.full(
            self.HRV_HIST_SIZE, np.nan
        )  # HR range based on local IBI extrema
        ...
        self.maxmin_values_hist = np.full(
            self.BR_HIST_SIZE, np.nan
        )  # Max-min HR in a breathing cycle
    ...
    def update_hrv_spectrum(self):
        ...
        ids = self.ibi_times_hist_rel_s > (
            self.ibi_times_hist_rel_s[-1] - 30
        )  # Hardcode 30 seconds
        ...
        self.ibi_values_interp_hist = np.interp(
            self.ibi_times_interp_hist, times, values
        )
        ...
        peak_indices = np.where(
            ...
            & (hrv_psd_freqs_interp <= pacer_freq + 0.015)
        )  # 0.03 Hz around the peak is recommended by R. McCraty
        peak_power = np.trapz(
        ...
        while True:
            ...
            while not self.polar_sensor.ibi_queue_is_empty():
                (
                    ...
                    self.polar_sensor.dequeue_ibi()
                )  # t is when value was added to the queue
                ...
                )  # seconds relative to the last value
    ...
    def update_breathing_rate(self):
        ...
        if np.isnan(self.br_times_hist[-1]):
            ...
            ibi_indices_in_cycle = self.ibi_times_hist_rel_s > (
                self.br_times_hist[-2] - time.time_ns() / 1.0e9
            )
    ...
    def update_breathing_spectrum(self):
        ...
        ids = self.breath_acc_times > (
            self.breath_acc_times[-1] - 30
        )  # Hardcode 30 seconds
    ...
    def get_breath_circle_coords(self):
        if ~np.isnan(self.breath_acc_hist[-1]):
            self.breathing_circle_radius = (
                0.7 * self.breath_acc_hist[-1]
                + (1 - 0.7) * self.breathing_circle_radius
            )
        else:
            self.breathing_circle_radius = -0.5
