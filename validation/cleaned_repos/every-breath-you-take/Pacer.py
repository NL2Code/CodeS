...
class Pacer(QObject):
    ...
    def breathing_pattern(self, breathing_rate, time):
        ...
        """
        if (
            breathing_rate != self.last_breathing_rate
        ):  # Maintaining continuity by adjusting phase when breathing rate is changes
            self.phase = (
                time - self.last_breathing_rate * (time - self.phase) / breathing_rate
            )
            self.last_breathing_rate = breathing_rate

        radius = 0.5 + 0.5 * np.sin(
            2 * np.pi * breathing_rate / 60 * (time - self.phase)
        )
        return radius
