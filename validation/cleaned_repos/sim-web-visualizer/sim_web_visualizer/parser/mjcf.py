        ...
        for geom_id, geom in enumerate(geoms):  # type: int, mjcf.Element
            ...
            if geom_type == "capsule":
                ...
                if from_to is not None:
                    ...
                    length = (
                        size[1] * 2
                    )  # MuJoCo mjcf uses half-length but three js uses full-length
                    center = np.array([0, 0, 0])

                geometry = g.Capsule(radius=radius, length=length)
                capsule_transformation = compute_vector_rotation(
                    np.array([0, 1.0, 0]), vector.astype(float)
                )
                ...
                if from_to is None:
                    ...
                    pose[:3, :3] = pose[:3, :3] @ transforms3d.euler.euler2mat(
                        np.pi / 2, 0, 0
                    )
                else:
                    vector = from_to[3:6] - from_to[0:3]
                    ...
                    cylinder_transformation = compute_vector_rotation(
                        np.array([0, 1.0, 0]), vector.astype(float)
                    )
                ...
                mesh = geom.mesh
                scale = (
                    mesh.scale if mesh.scale is not None else np.array([1.0, 1.0, 1.0])
                )
                ...
                if mesh.file.extension == ".stl":
                    stl_dict = trimesh.exchange.stl.load_stl_binary(
                        trimesh.util.wrap_as_stream(mesh.file.contents)
                    )
            ...
            if mat is None:
                ...
                mat = g.MeshLambertMaterial(
                    color=rgb_to_hex(rgba[:3]), opacity=rgba[3] / 255.0
                )

            if isinstance(geometry, List):
                sub_geom_id = 0
                for each_geometry, each_mat, each_pose in zip(geometry, mat, pose):
                    offline_data_dict[f"{link_name}/{geom_id}/{sub_geom_id}"] = (
                        each_geometry,
                        each_mat,
                    )
                    offline_pose_dict[
                        f"{link_name}/{geom_id}/{sub_geom_id}"
                    ] = each_pose
                    geom_id += 1
            else:
...
def load_mjcf_into_viewer_kinpy(
    ...
    visual_maps = {
        k: v for k, v in original_visual_maps.items() if not k.endswith("_child")
    }
    child_maps = {
        k[:-6]: v for k, v in original_visual_maps.items() if k.endswith("child")
    }
    ...
    for link, geoms in visual_maps.items():
        ...
        for geom_id, geom in enumerate(geoms):
            ...
            if dry_run:
                offline_data_dict[f"{link}/{geom_id}"] = (
                    geometry,
                    g.MeshPhongMaterial(),
                )
