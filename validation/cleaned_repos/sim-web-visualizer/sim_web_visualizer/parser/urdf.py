    ...
    for link_name, link_info in robot.link_map.items():
        for visual in link_info.visuals:
            ...
            if visual.material is not None:
                urdf_rgba = (
                    np.clip(visual.material.color.rgba, 0, 1)
            ...
            if geom.mesh is not None:
                ...
                if mesh_filename.lower().endswith(
                    "obj"
                ) or mesh_filename.lower().endswith("dae"):
                    trimesh_geom = trimesh.load(mesh_filename, ignore_broken=True)

                    assert mesh_filename.lower().endswith("obj") or isinstance(
                        trimesh_geom, trimesh.Scene
                    )
                    ...
                    if isinstance(trimesh_geom, trimesh.Scene):
                        for name, scene_geometry in trimesh_geom.geometry.items():
                            ...
                            mats.append(
                                get_trimesh_geometry_material(scene_geometry, urdf_rgba)
                            )
                        ...
                        geometries = [
                            g.ObjMeshGeometry.from_stream(
                                trimesh.util.wrap_as_stream(exp_mesh)
                            )
                        ]
                    ...
                    scene = trimesh.load(
                        mesh_filename, ignore_broken=True, force="scene"
                    )
                    geom_name_map = {
                        scene.graph[node_name][1]: node_name
                    ...
                    for name, scene_geometry in scene.geometry.items():
                        ...
                        poses.append(
                            visual_pose @ scene.graph.get(geom_name_map[name])[0]
                        )
                else:
            ...
            # Primitive geometry
                ...
                if geom.sphere is not None:
                    ...
                    geometries = [
                        g.Cylinder(geom.cylinder.length, geom.cylinder.radius)
                    ]
                    visual_pose[:3, :3] = visual_pose[
                        :3, :3
                    ] @ transforms3d.euler.euler2mat(np.pi / 2, 0, 0)
                ...
                if urdf_rgba is not None:
                    ...
                    mats.append(
                        g.MeshPhongMaterial(color=rgb_to_hex(rgb), opacity=urdf_rgba[3])
                    )
                else:
                    mats.append(None)
            ...
            for each_geometry, each_mat, each_pose in zip(geometries, mats, poses):
                offline_data_dict[f"{meshcat_link_name}/{geom_id}"] = (
                    each_geometry,
                    each_mat,
                )
