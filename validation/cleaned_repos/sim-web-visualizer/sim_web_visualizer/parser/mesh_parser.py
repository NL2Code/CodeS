    ...
    if mesh is None:
        try:
            ...
            mesh: trimesh.Trimesh = trimesh.load(
                mesh_file, force="mesh", ignore_broken=True
            )
...
def get_trimesh_geometry_material(
    ...
    if isinstance(geom.visual, trimesh.visual.texture.TextureVisuals):
        ...
        if isinstance(material, trimesh.visual.material.SimpleMaterial):
            ...
            mat = g.MeshPhongMaterial(
                shininess=material.glossiness,
                specular=rgb_to_hex(material.specular[:3]),
                side=0,
            )
            ...
            mat = g.MeshStandardMaterial(
                metalness=material.metallicFactor, roughness=material.roughnessFactor
            )
            if not material.doubleSided:
                mat.side = 0
            if material.baseColorTexture is not None:
                output = io.BytesIO()
                material.baseColorTexture.save(
                    output, format=material.baseColorTexture.format
                )
                hex_data = output.getvalue()
