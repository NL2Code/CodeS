    ...
    if visual.type == "Box":
        ...
        if isinstance(trimesh_geom, trimesh.Scene):
            for name, scene_geometry in trimesh_geom.geometry.items():
                ...
                mats.append(
                    get_trimesh_geometry_material(
                        scene_geometry, default_rgba=default_rgba
                    )
                )
            ...
            geoms = [
                g.ObjMeshGeometry.from_stream(trimesh.util.wrap_as_stream(exp_mesh))
            ]
...
class MeshCatVisualizerSapien(MeshCatVisualizerBase):
    ...
    def _override_urdf_loader(self, mimic: MimicEntity):
        ...
        def load_from_string(
            urdf_string: str, srdf_string: str, config=None
        ) -> sapien.Articulation:
            if config is None:
                config = {}
        ...
        mimic.add_method(
            "load_file_as_articulation_builder", load_file_as_articulation_builder
        )
    ...
    def _override_actor_builder(self, mimic: MimicEntity):
        def add_actor_visual(act: sapien.ActorBase):
            ...
            for i, visual in enumerate(mimic.get_visuals()):
                geom_num = add_visual_to_viz(
                    self.viz, visual, actor_tree_path, geom_num
                )
        ...
        def add_robot_visual(art: sapien.ArticulationBase):
            ...
            for builder in mimic.entity.get_link_builders():
                ...
                for i, visual in enumerate(builder.get_visuals()):
                    geom_num = add_visual_to_viz(
                        self.viz, visual, link_tree_path, geom_num
                    )
    ...
    def _scene_override_builder_create_fn(self):
        ...
        self.new_scene.add_method(
            "create_articulation_builder", create_articulation_builder
        )
    ...
    def _scene_override_misc_fn(self):
        def add_ground(
            ...
            ground = self.original_scene.add_ground(
                altitude, render, material, render_material, render_half_size
            )
            if render:
                ...
                color = np.clip(render_material.base_color[:3] * 255, 0, 255).astype(
                    np.uint8
                )
    ...
    def _scene_override_visualization_fn(self):
        def update_render():
            ...
            for actor in self.original_scene.get_all_actors():
                ...
                self.viz[actor_tree_path].set_transform(
                    actor.get_pose().to_transformation_matrix().astype(float)
                )
            ...
            for art in self.original_scene.get_all_articulations():
                ...
                for link in art.get_links():
                    ...
                    self.viz[link_tree_path].set_transform(
                        link.get_pose().to_transformation_matrix().astype(float)
                    )
