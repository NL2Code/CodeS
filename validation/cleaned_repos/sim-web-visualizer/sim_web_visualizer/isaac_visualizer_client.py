...
class MeshCatVisualizerIsaac(MeshCatVisualizerBase):
    ...
    def _override_asset_create_fn(self):
        ...
        def create_sphere(
            sim: gymapi.Sim, radius: float, options: gymapi.AssetOptions
        ) -> gymapi.Asset:
    ...
    def _overload_create_env(self):
        def create_env(
            arg0: gymapi.Sim, arg1: gymapi.Vec3, arg2: gymapi.Vec3, arg3: int
        ) -> gymapi.Env:
            ...
            if num_env_now <= self.max_env:
                ...
                if self.env_handle_request > 0:
                    ...
                    set_env_pose(
                        num_env, self.num_per_row, self.env_size, np.zeros(2), self.viz
                    )
    ...
    def _override_create_actor(self):
        def create_actor(
            ...
            actor = self.original_gym.create_actor(
                env, asset, pose, name, collision_group, collision_filter, seg_id
            )
            ...
            if env in self.env_map:
                ...
                if id(asset) in self.asset_resource_map:
                    ...
                    rigid_body_name = self.original_gym.get_actor_rigid_body_names(
                        env, actor
                    )[0]
    ...
    def _override_add_fn(self):
        ...
        def add_triangle_mesh(
            ...
            self.viz[f"/Sim/triangle_mesh/{self.triangle_mesh_count}"].set_transform(
                pose
            )
    ...
    def _override_set_fn(self):
        def set_rigid_body_color(
            arg0: gymapi.Env,
            arg1: int,
            arg2: int,
            arg3: gymapi.MeshType,
            arg4: gymapi.Vec3,
        ):
            self.original_gym.set_rigid_body_color(arg0, arg1, arg2, arg3, arg4)
            if arg0 in self.env_map:
                body_name = self.original_gym.get_actor_rigid_body_names(arg0, arg1)[
                    arg2
                ]
    ...
    def _override_viewer_fn(self):
        def create_viewer(
            sim: gymapi.Sim, cam_prop: gymapi.CameraProperties
        ) -> Union[MimicViewer, gymapi.Viewer]:
        ...
        def subscribe_viewer_keyboard_event(
            viewer: Union[MimicViewer, gymapi.Viewer],
            keyboard_input: gymapi.KeyboardInput,
            event_name: str,
        ):
            if self.keep_default_viewer:
                self.original_gym.subscribe_viewer_keyboard_event(
                    viewer, keyboard_input, event_name
                )
            self.viewer_keyboard_event[keyboard_input.name] = event_name

        def subscribe_viewer_mouse_event(
            viewer: Union[MimicViewer, gymapi.Viewer],
            mouse_input: gymapi.MouseInput,
            event_name: str,
        ):
            if self.keep_default_viewer:
                self.original_gym.subscribe_viewer_mouse_event(
                    viewer, mouse_input, event_name
                )
        ...
        def query_viewer_action_events(
            viewer: Union[MimicViewer, gymapi.Viewer]
        ) -> List[gymapi.ActionEvent]:
            if self.keep_default_viewer:
        ...
        def draw_viewer(
            ...
            if self.env_handle_request > 0:
                num_env = (
                    min(self.original_gym.get_env_count(self.sim), self.max_env)
                    - self.env_handle_request
                )
                set_env_pose(
                    num_env, self.num_per_row, self.env_size, np.zeros(2), self.viz
                )
            ...
            if USE_GPU_PIPELINE and not self.prepared:
                ...
                rigid_body_tensor = self.original_gym.acquire_rigid_body_state_tensor(
                    self.sim
                )
                self.original_gym.refresh_rigid_body_state_tensor(self.sim)
                self.rigid_body_state_tensor = gymtorch.wrap_tensor(
                    rigid_body_tensor
                ).view(num_envs, -1, 13)
            ...
            if USE_GPU_PIPELINE:
                self.original_gym.refresh_rigid_body_state_tensor(self.sim)
                rigid_body_state = (
                    self.rigid_body_state_tensor[: len(self.env_map), :, :7]
                    .cpu()
                    .numpy()
                )
                pos = rigid_body_state[:, :, :3]
                quat = np.concatenate(
                    [rigid_body_state[..., 6:7], rigid_body_state[..., 3:6]], axis=-1
                )  # xyzw -> wxyz
                ...
                for i in range(len(self.env_map)):
                    ...
                    for k in range(self.env_actor_count[i]):
                        ...
                        for rigid_body_idx in range(
                            body_count, body_count + rigid_body_count
                        ):
                            actor_viz[
                                rigid_body_name[rigid_body_idx - body_count]
                            ].set_transform(pose[i, rigid_body_idx])
                ...
                for i in range(len(self.env_map)):
                    ...
                    for k in range(self.env_actor_count[i]):
                        actor_viz = env_viz[f"actor:{k}"]
                        actor_states = self.original_gym.get_actor_rigid_body_states(
                            env_ptr, k, gymapi.STATE_POS
                        )
                        rigid_body_name = self.actor_rigid_body_name_map[k]
                        rigid_body_count = len(rigid_body_name)
                        ...
                        for rigid_body_idx in range(rigid_body_count):
                            actor_viz[rigid_body_name[rigid_body_idx]].set_transform(
                                pose[rigid_body_idx]
                            )

        self.new_gym.add_method("create_viewer", create_viewer)
        self.new_gym.add_method(
            "subscribe_viewer_keyboard_event", subscribe_viewer_keyboard_event
        )
        self.new_gym.add_method(
            "subscribe_viewer_mouse_event", subscribe_viewer_mouse_event
        )
        ...
        self.new_gym.add_method(
            "query_viewer_action_events", query_viewer_action_events
        )
        ...
        self.new_gym.add_method(
            "poll_viewer_events",
            partial(draw_viewer, sim=self.sim, render_collision=False),
        )
...
def create_isaac_visualizer(
    ...
    visualizer = MeshCatVisualizerIsaac(
        port, host, keep_default_viewer, max_env=max_env, **kwargs
    )
...
def bind_visualizer_to_gym(gym: gymapi.Gym, sim: gymapi.Sim):
    if len(_REGISTERED_VISUALIZER) <= 0:
        raise RuntimeError(
            f"Web Visualizer has not been created yet! Call create_visualizer before register it to env"
        )
