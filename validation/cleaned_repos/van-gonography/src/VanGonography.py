...
SINGLE_RGB_BIT_SIZE = (
    8  # Each RGB value is composed of 3 colors, each color is composed of 8 bits
)
SINGLE_RGB_PIXEL_BIT_SIZE = (
    SINGLE_RGB_BIT_SIZE * 3
)  # Each pixel is composed of 3 RGB values, each RGB value is composed of 3 colors, each color is composed of 8 bits
...
def add_header(
    ...
    max_bits_available = (
        (height - 1) * 3 * 8
    )  # Height - 1 pixels, 3 channels (RGB), 8 bits per channel
    ...
    pixels_needed = np.ceil(extension_length / pixel_bits_used).astype(
        int
    )  # Number of pixels needed to store the extension

    for i in range(pixels_needed):
        # Get the RGB values of the current pixel
        r, g, b = cover_array[
            i + 1, 0
        ]  # We start at 1 because we already used the first pixel
        ...
        r = (
            r & 0b11111000
        ) | extension_int  # 0b11111000 is used to clear the last 3 bits of the red channel
...
def encode_image(
    ...
    data = "".join(
        [f"{byte:08b}" for byte in data]
    )  # Convert the data back to binary (zlib compresses the data and converts it to bytes)
    ...
    if encrypt:
        ...
        try:
            ...
            data = "".join(
                [f"{byte:08b}" for byte in data]
            )  # Convert the data back to binary (Fernet encrypts the data and converts it to bytes)
        except Exception as e:
...
def decode_image(
    ...
    binary_string = binary_string[
        :data_length
    ]  # Truncate the binary string to the length of the data
    ...
    if compressed:
        ...
        binary_string = "".join(
            [f"{byte:08b}" for byte in binary_string]
        )  # Convert the data back to binary (zlib compresses the data and converts it to bytes)
...
def main():
    ...
    if args.json:
        with open(args.json, "r") as json_file:
            ...
            json_data.pop(
                "desc"
            )  # Removing the description argument for not causing errors when setting the attributes
    ...
    if args.cli:
        ...
        if (
            args.cover
        ):  # Checking if a cover image is given (essential for both decoding and encoding)
            ...
            if args.encode:  # Encode
                ...
                try:
                    ...
                    logging.info(
                        f"Encoding {args.file} in {args.cover}"
                    )  # Logging the file and cover image
                    ...
                    logging.info(
                        f"File hidden successfully in {args.cover}."
                    )  # Logging the success message, this is also useful for checking the time it took to hide the file
                except Exception as e:
            ...
            # Checking for decode
                ...
                try:
                    ...
                    logging.info(
                        f"File revealed successfully from {args.cover}."
                    )  # Same as above
                except Exception as e:
                    print(f"An error occurred: {e}")
                    logging.error(f"An error occurred: {e}")
            ...
            # Checking for differentiating images
                ...
                try:
                    ...
                    logging.info(
                        f"Differentiating {args.cover} and {args.file}"
                    )  # Logging the source and cover images
