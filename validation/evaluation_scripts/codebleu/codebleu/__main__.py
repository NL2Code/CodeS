...
def main(
    ...
    if repo_bleu:
        repo_bleu_score = calc_repobleu(
            [Path(x[0]) for x in references],
            [Path(x) for x in hypothesis],
            lang,
            weights=weights,
        )
        print("Repo-level CodeBLEU score: ", repo_bleu_score["repo_codebleu"])
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repo", action="store_true", help="whether to use repo-level codebleu")
