...
class LocalBootstrapSource(BaseBootstrapSource):
        ...
        for entry in raw_data:
            if "type" not in entry:
                raise BootstrapSourceError(
                    self.ERROR_PREFIX + "No type defined for entry"
                )
            ...
            if entry_type == "host":
                ...
                raise BootstrapSourceError(
                    self.ERROR_PREFIX + "Invalid type: '%s'" % entry_type
                )

    def _parse_host_entry(self, host_data):
        if "name" not in host_data:
            raise BootstrapSourceError(
                self.ERROR_PREFIX + "Missing key 'name' on host item"
            )
        if "cdn" not in host_data:
            raise BootstrapSourceError(
                self.ERROR_PREFIX + "Missing key 'cdn' on host item"
            )
        ...
        if any([s in main_domain for s in ("*", "?")]):
            raise BootstrapSourceError(
                self.ERROR_PREFIX + "Wildcards only allowed in subdomains"
            )
    ...
    def _parse_cdn_entry(self, cdn_data):
        if "id" not in cdn_data:
            raise BootstrapSourceError(
                self.ERROR_PREFIX + "Missing key 'id' on cdn item"
            )
...
class RemoteBootstrapSource(BaseBootstrapSource):
    ...
    def lookup_host(self, hostname):
        ...
        host_data = {"hostname": hostname, "cdn": data["cdn"], "ssl": data["ssl"]}
    ...
    def lookup_cdn(self, cdn_id):
        ...
        return {"id": cdn_id, "name": data["name"], "edge": data["edges"][0]}
    ...
    def _request(self, path):
        ...
        try:
            ...
            logger.warning(
                "Connection with remote bootstrap '{}' failed".format(str(self))
            )
            return None
        ...
        if response.status_code == 404:
            ...
            raise BootstrapSourceError(
                "Remote bootstrapper request failed: %d" % response.status_code
            )
...
class Bootstrapper(object):
    ...
    def lookup_host(self, hostname):
        for source in self.sources:
            logger.debug(
                "Looking up host bootstrap info for '{}' from {} source '{}'".format(
                    hostname,
    ...
    def lookup_cdn(self, cdn_id):
        for source in self.sources:
            logger.debug(
                "Looking up cdn bootstrap info for '{}' from {} source '{}'".format(
                    cdn_id,
                    "local" if isinstance(source, LocalBootstrapSource) else "remote",
                    str(source),
                )
    ...
    def _validate_host_name(cls, hostname):
        if any(s in hostname for s in ["http", "/"]):
            raise BootstrapValidationError(
                "invalid hostname to be bootstrapped: '%s'" % hostname
            )
        return hostname
    ...
    def _validate_host_data(cls, hostname, host_data):
        for key in ["hostname", "cdn"]:
            if key not in host_data:
                raise BootstrapValidationError(
                    "invalid host data received, missing '%s' key" % key
                )
    ...
    def _validate_cdn_data(cls, cdn_id, cdn_data):
        for key in ["id", "edge_server"]:
            if key not in cdn_data:
                raise BootstrapValidationError(
                    "invalid cdn data received, missing '%s' key" % key
                )

        if cdn_id != cdn_data["id"]:
            raise BootstrapValidationError(
                "host data mismatch. Expecting data for %s but got data for %s"
                % (cdn_id, cdn_data["id"])
            )
        return cdn_data
