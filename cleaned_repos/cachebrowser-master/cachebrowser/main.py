...
from cachebrowser.proxy import ProxyConfig, ProxyController
from cachebrowser.settings import (
    DevelopmentSettings,
    ProductionSettings,
...
@click.option(
    "-c", "--config", type=click.File("r"), help="Path to configuration file."
)
...
@click.option(
    "--sni",
    type=click.Choice(["empty", "front", "original"]),
    help="The default SNI policy to use.",
)
...
def start_cachebrowser_server(context):
    ...
    try:
        logger.info(
            "Listening for proxy connections on port {}".format(context.settings.port)
        )
        return m.run()
    ...
    for data_file in data_files:
        ...
        if not os.path.isfile(data_path):
            ...
            with open(data_path, "w") as f:
                f.write(
                    pkgutil.get_data("cachebrowser", os.path.join("data", data_file))
                )
