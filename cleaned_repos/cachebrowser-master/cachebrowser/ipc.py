...
class IPCRouter(object):
    ...
    def rpc_request(self, client_id, request_id, method, params):
        if method not in self.rpc_clients:
            logger.error(
                "RPC request received for '{}', but route does not exist".format(method)
            )
...
class WebSocketIPCClient(tornado.websocket.WebSocketHandler, IPCClient):
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            logger.error(
                "Uncaught exception occurred while handling IPC message: {}".format(
                    message
                )
            )
            traceback.print_exc()
    ...
    def send_publish(self, channel, message):
        self.send({"type": "pub", "channel": channel, "message": message})
    ...
    def send_rpc_response(self, request_id, response):
        self.send({"type": "rpc_resp", "request_id": request_id, "message": response})
