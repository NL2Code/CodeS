...
class ScramblerPipe(FlowPipe):
    ...
    def start(self):
        ...
        self.adblocker.load_blacklist(
            self.context.settings.data_path("scrambler/ad-domains"),
            self.context.settings.data_path("scrambler/blacklist"),
        )
        self.decoymaker.load_decoys(
            self.context.settings.data_path("scrambler/decoy.json")
        )
    ...
    def print_stats(self):
        ...
        print(
            "Sent: {}  Received: {}  Overhead: {}  Traffic: {}   Overhead: {}  Traffic: {} ".format(
                self.decoysent,
                self.decoyreceived,
                pretty_bytes(self.downstream_overhead),
                pretty_bytes(self.downstream_traffic),
                pretty_bytes(self.upstream_overhead),
                pretty_bytes(self.upstream_traffic),
            )
        )
        ...
        if is_decoy:
            ...
            if self.BLOCK_ADS and self.adblocker.should_block(flow):
                ...
                for i in range(6):
                    ...
                    if (
                        actual + self.decoymaker.inflight
                        < wanted + wanted * self.overhead
                    ):
    ...
    def handle_ads(self, flow):
        ...
        if (
            self.adblocker.should_block(flow)
            and self.drop_ads
            and should_i(self.PROB_AD_BLOCK)
        ):
            self.dummy_response(flow)
