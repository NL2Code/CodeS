...
UNCONVERTED_KEYS = [("AWS::StepFunctions::StateMachine", "DefinitionString")]
            ...
            if key == "Type":
                for item in UNCONVERTED_KEYS:
                    if value == item[0]:
                        ...
                        if source.get("Properties") and source.get(
                            "Properties", {}
                        ).get(item[1]):
                            if isinstance(
                                source["Properties"][item[1]], dict
                            ) and not has_intrinsic_functions(
                                source["Properties"][item[1]].keys()
                            ):
                                source["Properties"][item[1]] = LiteralString(
                                    "{}".format(
                                        json.dumps(
