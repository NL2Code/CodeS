...
_CONFIG_DEFAULTS: Dict[str, _ConfigArg] = {
    "max_col_width": _ConfigArg(
        dtype=int, nullable=False, has_default=True, default=200
    )
}
...
class _Config:
    ...
    def _load_config(self, name, conf):
        ...
        if env_var is not None:
            value = self._apply_type(
                name=name,
                value=env_var,
                dtype=_CONFIG_DEFAULTS[name].dtype,
                nullable=_CONFIG_DEFAULTS[name].nullable,
            )
    ...
    def __setattr__(self, key: str, value: Any) -> Any:
        if key not in _CONFIG_DEFAULTS:
            raise TypeError(
                ...
                f"Please use: {list(_CONFIG_DEFAULTS.keys())}"
            )
        value = self._apply_type(
            name=key,
            value=value,
            dtype=_CONFIG_DEFAULTS[key].dtype,
            nullable=_CONFIG_DEFAULTS[key].nullable,
        )
    ...
    def _apply_type(name: str, value: Any, dtype: Type, nullable: bool) -> Any:
        if _Config._is_null(value=value) is True:
            if nullable is True:
                return None
            ValueError(
                f"{name} configuration does not accept null value."
                f" Please pass {dtype}."
            )
    ...
    def _is_null(value: Any) -> bool:
        if value is None:
            return True
        if (isinstance(value, str) is True) and (
            value.lower() in ("none", "null", "nil")
        ):
            return True
        return False
...
def apply_configs(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        available_configs = [
            x
            for x in valid_configs
            if (x not in received_args) and (hasattr(config, x) is True)
        ]
