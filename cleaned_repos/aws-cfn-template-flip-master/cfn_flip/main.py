...
@click.option(
    ...
    help="Specify the input format. Overrides -j and -y flags.",
)
@click.option(
    "--output",
    "-o",
    "out_format",
    type=click.Choice(["json", "yaml"]),
    help="Specify the output format. Overrides -j, -y, and -n flags.",
)
@click.option(
    "--json",
    "-j",
...
@click.option(
    ...
    "-n",
    is_flag=True,
    help="Perform other operations but do not flip the output format.",
)
