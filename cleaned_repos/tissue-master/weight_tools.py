...
class formula_prop(PropertyGroup):
    ...
    float_var: FloatVectorProperty(
        name="", description="", default=(0, 0, 0, 0, 0), size=5
    )
...
class reaction_diffusion_prop(PropertyGroup):
    run: BoolProperty(
        default=False,
        update=reaction_diffusion_add_handler,
    ...
    time_steps: IntProperty(
        name="Steps", default=10, min=0, soft_max=50, description="Number of Steps"
    )

    dt: FloatProperty(
        name="dt", default=0.5, min=0, soft_max=1, description="Time Step"
    )
    ...
    vertex_group_diff_a: StringProperty(
        name="Diff A", default="", description="Vertex Group used for A diffusion"
    )

    vertex_group_diff_b: StringProperty(
        name="Diff B", default="", description="Vertex Group used for B diffusion"
    )

    vertex_group_scale: StringProperty(
        name="Scale", default="", description="Vertex Group used for Scale value"
    )

    vertex_group_f: StringProperty(
        name="f", default="", description="Vertex Group used for Feed value (f)"
    )

    vertex_group_k: StringProperty(
        name="k", default="", description="Vertex Group used for Kill value (k)"
    )

    vertex_group_brush: StringProperty(
        name="Brush", default="", description="Vertex Group used for adding/removing B"
    )

    invert_vertex_group_diff_a: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group Diff A"
    )

    invert_vertex_group_diff_b: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group Diff B"
    )

    invert_vertex_group_scale: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group Scale"
    )

    invert_vertex_group_f: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group f"
    )

    invert_vertex_group_k: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group k"
    )
    ...
    cache_frame_start: IntProperty(
        name="Start", default=1, description="Frame on which the simulation starts"
    )

    cache_frame_end: IntProperty(
        name="End", default=250, description="Frame on which the simulation ends"
    )
    ...
    vector_field_mode: EnumProperty(
        items=(
            ...
            (
                "XYZ",
                "x, y, z",
                "Vector field defined by vertex groups 'x', 'y' and 'z'",
            ),
        ),
        default="NONE",
    ...
    perp_vector_field: BoolProperty(
        default=False,
...
def compute_formula(
    ob=None, formula="rx", float_var=(0, 0, 0, 0, 0), int_var=(0, 0, 0, 0, 0)
):
...
class weight_formula_wiki(Operator):
    ...
    def execute(self, context):
        bpy.ops.wm.url_open(
            url="https://github.com/alessandro-zomparelli/tissue/wiki/Weight-Tools#weight-formula"
        )
        return {"FINISHED"}
...
class weight_formula(Operator):
    ...
    _ex_items = list(
        (str(i), "{}   ( {} )".format(s[0], s[1]), s[1]) for i, s in enumerate(ex_items)
    )
    _ex_items.append(("CUSTOM", "User Formula", ""))

    examples: EnumProperty(items=_ex_items, default="CUSTOM", name="Examples")
    ...
    slider_f01: FloatProperty(name="f1", default=1, description="Slider Float 1")
    slider_f02: FloatProperty(name="f2", default=1, description="Slider Float 2")
    slider_f03: FloatProperty(name="f3", default=1, description="Slider Float 3")
    slider_f04: FloatProperty(name="f4", default=1, description="Slider Float 4")
    slider_f05: FloatProperty(name="f5", default=1, description="Slider Float 5")
    slider_i01: IntProperty(name="i1", default=1, description="Slider Integer 1")
    slider_i02: IntProperty(name="i2", default=1, description="Slider Integer 2")
    slider_i03: IntProperty(name="i3", default=1, description="Slider Integer 3")
    slider_i04: IntProperty(name="i4", default=1, description="Slider Integer 4")
    slider_i05: IntProperty(name="i5", default=1, description="Slider Integer 5")
    ...
    def draw(self, context):
        ...
        layout.separator()
        if "f1" in formula:
            layout.prop(self, "slider_f01")
        if "f2" in formula:
            layout.prop(self, "slider_f02")
        if "f3" in formula:
            layout.prop(self, "slider_f03")
        if "f4" in formula:
            layout.prop(self, "slider_f04")
        if "f5" in formula:
            layout.prop(self, "slider_f05")
        if "i1" in formula:
            layout.prop(self, "slider_i01")
        if "i2" in formula:
            layout.prop(self, "slider_i02")
        if "i3" in formula:
            layout.prop(self, "slider_i03")
        if "i4" in formula:
            layout.prop(self, "slider_i04")
        if "i5" in formula:
            layout.prop(self, "slider_i05")
        ...
        layout.label(
            text="rx, ry, rz: Local Coordinates (0 to 1)", icon="NORMALIZE_FCURVES"
        )
        ...
        layout.separator()
        layout.label(
            text="f1, f2, f3, f4, f5: Float Sliders", icon="MOD_HUE_SATURATION"
        )  # PROPERTIES
        layout.label(
            text="i1, i2, i3, i4, i5: Integer Sliders", icon="MOD_HUE_SATURATION"
        )
        layout.separator()
    ...
    def execute(self, context):
        ...
        weight = compute_formula(
            ob, formula=formula, float_var=f_sliders, int_var=i_sliders
        )
        if type(weight) == str:
...
class update_weight_formula(Operator):
    ...
    def execute(self, context):
        ...
        if type(weight) == str:
            self.report(
                {"ERROR"}, "The name of the active Vertex Group\nis not a valid Formula"
            )
            return {"CANCELLED"}
...
class _weight_laplacian(Operator):
    ...
    mode: EnumProperty(
        items=(("LENGTH", "Length Weight", ""), ("SIMPLE", "Simple", "")),
        default="SIMPLE",
    ...
    def execute(self, context):
        ...
        stdev_lap = stdev(lap)
        filter_lap = [
            i for i in lap if mean_lap - 2 * stdev_lap < i < mean_lap + 2 * stdev_lap
        ]
        if self.bounds == "MANUAL":
...
class reaction_diffusion(Operator):
    ...
    diff_a: FloatProperty(
        name="Diff A", default=1, min=0, soft_max=2, description="Diffusion A"
    )

    diff_b: FloatProperty(
        name="Diff B", default=0.5, min=0, soft_max=2, description="Diffusion B"
    )
...
class edges_deformation(Operator):
    ...
    mode: EnumProperty(
        items=(("MAX", "Max Deformation", ""), ("MEAN", "Average Deformation", "")),
        default="MEAN",
        name="Evaluation Mode",
    )
    ...
    def execute(self, context):
        ...
        if self.mode == "MEAN":
            group_name = "Average Deformation"
        elif self.mode == "MAX":
            group_name = "Max Deformation"
        ob.vertex_groups.new(name=group_name)
        me0 = ob.data
        ...
        if len(me.vertices) != len(me0.vertices) or len(me.edges) != len(me0.edges):
            self.report({"ERROR"}, "The topology of the object should be" + "unaltered")
            return {"CANCELLED"}
        ...
        for v in bm.verts:
            ...
            if self.mode == "MEAN":
                ...
                v_deformations.append(max(vdef, key=abs))
...
class weight_contour_displace(Operator):
    ...
    use_modifiers: BoolProperty(
        name="Use Modifiers", default=True, description="Apply all the modifiers"
    )
    min_iso: FloatProperty(
        name="Min Iso Value", default=0.49, min=0, max=1, description="Threshold value"
    )
    max_iso: FloatProperty(
        name="Max Iso Value", default=0.51, min=0, max=1, description="Threshold value"
    )
    n_cuts: IntProperty(
        name="Cuts",
        default=2,
    ...
    def execute(self, context):
        ...
        for i_cut in range(self.n_cuts):
            ...
            if delta_iso == 0:
                ...
                iso_val = i_cut / (self.n_cuts - 1) * delta_iso + min_iso
            else:
                iso_val = (self.max_iso + self.min_iso) / 2
...
class weight_contour_mask(Operator):
    ...
    offset: FloatProperty(name="Offset", default=1, min=0, max=1, description="Offset")
    thickness: FloatProperty(
        name="Thickness", default=0.5, soft_min=0, soft_max=1, description="Thickness"
    )
...
class weight_contour_mask_wip(Operator):
    ...
    def execute(self, context):
        ...
        for g in ob0.vertex_groups:
            ob.vertex_groups.new(name=g.name)
        weight = get_weight_numpy(
            ob.vertex_groups[vertex_group_name], len(me0.vertices)
        )
...
class vertex_colors_to_vertex_groups(Operator):
    ...
    green: BoolProperty(
        name="green channel", default=False, description="convert green channel"
    )
    blue: BoolProperty(
            ...
            if ob.data.color_attributes.active_color.domain == "POINT":
                for v in ob.data.vertices:
                    ...
                    if self.value:
                        ...
                        gr[
                            min(len(gr) - sub_value, id_value)
                        ].weight = self.invert + mult * (
...
class vertex_group_to_vertex_colors(Operator):
    ...
    def execute(self, context):
        ...
        if self.channel == "FALSE_COLORS":
            ...
            mask = np.where(
                np.logical_and(splitted_weight >= 0.25, splitted_weight < 0.5)
            )
            g[mask] = np.ones(splitted_weight.shape)[mask]
            b[mask] = 1 - (splitted_weight[mask] - 0.25) * 4

            mask = np.where(
                np.logical_and(splitted_weight >= 0.5, splitted_weight < 0.75)
            )
...
class vertex_group_to_uv(Operator):
    ...
    vertex_group_u: StringProperty(
        name="U", default="", description="Vertex Group used for the U coordinate"
    )
    vertex_group_v: StringProperty(
        name="V", default="", description="Vertex Group used for the V coordinate"
    )
    normalize_weight: BoolProperty(
        name="Normalize Weight", default=True, description="Normalize weight values"
    )
    invert_u: BoolProperty(name="Invert U", default=False, description="Invert U")
    invert_v: BoolProperty(name="Invert V", default=False, description="Invert V")
...
class curvature_to_vertex_groups(Operator):
    ...
    invert: BoolProperty(
        name="Invert", default=False, description="Invert the curvature map"
    )
...
class face_area_to_vertex_groups(Operator):
    ...
    bl_label = "Area"
    bl_options = {"REGISTER", "UNDO"}
    bl_description = "Generate a Vertex Group based on the area of individual" "faces."

    invert: BoolProperty(name="invert", default=False, description="invert values")
    bounds: EnumProperty(
        items=(("MANUAL", "Manual Bounds", ""), ("AUTOMATIC", "Automatic Bounds", "")),
        default="AUTOMATIC",
        name="Bounds",
    )
...
class random_weight(Operator):
    ...
    min_val: FloatProperty(
        name="Min", default=0, soft_min=0, soft_max=1, description="Minimum Value"
    )

    max_val: FloatProperty(
        name="Max", default=1, soft_min=0, soft_max=1, description="Maximum Value"
    )
...
class harmonic_weight(Operator):
    ...
    midlevel: FloatProperty(
        name="Midlevel", default=0, min=-1, max=1, description="Midlevel"
    )

    add: FloatProperty(
        name="Add", default=0, min=-1, max=1, description="Add to the Weight"
    )

    mult: FloatProperty(
        name="Multiply", default=0, min=0, max=1, description="Multiply for he Weight"
    )
    ...
    def execute(self, context):
        ...
        if len(ob.vertex_groups) > 0:
            ...
            for i in range(len(ob.data.vertices)):
                try:
                    val = ob.vertex_groups[group_id].weight(i)
                except:
                    val = 0
                weight = (
                    self.amp * (math.sin(val * self.freq) - self.midlevel) / 2
                    + 0.5
                    + self.add * val * (1 - (1 - val) * self.mult)
                )
                ob.vertex_groups[-1].add([i], weight, "REPLACE")
            ob.data.update()
        else:
...
class tissue_weight_distance(Operator):
    ...
    min_value: FloatProperty(
        name="Min", default=0, min=0, soft_max=100, description="Minimum Distance"
    )

    max_value: FloatProperty(
        name="Max", default=10, min=0, soft_max=100, description="Max Distance"
    )
    ...
    def fill_neighbors(self, verts, weight):
        ...
        if len(neigh) == 0:
            ...
            return self.fill_neighbors(neigh.keys(), weight)
...
class TISSUE_PT_weight(Panel):
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        try:
            row.operator("object.weight_formula", icon="CON_TRANSFORM")
        except:
            row.operator("object.weight_formula")  # , icon="CON_TRANSFORM")
        row.operator(
            "object.update_weight_formula", icon="FILE_REFRESH", text=""
        )  # , icon="CON_TRANSFORM")
        ...
        col.operator(
            "object.edges_deformation", icon="DRIVER_DISTANCE"
        )  # FULLSCREEN_ENTER")
        col.operator(
            "object.edges_bending", icon="DRIVER_ROTATIONAL_DIFFERENCE"
        )  # "MOD_SIMPLEDEFORM")
        col.separator()
        ...
        op = col.operator(
            "object.tissue_weight_contour_curves_pattern", icon="FORCE_TURBULENCE"
        )
        ...
        col.operator("object.vertex_group_to_uv", icon="UV", text="Convert to UV")
...
class start_reaction_diffusion(Operator):
    ...
    run: BoolProperty(
        ...
        default=True,
        description="Compute a new iteration on frame changes",
    )
    ...
    diff_a: FloatProperty(
        name="Diff A", default=0.18, min=0, soft_max=2, description="Diffusion A"
    )

    diff_b: FloatProperty(
        name="Diff B", default=0.09, min=0, soft_max=2, description="Diffusion B"
    )
...
class bake_reaction_diffusion(Operator):
    ...
    def execute(self, context):
        ...
        if props.fast_bake:
            ...
            fast_bake_def(
                ob, frame_start=props.cache_frame_start, frame_end=props.cache_frame_end
            )
...
def reaction_diffusion_def(ob, bake=False):
    ...
    if not props.bool_cache:
        ...
        # store weight values
        if "dB" in ob.vertex_groups:
            db = np.zeros(n_verts)
        if "grad" in ob.vertex_groups:
            grad = np.zeros(n_verts)
        ...
        if props.vertex_group_diff_a != "":
            ...
            diff_a = bmesh_get_weight_numpy(
                group_index, dvert_lay, bm.verts, normalized=True
            )
                ...
                if props.vector_field_mode == "GRADIENT":
                    ...
                    if props.vertex_group_gradient:
                        if props.vertex_group_gradient in ob.vertex_groups.keys():
                            ...
                            group_index = ob.vertex_groups[
                                props.vertex_group_gradient
                            ].index
                            weight = bmesh_get_weight_numpy(
                                group_index, dvert_lay, bm.verts
                            )
        ...
        try:
            ...
            if len(field_mult) == 1:
                ...
                a, b = numba_reaction_diffusion_anisotropic(
                    n_verts,
                    n_edges,
                    edge_verts,
                    a,
            ...
            if len(field_mult) == 1:
                ...
                mult = field_mult[arr]  # second vertex indices for each edge
            for i in range(time_steps):
                ...
                lap_a0 = (
                    a[id1] - a[id0]
                ) * mult  # laplacian increment for first vertex of each edge
                lap_b0 = (
                    b[id1] - b[id0]
                ) * mult  # laplacian increment for first vertex of each edge
    ...
    if props.update_uv:
        ...
        if not splitted:
            loops_size = get_attribute_numpy(
                me.polygons, attribute="loop_total", mult=1
            )
            n_data = np.sum(loops_size)
            v_id = np.ones(n_data)
            me.polygons.foreach_get("vertices", v_id)
...
def create_fast_bake_def(ob, frame_start=1, frame_end=250):
    ...
    if not props.bool_cache:
        ...
        field = np.concatenate(
            (vec_x[:, None], vec_y[:, None], vec_y[:, None] * 0), axis=1
        )
        field = field * 2 - 1
        field[:, 2] = 0
        ...
        for j in range(props.cache_frame_start, props.cache_frame_end + 1):
            ...
            print("{:6d} Reaction-Diffusion: {}".format(j, ob.name))
            if bool_run:
                values = integrate_field(
                    n_edges, id0, id1, values, edge_flow, mult, time_steps
                )
            else:
                bool_run = True
...
class tissue_weight_streamlines(Operator):
    ...
    rand_seed: IntProperty(
        name="Seed", default=0, min=0, soft_max=10, description="Random Seed"
    )
    ...
    bevel_depth: FloatProperty(
        name="Bevel Depth", default=0, min=0, soft_max=1, description=""
    )
    min_bevel_depth: FloatProperty(
        name="Min Bevel Depth", default=0.1, min=0, soft_max=1, description=""
    )
    max_bevel_depth: FloatProperty(
        name="Max Bevel Depth", default=1, min=0, soft_max=1, description=""
    )
    ...
    vertex_group_bevel: StringProperty(
        name="Bevel", default="", description="Variable Bevel depth"
    )

    object_name: StringProperty(name="Active Object", default="", description="")
    ...
    vertex_group_streamlines: StringProperty(
        name="Flow", default=vg_name, description="Vertex Group used for streamlines"
    )
    ...
    def execute(self, context):
        ...
        try:
            weight = get_weight_numpy(
                ob.vertex_groups[self.vertex_group_streamlines], n_verts
            )
        except:
        ...
        try:
            if self.min_bevel_depth == self.max_bevel_depth:
                ...
                bevel_weight = get_weight_numpy(
                    ob.vertex_groups[self.vertex_group_bevel], n_verts
                )
                if self.min_bevel_depth > self.max_bevel_depth:
        ...
        for i in seeds:
            ...
            for j in range(self.pos_steps):
                ...
                if next > 0:
                    if next not in next_pts:
                        next_pts.append(next)
                else:
                    break
        ...
        crv = nurbs_from_vertices(
            curves, co, bevel_weight, ob.name + "_Streamlines", True, self.interpolation
        )
