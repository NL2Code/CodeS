...
class random_materials(Operator):
    ...
    prefix: StringProperty(name="Prefix", default="Random.", description="Name prefix")

    color_A: FloatVectorProperty(
        name="Color A", subtype="COLOR_GAMMA", min=0, max=1, default=[0, 0, 0]
    )

    color_B: FloatVectorProperty(
        name="Color B", subtype="COLOR_GAMMA", min=0, max=1, default=[1, 1, 1]
    )

    hue: FloatProperty(name="Hue", min=0, max=1, default=0.5)
    hue_variation: FloatProperty(name="Hue Variation", min=0, max=1, default=0.6)

    seed: IntProperty(name="Seed", default=0, description="Random seed")
    ...
    generate_materials: BoolProperty(
        ...
        default=False,
        description="Automatically generates new materials",
    )

    random_colors: BoolProperty(
        name="Random Colors",
        default=True,
        description="Colors are automatically generated",
    )
            ...
            for i in range(count):
                ...
                if self.random_colors:
                    mat.diffuse_color = colorsys.hsv_to_rgb(
                        (h1 + (h2 - h1) / (count) * i) % 1, 1, 1
                    )[:] + (1,)
                else:
                    mat.diffuse_color = list(colA + (colB - colA) / (count - 1) * i) + [
                        1
                    ]
...
class weight_to_materials(Operator):
    ...
    prefix: StringProperty(name="Prefix", default="Weight.", description="Name prefix")
