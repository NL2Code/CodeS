...
try:
    from .numba_functions import run_tex_rd, run_tex_rd_ani
except:
    pass
...
class tex_reaction_diffusion_prop(PropertyGroup):
    run: BoolProperty(
        default=False,
        update=tex_reaction_diffusion_add_handler,
    ...
    dt: FloatProperty(
        name="dt", default=1, min=0, soft_max=0.2, description="Time Step"
    )
    ...
    img_a: StringProperty(
        name="A", default="", description="Image used for the chemical A"
    )

    img_b: StringProperty(
        name="B", default="", description="Image used for the chemical B"
    )

    img_diff_a: StringProperty(
        name="Diff A", default="", description="Image used for A diffusion"
    )

    img_diff_b: StringProperty(
        name="Diff B", default="", description="Image used for B diffusion"
    )

    img_scale: StringProperty(
        name="Scale", default="", description="Image used for Scale value"
    )

    img_f: StringProperty(
        name="f", default="", description="Image used for Feed value (f)"
    )

    img_k: StringProperty(
        name="k", default="", description="Image used for Kill value (k)"
    )

    img_brush: StringProperty(
        name="Brush", default="", description="Image used for adding/removing B"
    )

    invert_img_diff_a: BoolProperty(
        default=False, description="Invert the value of the Vertex Group Diff A"
    )

    invert_img_diff_b: BoolProperty(
        default=False, description="Invert the value of the Vertex Group Diff B"
    )

    invert_img_scale: BoolProperty(
        default=False, description="Invert the value of the Vertex Group Scale"
    )

    invert_img_f: BoolProperty(
        default=False, description="Invert the value of the Vertex Group f"
    )

    invert_img_k: BoolProperty(
        default=False, description="Invert the value of the Vertex Group k"
    )

    invert_img_vector_field: BoolProperty(
        default=False, description="Use the perpendicular direction"
    )
...
def tex_reaction_diffusion_def(ob, bake=False):
    ...
    if img_diff_a:
        diff_a = np_remap_image_values(
            img_diff_a,
            channel=0,
            min=diff_a_min,
            max=diff_a_max,
            invert=props.invert_img_diff_a,
        )
    else:
        diff_a = np.ones((nx, ny)) * props.diff_a

    if img_diff_b:
        diff_b = np_remap_image_values(
            img_diff_b,
            channel=0,
            min=diff_b_min,
            max=diff_b_max,
            invert=props.invert_img_diff_b,
        )
    else:
        diff_b = np.ones((nx, ny)) * props.diff_b

    if img_scale:
        scale = np_remap_image_values(
            img_scale,
            channel=0,
            min=min_scale,
            max=max_scale,
            invert=props.invert_img_scale,
        )
        diff_a *= scale
        diff_b *= scale
    else:
    ...
    if img_f:
        f = np_remap_image_values(
            img_f, channel=0, min=f_min, max=f_max, invert=props.invert_img_f
        )
    else:
        f = np.ones((nx, ny)) * props.f

    if img_k:
        k = np_remap_image_values(
            img_k, channel=0, min=k_min, max=k_max, invert=props.invert_img_k
        )
    else:
    ...
    start_time = timeit.default_timer()
    run_tex_rd_ani(
        a, b, lap_a, lap_b, diff_a, diff_b, f, k, dt, time_steps, vf1, vf2, brush
    )
...
class reset_tex_reaction_diffusion(Operator):
    ...
    diff_a: FloatProperty(
        name="Diff A", default=0.14, min=0, soft_max=2, description="Diffusion A"
    )

    diff_b: FloatProperty(
        name="Diff B", default=0.07, min=0, soft_max=2, description="Diffusion B"
    )
...
class TISSUE_PT_tex_reaction_diffusion(Panel):
    ...
    def draw(self, context):
        ...
        if not (img_a and img_b in bpy.data.images):
            row.operator("object.start_tex_reaction_diffusion", icon="EXPERIMENTAL")
            col = layout.column(align=True)
            row = col.row(align=True)
            ...
            row.operator("object.reset_tex_reaction_diffusion", icon="EXPERIMENTAL")
            row = col.row(align=True)
...
def insert_image_parameter(col, ob, name, text=""):
    ...
    if name not in ("brush"):
        if name == "vector_field":
            icon = "DRIVER_ROTATIONAL_DIFFERENCE"  #'ORIENTATION_VIEW'
        else:
