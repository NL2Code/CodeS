...
def tissue_time(start_time, name, levels=0):
    ...
    if levels < ps:
        ...
        if start_time:
            print(
                "{}{}{} in {:.4f} sec".format(
                    head, "|   " * levels, name, end_time - start_time
                )
            )
        else:
...
def _np_broadcast(arrays):
    ...
    for i in range(len(shapes[0])):
        ...
        for j in range(len(arrays)):
            leng = ish[j]
            if leng == 1:
                arrays[j] = np.repeat(arrays[j], max_len, axis=i)
    for arr in arrays:
        arr = arr.flatten()
    # vt = v0 + (v1 - v0) * t
    return arrays
...
def calc_thickness(co2, n2, vz, a, weight):
    if "numba" in sys.modules and use_numba_tess():
        if len(co2.shape) == 3:
            ...
            for i in range(n_sk):
                co3[i] = numba_calc_thickness_area_weight(
                    co2[:, i],
                    n2[:, i],
                    vz[:, i],
                    a[:, min(i, na)],
...
def convert_object_to_mesh(
    ob, apply_modifiers=True, preserve_status=True, mirror_correction=True
):
                ...
                if m.type == "MIRROR" and vis:
                    ...
                    if n_axis > 0:
                        ...
                        if n_axis == 1:
                            ...
                            fraction_val = [0, 1, 1, 0]
                        elif n_axis == 3:
                            fraction_val = [0, 1, 1, 0, 1, 0, 0, 1]
...
def _join_objects(context, objects, link_to_scene=True, make_active=True):
    ...
    bm.faces.ensure_lookup_table()
    # assign new indexes
    for index, f in zip(faces_materials, bm.faces):
...
def get_mesh_before_subs(ob):
    ...
    for m in hide_mods:
        m.show_viewport = False
    me = simple_to_mesh_mirror(ob)
    for m, vis in zip(hide_mods, mods_visibility):
        m.show_viewport = vis
    return me, subs
...
def get_patches____(
    me_low, me_high, sides, subs, bool_selection, bool_material_id, material_id
):
    ...
    if sides == 4:
        ...
        if subs != 0:
            ...
            inners = np.array(
                [[j * (n - 2) + i for j in range(n - 2)] for i in range(n - 2)]
            )
        ...
        for sk in ob1.data.shape_keys.key_blocks:
            ...
            for i, sk_v in enumerate(source):
                if mode == "BOUNDS":
                    ...
                    if scale_mode == "CONSTANT" or normals_mode in (
                        "OBJECT",
                        "SHAPEKEYS",
                    ):
                        if not use_origin_offset:
                            sk_vert[2] = (
                                sk_vert[2] / bb[2] if bb[2] != 0 else sk_vert[2]
                            )
        ...
        for co, norm, bound in zip(planes_co, planes_no, bounds):
            ...
            while True:
                ...
                bisect = bmesh.ops.bisect_plane(
                    ...
                    clear_outer=bound == "CLIP",
                    clear_inner=False,
                )
                geom = bisect["geom"]
                cut_edges = [
                    g for g in bisect["geom_cut"] if type(g) == bmesh.types.BMEdge
                ]
                cut_verts = [
                    g for g in bisect["geom_cut"] if type(g) == bmesh.types.BMVert
                ]
                ...
                if bound == "CYCLIC":
                    ...
                    if len(geom_verts) > 0:
                        ...
                        geom = bmesh.ops.split(
                            bm, geom=geom["geom"], use_only_faces=False
                        )
                        vec = Vector(norm)
                        move_verts = [
                            g for g in geom["geom"] if type(g) == bmesh.types.BMVert
                        ]
...
def get_patches(
    me_low, me_high, sides, subs, bool_selection
    ...
    if sides == 4:
        ...
        if subs != 0:
            ...
            ids = pick_verts[dir.astype("int8")][
                :, None, :
            ]  # indexes order along the side
            patches[patch_index, ids, 0] = edge_verts[:, None, :]  # assign indexes
...
def get_attribute_numpy(elements_list, attribute="select", mult=1, size=None):
    ...
    if mult > 1:
        values = values.reshape((n_elements, mult))
    return values
...
def curve_from_points(points, name="Curve"):
    ...
    for c in points:
        ...
        for i, p in enumerate(c):
            s.points[i].co = p.xyz + [1]
    ob_curve = bpy.data.objects.new(name, curve)
    return ob_curve


def curve_from_pydata(
    points,
    radii,
    indexes,
    name="Curve",
    ...
    for c in indexes:
        ...
        if skip_open and not bool_cyclic:
            continue
...
def update_curve_from_pydata_simple(
    curve,
    points,
    radii,
    indexes,
...
def update_curve_from_pydata(
    ...
    for ic, c in enumerate(indexes):
        ...
        patt1 = (
            series
            + (series - series % pattern[1]) / pattern[1] * pattern[0]
            + pattern[0]
        )
...
def nurbs_from_vertices(
    indexes, co, radii=[], name="Curve", set_active=True, interpolation="POLY"
):
    curve = bpy.data.curves.new(name, "CURVE")
    curve.dimensions = "3D"
...
def get_weight(vertex_group, n_verts):
    ...
    for i in range(n_verts):
        try:
            weight[i] = vertex_group.weight(i)
        except:
            pass
    return weight
...
def mesh_diffusion(me, values, iter, diff=0.2, uv_dir=0):
    ...
    for ii in range(iter):
        ...
        if uv_dir != 0:
            lap0 = (
                values[id1] - values[id0]
            ) * uv_mult  # laplacian increment for first vertex of each edge
        else:
...
def fill_neighbors_attribute(verts, weight, attribute):
    ...
    if len(neigh) == 0:
        ...
        return fill_neighbors_attribute(neigh.keys(), weight, attribute)
