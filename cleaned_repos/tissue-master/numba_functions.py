...
try:
    ...
    from numba import float64, guvectorize, int32, jit, njit, prange
...
if bool_numba:
    ...
    def tex_laplacian(lap, arr):
        ...
        for i in prange(nx):
            for j in prange(ny):
                ...
                lap[i, j] = (
                    (arr[i0, j] + arr[i1, j] - arr2[i, j])
                    + (arr[i, j0] + arr[i, j1] - arr2[i, j])
                    + (arr[i0, j0] + arr[i1, j1] - arr2[i, j]) * diag
                    + (arr[i1, j0] + arr[i0, j1] - arr2[i, j]) * diag
                ) * 0.75
    ...
    def tex_laplacian_ani(lap, arr, VF):
        ...
        for i in prange(nx):
            for j in prange(ny):
                lap[i, j] = (
                    (arr[i0[i], j] + arr[i1[i], j] - arr2[i, j]) * VF[0, i, j]
    ...
    @njit(parallel=True)
    def run_tex_rd_ani(
        A, B, lap_A, lap_B, diff_A, diff_B, f, k, dt, steps, vf1, vf2, brush
    ):
        for t in range(steps):
            ...
            for i in prange(nx):
                for j in prange(ny):
                    ...
                    ab2 = A[i, j] * B[i, j] ** 2
                    A[i, j] += (
                        lap_A[i, j] * diff_A[i, j] - ab2 + f[i, j] * (1 - A[i, j])
                    ) * dt
                    B[i, j] += (
                        lap_B[i, j] * diff_B[i, j] + ab2 - (k[i, j] + f[i, j]) * B[i, j]
                    ) * dt

    @njit(parallel=True)
    def numba_reaction_diffusion(
        n_verts, n_edges, edge_verts, a, b, brush, diff_a, diff_b, f, k, dt, time_steps
    ):
    ...
    @njit(parallel=True)
    def numba_reaction_diffusion_anisotropic(
        n_verts,
        n_edges,
        edge_verts,
        a,
        ...
        for i in prange(n_patches):
            for j in prange(n_verts):
                for k in prange(n_co):
                    co3[i, j, k] = (
                        co2[i, j, k]
                        + n2[i, min(j, nn), k] * vz[0, j, 0] * weight[i, min(j, nw), 0]
                    )
        return co3
            ...
            for sk in prange(n_sk):
                for j in prange(n_verts):
                    for k in prange(n_co):
                        co0 = (
                            v00[i, sk, j, k]
                            + (v10[i, sk, j, k] - v00[i, sk, j, k]) * vx[0, sk, j, 0]
                        )
                        co1 = (
                            v01[i, sk, j, k]
                            + (v11[i, sk, j, k] - v01[i, sk, j, k]) * vx[0, sk, j, 0]
                        )
