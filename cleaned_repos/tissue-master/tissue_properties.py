...
def anim_tessellate(scene, depsgraph=None):
    ...
    try:
        ...
        active_object = old_mode = selected_objects = None

    if old_mode in ("OBJECT", "PAINT_WEIGHT"):
        update_objects = []
        for ob in scene.objects:
            if ob.tissue.bool_run and not ob.tissue.bool_lock:
                if ob not in update_objects:
                    update_objects.append(ob)
...
def remove_tessellate_handler():
    ...
    for h in tissue_handlers:
        blender_handlers.remove(h)
...
class tissue_tessellate_prop(PropertyGroup):
    ...
    warning_message: StringProperty(name="Warning Message", default="")
    warning_message_thickness: StringProperty(
        name="Warning Message Thickness", default=""
    )
    warning_message_merge: StringProperty(name="Warning Message Merge", default="")
...
def store_parameters(operator, ob):
    ...
    if operator.component_coll in bpy.data.collections.keys():
        ob.tissue_tessellate.component_coll = bpy.data.collections[
            operator.component_coll
        ]
    ...
    ob.tissue_tessellate.invert_vertex_group_distribution = (
        operator.invert_vertex_group_distribution
    )
    ob.tissue_tessellate.vertex_group_distribution_factor = (
        operator.vertex_group_distribution_factor
    )
    ...
    ob.tissue_tessellate.invert_vertex_group_bridge = (
        operator.invert_vertex_group_bridge
    )
    ...
    ob.tissue_tessellate.invert_vertex_group_smooth_normals = (
        operator.invert_vertex_group_smooth_normals
    )
    ...
    ob.tissue_tessellate.vertex_group_scale_normals = (
        operator.vertex_group_scale_normals
    )
    ob.tissue_tessellate.invert_vertex_group_scale_normals = (
        operator.invert_vertex_group_scale_normals
    )
...
def props_to_dict(ob):
    ...
    ] = props.invert_vertex_group_distribution
    tessellate_dict[
        "vertex_group_distribution_factor"
    ] = props.vertex_group_distribution_factor
    ...
    ] = props.invert_vertex_group_smooth_normals
    ...
    ] = props.invert_vertex_group_scale_normals
