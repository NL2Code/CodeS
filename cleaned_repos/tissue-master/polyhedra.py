...
class tissue_polyhedra_prop(PropertyGroup):
    ...
    mode: EnumProperty(
        items=(
            (
                "POLYHEDRA",
                "Polyhedra",
                "Polyhedral Complex Decomposition, the result are disconnected polyhedra geometries",
            ),
    ...
    bool_modifiers: BoolProperty(
        name="Use Modifiers", description="", default=True, update=anim_polyhedra_active
    )
    ...
    selective_wireframe: EnumProperty(
        ...
        items=(
            ...
            (
                "THICKNESS",
                "Thickness",
                "Wireframe only on bigger cells compared to the thickness",
            ),
...
class polyhedral_wireframe(Operator):
    ...
    crease: FloatProperty(
        name="Crease", default=0, min=0, max=1, description="Crease Inner Loops"
    )
    ...
    thicken_all: BoolProperty(
        name="Thicken all", description="Thicken original faces as well", default=True
    )
...
class tissue_update_polyhedra(Operator):
    ...
    def execute(self, context):
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, subdivide edges in {:.4f} sec".format(
                end_time - start_time
            )
        )
        start_time = time.time()
        ...
        if props.vertex_group_thickness in ob.vertex_groups.keys():
            ...
            thickness_weight = bmesh_get_weight_numpy(
                group_index_thickness, dvert_lay, bm.verts
            )
        ...
        if props.selective_wireframe == "THICKNESS":
            ...
            if props.vertex_group_selective in ob.vertex_groups.keys():
                dvert_lay = bm.verts.layers.deform.active
                group_index_selective = ob.vertex_groups[
                    props.vertex_group_selective
                ].index
                thresh = props.vertex_group_selective_threshold
                selective_weight = bmesh_get_weight_numpy(
                    group_index_selective, dvert_lay, bm.verts
                )
                ...
                if invert:
                    ...
                    selective_weight = selective_weight >= thresh
                selective_dict = dict(
                    zip([tuple(v.co) for v in bm.verts], selective_weight)
                )
            else:
                filter_faces = False
        else:
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, found {} polyhedra in {:.4f} sec".format(
                len(polyhedra), end_time - start_time
            )
        )
        start_time = time.time()
        ...
        ) = combine_polyhedra_faces(bm1, polyhedra)
        ...
        if props.mode == "POLYHEDRA":
            ...
            end_time = time.time()
            print(
                "Tissue: Polyhedral wireframe in {:.4f} sec".format(
                    end_time - start_time
                )
            )
            return {"FINISHED"}
        ...
        wireframe_faces_id = [
            i for i in wireframe_faces if i not in not_wireframe_faces
        ]
        wireframe_faces = [all_faces_dict[i] for i in wireframe_faces_id]
        # flat_faces = [all_faces_dict[i] for i in flat_faces]
        delete_faces = [
            all_faces_dict[i]
            for i in delete_faces
            if all_faces_dict[i] not in outer_faces
        ]

        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, merge and delete in {:.4f} sec".format(
                end_time - start_time
            )
        )
        start_time = time.time()
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, frames in {:.4f} sec".format(
                end_time - start_time
            )
        )
        start_time = time.time()
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, corners displace in {:.4f} sec".format(
                end_time - start_time
            )
        )
        start_time = time.time()
        ...
        if props.dissolve != "NONE":
            if props.dissolve == "INNER":
                dissolve_id = 2
            if props.dissolve == "OUTER":
                dissolve_id = 0
            bm1.edges.index_update()
            dissolve_edges = []
            for f in bm1.faces:
            ...
            bmesh.ops.dissolve_edges(
                bm1, edges=dissolve_edges, use_verts=True, use_face_split=False
            )
        ...
        bmesh.ops.dissolve_verts(
            bm1, verts=dissolve_verts, use_face_split=False, use_boundary_tear=False
        )

        # clean meshes
        bm1.to_mesh(me)
        if props.bool_smooth:
            me.shade_smooth()
        me.update()
        old_me = ob.data
        ob.data = me
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe, clean mesh {:.4f} sec".format(
                end_time - start_time
            )
        )
        start_time = time.time()
        ...
        end_time = time.time()
        print(
            "Tissue: Polyhedral wireframe in {:.4f} sec".format(end_time - begin_time)
        )
        return {"FINISHED"}
    ...
    for loops_index, loops in enumerate(all_loops):
        ...
        if loops_keys[0] in frames_verts_dict:
            ...
            for loop in loops:
                tan = (
                    loop.calc_tangent()
                )  # nor.cross(loop.calc_tangent().cross(nor)).normalized()
    ...
    for e in bm.edges:
        ...
        for id1 in range(n_radial_faces - 1):
            ...
            faceskeys2, normals2 = get_second_faces(
                link_faces, vert0.index, vert1.index, ref_loop_dir, f1
            )
            ...
            facekey2_pos = get_closest_face(
                faceskeys2, tangents2, tan1, edge_vec1, True
            )
            polyhedra_from_facekey, count, to_merge = store_neighbor_faces(
                facekey1, facekey2_pos, polyhedra_from_facekey, count, to_merge
            )
            # negative side
            facekey2_neg = get_closest_face(
                faceskeys2, tangents2, tan1, edge_vec1, False
            )
            polyhedra_from_facekey, count, to_merge = store_neighbor_faces(
                -facekey1, facekey2_neg, polyhedra_from_facekey, count, to_merge
            )
...
def store_neighbor_faces(key1, key2, polyhedra, polyhedra_count, to_merge):
...
def add_polyhedron(bm, source_faces):
    ...
    for verts_keys in faces_verts_key:
        new_faces[count] = bm.faces.new(
            [polyhedron_verts_dict.get(key) for key in verts_keys]
        )
        count += 1
...
class TISSUE_PT_polyhedra_object(Panel):
    ...
    def draw(self, context):
        ...
        if not bool_polyhedra:
            layout.label(
                text="The selected object is not a Polyhedral object", icon="INFO"
            )
        else:
            ...
            if props.mode == "WIREFRAME":
                ...
                ob0 = props.object
                row.prop_search(
                    props, "vertex_group_thickness", ob0, "vertex_groups", text=""
                )
                col2 = row.column(align=True)
                ...
                if props.selective_wireframe == "THICKNESS":
                    ...
                    row = col.row(align=True)
                    row.prop_search(
                        props, "vertex_group_selective", ob0, "vertex_groups", text=""
                    )
                    col2 = row.column(align=True)
                    row2 = col2.row(align=True)
                    row2.prop(
                    ...
                    row2.enabled = (
                        props.vertex_group_selective in ob0.vertex_groups.keys()
                    )
