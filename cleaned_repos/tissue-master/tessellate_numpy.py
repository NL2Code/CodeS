...
def tessellate_patch(props):
    ...
    if normals_mode in ("SHAPEKEYS", "OBJECT"):
        if fill_mode == "PATCH":
            ...
            if fill_mode == "FAN":
                ob0_sk = convert_to_fan(target, _props, add_id_layer=id_layer)
            elif fill_mode == "FRAME":
                ob0_sk = convert_to_frame(target, _props)
            elif fill_mode == "TRI":
                ob0_sk = convert_to_triangles(target, _props)
            elif fill_mode == "QUAD":
                ob0_sk = reduce_to_quads(target, _props)
        me0_sk = ob0_sk.data
        ...
        if normals_mode == "SHAPEKEYS":
            ...
            for sk in _ob0.data.shape_keys.key_blocks:
                sk.value = 0
    # Base mesh
    if fill_mode == "PATCH":
        ...
        if fill_mode == "FAN":
            ...
            ob0 = convert_to_frame(_ob0, _props)
        elif fill_mode == "TRI":
            ob0 = convert_to_triangles(_ob0, _props)
        elif fill_mode == "QUAD":
            ob0 = reduce_to_quads(_ob0, _props)
            ...
            if scale_mode == "CONSTANT":
                ...
                if not even_thickness:
                    ...
                    for com in components:
                        if com:
                            com = convert_object_to_mesh(
                                com, com_modifiers, False, False
                            )
                            ...
                            if first_component:
                                ...
                                all_com_verts = np.concatenate(
                                    (all_com_verts, com_verts), axis=0
                                )
                    ...
                    for sgn, stp in zip(sign, step_dist):
                        if stp == 0:
                            if sgn == 1:
                                verts0_normal_pos = verts0_normal
                            if sgn == -1:
                                verts0_normal_neg = verts0_normal
                            continue
                        for i in range(even_thickness_iter):
                            ...
                            projected_vectors = np.multiply(
                                closest_vec, closest_nor
                            ).sum(1)[:, None]
                            closest_dist = np.linalg.norm(projected_vectors, axis=1)[
                                :, None
                            ]
                            mult = (
                                mult * 0.2 + test_dist / closest_dist * 0.8
                            )  # Reduces bouncing effect
                        if sgn == 1:
                            verts0_normal_pos = verts0_normal * mult
                        if sgn == -1:
                            verts0_normal_neg = verts0_normal * mult
    ...
    if fill_mode == "PATCH":
        all_verts, mask, materials = get_patches(
            before_subsurf, me0, 4, levels, bool_selection
        )
    else:
    ...
    bool_weight_cap = (
        vertex_group_cap_owner == "BASE"
        and vertex_group_cap in ob0.vertex_groups.keys()
    )
    bool_weight_bridge = (
        vertex_group_bridge_owner == "BASE"
        and vertex_group_bridge in ob0.vertex_groups.keys()
    )
    bool_weight_normals = vertex_group_scale_normals in ob0.vertex_groups.keys()

    read_vertex_groups = (
        bool_vertex_group
        or rotation_mode == "WEIGHT"
        or bool_weight_thickness
        or bool_weight_cap
    ...
    if component_mode == "COLLECTION":
        ...
        if bool_weight_distribution:
            ...
            v11 = all_verts[:, -1, -1]
            # Average method
            face_weight = np.average(
                weight_distribution[all_verts.reshape((all_verts.shape[0], -1))], axis=1
            ) * len(components)
            # Corners Method
            ...
            coll_materials = (
                face_weight
                + (coll_materials - face_weight) * vertex_group_distribution_factor
            )
    ...
    if smooth_normals:
        ...
        verts0_normal = mesh_diffusion_vector(
            me0,
            verts0_normal,
            smooth_normals_iter,
        ...
        if scale_mode == "CONSTANT":
            ...
            original_normals = get_normals_numpy(me0)
            verts0_normal /= np.multiply(verts0_normal, original_normals).sum(1)[
                :, None
            ]
    ...
    if normals_mode in ("FACES", "VERTS"):
        ...
        if normals_x < 1:
            verts0_normal[:, 0] *= w_normals_x
        if normals_y < 1:
            verts0_normal[:, 1] *= w_normals_y
        if normals_z < 1:
            verts0_normal[:, 2] *= w_normals_z
    ...
    for mat_id, _ob1 in enumerate(components):
        ...
        if com_modifiers or _ob1.type != "MESH":
        ...
        if bool_vertex_group:
            ...
            store_weight = np_lerp2(
                w00, w10, w01, w11, vx[None, :, :, :], vy[None, :, :, :], "weight"
            )
        ...
        if normals_mode == "FACES":
            ...
            if (
                normals_mode in ("SHAPEKEYS", "OBJECT")
                and scale_mode == "CONSTANT"
                and even_thickness
            ):
        ...
        if bool_shapekeys:
            ...
            v00 = verts_xyz[:, np_u, np_v].reshape(
                (n_patches, n_sk, n_verts1, 3)
            )  # .swapaxes(0,1)
            v10 = verts_xyz[:, np_u1, np_v].reshape(
                (n_patches, n_sk, n_verts1, 3)
            )  # .swapaxes(0,1)
            v01 = verts_xyz[:, np_u, np_v1].reshape(
                (n_patches, n_sk, n_verts1, 3)
            )  # .swapaxes(0,1)
            v11 = verts_xyz[:, np_u1, np_v1].reshape(
                (n_patches, n_sk, n_verts1, 3)
            )  # .swapaxes(0,1)
            ...
            if normals_mode == "FACES":
                ...
                if (
                    ...
                    verts_norm_neg = verts0_normal_neg[masked_verts]
                    nor_mask = (vz < 0).reshape((-1))
                    n00 = verts_norm_pos[:, np_u, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n10 = verts_norm_pos[:, np_u1, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n01 = verts_norm_pos[:, np_u, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n11 = verts_norm_pos[:, np_u1, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n00_neg = verts_norm_neg[:, np_u, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n10_neg = verts_norm_neg[:, np_u1, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n01_neg = verts_norm_neg[:, np_u, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n11_neg = verts_norm_neg[:, np_u1, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n00[:, :, nor_mask] = n00_neg[:, :, nor_mask]
                    n10[:, :, nor_mask] = n10_neg[:, :, nor_mask]
                    ...
                    n00 = verts_norm[:, np_u, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n10 = verts_norm[:, np_u1, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n01 = verts_norm[:, np_u, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
                    n11 = verts_norm[:, np_u1, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 3)
                    )
            ...
            if (
                scale_mode == "ADAPTIVE"
            ):  # and normals_mode not in ('OBJECT', 'SHAPEKEYS'): ### not sure
                if normals_mode == "FACES":
                    a2 = mean_area
                else:
                    a00 = verts_area[:, np_u, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 1)
                    )
                    a10 = verts_area[:, np_u1, np_v].reshape(
                        (n_patches, n_sk, n_verts1, 1)
                    )
                    a01 = verts_area[:, np_u, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 1)
                    )
                    a11 = verts_area[:, np_u1, np_v1].reshape(
                        (n_patches, n_sk, n_verts1, 1)
                    )
                    # remapped z scale
        ...
        store_coordinates = (
            np.concatenate(store_coordinates, axis=0).reshape((-1)).tolist()
        )
        ...
        for area in bpy.context.screen.areas:
            for space in area.spaces:
                try:
                    new_patch.local_view_set(space, True)
                except:
                    pass
        ...
        if bool_shapekeys:
            ...
            for i in range(n_sk):
                ...
                new_patch.data.shape_keys.key_blocks[i + 1].data.foreach_set(
                    "co", coordinates
                )
...
class tissue_tessellate(Operator):
    ...
    bool_hold: BoolProperty(name="Hold", description="Wait...", default=False)
    object_name: StringProperty(name="", description="Name of the generated object")
    ...
    mode: EnumProperty(
        items=(
            (
                "BOUNDS",
                "Bounds",
                "The component fits automatically the size of the target face",
            ),
    ...
    fill_mode: EnumProperty(
        items=(
            ...
            (
                "PATCH",
                "Patch",
                "Curved tessellation according to the last "
    ...
    combine_mode: EnumProperty(
        items=(
            ...
            (
                "UNUSED",
                "Unused",
                "Combine each iteration with the unused faces of the previous iteration. Used for branching systems",
            ),
    ...
    merge: BoolProperty(
        name="Merge", default=False, description="Merge vertices in adjacent duplicates"
    )
    merge_open_edges_only: BoolProperty(
        name="Open edges only", default=True, description="Merge only open edges"
    )
    ...
    bool_random: BoolProperty(
        name="Randomize", default=False, description="Randomize component rotation"
    )
    rand_seed: IntProperty(
        name="Seed", default=0, soft_min=0, soft_max=10, description="Random seed"
    )
    coll_rand_seed: IntProperty(
        name="Seed", default=0, soft_min=0, soft_max=10, description="Random seed"
    )
    rand_step: IntProperty(
        name="Step", default=1, min=1, soft_max=2, description="Random step"
    )
    ...
    generator: StringProperty(
        name="", description="Base object for the tessellation", default=""
    )
    component: StringProperty(
        name="", description="Component object for the tessellation", default=""
    )
    component_coll: StringProperty(
        name="", description="Components collection for the tessellation", default=""
    )
    target: StringProperty(
        name="", description="Target object for custom direction", default=""
    )
    ...
    bool_dissolve_seams: BoolProperty(
        name="Dissolve Seams", default=False, description="Dissolve all seam edges"
    )
    material_id: IntProperty(
        name="Material ID", default=0, min=0, description="Material ID"
    )
    ...
    bool_advanced: BoolProperty(
        name="Advanced Settings", default=False, description="Show more settings"
    )
    ...
    close_mesh: EnumProperty(
        items=(
            ...
            (
                "BRIDGE_CAP",
                "Custom",
                "Bridge loop pairs and cap holes according to vertex groups",
            ),
        ),
        default="NONE",
        name="Close Mesh",
    ...
    cap_faces: BoolProperty(
        name="Cap Holes", default=False, description="Cap open edges loops"
    )
    frame_boundary: BoolProperty(
        name="Frame Boundary", default=False, description="Support face boundaries"
    )
    ...
    bridge_smoothness: FloatProperty(
        name="Smoothness", default=1, min=0, max=1, description="Bridge Smoothness"
    )
    ...
    frame_mode: EnumProperty(
        items=(
            ...
            (
                "CENTER",
                "Center",
                "Toward the center of the face (uses Incenter for Triangles)",
            ),
        ),
        default="CONSTANT",
        name="Offset",
    ...
    bridge_cuts: IntProperty(
        name="Cuts", default=0, min=0, max=20, description="Bridge Cuts"
    )
    ...
    invert_vertex_group_thickness: BoolProperty(
        name="Invert", default=False, description="Invert the vertex group influence"
    )
    ...
    vertex_group_rotation: StringProperty(
        name="Rotation weight", default="", description="Vertex Group used for rotation"
    )
    ...
    smooth_normals_iter: IntProperty(
        name="Iterations", default=5, min=0, description="Smooth iterations"
    )
    ...
    def draw(self, context):
        ...
        if self.component_mode == "OBJECT":
            if len(sel) != 2 and not bool_working:
                ...
                layout.label(
                    icon="INFO",
                    text="Please, select two different objects. Select first the",
                )
                ...
                layout.label(
                    icon="ERROR",
                    text="Please, select Mesh, Curve, Surface, Meta or Text",
                )
                return
            ...
            for o in bpy.data.collections[self.component_coll].objects:
                if (
                    o.type in ("MESH", "CURVE", "META", "SURFACE", "FONT")
                    and o is not ob0
                ):
                    no_components = False
                    break
            if no_components:
                layout = self.layout
                layout.label(
                    icon="OUTLINER_COLLECTION", text="Components from Active Collection"
                )
                layout.label(
                    icon="INFO",
                    text="The Active Collection does not containt any Mesh,",
                )
            ...
            if no_components:
                ...
                layout.label(
                    text="Can't find any object according to the materials name."
                )
                return
        ...
        if self.component_mode == "OBJECT":
            ...
            row.label(
                text="Collection : " + self.component_coll, icon="OUTLINER_COLLECTION"
            )
        elif self.component_mode == "MATERIALS":
        ...
        if self.fill_mode == "FRAME":
            ...
            row = col.row(align=True)
            row.prop_search(
                self, "vertex_group_frame_thickness", ob0, "vertex_groups", text=""
            )
            col2 = row.column(align=True)
            row2 = col2.row(align=True)
            row2.prop(
                self,
                "invert_vertex_group_frame_thickness",
                text="",
                toggle=True,
        ...
        if self.rotation_mode == "UV":
            ...
            if ob0.type != "MESH":
                row = col.row(align=True)
                row.label(
                    text="UV rotation supported only for Mesh objects", icon="ERROR"
                )
                uv_error = True
            else:
                if len(ob0.data.uv_layers) == 0:
                    ...
                    row.label(
                        text="'" + check_name + "' doesn't have UV Maps", icon="ERROR"
                    )
                    uv_error = True
            if uv_error:
                row = col.row(align=True)
        ...
        col.separator()
        col.label(
            text="More settings in the Object Data Properties panel...",
            icon="PROPERTIES",
        )
    ...
    def execute(self, context):
        ...
        if context.object == ob0:
            ...
            new_ob = convert_object_to_mesh(
                ob0, False, False, self.rotation_mode != "UV"
            )  # ///
...
class tissue_update_tessellate_deps(Operator):
    ...
    bl_description = (
        "Fast update the tessellated mesh according to base and " "component changes."
    )
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if not self.go:
            ...
            invert_vertex_group_thickness = (
                ob.tissue_tessellate.invert_vertex_group_thickness
            )
            vertex_group_thickness_factor = (
                ob.tissue_tessellate.vertex_group_thickness_factor
            )
            vertex_group_frame_thickness = (
                ob.tissue_tessellate.vertex_group_frame_thickness
            )
            invert_vertex_group_frame_thickness = (
                ob.tissue_tessellate.invert_vertex_group_frame_thickness
            )
            vertex_group_frame_thickness_factor = (
                ob.tissue_tessellate.vertex_group_frame_thickness_factor
            )
            ...
            invert_vertex_group_distribution = (
                ob.tissue_tessellate.invert_vertex_group_distribution
            )
            vertex_group_distribution_factor = (
                ob.tissue_tessellate.vertex_group_distribution_factor
            )
            ...
            invert_vertex_group_smooth_normals = (
                ob.tissue_tessellate.invert_vertex_group_smooth_normals
            )
        ...
        try:
            ...
            self.report({"ERROR"}, "Active object must be Tessellated before Update")
            return {"CANCELLED"}
        ...
        if ob0.type == "META":
            base_ob = convert_object_to_mesh(
                ob0, False, True, props["rotation_mode"] != "UV"
            )
        else:
            base_ob = ob0.copy()
        ...
        for iter in range(iterations):
            ...
            if iter > 0 and len(iter_objects) == 0:
                break
            ...
            if (bool_selection or bool_material_id) and combine_mode == "UNUSED":
                ...
                if bool_selection:
                    if component_mode == "MATERIALS" or bool_material_id:
                        remove_faces = [
                            f
                            for f in bm.faces
                            if f.material_index in remove_materials and f.select
                        ]
                    else:
                        remove_faces = [f for f in bm.faces if f.select]
                else:
                    remove_faces = [
                        f for f in bm.faces if f.material_index in remove_materials
                    ]
                ...
                base_ob = convert_object_to_mesh(
                    new_ob, True, True, props["rotation_mode"] != "UV"
                )
                if iter < iterations - 1:
                    new_ob.data = base_ob.data
        ...
        if new_ob == 0:
            ...
            return {"CANCELLED"}
        errors = {}
        errors["modifiers_error"] = (
            "Modifiers that change the topology of the mesh \n"
        ...
        is_multiple = (
            iterations > 1 or combine_mode != "LAST"
        )  # or bool_multi_components
        if merge and is_multiple:
            use_bmesh = not (
                bool_shapekeys and fill_mode == "PATCH" and component_mode != "OBJECT"
            )
...
class TISSUE_PT_tessellate(Panel):
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        row.operator(
            "object.tissue_tessellate", text="Tessellate", icon="OBJECT_DATA"
        ).component_mode = "OBJECT"
        tss = row.operator(
            "object.tissue_tessellate", text="", icon="OUTLINER_COLLECTION"
        )
        tss.component_mode = "COLLECTION"
        tss.component_coll = context.collection.name
        row.operator(
            "object.tissue_tessellate", text="", icon="MATERIAL"
        ).component_mode = "MATERIALS"
        # col.operator("object.tissue_tessellate_multi", text='Tessellate Multi')
        col.operator(
            "object.dual_mesh_tessellated", text="Dual Mesh", icon="SEQ_CHROMA_SCOPE"
        )
        col.separator()
        ...
        op = col.operator(
            ...
            icon="MESH_CUBE",
            text="Polyhedral Decomposition",
        )
        op.mode = "POLYHEDRA"
        op = col.operator(
            "object.polyhedral_wireframe",
            icon="MOD_WIREFRAME",
            text="Polyhedral Wireframe",
        )
        op.mode = "WIREFRAME"
        col.separator()
        ...
        col.separator()
        col.operator(
            "object.tissue_update_tessellate_deps", icon="FILE_REFRESH", text="Refresh"
        )  #####
        ...
        row.operator(
            "mesh.tissue_rotate_face_right", text="Right", icon="LOOP_FORWARDS"
        )
...
class TISSUE_PT_tessellate_object(Panel):
    ...
    def draw(self, context):
        ...
        if not bool_tessellated:
            layout.label(
                text="The selected object is not a Tessellated object", icon="INFO"
            )
        else:
...
class TISSUE_PT_tessellate_frame(Panel):
    ...
    def poll(cls, context):
        try:
            ...
            return (
                ...
                and context.object.tissue.tissue_type == "TESSELLATE"
            )
        except:
            return False
    ...
    def draw(self, context):
        ...
        ob0 = props.generator
        row.prop_search(
            props, "vertex_group_frame_thickness", ob0, "vertex_groups", text=""
        )
        col2 = row.column(align=True)
        row2 = col2.row(align=True)
        row2.prop(
    ...
    def draw(self, context):
        ...
        if props.component_mode == "OBJECT":
            ...
            if props.component_coll in list(bpy.data.collections):
                ...
                if n_comp == 0:
                    col.label(
                        text="Can't find components in the Collection.", icon="ERROR"
                    )
                else:
                    ...
                    row.prop(
                        props, "com_modifiers", text="Use Modifiers", icon="MODIFIER"
                    )
            else:
            ...
            ob0 = props.generator
            row.prop_search(
                props, "vertex_group_distribution", ob0, "vertex_groups", text=""
            )
            col2 = row.column(align=True)
            row2 = col2.row(align=True)
            row2.prop(
            ...
            if props.fill_mode == "FAN":
                col.prop(props, "consistent_wedges")
        else:
            components = []
            for mat in props.generator.material_slots.keys():
                if mat in bpy.data.objects.keys():
            ...
            if n_comp == 0:
                col.label(
                    text="Can't find components from the materials.", icon="ERROR"
                )
            else:
                col.separator()
                text = "{} Component{}".format(n_comp, "s" if n_comp > 1 else "")
        ...
        if props.rotation_mode == "UV":
            ...
            if props.generator.type != "MESH":
                ...
                if len(props.generator.data.uv_layers) == 0:
                    row = col.row(align=True)
                    row.label(
                        text="'" + props.generator.name + " doesn't have UV Maps",
                        icon="ERROR",
                    )
                    uv_error = True
            if uv_error:
...
class TISSUE_PT_tessellate_thickness(Panel):
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        ob0 = props.generator
        row.prop_search(props, "vertex_group_thickness", ob0, "vertex_groups", text="")
        col2 = row.column(align=True)
        row2 = col2.row(align=True)
        row2.prop(
            props,
    ...
    def draw(self, context):
        ...
        if props.normals_mode != "FACES":
            ...
            if props.smooth_normals:
                ...
                row.separator()
                row.prop_search(
                    props, "vertex_group_smooth_normals", ob0, "vertex_groups", text=""
                )
                col2 = row.column(align=True)
                col2.prop(
                    props,
                ...
                col2.enabled = (
                    props.vertex_group_smooth_normals in ob0.vertex_groups.keys()
                )
        if props.normals_mode == "VERTS":
            ...
            row = col.row(align=True)
            row.prop_search(
                props, "vertex_group_scale_normals", ob0, "vertex_groups", text=""
            )
            col2 = row.column(align=True)
            col2.prop(
                props,
        ...
        if props.normals_mode in ("OBJECT", "SHAPEKEYS"):
            ...
            if props.even_thickness:
                row.prop(props, "even_thickness_iter")
        ...
        if props.merge:
            ...
            if props.close_mesh in ("BRIDGE", "BRIDGE_CAP"):
                ...
                if props.close_mesh == "BRIDGE_CAP":
                    if props.vertex_group_bridge_owner == "BASE":
                        ob_bridge = ob0
                    else:
                        ob_bridge = ob1
                    row = col.row(align=True)
                    row.prop_search(
                        props, "vertex_group_bridge", ob_bridge, "vertex_groups"
                    )
                    row.prop(
                        props,
            ...
            if props.close_mesh in ("CAP", "BRIDGE_CAP"):
                ...
                col.separator()
                if props.close_mesh == "BRIDGE_CAP":
                    if props.vertex_group_cap_owner == "BASE":
                        ob_cap = ob0
                    else:
                        ob_cap = ob1
                    row = col.row(align=True)
                    row.prop_search(props, "vertex_group_cap", ob_cap, "vertex_groups")
                    row.prop(
                        props,
...
class TISSUE_PT_tessellate_morphing(Panel):
    ...
    def draw(self, context):
        ...
        if tessellated(ob):
                ...
                try:
                    ...
                    try:
                        if o.data.shape_keys is None:
                            continue
                        elif len(o.data.shape_keys.key_blocks) < 2:
                            continue
                        else:
                            allow_shapekeys = not props.com_modifiers
                    except:
            ...
            row2 = col2.row(align=True)
            row2.prop(
                props, "bool_shapekeys", text="Use Shape Keys", icon="SHAPEKEY_DATA"
            )
            ...
            if not allow_shapekeys:
                ...
                row2 = col2.row(align=True)
                row2.label(
                    text="Component's Shape Keys cannot be used together with Component's Modifiers",
                    icon="INFO",
                )
...
class TISSUE_PT_tessellate_selective(Panel):
    ...
    def draw(self, context):
        ...
        col2 = row.column(align=True)
        col2.prop(
            props, "bool_material_id", icon="MATERIAL_DATA", text="Material Index"
        )
...
class tissue_rotate_face_right(Operator):
    ...
    def execute(self, context):
        ...
        for o in [
            ...
            if obj.tissue_tessellate.generator == ob and obj.visible_get()
        ]:
...
def convert_to_frame(ob, props, use_modifiers=True):
    new_ob = convert_object_to_mesh(
        ob, use_modifiers, True, props["rotation_mode"] != "UV"
    )
        ...
        if len(selected_edges) > 0:
            ...
            while True:
                ...
                for e1 in selected_edges:
                    if e1.verts[0] in e0.verts:
                        new_vert = e1.verts[1]
                    elif e1.verts[1] in e0.verts:
                        new_vert = e1.verts[0]
                    if new_vert != None:
                        if len(loop) == 0:
                        ...
                        face = e0.link_faces[0]
                        boundary_mat.append(
                            face.material_index + props["boundary_mat_offset"]
                        )
    ...
    if props["frame_mode"] == "RELATIVE":
        ...
        for v in bm.verts:
            ...
            if len(linked_faces) > 0:
                area = (
                    sum([sqrt(f.calc_area()) / len(f.verts) for f in v.link_faces]) * 2
                )
    ...
    bool_weight_thick = (
        props["vertex_group_frame_thickness"] in new_ob.vertex_groups.keys()
    )
...
def reduce_to_quads(ob, props):
    ...
    """
    new_ob = convert_object_to_mesh(
        ob, props["gen_modifiers"], True, props["rotation_mode"] != "UV"
    )
    me = new_ob.data
...
def merge_components(ob, props, use_bmesh):
    if not use_bmesh and False:
        ...
        bpy.ops.mesh.remove_doubles(threshold=props.merge_thres, use_unselected=False)
        ...
        if props.close_mesh != "NONE":
            ...
            if props.close_mesh == "CAP":
                ...
                for f in ob.data.polygons:
                    if f.select:
                        f.material_index += props.cap_material_offset
                ...
                try:
                    ...
                    for f in ob.data.polygons:
                        if f.select:
                            f.material_index += props.bridge_material_offset
                except:
                    pass
        ...
        if props.bool_dissolve_seams:
            ...
            bmesh.ops.dissolve_edges(
                bm, edges=seam_edges, use_verts=True, use_face_split=False
            )
        if props.close_mesh != "NONE":
            bm.edges.ensure_lookup_table()
            # set crease
            ...
            if props.close_mesh == "BRIDGE":
                try:
                    ...
                    closed = bmesh.ops.bridge_loops(
                        bm, edges=boundary_edges, use_pairs=True
                    )
                    if n_materials >= 0:
                        for f in closed["faces"]:
                            f.material_index = min(
                                f.material_index + props.bridge_material_offset,
                                n_materials,
                            )
                except:
                ...
                if n_materials >= 0:
                    for f in closed["faces"]:
                        f.material_index = min(
                            f.material_index + props.cap_material_offset, n_materials
                        )
            elif props.close_mesh == "BRIDGE_CAP":
                # BRIDGE
                ...
                try:
                    ...
                    if props.invert_vertex_group_bridge:
                        bw = 1 - bw
                    bridge_edges = [
                        e
                        for e in boundary_edges
                        if bw[e.verts[0].index] * bw[e.verts[1].index] >= 1
                    ]
                    for e in bridge_edges:
                        e[crease_layer] = props.bridge_edges_crease
                    closed = bmesh.ops.bridge_loops(
                        bm, edges=bridge_edges, use_pairs=True
                    )
                    if n_materials >= 0:
                        for f in closed["faces"]:
                            f.material_index = min(
                ...
                try:
                    ...
                    bw = bmesh_get_weight_numpy(group_index, dvert_lay, bm.verts)
                    if props.invert_vertex_group_cap:
                        bw = 1 - bw
                    cap_edges = [
                        e
                        for e in boundary_edges
...
def offset_boundary_materials(
    bm,
    boundary_mat_offset=0,
    boundary_variable_offset=False,
    auto_rotate_boundary=False,
):
