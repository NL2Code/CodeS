...
class tissue_to_curve_prop(PropertyGroup):
    ...
    subdivision_mode: EnumProperty(
        items=(("ALL", "All", ""), ("CAGE", "Cage", ""), ("INNER", "Inner", "")),
        default="CAGE",
...
class tissue_convert_to_curve(Operator):
    ...
    object: StringProperty(name="", description="Source object", default="")
    bool_smooth: BoolProperty(
        name="Smooth Shading",
        default=True,
        description="Output faces with smooth shading rather than flat shaded",
    ...
    nurbs_order: IntProperty(
        name="Order", default=4, min=2, max=6, description="Nurbs order"
    )
    system: IntProperty(
        name="System", default=0, min=0, description="Particle system index"
    )
    clean_distance: FloatProperty(
        name="Merge Distance",
        default=0,
    ...
    vertex_group: StringProperty(
        name="Radius", default="", description="Vertex Group used for variable radius"
    )
    invert_vertex_group: BoolProperty(
        default=False, description="Inverte the value of the Vertex Group"
    )
    vertex_group_factor: FloatProperty(
        name="Factor",
        default=0,
    ...
    only_sharp: BoolProperty(
        default=False, name="Only Sharp Edges", description="Convert only Sharp edges"
    )
    pattern_depth: FloatProperty(
        name="Depth",
        default=0.02,
    ...
    pattern0: IntProperty(
        name="Step 0", default=0, min=0, soft_max=10, description="Pattern step 0"
    )
    pattern1: IntProperty(
        name="Step 1", default=0, min=0, soft_max=10, description="Pattern step 1"
    )
    ...
    def poll(cls, context):
        try:
            ...
            ob = context.object
            return (
                ob.type in ("MESH", "CURVE", "SURFACE", "FONT") and ob.mode == "OBJECT"
    ...
    def execute(self, context):
        ...
        if props.mode == "PARTICLES":
            ...
            if props.mode in ("LOOPS", "EDGES"):
                ...
                if props.bounds_selection == "BOUNDS":
                    todo_edges = [e for e in todo_edges if len(e.link_faces) < 2]
                elif props.bounds_selection == "INNER":
                    todo_edges = [e for e in todo_edges if len(e.link_faces) > 1]

                if props.mode == "EDGES":
                    ordered_points = [
                        [e.verts[0].index, e.verts[1].index] for e in todo_edges
                    ]
                    ...
                    if props.only_sharp:
                        ...
                        ordered_points = [
                            [v.index for v in loop] for loop in vert_loops
                        ]
            ...
            try:
                weight = get_weight_numpy(
                    ob0.vertex_groups[props.vertex_group], n_verts
                )
        ...
        if not props.bool_smooth:
            bpy.ops.object.shade_flat()

        end_time = time.time()
        print(
            'Tissue: object "{}" converted to Curve in {:.4f} sec'.format(
                ob.name, end_time - start_time
            )
        )
...
class TISSUE_PT_convert_to_curve(Panel):
    ...
    def draw(self, context):
        ...
        if props.object.type == "MESH":
            ...
            row = col.row(align=True)
            row.prop_search(
                props, "vertex_group", props.object, "vertex_groups", text=""
            )
            row.prop(
                props,
                "invert_vertex_group",
