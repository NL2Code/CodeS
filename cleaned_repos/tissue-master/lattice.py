...
def grid_from_mesh(mesh, swap_uv):
    ...
    while running_grid:
        ...
        if len(faces_grid) == 0:
            ...
            verts_candidates = [
                v
                for v in bm.faces[faces_grid[-1][0]].verts
                if not_in(v.index, verts_grid)
            ]
        ...
        if not is_last:
            for vert in verts_candidates:
                new_link_faces = [
                    f for f in vert.link_faces if not_in(f.index, faces_grid)
                ]
            ...
            if len(verts_loop) == 1:  # only one vertex stored in the loop
                if len(faces_grid) == 0:  # first loop #
                    ...
                    for edge in bm.faces[faces_grid[-1][0]].edges:
                        ...
                        if (
                            bm.verts[verts_loop[0]] in edge.verts
                            and bm.verts[verts_grid[-1][0]] not in edge.verts
                        ):
                            for vert in edge.verts:
                                if vert.index != id:
                                    vert.select = True
            ...
            # continuing the loop
                ...
                for edge in link_edges:
                    for vert in edge.verts:
                        store_data = False
                        if (
                            not_in(vert.index, verts_grid)
                            and vert.index not in verts_loop
                        ):
                            if len(faces_loop) > 0:
...
class lattice_along_surface(Operator):
    ...
    flipNormals: BoolProperty(
        name="Flip Normals", default=False, description="Flip normals direction"
    )
    swapUV: BoolProperty(
        name="Swap UV", default=False, description="Flip grid's U and V"
    )
    flipU: BoolProperty(name="Flip U", default=False, description="Flip grid's U")

    flipV: BoolProperty(name="Flip V", default=False, description="Flip grid's V")
    flipW: BoolProperty(name="Flip W", default=False, description="Flip grid's W")
    ...
    scale_x: FloatProperty(
        name="Scale X", default=1, min=0.001, max=1, description="Object scale"
    )
    scale_y: FloatProperty(
        name="Scale Y", default=1, min=0.001, max=1, description="Object scale"
    )
    scale_z: FloatProperty(
        name="Scale Z", default=1, min=0.001, max=1, description="Object scale"
    )
    ...
    def execute(self, context):
        if self.source_object == self.grid_object == "" or True:
            ...
            if grid_obj.type not in ("MESH", "CURVE", "SURFACE"):
                self.report(
                    {"ERROR"},
                    "The surface object is not valid. Only Mesh,"
            ...
            try:
                ...
                obj_me = simple_to_mesh(
                    obj
                )  # obj.to_mesh(bpy.context.depsgraph, apply_modifiers=True)
            except:
                self.report(
                    {"ERROR"},
                    "The object to deform is not valid. Only "
        ...
        lattice.scale = Vector(
            (bb.x / self.scale_x, bb.y / self.scale_y, bb.z / self.scale_z)
        )
