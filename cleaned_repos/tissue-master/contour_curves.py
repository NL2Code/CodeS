...
class tissue_contour_curves_prop(PropertyGroup):
    ...
    min_value: FloatProperty(
        name="Offset Value",
        default=0.0,  # soft_min=0, soft_max=1,
        description="Offset contouring values",
        update=anim_contour_curves,
    )
    range_value: FloatProperty(
        name="Range Values",
        default=100,  # soft_min=0, soft_max=1,
    ...
    object_name: StringProperty(
        name="Active Object", default="", description="", update=anim_contour_curves
    )
    ...
    contour_mode: EnumProperty(
        items=(
            (
                "VECTOR",
                "Vector",
                "Orient the Contour to a given vector starting from the origin of the object",
            ),
            ...
            (
                "GEODESIC",
                "Geodesic Distance",
                "Contour based on the geodesic distance from the chosen vertices",
            ),
            (
                "TOPOLOGY",
                "Topology Distance",
                "Contour based on the topology distance from the chosen vertices",
            ),
        ),
        default="VECTOR",
    ...
    seeds_mode: EnumProperty(
        items=(
            (
                "BOUND",
                "Boundary Edges",
                "Compute the distance starting from the boundary edges",
            ),
            (
                "SHARP",
                "Sharp Edges",
                "Compute the distance starting from the sharp edges",
            ),
            (
                "WEIGHT",
                "Weight",
                "Compute the distance starting from the selected vertex group",
            ),
        ),
        default="BOUND",
...
class tissue_weight_contour_curves_pattern(Operator):
    ...
    object: StringProperty(name="Object", description="Source object", default="")
    ...
    variable_bevel: BoolProperty(
        name="Variable Bevel", default=False, description="Variable Bevel"
    )
    ...
    displace_x: BoolProperty(
        name="Use X", default=True, description="Displace along X axis"
    )
    displace_y: BoolProperty(
        name="Use Y", default=True, description="Displace along Y axis"
    )
    displace_z: BoolProperty(
        name="Use Z", default=True, description="Displace along Z axis"
    )

    merge: BoolProperty(name="Merge Vertices", default=True, description="Merge points")
    merge_thres: FloatProperty(
        name="Merge Threshold",
        default=0.01,
        min=0,
    ...
    min_bevel_depth: FloatProperty(
        name="Min Bevel Depth", default=0.05, min=0, soft_max=1, description=""
    )
    max_bevel_depth: FloatProperty(
        name="Max Bevel Depth", default=0.20, min=0, soft_max=1, description=""
    )
    remove_open_curves: BoolProperty(
        name="Remove Open Curves", default=False, description="Remove Open Curves"
    )
    ...
    vertex_group_contour: StringProperty(
        name="Contour", default=vg_name, description="Vertex Group used for contouring"
    )
    clean_distance: FloatProperty(
        name="Clean Distance",
        default=0.005,
    ...
    spiral_rotation: FloatProperty(
        name="Spiral Rotation", default=0, min=0, max=2 * pi, description=""
    )
    ...
    contour_vector_object: StringProperty(
        name="Object", description="Target object", default=""
    )
    contour_offset: FloatProperty(
        name="Offset",
        default=0.05,
    ...
    def draw(self, context):
        ...
        if self.contour_mode == "VECTOR":
            ...
            col.prop_search(
                self, "vertex_group_contour", ob, "vertex_groups", text="Group"
            )
            ...
            if self.seeds_mode == "WEIGHT":
                ...
                # row.label(text='')
                col.prop_search(
                    self, "vertex_group_seed", ob, "vertex_groups", text="Group"
                )
        elif self.contour_mode == "OBJECT":
            col.prop_search(
                self, "contour_vector_object", context.scene, "objects", text="Object"
            )
        col.separator()
    ...
    def execute(self, context):
        ...
        if props.contour_mode in ("VECTOR", "OBJECT"):
            ...
            if props.contour_mode == "OBJECT" and props.contour_vector_object:
                ...
                vec_ob_matrix = np.matrix(
                    vec_ob.matrix_world.to_3x3().inverted().transposed()
                )
            ...
            try:
                weight = get_weight_numpy(
                    ob0.vertex_groups[props.vertex_group_contour], len(me0.vertices)
                )
            except:
                bm.free()
                bpy.data.objects.remove(ob0)
            ...
            if props.seeds_mode == "WEIGHT":
                try:
                    seeds = get_weight_numpy(
                        ob0.vertex_groups[props.vertex_group_seed], len(me0.vertices)
                    )
                except:
                    bm.free()
                    bpy.data.objects.remove(ob0)
        ...
        try:
            pattern_weight = get_weight_numpy(
                ob0.vertex_groups[props.vertex_group_pattern], len(me0.vertices)
            )
        except:
        ...
        if props.variable_bevel:
            try:
                bevel_weight = get_weight_numpy(
                    ob0.vertex_groups[props.vertex_group_bevel], len(me0.vertices)
                )
                weight_bevel = True
            except:
        ...
        if props.contour_mode in ("VECTOR", "OBJECT", "GEODESIC"):
            ...
            n_curves = min(
                int((np.max(weight) - props.min_value) / delta_iso) + 1, props.n_curves
            )
        else:
            if n_curves == 1:
        ...
        if False:
            edges_verts = get_attribute_numpy(me0.edges, "vertices", mult=2).astype(
                "int"
            )
        ...
        for c in range(n_curves):
            ...
            if len(edges_index) > 0:
                if (
                    props.variable_bevel
                    and props.max_bevel_depth != props.min_bevel_depth
                    and False
                ):
        ...
        if len(total_segments) > 0:
            ...
            ordered_points, ordered_points_edge_id = find_curves_attribute(
                total_segments, len(total_verts), total_edges_index
            )
            ...
            if props.variable_bevel:  # and not weight_bevel:
                total_radii = np.interp(
                    total_radii,
                    (total_radii.min(), total_radii.max()),
                    (props.min_bevel_depth, props.max_bevel_depth),
                )
            ob.data = curve_from_pydata(
                total_verts,
                total_radii,
                ordered_points,
                ob0.name + "_ContourCurves",
                props.remove_open_curves,
                merge_distance=props.clean_distance,
                only_data=True,
                curve=ob.data,
                spline_type=props.spline_type,
            )
            ...
            if props.variable_bevel:
                if not weight_bevel:
                    ...
                    ob.data.bevel_depth = max(
                        props.max_bevel_depth, props.min_bevel_depth
                    )
...
class TISSUE_PT_contour_curves(Panel):
    ...
    def draw(self, context):
        ...
        if props.contour_mode == "VECTOR":
            ...
            col.prop_search(
                props, "vertex_group_contour", ob0, "vertex_groups", text="Group"
            )
            ...
            if props.seeds_mode == "WEIGHT":
                ...
                # row.label(text='')
                col.prop_search(
                    props, "vertex_group_seed", ob0, "vertex_groups", text="Group"
                )
        elif props.contour_mode == "OBJECT":
            col.prop_search(
                props, "contour_vector_object", context.scene, "objects", text="Object"
            )
        col.separator()
