...
def get_ssl_certificate_info(host):
    try:
        ...
        issuer = certificate.issuer.get_attributes_for_oid(x509.NameOID.COMMON_NAME)[
            0
        ].value
        ...
        print(
            f"{Fore.RED}Error extracting SSL certificate information: {e}{Fore.RESET}"
        )
        return None
            ...
            if real_ip:
                ...
                if ssl_info:
                    ...
                    for key, value in ssl_info.items():
                        print(
                            f"{Fore.RED}      \u2514\u27A4 {Fore.CYAN}{key}:{W} {value}"
                        )
...
def read_config():
    ...
    if not os.path.exists("config.ini"):
        ...
        config["DEFAULT"] = {"securitytrails_api_key": "your_api_key"}
...
def securitytrails_historical_ip_address(domain):
    if read_config():
        ...
        headers = {"accept": "application/json", "APIKEY": read_config()}
        try:
            ...
            print(
                f"{Fore.RED}Error extracting Historical IP Address information from SecurityTrails{Fore.RESET}"
            )
            None
    else:
        print(
            f"{Fore.RED}Please add your {C}SecurityTrails{Fore.RED} API Key in config.ini file{Fore.RESET}"
        )
        None
...
if __name__ == "__main__":
    domain = sys.argv[
        1
    ]  # pass domain in command-line argument ex: python3 cloakquest3r.py top.gg
    ...
    if is_using_cloudflare(domain):
        ...
        proceed = input(
            f"\n{Fore.YELLOW}> Do you want to proceed? {Fore.GREEN}(yes/no): "
        ).lower()
