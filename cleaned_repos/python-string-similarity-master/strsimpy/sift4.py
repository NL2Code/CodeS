...
class SIFT4Options(MetricStringDistance):
    def __init__(self, options=None):
        ...
        otheroptions = {
            ...
            "matchingevaluator": {
                "sift4matchingevaluator": self.sift4matchingevaluator
            },
            ...
            "transpositioncostevaluator": {
                "longertranspositionsaremorecostly": self.longertranspositionsaremorecostly
            },
                    ...
                    if k == "maxdistance":
                        ...
                        if callable(v):
                            ...
                            if v in otheroptions[k].keys():
                                ...
                                msg = "Option {} should be callable or one of [{}]".format(
                                    k, ", ".join(otheroptions[k].keys())
                                )
    ...
    def distance(self, s1, s2, maxoffset=5, options=None):
        ...
        while (c1 < l1) and (c2 < l2):
                ...
                while i < len(offset_arr):
                    ...
                    if (c1 <= ofs["c1"]) or (c2 <= ofs["c2"]):
                        ...
                        if isTrans:
                            ...
                            if not ofs["trans"]:
                                ofs["trans"] = True
                                trans += options.transpositioncostevaluator(
                                    ofs["c1"], ofs["c2"]
                                )
                        break
                    else:
            ...
            if options.maxdistance:
                temporarydistance = options.locallengthevaluator(
                    max(c1, c2)
        ...
        lcss += options.locallengthevaluator(local_cs)
        return round(
            options.locallengthevaluator(max(l1, l2))
            - options.transpositionsevaluator(lcss, trans)
        )
