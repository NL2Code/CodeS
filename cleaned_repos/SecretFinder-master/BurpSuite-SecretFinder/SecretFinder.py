    ...
    def doActiveScan(self, baseRequestResponse, pa):
        ...
        for reg in self.regexs.items():
            tmp_issues = self._CustomScans.findRegEx(
                ...
                BurpExtender.issuename
                % (" ".join([x.title() for x in reg[0].split("_")])),
...
class CustomScans:
    def __init__(self, requestResponse, callbacks):
        ...
        self._mime_type = self._helpers.analyzeResponse(
            self._requestResponse.getResponse()
        ).getStatedMimeType()
        return
                        ...
                        ScanIssue(
                            ...
                            [
                                self._callbacks.applyMarkers(
                                    self._requestResponse, None, offsets
                                )
                            ],
                            issuename,
                            issuelevel,
                            issuedetail.replace(r"%%regex%%", ref),
                        )
                    )
                except:
                    continue
...
class ScanIssue(IScanIssue):
    def __init__(
        self, httpservice, url, requestresponsearray, name, severity, detailmsg
    ):
        self._url = url
