...
from ..exceptions import MethodNotAllowed, MissingComponent, NotFound, ReverseNotFound
from ..limits import RouteLimits
from ..request.request import Request
from ..responses.responses import RedirectResponse, Response, WebsocketHandshakeResponse
        ...
        for http_code in [404, 405]:
            if self.default_handlers.get(http_code) is None:
                raise NotImplementedError(
                    f"Please implement the default {http_code} route."
                )
...
class Route:
    def __init__(
        ...
        (
            self.regex,
            self.params_book,
            self.simplified_pattern,
        ) = PatternParser.extract_params(pattern)
