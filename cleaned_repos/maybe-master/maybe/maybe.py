...
from ptrace.syscall import DIRFD_ARGUMENTS, RETURN_VALUE_REGISTER, SYSCALL_REGISTER
    ...
    while True:
        ...
        if syscall and syscall_state.next_event == "exit":
            ...
            if syscall.name in syscall_filters:
                ...
                operation, return_value = filter_function(
...
def main(argv=sys.argv[1:]):
    ...
    arg_parser.add_argument(
        "command", nargs="+", help="the command to run under maybe's control"
    )
    ...
    if args.plugin is not None:
        for plugin_path in args.plugin:
            try:
                ...
                print(
                    T.red(
                        "Error loading %s: %s." % (T.bold(plugin_path) + T.red, error)
                    )
                )
                return 1
    ...
    command = " ".join(
        [(("'%s'" % arg) if (" " in arg) else arg) for arg in args.command]
    )
    ...
    try:
        ...
        print(
            T.yellow(
                "%s terminated by keyboard interrupt." % (T.bold(command) + T.yellow)
            )
        )
        return 2
    finally:
    ...
    if operations:
        ...
        print(
            "%s has not detected any file system operations from %s."
