...
def filter_write(process, file_descriptor, byte_count):
    if process.is_tracked_descriptor(file_descriptor):
        ...
        return (
            "%s %s to %s"
            % (T.red("write"), T.bold("%d bytes" % byte_count), T.underline(path)),
            byte_count,
        )
    else:
        return None, None
...
def filter_dup(process, file_descriptor_old, file_descriptor_new=None):
    if process.is_tracked_descriptor(file_descriptor_old):
        ...
        return None, process.register_path(
            process.descriptor_path(file_descriptor_old), file_descriptor_new
        )
    else:
        return None, None
...
register_filter(
    "open",
    lambda process, args: filter_open(process, process.full_path(args[0]), args[1]),
)
register_filter(
    "creat",
    lambda process, args: filter_open(
        process, process.full_path(args[0]), O_CREAT | O_WRONLY | O_TRUNC
    ),
)
register_filter(
    "openat",
    lambda process, args: filter_open(
        process, process.full_path(args[1], args[0]), args[2]
    ),
)
register_filter(
    "mknod", lambda process, args: filter_mknod(process.full_path(args[0]), args[1])
)
register_filter(
    "mknodat",
    lambda process, args: filter_mknod(process.full_path(args[1], args[0]), args[2]),
)
...
register_filter(
    "pwritev", lambda process, args: filter_write(process, args[0], args[2])
)
