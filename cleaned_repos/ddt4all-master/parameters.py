...
class ecuCommand(widgets.QDialog):
    ...
    def compute_frame(self, check=True):
        ...
        stream_to_send = " ".join(
            self.current_request.build_data_stream(data_to_stream)
        )
    ...
    def req_changed(self, item):
        ...
        self.com_table.setHorizontalHeaderLabels(
            ["Key", "Value", "Unit", "Data type", "Description"]
        )
...
class paramWidget(widgets.QWidget):
    def __init__(
        self, parent, ddtfile, ecu_addr, ecu_name, logview, prot_status, canline
    ):
    ...
    def saveEcu(self, name=None):
        if not name:
            filename_tuple = widgets.QFileDialog.getSaveFileName(
                self,
                _("Save ECU (keep '.json' extension)"),
                "./json/myecu.json",
                "*.json",
            )
        ...
        if self.targetsdata:
            ...
            ecu_idents = options.ecu_scanner.ecu_database.getTargetsByHref(
                os.path.basename(self.ddtfile)
            )
            if len(ecu_idents):
                js_targets = []
    ...
    def addParameter(self, requestname, issend, screenname, item):
        if self.parser != "json":
            self.logview.append(
                "<font color=red>"
                + _("To be able to edit your screen, first export it in JSON format")
                + "</font>"
            )
            return
    ...
    def changeLabelColor(self):
        ...
        if isinstance(self.currentwidget, displaymod.labelWidget):
            ...
            if color.isValid():
                self.currentwidget.jsondata["color"] = "rgb(%i,%i,%i)" % (
                    color.red(),
                    color.green(),
                    color.blue(),
                )
        ...
        if isinstance(self.currentwidget, displaymod.labelWidget):
            for label in self.layoutdict["screens"][self.current_screen]["labels"]:
                ...
                if txt == self.currentwidget.text():
                    nln = widgets.QInputDialog.getText(
                        self, "DDT4All", _("Enter label name")
                    )
                    if not nln[1]:
                        return
                    newlabelname = nln[0]
    ...
    def removeElement(self):
        ...
        if isinstance(self.currentwidget, displaymod.displayWidget):
            ...
            for display in self.layoutdict["screens"][self.current_screen]["displays"]:
                txt = display["text"]
                if txt == self.currentwidget.qlabel.text():
                    self.layoutdict["screens"][self.current_screen]["displays"].pop(
                        count
                    )
                    break
                count += 1
        ...
        if isinstance(self.currentwidget, displaymod.buttonRequest):
            ...
            for button in self.layoutdict["screens"][self.current_screen]["buttons"]:
                ...
                if txt == self.currentwidget.uniquename:
                    self.layoutdict["screens"][self.current_screen]["buttons"].pop(
                        count
                    )
                    break
                count += 1
    ...
    def mouseMoveEvent(self, event):
        if options.simulation_mode and options.mode_edit == True:
            if len(self.movingwidgets):
                sizemodifier = (
                    widgets.QApplication.keyboardModifiers() == core.Qt.ControlModifier
                )
                ratiomodifier = (
                    widgets.QApplication.keyboardModifiers() == core.Qt.ShiftModifier
                )
        ...
        if self.sliding:
            ...
            self.scrollarea.verticalScrollBar().setValue(
                self.scrollarea.verticalScrollBar().value() - mouseY
            )
            self.scrollarea.horizontalScrollBar().setValue(
                self.scrollarea.horizontalScrollBar().value() - mouseX
            )
    ...
    def hexeditor(self):
        ...
        self.diagsession.addItem("None")
        self.request_editor_sds["None"] = ""

        for diag in rqsts:
            if (
                "start" in diag.lower()
                and "session" in diag.lower()
                and "diag" in diag.lower()
            ):
                sds = self.ecurequestsparser.requests[diag]

                if "Session Name" in sds.sendbyte_dataitems:
                    session_names = self.ecurequestsparser.data["Session Name"]
                    for name in session_names.items.keys():
                        sds_stream = " ".join(
                            sds.build_data_stream({"Session Name": name})
                        )
                        name = name + "[" + sds_stream + "]"
                        self.request_editor_sds[name] = sds_stream
                        self.diagsession.addItem(name)
                    print(sds.sendbyte_dataitems["Session Name"])
    ...
    def initELM(self):
        ...
        if self.main_protocol_status:
            if self.ecurequestsparser.ecu_protocol == "CAN":
                ...
                txrx = "(Tx0x%s/Rx0x%s)@%iK" % (
                    self.ecurequestsparser.ecu_send_id,
                ...
                self.startDiagnosticSession()
                self.main_protocol_status.setText(
                    "KWP @ " + self.ecurequestsparser.funcaddr
                )
            elif self.ecurequestsparser.ecu_protocol == "ISO8":
                self.main_protocol_status.setText(
                    "ISO8 @ " + self.ecurequestsparser.funcaddr
                )
            else:
                self.main_protocol_status.setText(
                    "??? @ " + self.ecurequestsparser.funcaddr
                )
    ...
    def initJSON(self):
        ...
        if os.path.exists(targetsfile):
            ...
            tgt = options.main_window.ecu_scan.ecu_database.getTargetsByHref(
                self.ddtfile[5:]
            )
            if tgt:
                self.targetsdata = []
    ...
    def initXML(self):
        ...
        if ".json" in self.ddtfile:
            ...
            target = self.getChildNodesByName(xdoc, "Target")[0]
            if not target:
                self.logview.append(_("Invalid DDT file"))
                return

            categories = self.getChildNodesByName(target, "Categories")

            for cats in categories:
                xml_cats = self.getChildNodesByName(cats, "Category")
                for category in xml_cats:
                    category_name = category.getAttribute("Name")
                    self.categories[category_name] = []
                    screens_name = self.getChildNodesByName(category, "Screen")
        ...
        for reqname, request in self.ecurequestsparser.requests.items():
            ...
            if "START" in uppername and "DIAG" in uppername and "SESSION" in uppername:
                ...
                for di in request.sendbyte_dataitems.keys():
                    ...
                    if "SESSION" in dataitemnameupper and "NAME" in dataitemnameupper:
        ...
        if self.ecurequestsparser.ecu_protocol == "CAN":
            ...
            for k in reqk:
                if "tester" in k.lower() and "present" in k.lower():
                    self.tester_presend_command = self.ecurequestsparser.requests[
                        k
                    ].sentbytes
                    break
    ...
    def sendElm(self, command, auto=False, force=False):
        ...
        if not options.simulation_mode:
            ...
            if command.startswith("10"):
                self.logview.append(
                    "<font color=blue>"
                    + _("Switching to session mode")
                    + "</font> <font color=orange>%s</font>" % command
                )
            ...
            if self.ecurequestsparser.ecu_send_id == "745":
                ...
                if "210B" in command:
                    elm_response = (
                        "61 0B 02 00 6C 10 80 00 02 01 02 01 02 00 00 05 FF FF FF FF"
                    )
                if "2147" in command:
                    elm_response = "61 47 00 30 50 50 20 1E 0A 20 FF FF FF"
                if "2126" in command:
                    elm_response = (
                        "61 26 14 0F 1C 28 64 5A 50 46 3C 32 28 1E 14 0A 00 5A 0A FF"
                    )
                if "2125" in command:
            ...
            if "210A" in command:
                ...
                # Test for ACU4
                elm_response = (
                    "57 06 90 07 41 90 08 41 90 42 52 90 08 42 90 07 42 90 7C 40"
                )
            ...
            txt = (
                "<font color=green>" + _("Sending simulated ELM request :") + "</font>"
            )
        ...
        if elm_response.startswith("WRONG"):
            self.logview.append(
                "<font color=red>" + _("Bad ELM response :") + "</font> " + elm_response
            )
    ...
    def drawDisplays(self, screen):
        ...
        if self.parser == "xml":
            ...
            for disp in displays:
                displaywidget = displaymod.displayWidget(
                    self.panel, self.uiscale, self.ecurequestsparser
                )
    ...
    def drawButtons(self, screen):
        ...
        if self.parser == "xml":
            ...
            for button in buttons:
                qbutton = displaymod.buttonRequest(
                    self.panel, self.uiscale, self.ecurequestsparser, button_count
                )
                ...
                qbutton.clicked.connect(
                    lambda state, btn=qbutton.butname: self.buttonClicked(btn)
                )
        else:
            button_count = 0
            for button in screen["buttons"]:
                ...
                qbutton.clicked.connect(
                    lambda state, btn=qbutton.uniquename: self.buttonClicked(btn)
                )
    ...
    def drawLabels(self, screen):
        ...
        for key, value in [
            (k, labeldict[k])
            for k in sorted(labeldict, key=labeldict.get, reverse=True)
        ]:
    ...
    def drawInputs(self, screen):
        ...
        if self.parser == "xml":
            ...
            for inp in inputs:
                inputwidget = displaymod.inputWidget(
                    self.panel, self.uiscale, self.ecurequestsparser
                )
    ...
    def buttonClicked(self, txt):
        if not txt in self.button_requests:
            self.logview.append(
                "<font color=red>" + _("Button request not found : ") + txt + "</font>"
            )
            return
        ...
        for req in request_list:
            ...
            for k in sendbytes_data_items.keys():
                ...
                elm_data_stream = ecu_data.setValue(
                    input_value,
                    elm_data_stream,
                    dataitem,
                ...
                if not elm_data_stream:
                    ...
                    self.logview.append(
                        _("Request aborted (look at red paramters entries): ")
                        + str(input_value)
                    )
                    return
            ...
            for key in rcvbytes_data_items.keys():
                if request_name in self.displaydict:
                    ...
                    value = dd_ecu_data.getDisplayValue(
                        elm_response, data_item, self.ecurequestsparser.endianness
                    )
    ...
    def startDiagnosticSession(self, sds=""):
        ...
        self.logview.append(
            "<font color=blue>"
            + _("ECU uses SDS ")
            + "</font> <font color=orange>%s</font>" % sds
        )
    ...
    def updateDisplay(self, request_name, update_inputs=False):
        ...
        for data_struct in request_data.data:
            ...
            ecu_data = data_struct.data
            data_item = request.dataitems[ecu_data.name]
            value = ecu_data.getDisplayValue(
                elm_response, data_item, request.ecu_file.endianness
            )
        ...
        if self.updatelog and self.logfile is not None:
            ...
            self.logfile.write(
                "\tScreen : " + self.pagename + "\tRequest : " + request_name + "\n"
            )
    ...
    def clearDTC(self):
        ...
        if "ClearDiagnosticInformation.All" in self.ecurequestsparser.requests:
            ...
            self.logview.append(
                _("No ClearDTC request for that ECU, will send default 14FF00")
            )
            request = "14FF00"

        msgbox = widgets.QMessageBox()
        msgbox.setText(
            _("<center>You are about to clear diagnostic troubles codes</center>")
        ...
        if "WRONG" in response:
            ...
            options.main_window.logview.append(
                "<font color=red>Clear DTC failed</font>"
            )
        else:
            options.main_window.logview.append(
                "<font color=green>Clear DTC successfully done</font>"
            )
    ...
    def readDTC(self):
        ...
        for dn in range(0, numberofdtc):
            ...
            for k in request.dataitems.keys():
                ...
                value_hex = ecu_data.getHexValue(
                    " ".join(can_response), dataitem, request.ecu_file.endianness
                )
...
def dumpAddressing(file):
    ...
    xdoc = xdom.documentElement
    dict = ecu.addressing
    xml_funcs = getChildNodesByName(xdoc, "Function")
    for func in xml_funcs:
        shortname = func.getAttribute("Name")
        address = func.getAttribute("Address")
        for name in getChildNodesByName(func, "Name"):
            try:
...
def dumpSNAT(file):
    ...
    for func in xml_funcs:
        address = func.getAttribute("Address")
        protolist = getChildNodesByName(func, "ProtocolList")
        for rid in protolist:
            proto = getChildNodesByName(rid, "Protocol")
            for prtc in proto:
                grid = getChildNodesByName(prtc, "Address")
                for ok in grid:
                    rid_add = ok.getAttribute("Rid")
                    strHex = "%0.2X" % int(address)
                    dict[strHex] = rid_add
                    break
    return dict
...
def dumpDNAT(file):
    ...
    for func in xml_funcs:
        ...
        for xid in protolist:
            proto = getChildNodesByName(xid, "Protocol")
            for prtc in proto:
                gxid = getChildNodesByName(prtc, "Address")
                for ok in gxid:
                    xid_add = ok.getAttribute("Xid")
                    strHex = "%0.2X" % int(address)
                    dict[strHex] = xid_add
                    break
    return dict
...
def dumpDOC(xdoc):
    target = getChildNodesByName(xdoc, "Target")
    if not target:
        return None
    ...
    xml_categories = getChildNodesByName(target, "Categories")

    xmlscreens = {}
    js_categories = {}

    for cats in xml_categories:
        xml_cats = getChildNodesByName(cats, "Category")
        for category in xml_cats:
            category_name = category.getAttribute("Name")
            js_categories[category_name] = []
            screens_name = getChildNodesByName(category, "Screen")
            for screen in screens_name:
                screen_name = screen.getAttribute("Name")
    ...
    for scrname, screen in xmlscreens.items():
        ...
        presend = []
        for elem in getChildNodesByName(screen, "Send"):
        ...
        for label in labels:
            ...
            label_dict["bbox"] = getRectangleXML(
                getChildNodesByName(label, "Rectangle")[0]
            )
        ...
        for display in displays:
            ...
            display_dict["rect"] = getRectangleXML(
                getChildNodesByName(display, "Rectangle")[0]
            )
        ...
        for button in buttons:
            ...
            button_dict["rect"] = getRectangleXML(
                getChildNodesByName(button, "Rectangle")[0]
            )
        ...
        for input in inputs:
            ...
            input_dict["rect"] = getRectangleXML(
                getChildNodesByName(input, "Rectangle")[0]
            )
...
def dumpVehicles(file="vehicles/projects.xml"):
    ...
    dict["projects"]["All"]["addressing"] = dumpAddressing(
        "./vehicles/GenericAddressing.xml"
    )
    ...
    manufacturers = getChildNodesByName(xdoc, "Manufacturer")
    for manufacturer in manufacturers:
        name = (
            str(manufacturer.getElementsByTagName("name")[0].childNodes[0].nodeValue)
            .lower()
            .title()
        )
        projects = getChildNodesByName(manufacturer, "project")
        for project in projects:
            code = project.getAttribute("code")
            p_name = project.getAttribute("name")
            project_name = "%s %s" % (name, p_name)
            if name.lower().replace("_", "").replace("-", "") in p_name.lower().replace(
                "_", ""
            ).replace("-", ""):
                project_name = p_name
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--convert", action="store_true", default=None, help="Convert all XML to JSON"
    )
    parser.add_argument(
        "--zipconvert",
        action="store_true",
    ...
    parser.add_argument(
        "--dumpprojects", action="store_true", default=None, help="Dump Vehicles"
    )
