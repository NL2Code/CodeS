...
class labelWidget(widgets.QLabel):
    ...
    def initXML(self, xmldata):
        ...
        rect = getRectangleXML(
            getChildNodesByName(xmldata, "Rectangle")[0], self.uiscale
        )
        ...
        self.setStyleSheet(
            "background: %s; color: %s" % (colorConvert(color), getFontColor(xmldata))
        )
...
class screenWidget(widgets.QFrame):
    ...
    def initXML(self, xmldata):
        ...
        for elem in getChildNodesByName(xmldata, "Send"):
...
class displayWidget(widgets.QWidget):
    ...
    def toggle_selected(self, sel):
        if sel:
            ...
            self.qlabelval.setFrameStyle(
                widgets.QFrame.Panel | widgets.QFrame.StyledPanel
            )
        else:
    ...
    def initXML(self, display, displaydict):
        ...
        rect = getRectangleXML(
            getChildNodesByName(display, "Rectangle")[0], self.uiscale
        )
        ...
        self.qlabel.setStyleSheet(
            ...
            % (colorConvert(color), getFontColor(display))
        )
        ...
        self.qlabelval.setDefaultStyle(
            "background-color: %s; color: %s"
            % (colorConvert(color), getFontColor(display))
        )
        ...
        infos += (
            "Request="
            + unicode(req.sentbytes)
            + " ManualRequest="
            + unicode(req.manualsend)
        )
        infos += "\nNumBits=" + unicode(data.bitscount)
        infos += " FirstByte=" + unicode(dataitem.firstbyte)
        infos += " BitOffset=" + unicode(dataitem.bitoffset)
        infos += " Endianess=" + unicode(endianess)
    ...
    def initJson(self, display, displaydict):
        ...
        self.qlabel.setStyleSheet(
            "background-color: %s; color: %s" % (color, fontcolor)
        )
        self.qlabel.setAlignment(core.Qt.AlignLeft)
        self.qlabel.setWordWrap(True)
...
class inputWidget(widgets.QWidget):
    ...
    def initXML(self, input, inputdict):
        ...
        self.qlabel.setText(text)
        self.qlabel.setStyleSheet(
            "background:%s; color:%s" % (colorConvert(color), getFontColor(input))
        )
    ...
    def initJson(self, jsoninput, inputdict):
        ...
        if len(self.ecurequestsparser.data[text].items) > 0:
            ...
            self.editwidget.setToolTip(infos)
            self.editwidget.setStyleSheet("background:%s; color:%s" % (color, fntcolor))
