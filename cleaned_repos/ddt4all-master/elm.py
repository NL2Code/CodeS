...
class ELM:
    ...
    def __init__(self, portName, rate, adapter_type="STD", maxspeed="No"):
        for speed in [
            ...
            if len(options.log) > 0:
                self.lf = open(
                    "./logs/elm_" + options.log + ".txt", "at", encoding="utf-8"
                )
                self.vf = open(
                    "./logs/ecu_" + options.log + ".txt", "at", encoding="utf-8"
                )
                self.vf.write(
    ...
    def cmd(self, command, serviceDelay="0"):
        ...
        if ((tb - self.lastCMDtime) < (self.busLoad + self.srvsDelay)) and (
            "AT" not in command.upper() or "ST" not in command.upper()
        ):
        ...
        if (
            (tb - self.lastCMDtime) > self.keepAlive
    ...
    def send_cmd(self, command):
        if (
            "AT" in command.upper()
            or "ST" in command.upper()
            or self.currentprotocol != "can"
        ):
    ...
    def set_can_addr(self, addr, ecu, canline=0):
        ...
        if canline == 0:
            ...
            if (
                0 and "brp" in ecu.keys() and ecu["brp"] == "1"
            ):  # I suppose that brp=1 means 250kBps CAN
                if extended_can:
                    self.cmd("AT SP 9")
                else:
                    self.cmd("AT SP 8")
    ...
    def set_iso8_addr(self, addr, ecu):
        if (
            self.currentprotocol == "iso"
            and self.currentaddress == addr
            and self.currentsubprotocol == ecu["protocol"]
        ):
            return
...
def elm_checker(port, speed, logview, app):
    ...
    if pycom > 0:
        logview.append(
            "<font color=red>" + _("Incompatible adapter on ARM core") + "</font> \n"
        )
    logview.append(
        _("Result: ")
        + str(good)
