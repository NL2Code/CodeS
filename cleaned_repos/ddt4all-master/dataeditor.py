        ...
        if index.column() == 0:
            ...
            for sn in options.main_window.screennames:
                ...
                sa.triggered.connect(
                    lambda state, name=sn, it=item_name: self.add_to_screen(name, it)
                )
...
class requestTable(widgets.QTableWidget):
    ...
    def set_sds(self, view):
        ...
        self.ecureq[self.currentreq].sds[
            "aftersales"
        ] = view.checkaftersales.isChecked()
        self.ecureq[self.currentreq].sds[
            "engineering"
        ] = view.checkengineering.isChecked()
...
class paramEditor(widgets.QFrame):
    ...
    def update_bitview(self, dataname):
        ...
        bytesarray = ecudata.setValue(
            ...
            bytesarray,
            dataitem,
            self.current_request.ecu_file.endianness,
            True,
        )
    ...
    def update_bitview_value(self, dataname):
        ...
        byteslisttosend = [
            bytestosend[a * 2 : a * 2 + 2] for a in range(len(bytestosend) // 2)
        ]
    ...
    def init(self, req):
        ...
        for k in datak:
            ...
            ln = dataitem.firstbyte + math.ceil(float(ecudata.bitscount) / 8.0)
            if ln > bytescount:
                bytescount = ln
            ...
            item_bc.setTextAlignment(
                int(core.Qt.AlignHCenter) | int(core.Qt.AlignVCenter)
            )

            item_sb.valueChanged.connect(
                lambda state, di=dataitem, slf=item_sb: self.start_byte_changed(di, slf)
            )
            item_boff.valueChanged.connect(
                lambda state, di=dataitem, slf=item_boff: self.bit_offset_changed(
                    di, slf
                )
            )
            endian_combo.activated.connect(
                lambda state, di=dataitem, slf=endian_combo: self.endian_changed(
                    di, slf
                )
            )
            ...
            self.table.setItem(
                count, 1, widgets.QTableWidgetItem(str(dataitem.firstbyte).zfill(5))
            )
...
class buttonData(widgets.QFrame):
    ...
    def check_data(self):
        ...
        if len(itemsfound) == numfound:
            options.main_window.logview.append(
                _(
                    "<font color=green>Request <font color=blue>'%s'</font> has no missing input values</font>"
                )
                % requestname
            )
            return
        ...
        options.main_window.logview.append(
            _(
                "<font color=red>Request <font color=blue>'%s'</font> has missing inputs :</font>"
            )
            % requestname
        )
    ...
    def add_request(self):
        ...
        smap = {"Delay": str(delay), "RequestName": request}
...
class buttonEditor(widgets.QWidget):
    ...
    def name_changed(self, r, c):
        ...
        buttondata["uniquename"] = textdata + "_" + str(idx)
        self.init()
...
class ecuParamEditor(widgets.QFrame):
    ...
    def add_ident(self):
        ...
        if self.protocolcombo.currentIndex() == 0:
            ...
            protocol = "KWP2000 FastInit MonoPoint"
        elif self.protocolcombo.currentIndex() == 2:
            protocol = "KWP2000 Init 5 Baud Type I and II"
