...
parser.add_argument(
    "-git_test", "--git_workfallowmode", action="store_true", help="Mode build test's"
)
...
class Ecu_list(widgets.QWidget):
    ...
    def scanselvehicle(self):
        ...
        ecu.addressing = vehicles["projects"][self.vehicle_combo.currentText()][
            "addressing"
        ]
        ...
        for ecu in self.ecuscan.ecu_database.targets:
            if ecu.addr in self.ecuscan.ecu_database.addr_group_mapping:
                ...
                if ecu.addr in self.ecuscan.ecu_database.addr_group_mapping_long:
                    longgroupnames[
                        grp
                    ] = self.ecuscan.ecu_database.addr_group_mapping_long[ecu.addr]
            else:
                grp = "?"
    ...
    def filterProject(self):
        ...
        for root_item in root_items:
            ...
            for item in items:
                if (
                    project.upper() in str(item.text(7)).upper().split("/")
                ) or project == "ALL":
...
class Main_widget(widgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.diagaction = widgets.QAction(
            gui.QIcon("dtt4all_data/icons/dtc.png"), _("Read DTC"), self
        )
        self.diagaction.triggered.connect(self.readDtc)
        self.diagaction.setEnabled(False)

        self.log = widgets.QAction(
            gui.QIcon("dtt4all_data/icons/log.png"), _("Full log"), self
        )
        ...
        self.expert = widgets.QAction(
            gui.QIcon("dtt4all_data/icons/expert.png"),
            _("Expert mode (enable writing)"),
            self,
        )
        ...
        self.autorefresh = widgets.QAction(
            gui.QIcon("dtt4all_data/icons/autorefresh.png"), _("Auto refresh"), self
        )
        ...
        self.refresh = widgets.QAction(
            gui.QIcon("dtt4all_data/icons/refresh.png"), _("Refresh (one shot)"), self
        )
    ...
    def wiki_about(self):
        url = core.QUrl(
            "https://github.com/cedricp/ddt4all/wiki", core.QUrl.TolerantMode
        )
    ...
    def git_update(self):
        url = core.QUrl(
            "https://github.com/cedricp/ddt4all/releases", core.QUrl.TolerantMode
        )
        gui.QDesktopServices().openUrl(url)
    ...
    def zipdb(self):
        filename_tuple = widgets.QFileDialog.getSaveFileName(
            self, _("Save database (keep '.zip' extension)"), "./ecu.zip", "*.zip"
        )
    ...
    def newScreen(self):
        ...
        if not item:
            self.logview.append(
                "<font color=red>"
                + _("Please select a category before creating new screen")
                + "</font>"
            )
            return
    ...
    def scan(self):
        ...
        if scancan:
            self.ecu_scan.scan(
                ...
                self.infostatus,
                None,
                self.canlinecombo.currentIndex(),
            )
        if scankwp:
    ...
    def saveEcus(self):
        filename_tuple = widgets.QFileDialog.getSaveFileName(
            self,
            _("Save vehicule (keep '.ecu' extension)"),
            "./vehicles/mycar.ecu",
            "*.ecu",
        )
    ...
    def newEcu(self):
        filename_tuple = widgets.QFileDialog.getSaveFileName(
            self, _("Save ECU (keep '.json' extension)"), "./json/myecu.json", "*.json"
        )
    ...
    def saveRecord(self):
        ...
        filename_tuple = widgets.QFileDialog.getSaveFileName(
            self, _("Save record (keep '.txt' extension)"), "./record.txt", "*.txt"
        )
        filename = str(filename_tuple[0])
...
class donationWidget(widgets.QLabel):
    ...
    def mousePressEvent(self, mousevent):
        ...
        msgbox.setText(
            _(
                "<center>This Software is free, but I need money to buy cables/ECUs and make this application more reliable</center>"
            )
        )
        okbutton = widgets.QPushButton(_("Yes I contribute"))
        msgbox.addButton(okbutton, widgets.QMessageBox.YesRole)
        msgbox.addButton(
            widgets.QPushButton(_("No, I don't")), widgets.QMessageBox.NoRole
        )
    ...
    def donate(self):
        ...
        msgbox.setText(
            _(
                "<center>Thank you for you contribution, if nothing happens, please go to : https://github.com/cedricp/ddt4all</center>"
            )
        )
        msgbox.exec_()
