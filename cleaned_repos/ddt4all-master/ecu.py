...
addressing_entries = {"E7": "SCRCM", "E8": "SVS"}
...
class Ecu_request:
    ...
    def build_data_stream(self, data):
        ...
        for k, v in data.items():
            if k in self.sendbyte_dataitems:
                ...
                raise KeyError(
                    "Ecurequest::build_data_stream : Data item %s does not exist" % k
                )
            ...
            if k in self.ecu_file.data:
                ...
                raise KeyError(
                    "Ecurequest::build_data_stream : Data %s does not exist" % k
                )
    ...
    def get_values_from_stream(self, stream):
        ...
        for k, v in self.dataitems.items():
            if k in self.ecu_file.data:
                ...
                raise KeyError(
                    "Ecurequest::get_values_from_stream : Data %s does not exist" % k
                )
        return values
    ...
    def get_formatted_sentbytes(self):
        ...
        return [
            str(bytes_to_send_ascii[i : i + 2])
            for i in range(0, len(bytes_to_send_ascii), 2)
        ]
...
class Ecu_data:
    ...
    def init(self, data):
        if isinstance(data, dict):
            ...
            if description:
                self.description = (
                    description.item(0)
                    .firstChild.nodeValue.replace("<![CDATA[", "")
                    .replace("]]>", "")
                )
    ...
    def setValue(self, value, bytes_list, dataitem, ecu_endian, test_mode=False):
        ...
        if self.scaled:
            if not test_mode:
                ...
                value = int(
                    (value * float(self.divideby) - float(self.offset))
                    / float(self.step)
                )
            else:
        ...
        numreqbytes = int(math.ceil(float(self.bitscount + start_bit) / 8.0))
    ...
    def getDisplayValue(self, elm_data, dataitem, ecu_endian):
        ...
        res = (float(value) * float(self.step) + float(self.offset)) / float(
            self.divideby
        )
    ...
    def getHexValue(self, resp, dataitem, ecu_endian):
        ...
        if little_endian:
            ...
            if totalremainingbits != 0:
                print(
                    "getHexValue >> abnormal remaining bytes ", bits, totalremainingbits
                )
...
class Ecu_file:
    def __init__(self, data, isfile=False):
        ...
        if isfile and ".json" in data:
            ...
            target = getChildNodesByName(self.xmldoc, "Target")

            if target:
                self.ecuname = target[0].getAttribute("Name")
                autoidents = getChildNodesByName(target[0], "AutoIdents")
                if autoidents:
                    autoident = getChildNodesByName(autoidents[0], "AutoIdent")
                    for ai in autoident:
                        autoident_dict = {}
                ...
                projects = getChildNodesByName(target[0], "Projects")
                if projects:
                    for project in projects[0].childNodes:
                        self.projects.append(project.nodeName)

                functions = getChildNodesByName(target[0], "Function")
                if functions:
                    self.funcaddr = (
                        hex(int(functions[0].getAttribute("Address")))[2:]
                        .upper()
                        .zfill(2)
                    )
                    self.funcname = functions[0].getAttribute("Name")

                can = getChildNodesByName(target[0], "CAN")
                if can:
                    ...
                    if send_ids:
                        ...
                        if can_id:
                            self.ecu_send_id = hex(
                                int(can_id[0].getAttribute("Value"))
                            )[2:].upper()
                ...
                k = getChildNodesByName(target[0], "K")
                if k:
                    kwp = getChildNodesByName(k[0], "KWP")
                    iso8 = getChildNodesByName(k[0], "ISO8")
                    ...
                    if kwp:
                        ...
                        self.kw1 = hex(
                            int(
                                getChildNodesByName(iso8[0], "KW1")[0].getAttribute(
                                    "Value"
                                )
                            )
                        )[2:].upper()
                        self.kw2 = hex(
                            int(
                                getChildNodesByName(iso8[0], "KW2")[0].getAttribute(
                                    "Value"
                                )
                            )
                        )[2:].upper()
    ...
    def connect_to_hardware(self, canline=0):
        ...
        if self.ecu_protocol == "CAN":
            ...
            if short_addr is None:
                print(
                    "Cannot retrieve functionnal address of ECU %s @ %s"
                    % (self.ecuname, self.ecu_send_id)
                )
                return False
            ecu_conf = {
                "idTx": self.ecu_send_id,
                "idRx": self.ecu_recv_id,
                "ecuname": str(ecuname),
            }
    ...
    def dump_idents(self):
        ...
        for ai in self.autoidents:
            aidict = {
                "diagnostic_version": ai["diagversion"],
                ...
                for target in targets:
                    ...
                    if projectselems:
                        for project in projectselems[0].childNodes:
                            ...
                            if not projname in self.vehiclemap:
                                self.vehiclemap[projname] = []
                            self.vehiclemap[projname].append(
                                (ecu_ident.protocol, address)
                            )
...
class Ecu_scanner:
    ...
    def identify_new(self, addr, label):
        ...
        if options.simulation_mode:
            ...
            if addr == "26":
                can_response = (
                    "62 F1 8A 43 4F 4E 54 49 4E 45 4E 54 41 4C 20 41 55 54 4F 4D 4F 54 49 56 45 20 20 20 20 "
                ...
                can_response = (
                    "62 F1 8A 56 69 73 74 65 6F 6E 5F 4E 61 6D 65 73 74 6F 76 6F 5F 30 39 36 20 20 20 20"
            ...
            if "WRONG" in can_response:
                return False
        supplier = bytes.fromhex(can_response.replace(" ", "")[6:132]).decode(
            "utf8", "ignore"
        )
        ...
        if options.simulation_mode:
            ...
            if addr == "26":
                can_response = (
                    "62 F1 94 31 34 32 36 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 20 20 20 20"
                )
            elif addr == "13":
                ...
                can_response = (
                    "62 F1 94 31 34 32 36 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 FF FF FF FF FF FF"
                )
            elif addr == "62":
                can_response = (
                    "62 F1 94 31 30 30 30 30 30 30 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 FF FF FF FF FF FF"
                )
            elif addr == "01":
                can_response = (
                    "62 F1 94 4E 33 32 52 41 46 30 30 30 31 31 00 00 00 00 00 00"
                )
            elif addr == "04":
        ...
        soft = bytes.fromhex(can_response.replace(" ", "")[6:38]).decode(
            "utf8", "ignore"
        )
        # Check soft version
        if options.simulation_mode:
            ...
            if addr == "26":
                can_response = (
                    "62 F1 95 31 30 30 30 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 20 "
                    "20 20 20 20 20 20 20 20 20"
                )
            elif addr == "13":
                ...
                can_response = (
                    "62 F1 95 30 35 30 31 30 30 30 32 31 37 30 30 FF FF FF FF FF"
                )
            elif addr == "01":
                can_response = "62 F1 95 46 30 37 2F 34 6F 00 00 00 03"
            elif addr == "04":
        ...
        soft_version = bytes.fromhex(can_response.replace(" ", "")[6:38]).decode(
            "utf8", "ignore"
        )
        if diagversion == "":
            return False
    ...
    def scan(self, progress=None, label=None, vehiclefilter=None, canline=0):
        ...
        if vehiclefilter:
            if vehiclefilter in self.ecu_database.vehiclemap:
                for proto, addr in self.ecu_database.vehiclemap[vehiclefilter]:
                    if proto == "CAN" and not addr in project_can_addresses:
        ...
        for addr in list(set(project_can_addresses)):
            ...
            print(
                f"{'Scanning address: ' + addr:<25} ECU: {self.ecu_database.addr_group_mapping[addr]:<30}"
            )
            if not options.simulation_mode:
                options.elm.init_can()
    ...
    def scan_kwp(self, progress=None, label=None, vehiclefilter=None):
        if options.simulation_mode:
            ...
            self.ecus["S2000_Atmo__SoftA3"] = Ecu_ident(
                "004",
                "213",
                "00A5",
                "8300",
        ...
        if vehiclefilter:
            if vehiclefilter in self.ecu_database.vehiclemap:
                for proto, addr in self.ecu_database.vehiclemap[vehiclefilter]:
                    if proto == "KWP2000" and not addr in project_kwp_addresses:
        ...
        for addr in project_kwp_addresses:
            ...
            if not options.simulation_mode:
                options.opt_si = True
                if not options.elm.set_iso_addr(
                    addr,
                    {"idTx": "", "idRx": "", "ecuname": "SCAN", "protocol": "KWP2000"},
                ):
                    continue
                options.elm.start_session_iso("10C0")
                can_response = options.elm.request(
                    req="2180", positive="61", cache=False
                )
            else:
    ...
    def check_ecu(self, can_response, label, addr, protocol):
        if len(can_response) > 59:
            ...
            supplier = bytes.fromhex(can_response[24:32].replace(" ", "")).decode(
                "utf-8"
            )
    ...
    def check_ecu2(self, diagversion, supplier, soft, version, label, addr, protocol):
        ...
        for target in self.ecu_database.targets:
            ...
            if target.checkWith(diagversion, supplier, soft, version, addr):
                ...
                href = target.href
                line = (
                    "<font color='green'>Identified ECU [%s]@%s : %s DIAGVERSION [%s]"
        ...
        if not found_exact and found_approximate:
            ...
            if kept_ecu:
                ...
                line = (
                    "<font color='red'>Found ECU [%s] (not perfect match) :"
        ...
        if not found_exact and not found_approximate:
            line = (
                "<font color='red'>Found ECU [%s] (no relevant ECU file found) :"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Create a zip filesystem of the XMLs",
    )
    parser.add_argument(
        "--testdb", action="store_true", default=None, help="Test ecudatabse loading"
    )
    parser.add_argument(
        "--testecufile",
        action="store_true",
        default=None,
        help="Test ecudatabse loading",
    )
    parser.add_argument(
        "--convertxml",
        action="store_true",
        default=None,
        help="Convert XML file to JSON",
    )
