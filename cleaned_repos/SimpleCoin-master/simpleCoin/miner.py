    ...
    while True:
        ...
        if not proof[0]:
            ...
            NODE_PENDING_TRANSACTIONS = requests.get(
                url=MINER_NODE_URL + "/txion", params={"update": MINER_ADDRESS}
            ).content
            ...
            a.send(BLOCKCHAIN)
            requests.get(
                url=MINER_NODE_URL + "/blocks", params={"update": MINER_ADDRESS}
            )
...
def transaction():
    ...
    if request.method == "POST":
        ...
        if validate_signature(
            new_txion["from"], new_txion["signature"], new_txion["message"]
        ):
...
if __name__ == "__main__":
    ...
    miner_process = Process(
        target=mine, args=(pipe_output, BLOCKCHAIN, NODE_PENDING_TRANSACTIONS)
    )
    miner_process.start()
