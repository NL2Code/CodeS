...
class PEDA(object):
    ...
    def prev_inst(self, address, count=1):
        ...
        for i in range(backward):
            ...
            code = self.execute_redirect(
                "disassemble %s, %s"
                % (to_hex(address - backward + i), to_hex(address + 1))
            )
    ...
    def xrefs(self, search="", filename=None):
        ...
        out = execute_external_command(
            "%s -M intel -z --prefix-address -d '%s' | grep '%s'"
    ...
    def stepuntil(self, inst, mapname=None, depth=None):
        ...
        while True:
            ...
            if status != old_status:
                if (
                    "SIG" in status
                    and status[3:] not in ["TRAP"]
                    and not to_int(status[3:])
                ):  # ignore TRAP and numbered signals
    ...
    def writemem(self, address, buf):
        ...
        if not out:  # try the slow way
            for i in range(len(buf)):
                if not self.execute(
                    "set {char}0x%x = 0x%x" % (address + i, ord(buf[i]))
                ):
                    return i
            return i + 1
    ...
    def search_address(self, searchfor="stack", belongto="binary"):
        ...
        step = self.intsize()
        for start, end, _, _ in searchfor_ranges[
            ::-1
        ]:  # dirty trick, to search in rw-p mem first
            mem = self.dumpmem(start, end)
            if not mem:
                continue
            for i in range(0, len(mem), step):
                search = "0x" + codecs.encode(mem[i : i + step][::-1], "hex").decode(
                    "utf-8"
                )
                addr = to_int(search)
    ...
    def examine_mem_value(self, value):
        ...
        def examine_data(value, bits=32):
            out = self.execute_redirect(
                "x/%sx 0x%x" % ("g" if bits == 64 else "w", value)
            )
            if out:
                v = out.split(":\t")[-1].strip()
    ...
    def examine_mem_reference(self, value, depth=5):
        ...
        while vn is not None:
            ...
            if to_int(vn) is None:
                break
            if to_int(vn) in [
                to_int(v) for (v, _, _) in result
            ]:  # point back to previous value
                break
    ...
    def format_search_result(self, result, display=256):
        ...
        if not result:
            ...
            for addr, v in result[:count]:
                ...
                text += "%s : %s" % (
                    vmrange[3].rjust(maxlen),
                    format_reference_chain(chain) + "\n",
                )
    ...
    def readelf_header(self, filename, name=None):
        ...
        for hname, start, size, attr in matches:
            start, end = to_int("0x" + start), to_int("0x" + start) + to_int(
                "0x" + size
            )
    ...
    def elfheader_solib(self, solib=None, name=None):
        ...
        def _elfheader_solib_all():
            ...
            for start, end, hname, libname in soheaders:
                ...
                result += [
                    (start, end, hname, os.path.realpath(libname))
    ...
    def checksec(self, filename=None):
        ...
        out = execute_external_command(
            '%s -W -a "%s" 2>&1' % (config.READELF, filename)
        )
        if "Error:" in out:
            return None
    ...
    def search_asm(self, start, end, asmcode, rop=0):
        ...
        for machine_code in buildcode():
            ...
            search = (
                search.replace(decode_hex_escape(b"dead"), b"..")
        ...
        if rop:
            ...
            for a, v in candidates:
                ...
                if gadget != []:
                    ...
                    if (
                        re.search(
                            re.escape(asmcode).replace("\ ", ".*").replace("\?", ".*"),
                            asmcode_rs,
                        )
                        and a not in result
                    ):
            ...
            result = []
            for a, v in candidates:
                asmcode = self.execute_redirect(
                    "disassemble 0x%x, 0x%x" % (a, a + (len(v) // 2))
                )
                if asmcode:
    ...
    def common_rop_gadget(self, mapname=None):
        ...
        for start, end, _, _ in maps:
            if not self.is_executable(start, maps):
                continue
    ...
    def payload_copybytes(self, target=None, data=None, template=0):
        ...
        if "ncp" in transfer or "mem" in transfer:  # memcpy() style
            function_template += "\n".join(
                [
                    "        payload += [%s, pop3ret, target+offset, addr, len(str)]"
                    % plt_func,
                    "        offset += len(str)",
                ]
            )
            ...
            function_template += "\n".join(
                [
                    "        payload += [%s, pop3ret, target+offset, len(str)+1, addr]"
                    % plt_func,
                    "        offset += len(str)",
                ]
            )
...
class PEDACmd(object):
    ...
    def __init__(self):
        ...
        self.commands = [
            c for c in dir(self) if callable(getattr(self, c)) and not c.startswith("_")
        ]
    ...
    def _get_helptext(self, *arg):
        ...
        if cmd is None:
            helptext = (
                red("PEDA", "bold")
                + blue(" - Python Exploit Development Assistance for GDB", "bold")
                + "\n"
            )
            ...
            for cmd in self.commands:
                if cmd.startswith("_"):
                    continue  # skip internal use commands
                func = getattr(self, cmd)
                helptext += "%s -- %s\n" % (
                    cmd,
                    green(trim(func.__doc__.strip("\n").splitlines()[0])),
                )
    ...
    def pyhelp(self, *arg):
        ...
        peda_methods = [
            "%s" % c
            for c in dir(PEDA)
            if callable(getattr(PEDA, c)) and not c.startswith("_")
        ]
    ...
    def hexprint(self, *arg):
        ...
        if bytes_ is None:
            ...
            while hexstr:
                text += '%s : "%s"\n' % (
                    blue(to_address(address + i * linelen)),
                    hexstr[: linelen * 4],
                )
    ...
    def aslr(self, *arg):
        ...
        if option is None:
            ...
            if option in ["on", "off"]:
                peda.execute(
                    "set disable-randomization %s" % ("off" if option == "on" else "on")
                )
    ...
    def distance(self, *arg):
        ...
        text += "%d bytes, %d dwords%s" % (
            dist,
            dist // 4,
            " (+%d bytes)" % (dist % 4) if (dist % 4 != 0) else "",
        )
        msg(text)
    ...
    def procinfo(self, *arg):
        ...
        for fd in fdlist:
            ...
            if sock:
                spath = execute_external_command(
                    "netstat -aen | grep %s" % sock.group(1)
                )
                if spath:
                    rpath = spath.strip()
    ...
    def waitfor(self, *arg):
        ...
        if name is None:
            ...
            if filename is None:
                warning_msg(
                    "please specify the file to debug or process name to attach"
                )
                return
            else:
    ...
    def tracecall(self, *arg):
        ...
        peda.save_user_command("hook-stop")  # disable hook-stop to speedup
        msg(
            "Tracing calls %s '%s', Ctrl-C to stop..."
            % ("match" if not inverse else "not match", ",".join(fnames))
        )
    ...
    def profile(self, *arg):
        ...
        peda.save_user_command("hook-stop")  # disable hook-stop to speedup
        msg(
            "Stepping %s instructions, Ctrl-C to stop..."
            % ("%d" % count if count else "all")
        )
    ...
    def vmmap(self, *arg):
        ...
        if maps is not None and len(maps) > 0:
            ...
            msg(
                "%s %s %s\t%s"
                % ("Start".ljust(l, " "), "End".ljust(l, " "), "Perm", "Name"),
                "blue",
                "bold",
            )
            for start, end, perm, name in maps:
                ...
                msg(
                    "%s %s %s\t%s"
                    % (
                        to_address(start).ljust(l, " "),
                        to_address(end).ljust(l, " "),
                        perm,
                        name,
                    ),
                    color,
    ...
    def patch(self, *arg):
        ...
        if bytes_ >= 0:
            ...
            warning_msg(
                "Failed to patch memory, try 'set write on' first for offline patching"
            )
        return
    ...
    def lookup(self, *arg):
        ...
        if option == "pointer":
            msg(
                "Searching for pointers on: %s pointed to: %s, this may take minutes to complete..."
                % (searchfor, belongto)
            )
            result = peda.search_pointer(searchfor, belongto)
        if option == "address":
            msg(
                "Searching for addresses on: %s belong to: %s, this may take minutes to complete..."
                % (searchfor, belongto)
            )
    ...
    def telescope(self, *arg):
        ...
        if not peda.is_address(address):  # cannot determine address
            ...
            for i in range(count):
                if not peda.execute(
                    "x/%sx 0x%x" % ("g" if step == 8 else "w", address + i * step)
                ):
                    break
            return
    ...
    def strings(self, *arg):
        ...
        for start, end, _, _ in maps:
            ...
            for m in found:
                text += "0x%x: %s\n" % (
                    start + m.start(),
                    string_repr(mem[m.start() : m.end()].strip(), show_quotes=False),
                )
    ...
    def elfheader(self, *arg):
        ...
        if len(result) == 0:
            warning_msg(
                "%s not found, did you specify the FILE to debug?"
                % (name if name else "headers")
            )
        elif len(result) == 1:
    ...
    def asmsearch(self, *arg):
        ...
        if end is None:
            ...
            for start, end, _, _ in maps:
                if not peda.is_executable(start, maps):
                    continue  # skip non-executable page
                result += peda.search_asm(start, end, asmcode)
        else:
            msg(
                "Searching for ASM code: %s in range: 0x%x - 0x%x"
                % (repr(asmcode), start, end)
            )
            result = peda.search_asm(start, end, asmcode)
    ...
    def ropsearch(self, *arg):
        ...
        if end is None:
            ...
            msg(
                "Searching for ROP gadget: %s in range: 0x%x - 0x%x"
                % (repr(asmcode), start, end)
            )
            result = peda.search_asm(start, end, asmcode, rop=1)
    ...
    def pattern_create(self, *arg):
        ...
        if filename is not None:
            ...
            msg(
                'Writing pattern of %d chars to filename "%s"'
                % (len(pattern), filename)
            )
        else:
    ...
    def pattern_search(self, *arg):
        ...
        for start, end, perm, name in maps:
            if "w" not in perm:
                continue  # only search in writable memory
    ...
    def pattern_env(self, *arg):
        ...
        peda.execute(
            "set env %s %s" % (env, cyclic_pattern(size, offset).decode("utf-8"))
        )
    ...
    def substr(self, *arg):
        ...
        if mapname:
            msg(
                "Searching for sub strings of: %s in: %s ranges"
                % (repr(search), mapname)
            )
            maps = peda.get_vmmap(mapname)
            for start, end, perm, _ in maps:
            ...
            msg(
                "Searching for sub strings of: %s in range: 0x%x - 0x%x"
                % (repr(search), start, end)
            )
            result = peda.search_substr(start, end, search)

        if result:
            msg(
                "# (address, target_offset), # value (address=0xffffffff means not found)"
            )
            offset = 0
            for k, v in result:
                msg(
                    "(0x%x, %d), # %s"
                    % ((0xFFFFFFFF if v == -1 else v), offset, string_repr(k))
                )
                offset += len(k)
        else:
            msg("Not found")
    ...
    def shellcode(self, *arg):
        ...
        if mode == "generate":
            ...
            if sctype in ["bindport", "connect"]:
                text += "# port=%s, host=%s\n" % (
                    port if port else "16706",
                    host if host else "127.127.127.127",
                )
            text += "shellcode = (\n"
            while hexstr:
                text += '    "%s"\n' % (hexstr[: linelen * 4])
