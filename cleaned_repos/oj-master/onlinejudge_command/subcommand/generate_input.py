...
""",
    ...
    subparser.add_argument(
        "-t", "--tle", type=float, help="set the time limit (in second) (default: inf)"
    )
    ...
    subparser.add_argument(
        ...
        type=int,
        default=3,
        help="specify the width of indices of cases. (default: 3)",
    )
    subparser.add_argument(
        "--name", help='specify the base name of cases. (default: "random")'
    )
    subparser.add_argument(
        "-c", "--command", help="specify your solution to generate output"
    )
    subparser.add_argument(
        "--hack-expected",
        dest="command",
        help="alias of --command. If this is not given, --hack runs until the actual solution fails with RE or TLE.",
    )
    subparser.add_argument(
        "--hack",
        "--hack-actual",
        dest="hack",
        help="specify your wrong solution to be compared with the reference solution given by --hack-expected",
    )
    subparser.add_argument(
        "generator", type=str, help="your program to generate test cases"
    )
    subparser.add_argument(
        "count",
        nargs="?",
        type=int,
        help="the number of cases to generate (default: 100)",
    )
    ...
    with lock or nullcontext:
        ...
        if print_data:
            ...
            logger.info(
                ...
                pretty_printers.make_pretty_large_file_content(
                    input_data, limit=40, head=20, tail=10
                ),
            )
        ...
        if output_data is not None:
            if print_data:
                ...
                logger.info(
                    pretty_printers.make_pretty_large_file_content(
                        output_data, limit=40, head=20, tail=10
                    )
                )
...
def check_randomness_of_generator(
    input_data: bytes,
    *,
    name: str,
    ...
    with lock or nullcontext:
        if len(generated_input_hashes) < limit:
            if input_digest in generated_input_hashes:
                ...
                if len(generated_input_hashes) == limit:
                    logger.info(
                        "Conflict checking of generated inputs is disabled now because it seems the given input generator has enough randomness."
                    )  # This prints a log line but it's safe because here is in a lock.
    return None
...
def run(args: argparse.Namespace) -> None:
    if args.hack and not args.command:
        logger.info(
            "--hack-actual is given but --hack-expected is not given. It will run until the actual solution gets RE or TLE."
        )
    ...
    def iterate_path():
        for i in itertools.count():
            ...
            input_path = fmtutils.path_from_format(
                args.directory, args.format, name=name, ext="in"
            )
            output_path = fmtutils.path_from_format(
                args.directory, args.format, name=name, ext="out"
            )
    ...
    if args.jobs is None:
        for name, input_path, output_path in itertools.islice(
            iterate_path(), args.count
        ):
            if not args.hack:
                # generate serially
                generate_input_single_case(
                    args.generator,
                    input_path=input_path,
                    output_path=output_path,
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as executor:
            ...
            if not args.hack:
                ...
                for _, input_path, output_path in itertools.islice(
                    iterate_path(), args.count
                ):
                    data = None
                    while data is None:
