...
""",
    ...
    subparser.add_argument(
        "url",
        nargs="?",
        help="the URL of the problem to submit. if not given, guessed from history of download command.",
    )
    ...
    with utils.new_session_with_our_user_agent(path=args.cookie) as sess:
        ...
        try:
            ...
            if is_logged_in:
                ...
                logger.error(
                    "You are not logged in. Please run $ oj login %s", problem.get_url()
                )
                return False

        # guess or select language ids
        language_dict: Dict[LanguageId, str] = {
        ...
        if guessed_unmatch:
            samples_text = (
                'samples of "{}'.format('", "'.join(guessed_urls))
                if guessed_urls
                else "no samples"
            )
            logger.warning(
        ...
        if not args.yes:
            if guessed_unmatch:
                problem_id = (
                    problem.get_url().rstrip("/").split("/")[-1].split("?")[-1]
                )  # this is too ad-hoc
        ...
        # submit
        try:
            submission = problem.submit_code(
                code, language_id=LanguageId(args.language), session=sess
            )
...
def guess_lang_ids_of_file(
    ...
    if ext in ("cpp", "cxx", "cc", "C"):
        ...
        lang_ids = list(
            filter(
                lambda lang_id: is_cplusplus_description(language_dict[lang_id]),
                lang_ids,
            )
        )
        if not lang_ids:
        ...
        if found_gcc and found_clang:
            ...
            if cxx_compiler == "gcc":
                ...
                lang_ids = list(
                    filter(
                        lambda lang_id: parse_cplusplus_compiler(language_dict[lang_id])
                        in ("gcc", None),
                        lang_ids,
                    )
                )
        ...
        if cxx_latest:
            ...
            for compiler in ("gcc", "clang"):  # use the latest for each compiler
                ...
                ids.sort(
                    key=lambda lang_id: (
                        parse_cplusplus_version(language_dict[lang_id]) or "",
                        language_dict[lang_id],
                    )
                )
        ...
        lang_ids = list(
            filter(
                lambda lang_id: is_python_description(language_dict[lang_id]), lang_ids
            )
        )
        if any(
            parse_python_interpreter(language_dict[lang_id]) == "pypy"
            for lang_id in lang_ids
        ):
        ...
        if python_interpreter != "all":
            lang_ids = list(
                filter(
                    lambda lang_id: parse_python_interpreter(language_dict[lang_id])
                    == python_interpreter,
                    lang_ids,
                )
            )
        ...
        if two_found and three_found:
            ...
            if python_version in ("2", "3"):
                ...
                for version in (2, 3):
                    if re.search(
                        r"python *(version:? *)?%d".encode() % version, s.lower()
                    ):
                        versions += [version]
                if not versions:
            ...
            lang_ids = list(
                filter(
                    lambda lang_id: parse_python_version(language_dict[lang_id])
                    in versions + [None],
                    lang_ids,
                )
            )
        ...
        for data in table:
            if ext in data["exts"]:
                for name in data["names"]:
                    lang_ids += select_ids_of_matched_languages(
                        [name],
                        language_dict.keys(),
                        language_dict=language_dict,
