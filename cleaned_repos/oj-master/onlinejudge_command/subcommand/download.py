...
""",
    ...
    subparser.add_argument(
        "-f",
        "--format",
        help='a format string to specify paths of cases (default: "sample-%%i.%%e" if not --system)',
    ...
    )  # default must be None for guessing in submit command
    subparser.add_argument(
...
def run(args: argparse.Namespace) -> bool:
    ...
    if problem is None:
        ...
        return False
    is_default_format = (
        args.format is None and args.directory is None
    )  # must be here since args.directory and args.format are overwritten
    ...
    def iterate_files_to_write(
        ...
        for ext in ["in", "out"]:
            ...
            path: pathlib.Path = args.directory / format_utils.percentformat(
                args.format, table
            )
            yield ext, path, data
