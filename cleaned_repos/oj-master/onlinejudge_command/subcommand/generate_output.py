...
def add_subparser(subparsers: argparse.Action) -> None:
    ...
    subparser = subparsers_add_parser(
        "generate-output",
        aliases=["g/o"],
        help="generate output files from input and reference implementation",
...
def generate_output_single_case(
    test_name: str,
    ...
    *,
    ...
    with lock or nullcontext:
        ...
        if proc.returncode is None:
            ...
            logger.info(
                "FIALURE: " + utils.red("RE") + ": return code %d", proc.returncode
            )
            logger.info("skipped.")
            return
        ...
        if match_result is not None:
            ...
            assert False
        test_output_path = fmtutils.path_from_format(
            args.directory, args.format, name=matched_name, ext="out"
        )
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.jobs is None:
        for name, paths in sorted(tests.items()):
            generate_output_single_case_exists_ok(
