...
def run_program(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    if args.version:
        print(
            "online-judge-tools {} (+ online-judge-api-client {})".format(
    ...
    logger.info(
        "online-judge-tools %s (+ online-judge-api-client %s)",
        version.__version__,
...
def main(args: Optional[List[str]] = None) -> "NoReturn":
    ...
    try:
        ...
        if not is_updated:
            logger.info(
                utils.HINT
                + "try updating the version of online-judge-tools: $ pip3 install -U online-judge-tools online-judge-api-client"
            )
        sys.exit(1)
