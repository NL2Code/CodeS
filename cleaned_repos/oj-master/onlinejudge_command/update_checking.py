...
def get_latest_version_from_pypi(package_name: str) -> str:
    ...
    if version_cache_path.exists():
        try:
            logger.debug(
                "load the cache for update checking: %s", str(version_cache_path)
            )
...
def run() -> bool:
    ...
    try:
        is_updated = run_for_package(
            package_name=version.__package_name__, current_version=version.__version__
        )
