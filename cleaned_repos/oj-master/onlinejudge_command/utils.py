...
def webbrowser_register_explorer_exe() -> None:
    ...
    if sys.version_info < (3, 7):
        webbrowser.register(
            "explorer", None, instance
        )  # TODO: remove this after Python 3.6 supprot is finished
    else:
        webbrowser.register(
            "explorer", None, instance, preferred=True
        )  # `preferred=True` solves the issue that terminal logs are cleared on cmd.exe with stopping using wslview via www-browser. TODO: remove `preferred=True` after https://github.com/wslutilities/wslu/issues/199 is fixed.
