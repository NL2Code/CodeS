...
def percentparse(
    s: str, format: str, table: Dict[str, str]
...
def match_with_format(
    directory: pathlib.Path, format: str, path: pathlib.Path
) -> Optional[Match[str]]:
    if os.name == "nt":
        format = format.replace("/", "\\")
    table = {}
    ...
    pattern = re.compile(
        ...
        + percentformat(re.escape(format).replace(re.escape("%"), "%"), table)
    )
    return pattern.match(str(path.resolve()))


def path_from_format(
    directory: pathlib.Path, format: str, name: str, ext: str
) -> pathlib.Path:
    table = {}
    table["s"] = name
    table["e"] = ext
...
def construct_relationship_of_files(
    paths: List[pathlib.Path], directory: pathlib.Path, format: str
