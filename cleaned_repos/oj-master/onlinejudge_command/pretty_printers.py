...
def _make_diff_between_line_and_line_by_comparing_word_by_word(
    a: str, b: str
...
def _tokenize_str_with_highlight(s: str, *, is_right: bool) -> List[_PrettyToken]:
    ...
    for token in _tokenize_str(s):
        if token.type == _PrettyTokenType.BODY:
            typ = (
                _PrettyTokenType.BODY_HIGHLIGHT_RIGHT
                if is_right
                else _PrettyTokenType.BODY_HIGHLIGHT_LEFT
            )
...
def _make_diff_between_line_and_line_by_difflib(
    a: str, b: str
) -> Tuple[List[_PrettyToken], List[_PrettyToken]]:
    tokens_a = []
    tokens_b = []
    ...
    if len(a.rstrip("\n")) < len(a):
        tokens_a.append(
            _PrettyToken(_PrettyTokenType.NEWLINE, a[len(a.rstrip("\n")) :])
        )
    if len(b.rstrip("\n")) < len(b):
        tokens_b.append(
            _PrettyToken(_PrettyTokenType.NEWLINE, b[len(b.rstrip("\n")) :])
        )
...
def _make_diff_between_line_and_line(
    a: str, b: str
) -> Tuple[List[_PrettyToken], List[_PrettyToken]]:
...
def _make_diff_between_file_and_file_by_comparing_line_by_line(
    a: str, b: str, *, compare_mode: CompareMode
) -> List[_LineDiffOp]:
    ...
    while i < min(len(lines_a), len(lines_b)):
        if not check_lines_match(lines_a[i], lines_b[i], compare_mode=compare_mode):
            tokens_a, tokens_b = _make_diff_between_line_and_line(
                lines_a[i], lines_b[i]
            )
...
def _make_diff_between_file_and_file(
    a: str, b: str, *, compare_mode: CompareMode
) -> List[_LineDiffOp]:
    assert compare_mode != CompareMode.IGNORE_SPACES_AND_NEWLINES
...
_MergedDiffOpDots = _MergedDiffOp(
    None, [], None, [], False
)  # This represents insertion of "...".


def _reconstruct_entire_diff(
    a: str, b: str, *, ops: List[_LineDiffOp]
) -> List[_MergedDiffOp]:
    lines_a = a.splitlines(keepends=True)
    lines_b = b.splitlines(keepends=True)
    i_a = 0
    ...
    while i_a < len(lines_a) and i_b < len(lines_b):
        if (
            stk
            and stk[-1].left is not None
            and stk[-1].right is not None
            and i_a == stk[-1].lineno
        ):
            ...
            stk
            and stk[-1].left is None
            and stk[-1].right is not None
            and i_b == stk[-1].lineno
        ):
    ...
    while stk:
        if stk[-1].left is not None and stk[-1].right is None and i_a == stk[-1].lineno:
            ...
            stk[-1].left is None and stk[-1].right is not None and i_b == stk[-1].lineno
        ):
            result.append(_MergedDiffOp(None, [], i_b, stk[-1].right, True))
            stk.pop()
            i_b += 1
...
def _add_lines_around_diff_lines(
    a: str, b: str, *, ops: List[_LineDiffOp], size: int
) -> List[_MergedDiffOp]:
...
def _add_dots_between_gaps(
    a: str, b: str, *, ops: List[_MergedDiffOp]
) -> List[_MergedDiffOp]:
    result: List[_MergedDiffOp] = []
...
def _tokens_from_line_diff_ops(
    ...
    tokens = []
    tokens.append(
        _PrettyToken(_PrettyTokenType.OTHERS, "output:".ljust(left_width) + "expected:")
    )
    tokens.append(_PrettyToken(_PrettyTokenType.NEWLINE, "\n"))
    for op in ops:
        if op == _MergedDiffOpDots:
            tokens.append(
                _PrettyToken(_PrettyTokenType.OTHERS, "...".ljust(left_width) + "...")
            )
            tokens.append(_PrettyToken(_PrettyTokenType.NEWLINE, "\n"))
            continue
        left_exists = False
        if op.left_lineno is not None:
            tokens.append(
                _PrettyToken(
                    _PrettyTokenType.LINENO,
                    str(op.left_lineno + 1).rjust(left_lineno_width),
                )
            )
            tokens.append(_PrettyToken(_PrettyTokenType.OTHERS, "| "))
            tokens.extend(
                [
                    _PrettyToken(token.type, token.value.replace("\n", ""))
        ...
        if op.right_lineno is not None:
            ...
            tokens.append(
                _PrettyToken(
                    _PrettyTokenType.LINENO,
                    str(op.right_lineno + 1).rjust(right_lineno_width),
                )
            )
            tokens.append(_PrettyToken(_PrettyTokenType.OTHERS, "| "))
...
def _tokenize_pretty_diff(
    ...
    *,
    ...
    if compare_mode == CompareMode.IGNORE_SPACES_AND_NEWLINES:
        logger.warning(
            "ignoring --compare-mode=%s and using --compare-mode=%s instead for generating diff...",
            str(compare_mode),
            str(CompareMode.IGNORE_SPACES),
        )
    ...
    tokens = _tokens_from_line_diff_ops(
        merged_ops[:limit] if limit != -1 else merged_ops, char_in_line=char_in_line
    )
    if limit != -1:
...
def make_pretty_diff(
    output_bytes: bytes, *, expected: str, compare_mode: CompareMode, limit: int
) -> str:
