...
def analyze_file(_file, result, settings):
    try:
        bad_expressions = (
            settings.bad_expressions if settings.bad_expressions else BAD_EXPRESSIONS
        )
        if bad_expressions:
            if bad_expression_verifier(_file, bad_expressions):
                logger.info(
                    "Bad expression has been found in a "
                    + _file
                ...
                with open(_file) as f:
                    for line in f:
                        for password in password_search(line, settings):
                            ...
                            result.put(data)
                            logger.info(
                                "potential password has been found in a file " + _file
                            )

            except Exception as e:
                logger.error(
                    "while trying to open " + str(_file) + ". Details:\n" + str(e)
                )
...
def found_high_entropy(_file, word, result, entropy):
    ...
    if (b64Entropy > entropy) and false_positive_filter(word):
        ...
        data = {
            "Finding": "High entropy",
            "File": _file,
            "Details": {"Entropy": b64Entropy, "String": word},
        }
        result.put(data)
        return True
    return False
...
def folder_reader(queue, settings):
    try:
        excluded_files = (
            settings.exclude_files if settings.exclude_files else EXCLUDED_FILES
        )
...
def password_search(line, settings):
    try:
        ...
        for string in potential_pass_list:
            if (not min_pass <= len(string) <= max_pass) or any(
                ch.isspace() for ch in string
            ):
                continue
...
def bad_expression_verifier(_file, bad_expressions):
    try:
        with open(_file, "rb", 0) as f, mmap.mmap(
        ...
        logger.error(
            "while trying to open " + str(_file) + " file. Details:\n" + str(e)
        )
