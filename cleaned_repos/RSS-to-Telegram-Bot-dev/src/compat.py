...
def ssl_create_default_context():
    ...
    if (
        _version_info[1] >= 10
    ):  # Python 3.10+ also disabled TLS 1.1, here we only care about cipher
...
def _parsing_utils_html_validator_minify_preprocess(
    html: str, drop_sr_only: bool
) -> str:
...
def parsing_utils_html_validator_minify(html: str) -> str:
    ...
    if (
        minify_html_onepass
        is None  # requires minify-html-onepass to workaround upstream issue
        or contains_sr_only  # clear sr-only first, otherwise minify-html cannot strip spaces around them
    ):
    ...
    if minify_html_onepass is not None:
        try:
            ...
            if not preprocessed:
                html = _parsing_utils_html_validator_minify_preprocess(
                    html, contains_sr_only
                )
...
class OpmlMixin(listparser.opml.OpmlMixin):
    ...
    def start_opml_outline(self, attrs):
        ...
        if url not in self.found_urls and append_to:
            ...
            obj = listparser.common.SuperDict(
                {"url": url, "title": title, "text": text, "title_orig": title_orig}
            )
