...
__configure_logging(
    level=colorlog.DEBUG if __bool_parser(os.environ.get("DEBUG")) else colorlog.INFO
)
...
# ----- load .env -----
dot_env_paths = (
    os.path.join(config_folder_path, ".env"),
    os.path.join(os.path.abspath("."), ".env"),
)
if is_self_run_as_a_whole_package:
    dot_env_paths = (
        os.path.normpath(os.path.join(self_path, "..", ".env")),
    ) + dot_env_paths
...
if is_self_run_as_a_whole_package:
    ...
    try:
        with open(
            os.path.normpath(os.path.join(self_path, "..", ".version")), "r"
        ) as v:
...
if _version_match:
    ...
    _version = f"v{__version__}" + (
        f"-{_version}" if _version and _version != "dirty" else ""
    )
...
try:
    ...
    _chatid = (
        int(_chatid)
        if isinstance(_chatid, str) and _chatid.lstrip("-").isdecimal()
        else _chatid
    )
    _manager = os.environ.get("MANAGER") or _chatid
    MANAGER: Final = (
        int(_manager)
        if isinstance(_manager, str) and _manager.lstrip("-").isdecimal()
        else _manager
    )
    del _chatid
    del _manager
...
HTTP_CONCURRENCY_PER_HOST: Final = int(
    os.environ.get("HTTP_CONCURRENCY_PER_HOST") or 16
)

# ----- img relay server config -----
_img_relay_server = (
    os.environ.get("IMG_RELAY_SERVER")
    or "https://rsstt-img-relay.rongrong.workers.dev/"
)
...
# ----- db config -----
_database_url = (
    os.environ.get("DATABASE_URL") or f"sqlite://{config_folder_path}/db.sqlite3"
)
DATABASE_URL: Final = (
    _database_url.replace("postgresql", "postgres", 1)
...
EXECUTOR_NICENESS_INCREMENT: Final = int(
    os.environ.get("EXECUTOR_NICENESS_INCREMENT") or (2 * hasattr(os, "nice"))
)
DEBUG: Final = __bool_parser(os.environ.get("DEBUG"))
__configure_logging(  # config twice to make .env file work
    level=colorlog.DEBUG if DEBUG else colorlog.INFO, force=True
)
if DEBUG:
...
if os.environ.get("DELAY"):
    logger.warning(
        'Env var "DELAY" is DEPRECATED and of no use!\n'
        "To avoid this warning, remove this env var."
    )

if os.environ.get("CHATID"):
    logger.warning(
        'Env var "CHATID" is DEPRECATED!\n'
        "To avoid this warning, remove this env var."
    )

if any(
    (
        os.environ.get("REDISHOST"),
        os.environ.get("REDISUSER"),
        os.environ.get("REDISPASSWORD"),
        os.environ.get("REDISPORT"),
        os.environ.get("REDIS_NUM"),
    )
):
    logger.warning(
        "Redis DB is DEPRECATED!\n"
...
if is_self_run_as_a_whole_package and os.path.exists(
    os.path.join(config_folder_path, "rss.db")
):
    os.rename(
        os.path.join(config_folder_path, "rss.db"),
        os.path.join(config_folder_path, "rss.db.bak"),
    )
    logger.warning(
        'Sqlite DB "rss.db" with old schema is DEPRECATED and renamed to "rss.db.bak" automatically!\n'
        "ALL SUBS IN THE OLD DB WILL NOT BE MIGRATED. "
        'IF YOU NEED TO BACKUP YOUR SUBS, DOWNGRADE AND USE "/export" COMMAND TO BACKUP.'
    )
