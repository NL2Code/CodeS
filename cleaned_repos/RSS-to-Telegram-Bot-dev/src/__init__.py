...
from . import aio_helper, env, log
...
async def lazy():
    ...
    manager_lang: Optional[str] = await db.User.get_or_none(id=env.MANAGER).values_list(
        "lang", flat=True
    )
    ...
    set_bot_commands_tasks = (
        ...
        *(
        ...
        loop.create_task(
            command.utils.set_bot_commands(
                scope=types.BotCommandScopePeer(types.InputPeerUser(env.MANAGER, 0)),
                lang_code="",
...
def main():
    ...
    try:
        ...
        scheduler.add_job(
            func=command.monitor.run_monitor_task,
