...
from .utils import WebResponse, YummyCookieJar, logger, proxy_filter, sentinel

DEFAULT_READ_BUFFER_SIZE: Final = 2**16

PROXY: Final = (
    env.R_PROXY.replace("socks5h", "socks5").replace("sock4a", "socks4")
    if env.R_PROXY
    else None
)
...
async def __norm_callback(
    ...
    if (
        ...
        if decode and body:
            ...
            if (
                xml_header.startswith(b"<?xml")
                and b"?>" in xml_header
                and b"encoding" in xml_header
            ):
...
async def get(
    ...
    return await _get(
        ...
        semaphore=semaphore,
        headers=headers,
        resp_callback=partial(
            __norm_callback,
            decode=decode,
            max_size=max_size,
            intended_content_type=intended_content_type,
        ),
        read_bufsize=min(max_size, DEFAULT_READ_BUFFER_SIZE)
...
async def _get(
    ...
    semaphore_to_use = (
        ...
        if semaphore in (None, True)
        else (semaphore or nullcontext())
    )
    v6_rr_set = None
    try:
        v6_rr_set = (
            (await asyncio.wait_for(resolve(host, "AAAA", lifetime=1), 1.1)).rrset
            if env.IPV6_PRIOR
            else None
        )
    except DNSException:
    ...
    async def _fetch():
        async with aiohttp.ClientSession(
            ...
            async with session.get(
                url, read_bufsize=read_bufsize, read_until_eof=read_until_eof
            ) as response:
        ...
        for (status0, url_obj0), (status1, url_obj1) in zip(
            status_url_history, status_url_history[1:]
        ):
            if not (
                status0 in STATUSES_PERMANENT_REDIRECT
                or url_obj0.with_scheme("https") == url_obj1
            ):
                break  # fail fast
        ...
        if auth_header := response.request_info.headers.get("Authorization"):
            ...
            url_obj = url_obj.with_user(auth.login or None).with_password(
    ...
    while tries < max_tries:
        ...
        try:
            ...
            if isinstance(e, RetryInIpv4):
                ...
                err_msg = str(e).strip()
                e = RetryInIpv4(
                    reason=f"{type(e).__name__}" + (f": {err_msg}" if err_msg else "")
                )
...
async def get_page_title(
    url: str, allow_hostname=True, allow_path: bool = False, allow_filename: bool = True
) -> Optional[str]:
    r = None
    # noinspection PyBroadException
    try:
        r = await get(
            url=url, decode=False, intended_content_type="text/html", max_size=2 * 1024
        )
        ...
        filename_match = (
            contentDispositionFilenameParser(content_disposition)
