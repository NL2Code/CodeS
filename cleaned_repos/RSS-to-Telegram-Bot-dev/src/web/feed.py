...
from .req import get
from .utils import WebError, WebFeed, sentinel
    ...
    try:
        ...
        if not rss_d.feed.get("title"):  # why there is no feed hospital?
            ...
            if not rss_d.entries and (
                rss_d.bozo or not (rss_d.feed.get("link") or rss_d.feed.get("updated"))
            ):
                ret.error = WebError(
                    error_name="feed invalid", url=resp.url, log_level=log_level
                )
                return ret
            rss_d.feed[
                "title"
                ...
                resp.url
            )  # instead of `rss_d.feed.title = resp.url`, which does not affect the dict
        ...
        ret.error = WebError(
            error_name="internal error", url=url, base_error=e, log_level=log.ERROR
        )
