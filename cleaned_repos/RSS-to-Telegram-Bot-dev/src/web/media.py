...
async def __medium_info_callback(response: aiohttp.ClientResponse) -> tuple[int, int]:
    ...
    preloaded_length = (
        content.total_bytes
    )  # part of response body already came with the response headers
    ...
    if "svg" in content_type:  # svg
        return -1, -1
    if (
        "webp" in content_type or "application" in content_type
    ):  # webp or other binary files
    ...
    with BytesIO() as buffer:
        while not exit_flag:
            ...
            while (
                ...
                or curr_chunk_length < IMAGE_ITER_CHUNK_SIZE
            ):
                if content.is_eof():
                ...
                if is_jpeg:
                    ...
                    for _ in range(3):
                        ...
                        if pointer + 9 <= len(file_header):
                            if (
                                file_header.count(EOI, 0, pointer)
                                != file_header.count(SOI, 0, pointer) - 1
                            ):
...
async def get_medium_info(url: str) -> Optional[tuple[int, int, int, Optional[str]]]:
    ...
    try:
        r = await _get(
            url,
            resp_callback=__medium_info_callback,
            read_bufsize=IMAGE_READ_BUFFER_SIZE,
