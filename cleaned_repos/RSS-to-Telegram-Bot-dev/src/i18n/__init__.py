...
ALL_LANGUAGES = tuple(
    sorted(lang[:-5] for lang in listdir(I18N_PATH) if lang.endswith(".json"))
)
...
REQUIRED_KEYS = {
    istr("lang_code"),
    istr("lang_native_name"),
    istr("select_lang_prompt"),
}
...
class _I18N:
    ...
    def __init__(self):
        ...
        self.lang_3_per_row = tuple(
            lang for lang in ALL_LANGUAGES if len(self[lang]["lang_native_name"]) <= 7
        )
        self.lang_2_per_row = tuple(
            lang
            for lang in ALL_LANGUAGES
            if 7 < len(self[lang]["lang_native_name"]) <= 12
        )
        self.lang_1_per_row = tuple(
            lang for lang in ALL_LANGUAGES if len(self[lang]["lang_native_name"]) > 12
        )
    ...
    def __getitem__(self, lang_code: Optional[str]) -> "_L10N":
        ...
        return (
            self.__l10n_d[lang_code]
            if lang_code in self.__l10n_d
            else self.get_fallback_l10n(lang_code)
        )

    def get_all_l10n_string(
        self, key: str, html_escaped: bool = False, only_iso_639: bool = False
    ) -> tuple[str, ...]:
    ...
    def set_help_msg_html(self):
        cmd_lang_description = " / ".join(
            self.get_all_l10n_string(
                "cmd_description_lang", html_escaped=True, only_iso_639=True
            )
        )
        for l10n in self.__l10n_d.values():
            ...
            _cmd_description_lang = (
                ...
                if not l10n["iso_639_code"]
                and l10n_cmd_description_lang not in cmd_lang_description
                else ""
...
class _L10N:
    ...
    def key_exist(self, key: str):
        return key in self.__l10n_lang and (
            self.__l10n_lang[key] or key in NO_FALLBACK_KEYS
        )
    ...
    def __getitem__(self, key: str) -> str:
        ...
        if self.__lang_code != FALLBACK_LANGUAGE:
            ...
            return _I18N().get_fallback_l10n(
                None if self.__l10n_lang["iso_639_code"] else self.__lang_code
            )[key]
...
def get_commands_list(
    lang: Optional[str] = None, manager: bool = False
) -> list[types.BotCommand]:
    commands = [
        types.BotCommand(
            command=command, description=i18n[lang][f"cmd_description_{command}"]
        )
        for command in COMMANDS
    ]
