...
class ContextTimeoutManager:
    ...
    def __call__(self, context: AbstractAsyncContextManager, timeout: float = None):
        ...
        if timeout is None:
            raise RuntimeError(
                "`timeout` must be set either when creating the instance or in the call"
            )
        curr_time = time()
...
async def user_flood_wait(
    user: _USER_LIKE, seconds: int, call_time: float = None
) -> bool:
    ...
    async with flood_lock:
        ...
        if time_left > 0.1:
            ...
            return True
        logger.info(
            f"Skipped flood wait for {user} because the wait had been finished before the lock was acquired"
        )
        return False


async def user_flood_wait_background(user: _USER_LIKE, seconds: int) -> asyncio.Task:
    task = env.loop.create_task(
        user_flood_wait(user=user, seconds=seconds, call_time=time())
    )
...
# ----- web locks -----
overall_web_semaphore = (
    asyncio.BoundedSemaphore(env.HTTP_CONCURRENCY)
    if env.HTTP_CONCURRENCY > 0
    else nullcontext()
)

if env.HTTP_CONCURRENCY_PER_HOST > 0:
    _hostname_semaphore_bucket: defaultdict[
        str, asyncio.BoundedSemaphore
