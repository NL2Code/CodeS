...
async def cmd_set_option(
    event: Union[events.NewMessage.Event, Message], *_, lang: Optional[str] = None, **__
):
...
@command_gatekeeper(
    only_manager=True, only_in_private_chat=False, timeout=None if env.DEBUG else 300
)
async def cmd_test(
...
async def __send(chat_id, entry, feed_title, link):
    ...
    logger.debug(
        f"Sending {entry.get('title', 'Untitled')} ({entry.get('link', 'No link')}) to {chat_id}..."
    )
...
async def cmd_user_info_or_callback_set_user(
    ...
    **__,
    ...
    if user_id:
        ...
        if len(args) < 2 or (
            not args[1].lstrip("-").isdecimal() and not args[1].startswith("@")
        ):
            await event.respond(
                i18n[lang]["cmd_user_info_usage_prompt_html"], parse_mode="html"
            )
            return
        user_entity_like = (
            int(args[1]) if args[1].lstrip("-").isdecimal() else args[1].lstrip("@")
        )
    ...
    try:
        ...
        if isinstance(entity, types.User):
            ...
            name = entity.title
            user_type = (
                i18n[lang]["channel"] if entity.broadcast else i18n[lang]["group"]
            )
    ...
    user, user_created = await db.User.get_or_create(
        id=user_id, defaults={"lang": "null"}
    )
    ...
    if user_created:
        ...
        _, sub_count, sub_limit, is_default_limit = await inner.utils.check_sub_limit(
            user_id, force_count_current=True
        )

    msg_text = (
        f"<b>{i18n[lang]['user_info']}</b>\n\n"
        + (f"{name}\n" if name else "")
        + (f"{user_type} " if user_type else "")
        + f"<code>{user_id}</code>\n"
        + (f"@{username}\n" if username else "")
        ...
        + (
            f"\n{i18n[lang]['participant_count']}: {participant_count}"
                    ...
                    *(
                        (
                            Button.inline(
                                (
                                    SELECTED_EMOJI
                                    if user.state == btn_state
                                    else UNSELECTED_EMOJI
                                )
                                + '{prompt} "{state}"'.format(
                                ...
                                data="null"
                                if user.state == btn_state
                                else f"set_user={user_id},{btn_state}",
                            ),
                        )
    ...
    await event.edit(
        msg_text, parse_mode="html", buttons=buttons
    ) if is_callback else await event.respond(
        msg_text, parse_mode="html", buttons=buttons
    )


@command_gatekeeper(only_manager=True)
async def callback_reset_sub_limit(
    event: events.CallbackQuery.Event, *_, lang: Optional[str] = None, **__
):
    """
    ...
    if user:
        user.sub_limit = None
        await user.save()
    await cmd_user_info_or_callback_set_user.__wrapped__(
        event, user_id=user_id, lang=lang
    )
    return


@command_gatekeeper(only_manager=True)
async def cmd_set_sub_limit(
    event: Union[events.NewMessage.Event, Message], *_, lang: Optional[str] = None, **__
):
    """
    ...
    """
    args = parse_command(
        event.raw_text, strip_target_chat=False, strip_inline_header=True
    )
    if len(args) < 2 or not args[1].lstrip("-").isdecimal():
        await event.respond(
            i18n[lang]["permission_denied_no_direct_use"] % "/user_info"
        )
        return
    if len(args) < 3 or not args[2].lstrip("-").isdecimal():
        await event.respond(
            i18n[lang]["cmd_set_sub_limit_prompt_html"], parse_mode="html"
        )
        return
    user_id, sub_limit = int(args[1]), int(args[2])
    sub_limit = max(sub_limit, -1)
    user, user_created = await db.User.get_or_create(
        id=user_id, defaults={"lang": "null", "sub_limit": sub_limit}
    )
    if not user_created:
