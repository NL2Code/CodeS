...
from telethon import Button, events, hints
...
from .. import db, env, errors_collection, locks, log
from ..compat import cached_async
from ..errors_collection import UserBlockedErrors
from ..i18n import ALL_LANGUAGES, i18n
from . import inner
    ...
    **kwargs,
    ...
    with suppress(UserBlockedErrors):  # silently ignore
        ...
        if isinstance(event, events.CallbackQuery.Event) and not event._answered:
            ...
            with suppress(
                QueryIdInvalidError
            ):  # callback query expired, respond instead
            ...
            if event._answered:
                return
            await event.answer(
                switch_pm=msg, switch_pm_param=str(event.id), private=True
            )
            return  # return if answering successfully

        async with locks.ContextWithTimeout(
            locks.user_flood_lock(event.chat_id), timeout=30
        ):
...
async def is_user_admin(
    chat_id: hints.EntityLike, user_id: hints.EntityLike
...
async def unsub_all_and_leave_chat(user_id: hints.EntityLike):
    await asyncio.gather(inner.sub.unsub_all(user_id), leave_chat(user_id))
...
def command_gatekeeper(
    ...
    *,
    ...
    only_in_private_chat = (
        only_manager if only_in_private_chat is None else only_in_private_chat
    )
    ...
    async def wrapper(
        ...
        **kwargs,
        ...
        def describe_user():
            ...
            if (chat_title or chat_id) and chat_id != sender_id:
                chat_info = f"{chat_title or chat_id}" + (
                    f" ({chat_id})" if chat_title and chat_id else ""
                )
            return (
        ...
        async def user_and_chat_permission_check():
            ...
            if sender_id:
                sender_in_db, _ = await db.User.get_or_create(
                    id=sender_id, defaults={"lang": "null"}
                )
                sender_state = sender_in_db.state
            if chat_id == sender_id:
                chat_state = sender_state
            elif chat_id:
                chat_in_db, _ = await db.User.get_or_create(
                    id=chat_id, defaults={"lang": "null"}
                )
                chat_state = chat_in_db.state

            permission_denied_not_manager = only_manager and sender_id != env.MANAGER
            permission_denied_no_permission = sender_id != env.MANAGER and (
                (not env.MULTIUSER and max(sender_state, chat_state) < 1)
                or min(sender_state, chat_state) < 0
            )
            if permission_denied_not_manager or permission_denied_no_permission:
                ...
                logger.warning(
                    ...
                    + (
                        ...
                        if permission_denied_not_manager
                        else "the user has no permission to use the command"
                    )
                )
                raise events.StopPropagation
        ...
        async def execute():
            ...
            if callback_msg_id and callback_msg_id in pending_callbacks:
                ...
                await respond_or_answer(
                    event, i18n[lang]["callback_already_running_prompt"], cache_time=0
                )
                raise events.StopPropagation
            ...
            try:
                ...
                await asyncio.wait_for(
                    func(
                        event, *args, lang=lang, chat_id=chat_id, **kwargs
                    ),  # execute the command!
                    timeout=timeout,
                )
            except locks.ContextTimeoutError:
                logger.error(
                    f"Cancel {command} for {describe_user()} due to flood wait timeout ({timeout}s)"
                )
                # await respond_or_answer(event, 'ERROR: ' + i18n[lang]['flood_wait_prompt'])
            except asyncio.TimeoutError as _e:
                logger.error(
                    f"Cancel {command} for {describe_user()} due to timeout ({timeout}s)",
                    exc_info=_e,
                )
                await respond_or_answer(
                    event, "ERROR: " + i18n[lang]["operation_timeout_error"]
                )
            finally:
                if callback_msg_id:
        ...
        try:
            command = (
                ...
                else f"(Callback){event.data.decode()}"
                if is_callback
                else f"(Inline){event.text}"
                if is_inline
                else f"(ChatAction, {event.action_message and type(event.action_message.action).__name__})"
                if is_chat_action
                else "(no command, other message)"
            )
            command_header = parse_command(
                command, max_split=1, strip_target_chat=False
            )[0]
            ...
            if is_chat_action and event.action_message:  # service message
                ...
                sender_id = (
                    event.sender_id
                )  # `None` if the sender is an anonymous admin in a group
            ...
            sender_fullname = (
                ...
                else "__anonymous_admin__"
                if not is_chat_action
                else "__chat_action__"
            )

            # get the user's lang
            lang_in_db = await db.User.get_or_none(id=chat_id).values_list(
                "lang", flat=True
            )
            ...
            if not lang and not ignore_tg_lang:
                ...
                if not lang and sender_id != chat_id:
                    lang = db.User.get_or_none(id=sender_id).values_list(
                        "lang", flat=True
                    )
            ...
            if is_inline:
                ...
                if (
                    ...
                    await respond_or_answer(
                        event, i18n[lang]["permission_denied_switch_pm"]
                    )
                    logger.warning(
                        f"Redirected {describe_user()} (using {command}) to the private chat with the bot"
                    )
                    raise events.StopPropagation
            ...
            pattern_match: re.Match = (
                event.pattern_match if not is_chat_action else None
            )
            target_chat_id = (
                pattern_match and pattern_match.groupdict().get("target")
            ) or ""
            ...
            if (
                target_chat_id.startswith(("-100", "+"))
                and target_chat_id.lstrip("+-").isdecimal()
            ):
            ...
            if (
                is_inline
                or event.is_private  # allow inline
                or (  # allow commands in private chats
                    (
            ...
            if (
                target_chat_id
                or event.is_group  # deal with operating-channel/group commands in private chats
                or event.is_channel  # deal with commands in groups  # deal with commands in channels
            ):
                if isinstance(sender, types.Channel):
                ...
                if only_in_private_chat or (target_chat_id and not event.is_private):
                    await respond_or_answer(
                        event, i18n[lang]["permission_denied_not_in_private_chat"]
                    )
                    logger.warning(
                        f"Refused {describe_user()} to use {command} "
                ...
                if target_chat_id:
                    try:
                        ...
                        await respond_or_answer(
                            event, i18n[lang]["channel_or_group_not_found"]
                        )
                        raise events.StopPropagation
                else:
                    ...
                    chat: Optional[
                        Union[types.Chat, types.Channel]
                ...
                if is_callback or target_chat_id:
                    ...
                    if (
                        self_is_admin is None
                    ):  # I am not a participant of the group/channel, none of my business!
                        await respond_or_answer(
                            event,
                            i18n[lang]["permission_denied_bot_not_member"],
                            cache_time=15,
                        )
                        raise events.StopPropagation
                ...
                if is_chat_action:
                    ...
                    if is_admin is None:
                        ...
                        logger.warning(
                            f"Refused Refused {describe_user()} to use {command} "
                ...
                if not is_admin:
                    await respond_or_answer(
                        event,
                        i18n[lang]["permission_denied_not_chat_admin"],
                        cache_time=15,
                    )
            ...
            try:
                if isinstance(e, (FloodError, locks.ContextTimeoutError)):
                    ...
                    await respond_or_answer(
                        event, "ERROR: " + i18n[lang]["flood_wait_prompt"]
                    )
                ...
                # usually occurred because the user hits the same button during auto flood wait
                    ...
                    await respond_or_answer(
                        event, "ERROR: " + i18n[lang]["edit_conflict_prompt"]
                    )
                elif isinstance(e, (EntitiesTooLongError, MessageTooLongError)):
                    await respond_or_answer(
                        event, "ERROR: " + i18n[lang]["message_too_long_prompt"]
                    )
...
class ReplyMessage(events.NewMessage):
    ...
    async def __reply_verify(self, event: Union[events.NewMessage.Event, Message]):
        if event.is_reply:
            ...
            if (
                reply_to_msg is not None
                and self.reply_to_peer_id == reply_to_msg.sender_id
            ):
                return True
        return False
...
class AddedToGroupAction(events.ChatAction):
    ...
    def __init__(self, chats=None, *, blacklist_chats=False):
        super().__init__(
            chats, blacklist_chats=blacklist_chats, func=self.__added_to_group
        )
    ...
    def __added_to_group(event: events.ChatAction.Event):
        ...
        if event.user_added or (
            ...
            and isinstance(event.action_message.action, types.MessageActionChatAddUser)
        ):
...
async def send_success_and_failure_msg(
    ...
    **__,
    ...
    for i in range(4):
        ...
        try:
            msg = await (
                message.edit(msg_html, parse_mode="html")
                if edit
...
def get_group_migration_help_msg(
    ...
    buttons, _ = inner.utils.get_lang_buttons(
        callback="get_group_migration_help", current_lang=lang
    )
    return msg, buttons


def get_callback_tail(
    event: Union[events.NewMessage.Event, Message, events.CallbackQuery.Event],
    chat_id: int,
) -> str:
...
async def check_sub_limit(
    ...
    if limit_reached:
        logger.warning(
            f"Refused user {user_id} to add new subscriptions due to limit reached ({curr_count}/{limit})"
        )
