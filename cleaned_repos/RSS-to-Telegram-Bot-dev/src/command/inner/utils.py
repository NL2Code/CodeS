...
def calculate_update(
    old_hashes: Optional[Sequence[str]], entries: Sequence[dict]
...
def filter_urls(urls: Optional[Iterable[str]]) -> tuple[str, ...]:
    return (
        tuple(
            filter(lambda x: x.startswith("http://") or x.startswith("https://"), urls)
        )
        if urls
        else tuple()
    )
...
def get_http_last_modified(headers: Optional[Mapping]) -> datetime:
    ...
    """
    last_modified = (
        headers.get("Last-Modified") or headers.get("Date") if headers else None
    )
    try:
        return (
            parsedate_to_datetime(last_modified) if last_modified else datetime.utcnow()
        )
    except ValueError:
        try:
            return datetime.fromisoformat(
                last_modified
            )  # why some websites are so freaky? I can't understand
        except ValueError:
            return datetime.utcnow()


def arrange_grid(
    to_arrange: Iterable, columns: int = 8, rows: int = 13
) -> tuple[tuple[Any, ...], ...]:
    """
    ...
    return (
        tuple(tuple(to_arrange[i : i + columns]) for i in range(0, counts, columns))
        if counts > 0
        else ()
    )
...
def get_lang_buttons(
    callback=str, current_lang: str = None
    ...
    buttons = (
        arrange_grid(
            (
                Button.inline(i18n[lang]["lang_native_name"], data=f"{callback}={lang}")
...
async def get_sub_list_by_page(
    user_id: int, page_number: int, size: int, desc: bool = True, *args, **kwargs
    ...
    page = (
        await db.Sub.filter(user=user_id, *args, **kwargs)
...
def get_page_buttons(
    ...
    page_number = min(page_number, page_count)
    page_info = f"{page_number} / {page_count}" + (
        f" ({total_count})" if total_count else ""
    )
    return [
        Button.inline(
            f'< {i18n[lang]["previous_page"]}',
            data=f"{get_page_callback}|{page_number - 1}{tail}",
        )
        if page_number > 1
        else emptyButton,
...
async def get_sub_choosing_buttons(
    user_id: int,
    page_number: int,
    callback: str,
    ...
    *args,
    **kwargs,
) -> Optional[tuple[tuple[KeyboardButtonCallback, ...], ...]]:
    """
    :param user_id: user id
    :param page_number: page number (1-based)
    ...
    size = columns * rows
    page_number, page_count, page, sub_count = await get_sub_list_by_page(
        user_id=user_id, page_number=page_number, size=size, *args, **kwargs
    )

    if page_count == 0:
        return None

    buttons_to_arrange = tuple(
        Button.inline(
            _sub.title or _sub.feed.title,
...
async def update_interval(feed: Union[db.Feed, db.Sub, int]):
    ...
    if not sub_exist:  # no sub subs the feed, del the feed
        ...
        return
    intervals = await feed.subs.filter(state=1, interval__not_isnull=True).values_list(
        "interval", flat=True
    )
    intervals += await feed.subs.filter(
        state=1, interval__isnull=True, user__interval__not_isnull=True
    ).values_list("user__interval", flat=True)
    some_using_default = await feed.subs.filter(
        state=1, interval__isnull=True, user__interval__isnull=True
    ).exists()
    if (
        not intervals and not some_using_default
    ):  # no active sub subs the feed, deactivate the feed
        if feed.state == 1:
            feed.state = 0
            feed.error_count = 0
...
async def check_sub_limit(
    user_id: int, force_count_current: bool = False
    ...
    if user_id != env.MANAGER:
        ...
        limit: Optional[int] = await db.User.get_or_none(id=user_id).values_list(
            "sub_limit", flat=True
        )
        if limit is None:
            limit: int = (
...
async def deactivate_feed(feed: db.Feed) -> db.Feed:
    ...
    await asyncio.gather(
        *(
            activate_or_deactivate_sub(
                sub.user_id, sub, activate=False, _update_interval=False
            )
            for sub in subs
        )
    )
...
async def activate_or_deactivate_sub(
    user_id: int, sub: Union[db.Sub, int], activate: bool, _update_interval: bool = True
) -> Optional[db.Sub]:
    """
    :param user_id: user id
...
async def activate_or_deactivate_all_subs(
    user_id: int, activate: bool
) -> tuple[Optional[db.Sub], ...]:
    """
    :param user_id: user id
    ...
    if feeds_to_update:
        await db.Feed.bulk_update(
            feeds_to_update, ["state", "error_count", "next_check_time"]
        )
    for task in tasks:
