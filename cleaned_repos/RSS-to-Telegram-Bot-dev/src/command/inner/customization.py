...
async def get_sub_info(
    sub: db.Sub, lang: Optional[str] = None, additional_guide: bool = False
) -> str:
    ...
    return (
        ...
        + (
            f"{i18n[lang]['hashtags']}: {construct_hashtags(sub.tags)}"
            if sub.tags
            else ""
        )
        + (
...
async def get_customization_buttons(
    ...
    if is_user:
        interval_d = (
            ...
            display_media_d
        ) = display_author_d = display_via_d = display_title_d = style_d = False
        all_default = None
    else:
    ...
    length_limit = (
        sub_or_user.user.length_limit if length_limit_d else sub_or_user.length_limit
    )
    ...
    link_preview = (
        sub_or_user.user.link_preview if link_preview_d else sub_or_user.link_preview
    )
    display_media = (
        sub_or_user.user.display_media if display_media_d else sub_or_user.display_media
    )
    display_author = (
        sub_or_user.user.display_author
        if display_author_d
        else sub_or_user.display_author
    )
    display_via = (
        sub_or_user.user.display_via if display_via_d else sub_or_user.display_via
    )
    display_title = (
        sub_or_user.user.display_title if display_title_d else sub_or_user.display_title
    )
    ...
    buttons = (
        (
            ...
            if is_user
                ...
                + i18n[lang][
                    "status_activated"
                    if sub_or_user.state == 1
                    else "status_deactivated"
                ],
        ...
        (
            Button.inline(
                ...
                + formatting_time(
                    minutes=interval or db.EffectiveOptions.default_interval
                ),
                data=(
...
async def get_set_interval_buttons(
    ...
    buttons_in_minute_and_hour_count = sum(
        ...
        for interval in chain(range(1, 5), range(5, 61, 5), range(2 * 60, 24 * 60, 60))
    )
    ...
    buttons = (
        (
            ...
            if is_user
                ...
                Button.inline(
                    ...
                    + i18n[lang]["use_user_default_button"],
        ...
        + arrange_grid(
            to_arrange=chain(
                (
                    Button.inline(
                        "1h" if interval == 60 else f"{interval}min",
                ...
                (
                    Button.inline(
                        ...
                        if is_user
                        else f"set={sub_or_user.id},interval,{interval * 60}|{page}{tail}",
                    )
                    for interval in range(2, 24)
                    if interval * 60 >= minimal_interval
                ),
                (
                    Button.inline(
                        f"{interval}d",
                        ...
                        if is_user
                        else f"set={sub_or_user.id},interval,{interval * 60 * 24}|{page}{tail}",
                    )
        ...
        + (
            (
                Button.switch_inline(
                    ...
                    query=(
                        (
                            f"/set_interval {sub_or_user.id} default "
                            if tail
        ...
        + (
            (
                Button.inline(
                    ...
                    if is_user
                    else f"set={sub_or_user.id}|{page}{tail}",
                ),
            ),
        )
    )
...
async def get_set_length_limit_buttons(
    sub_or_user: Union[db.Sub, db.User],
    lang: Optional[str] = None,
    page: Optional[int] = None,
    tail: str = "",
    ...
    buttons = (
        ...
        + (
            (
                Button.inline(
                    i18n[lang]["length_limit_unlimited"],
                    data=f"set_default=length_limit,0{tail}"
                    if is_user
                    else f"set={sub_or_user.id},length_limit,0|{page}{tail}",
                ),
            ),
        )
        + arrange_grid(
            to_arrange=(
                Button.inline(
                    ...
                    if is_user
                    else f"set={sub_or_user.id},length_limit,{length_limit}|{page}{tail}",
                )
...
async def set_interval(
    sub_or_user: Union[db.Sub, db.User], interval: int
) -> Union[db.Sub, db.User]:
    is_user = isinstance(sub_or_user, db.User)
    ...
    if (
        ...
        and (sub_or_user.id if is_user else sub_or_user.user_id) != env.MANAGER
    ):
        interval = minimal_interval
...
async def set_length_limit(
    sub_or_user: Union[db.Sub, db.User], length_limit: int
) -> Union[db.Sub, db.User]:
...
async def set_exhaustive_option(
    sub_or_user: Union[db.Sub, db.User], option: str
) -> Union[db.Sub, db.User]:
...
async def set_sub_hashtags(
    sub: db.Sub, hashtags: Union[Iterable[str], str, None]
) -> db.Sub:
