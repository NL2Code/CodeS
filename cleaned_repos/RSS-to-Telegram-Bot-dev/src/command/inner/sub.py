...
with open(
    path.normpath(path.join(path.dirname(__file__), "../..", "opml_template.opml")), "r"
) as __template:
...
) -> dict[str, Union[int, str, db.Sub, None]]:
    if (
        not bypass_feed_sniff
        and feed_url in FeedSnifferCache
        and FeedSnifferCache[feed_url]
    ):
        return await sub(
            user_id, FeedSnifferCache[feed_url], lang=lang, bypass_feed_sniff=True
        )

    ret = {"url": feed_url, "sub": None, "status": -1, "msg": None}
    ...
    try:
        ...
        if not feed or feed.state == 0:
            ...
            if rss_d is None:
                if not bypass_feed_sniff and wf.status == 200 and wf.content:
                    ...
                    if sniffed_feed_url:
                        sniff_ret = await sub(
                            user_id, sniffed_feed_url, lang=lang, bypass_feed_sniff=True
                        )
                        if sniff_ret["sub"]:
                            return sniff_ret
            ...
            feed, created_new_feed = await db.Feed.get_or_create(
                defaults={"title": title}, link=feed_url
            )
            if created_new_feed or feed.state == 0:
                ...
                feed.entry_hashes = list(
                    calculate_update(old_hashes=None, entries=rss_d.entries)[0]
                )
        ...
        if not created_new_sub:
            ...
            if _sub.title != sub_title:
                ...
                await _sub.save()
                logger.info(
                    f"Sub {feed_url} for {user_id} updated title to {sub_title}"
                )
            else:
                _sub.state = 1
                await _sub.save()
...
async def subs(
    ...
    dict[str, Union[tuple[dict[str, Union[int, str, db.Sub, None]], ...], str, int]]
]:
    if not feed_urls:
        return None
    ...
    if limit > 0:
        ...
        failure = [
            {"url": url, "msg": "ERROR: " + i18n[lang]["sub_limit_reached"]}
            for url in feed_urls[remaining:]
        ]
        if failure:
            logger.info(
                f"Sub limit reached for {user_id}, rejected {len(failure)} feeds of {len(feed_urls)}"
            )
    else:
    ...
    result = await asyncio.gather(
        *(sub(user_id, url, lang=lang) for url in remaining_feed_urls)
    )
    ...
    msg = success_msg + ("\n\n" if success and failure else "") + failure_msg
...
async def unsub(
    user_id: int, feed_url: str = None, sub_id: int = None, lang: Optional[str] = None
) -> dict[str, Union[str, db.Sub, None]]:
    ret = {"url": feed_url, "sub": None, "msg": None}
    ...
    try:
        if feed_url:
            ...
            sub_to_delete: Optional[db.Sub] = (
                await feed.subs.filter(user=user_id).first() if feed else None
            )
        else:  # elif sub_id:
            sub_to_delete: db.Sub = await db.Sub.get_or_none(
                id=sub_id, user=user_id
            ).prefetch_related("feed")
            feed: Optional[db.Feed] = (
                await sub_to_delete.feed if sub_to_delete else None
            )
...
async def unsubs(
    ...
    coroutines = (
        tuple(unsub(user_id, feed_url=url, lang=lang) for url in feed_urls)
        if feed_urls
        else tuple()
    ) + (
        tuple(unsub(user_id, sub_id=sub_id, lang=lang) for sub_id in sub_ids)
        if sub_ids
        else tuple()
    )
...
async def unsub_all(
    user_id: int, lang: Optional[str] = None
) -> Optional[dict[str, Union[dict[str, Union[int, str, db.Sub, None]], str]]]:
    user_sub_list = await db.Sub.filter(user=user_id).values_list("id", flat=True)
    return (
        await unsubs(user_id, sub_ids=user_sub_list, lang=lang)
        if user_sub_list
        else None
    )
...
async def export_opml(user_id: int) -> Optional[bytes]:
    ...
    create_time.string = opml.new_string(
...
async def feed_sniffer(url: str, html: AnyStr) -> Optional[str]:
    ...
    links = (
        soup.find_all(
            name="link", attrs={"rel": "alternate", "type": FeedLinkTypeMatcher}
        )
        or soup.find_all(
            name="link", attrs={"rel": "alternate", "href": FeedLinkHrefMatcher}
        )
