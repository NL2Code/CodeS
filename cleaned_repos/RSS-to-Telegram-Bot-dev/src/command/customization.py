...
from .utils import (
    ...
    parse_command_get_sub_or_user_and_param,
    parse_customization_callback_data,
)
...
async def cmd_set_or_callback_get_set_page(
    ...
    **__,
    ...
    if not page:
        _, page = (
            parse_callback_data_with_page(event.data) if is_callback else (None, 1)
        )
    ...
    await event.edit(msg, buttons=buttons) if is_callback else await event.respond(
        msg, buttons=buttons
    )
...
async def callback_set(
    ...
    **__,
    ...
    if sub_id is None and not set_user_default:
        await cmd_set_or_callback_get_set_page.__wrapped__(
            event, lang=lang, page=page, chat_id=chat_id
        )
        return
    ...
    sub_or_user: Union[db.Sub, db.User] = (
        ...
        if set_user_default
        else await db.Sub.get_or_none(id=sub_id, user=chat_id).prefetch_related(
            "feed", "user"
        )
    )
    ...
    if (
        ...
        if action == "interval" and (isinstance(param, int) or param == "default"):
            await inner.customization.set_interval(
                sub_or_user, param if param != "default" else -100
            )
        elif action == "length_limit" and (
            isinstance(param, int) or param == "default"
        ):
            await inner.customization.set_length_limit(
                sub_or_user, param if param != "default" else -100
            )
            ...
            action is not None
            and action in inner.customization.SUB_OPTIONS_EXHAUSTIVE_VALUES
        ):
    ...
    if action == "length_limit":
        if (
            ...
            await event.answer(
                i18n[lang]["length_limit_only_effective_if_send_mode_auto"], alert=True
            )
            return
...
async def cmd_set_default(
    ...
    **__,
    ...
    buttons = await inner.customization.get_customization_buttons(
        user, lang=lang, tail=callback_tail
    )
...
async def callback_reset(
    ...
    **__,
    ...
    sub.length_limit = (
        ...
        sub.display_author
    ) = sub.display_media = sub.display_title = sub.display_via = sub.style = -100
    await sub.save()
    ...
    buttons = await inner.customization.get_customization_buttons(
        sub, lang=lang, page=page, tail=callback_tail
    )
    await event.edit(info, buttons=buttons, parse_mode="html", link_preview=False)
    ...
    if await inner.utils.have_subs(chat_id):
        await event.edit(
            ...
            buttons=[
                [
                    Button.inline(
                        i18n[lang]["reset_all_confirm"],
                        data=f"reset_all{callback_tail}",
                    )
                ],
                [
                    Button.inline(
                        i18n[lang]["reset_all_cancel"],
                        data=f"set_default{callback_tail}",
                    )
                ],
            ],
        )
...
async def cmd_activate_or_deactivate_subs(
    ...
    **__,
    ...
    await callback_get_activate_or_deactivate_page.__wrapped__(
        event, activate, lang=lang, chat_id=chat_id, page=1
    )


@command_gatekeeper(only_manager=False)
async def callback_get_activate_or_deactivate_page(
    event: Union[events.CallbackQuery.Event, events.NewMessage.Event, Message],
    activate: bool,
    *_,
    lang: Optional[str] = None,
    page: Optional[int] = None,
    ...
    **__,
    ...
    msg = (
        i18n[lang][
            "choose_sub_to_be_activated" if sub_buttons else "all_subs_are_activated"
        ]
        if activate
        else i18n[lang][
            "choose_sub_to_be_deactivated"
            if sub_buttons
            else "all_subs_are_deactivated"
        ]
    )
    activate_or_deactivate_all_subs_str = (
        "activate_all_subs" if activate else "deactivate_all_subs"
    )
    buttons = (
        (
            (
...
async def callback_activate_or_deactivate_sub(
    ...
    **__,
    ...
    sub_id = int(sub_id)
    unsub_res = await inner.utils.activate_or_deactivate_sub(
        chat_id, sub_id, activate=activate
    )
    if unsub_res is None:
        await event.answer("ERROR: " + i18n[lang]["subscription_not_exist"], alert=True)
        return
    await callback_get_activate_or_deactivate_page.__wrapped__(
        event, activate, lang=lang, page=page, chat_id=chat_id
    )


@command_gatekeeper(only_manager=False)
async def callback_del_subs_title(
    event: events.CallbackQuery.Event, *_, chat_id: Optional[int] = None, **__
    ...
    for id_range_str in id_ranges_str:
        ...
        subs.extend(
            await db.Sub.filter(user_id=chat_id, id__range=(id_start, id_end)).all()
        )
...
async def cmd_set_title(
    ...
    **__,
    ...
    chat_id = chat_id or event.chat_id
    callback_tail = get_callback_tail(event, chat_id)
    sub, title = await parse_command_get_sub_or_user_and_param(
        event.raw_text, chat_id, max_split=2
    )
    ...
    if not title and not sub.title:
        await event.respond(
            i18n[lang]["cmd_set_title_usage_prompt_html"], parse_mode="html"
        )
        return
    ...
    await event.respond(
        ...
        buttons=(
            Button.inline(
                ...
                data=f"set={sub.id}" + callback_tail,
            ),
        ),
        parse_mode="html",
        link_preview=False,
...
async def cmd_set_interval(
    ...
    **__,
    ...
    if not interval:
        await event.respond(
            i18n[lang]["cmd_set_interval_usage_prompt_html"], parse_mode="html"
        )
        return
    if interval < minimal_interval and chat_id != env.MANAGER:
        await event.respond(
            i18n[lang]["set_interval_failure_too_small_html"] % minimal_interval,
            parse_mode="html",
        )
        return
...
async def cmd_set_hashtags(
    ...
    **__,
    ...
    chat_id = chat_id or event.chat_id
    callback_tail = get_callback_tail(event, chat_id)
    sub, hashtags = await parse_command_get_sub_or_user_and_param(
        event.raw_text, chat_id, max_split=2
    )
    ...
    if not hashtags and not sub.tags:
        await event.respond(
            i18n[lang]["cmd_set_hashtags_usage_prompt_html"], parse_mode="html"
        )
        return
