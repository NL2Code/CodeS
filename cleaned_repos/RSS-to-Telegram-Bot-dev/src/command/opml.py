...
async def cmd_import(
    ...
    **__,
    ...
    await event.respond(
        ...
        + (
            "\n\n" + i18n[lang]["import_for_channel_or_group_prompt"]
            if event.is_private
            else ""
        ),
        buttons=(
...
async def opml_import(
    ...
    **__,
    ...
    opml_d = await run_async(
        partial(bozo_exception_removal_wrapper, listparser.parse, opml_file),
    ...
    msg = await send_success_and_failure_msg(
        reply, **import_result, lang=lang, edit=True
    )
    ...
    if subs:
        ...
        while sub_ids:
            ...
            if next_id in [curr_id + 1, curr_id]:
                ...
                sub.title is not None
                for sub in subs
                if sub.id in range(curr_start, curr_id + 1)
            ):  # if any sub has custom title
