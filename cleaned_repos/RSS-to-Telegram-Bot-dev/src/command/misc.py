...
from .. import db, env
from ..i18n import ALL_LANGUAGES, get_commands_list, i18n
from . import inner
from .utils import (
    command_gatekeeper,
...
async def cmd_start(
    event: Union[events.NewMessage.Event, Message], *_, lang=None, **__
):
    if lang is None:
...
async def callback_set_lang(
    event: events.CallbackQuery.Event, *_, **__
...
async def cmd_or_callback_help(
    ...
    **__,
    ...
    msg = i18n[lang][
        "help_msg_html" if event.chat_id != env.MANAGER else "manager_help_msg_html"
    ]
    if event.is_private:
        msg += "\n\n" + i18n[lang]["usage_in_channel_or_group_prompt_html"]
    await event.respond(msg, parse_mode="html", link_preview=False) if isinstance(
        event, events.NewMessage.Event
    ) or not hasattr(event, "edit") else await event.edit(
        msg, parse_mode="html", link_preview=False
    )
...
async def inline_command_constructor(
    event: events.InlineQuery.Event, *_, lang: Optional[str] = None, **__
):
    ...
    if not text:
        ...
        return
    await event.answer(
        results=[builder.article(title=text, text=text)], cache_time=3600, private=False
    )
