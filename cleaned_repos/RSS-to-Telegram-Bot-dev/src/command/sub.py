    ...
    **__,
    ...
    if not filtered_urls:
        await event.respond(
            ...
            buttons=(
                Button.force_reply(
                    single_use=True, selective=True, placeholder="url1 url2 url3 ..."
                )
                if event.is_group
                else None
            ),
...
async def cmd_unsub(
    ...
    **__,
    ...
    if unsub_result is None:
        ...
        await event.respond(
            i18n[lang]["unsub_choose_sub_prompt_html"]
            if buttons
...
async def cmd_or_callback_unsub_all(
    ...
    **__,
    ...
    if is_callback:
        ...
        await send_success_and_failure_msg(
            event, **unsub_all_result, lang=lang, edit=True
        )
        return
    ...
    if await inner.utils.have_subs(chat_id):
        await event.respond(
            ...
            buttons=[
                [
                    Button.inline(
                        i18n[lang]["unsub_all_confirm"],
                        data=f"unsub_all{callback_tail}",
                    )
                ],
...
async def cmd_list_or_callback_get_list_page(
    ...
    **__,
    ...
    await event.edit(
        list_result, parse_mode="html", buttons=page_buttons
...
async def callback_unsub(
    ...
    **__,
    ...
    if unsub_result["success_count"]:  # successfully unsubed
        await callback_get_unsub_page.__wrapped__(
...
async def callback_get_unsub_page(
    ...
    **__,
    ...
    await event.edit(
        None if buttons else i18n[lang]["no_subscription"], buttons=buttons
    )
