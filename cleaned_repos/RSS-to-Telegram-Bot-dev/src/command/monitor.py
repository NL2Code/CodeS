...
from .. import db, env, locks, log, web
from ..errors_collection import EntityNotFoundError, UserBlockedErrors
from ..i18n import i18n
from ..parsing.post import Post, get_post_from_entry
from ..parsing.utils import html_space_stripper
from . import inner
from .inner.utils import calculate_update, deactivate_feed, update_interval
...
class MonitoringLogs:
    ...
    def log(
        ...
        cls.timeout += timeout
        logger.debug(
            f"Finished feeds monitoring task: "
            f"updated({updated}), "
    ...
    def print_summary(cls):
        ...
        cls.not_updated = (
            cls.cached
        ) = cls.empty = cls.failed = cls.updated = cls.skipped = cls.timeout = 0
...
async def run_monitor_task():
    ...
    result = await asyncio.gather(
        *(asyncio.wait_for(__monitor(feed), timeout=wait_for) for feed in feeds),
    ...
    if timeout_errors:
        logger.error(
            f"Timeout detected during a feeds monitoring task, "
            f"totally {timeout} feed(s) timed out after {wait_for}s:"
        )
        for feed, error in timeout_errors:
            logger.error(
                f"The TimeoutError of the feed ({feed.link}) in the task:",
                exc_info=error,
            )
...
async def __monitor(feed: db.Feed) -> str:
    ...
    try:
        ...
        if rss_d is None:  # error occurred
            ...
            if feed.error_count % 20 == 0:  # error_count is always > 0
                logger.warning(
                    f"Fetch failed ({feed.error_count}th retry, {wf.error}): {feed.link}"
                )
            ...
            if feed.error_count >= 10:  # too much error, delay next check
                ...
                next_check_interval = min(interval, 15) * min(
                    feed.error_count // 10 + 1, 5
                )
    ...
    await asyncio.gather(
        *(__notify_all(feed, subs, entry) for entry in reversed(updated_entries))
    )
...
async def __notify_all(feed: db.Feed, subs: Iterable[db.Sub], entry: MutableMapping):
    ...
    try:
        ...
        logger.error(
            f"Failed to parse the post {link} (feed: {feed.link}) from entry:",
            exc_info=e,
        )
        try:
            error_message = Post(
                f"Something went wrong while parsing the post {link} "
                f"(feed: {feed.link}). "
            ...
            logger.error(
                f"Failed to send parsing error message for {link} (feed: {feed.link}):",
                exc_info=e,
            )
    ...
    for sub, exc in zip(subs, res):
        ...
        if not isinstance(
            ...
            raise exc
        logger.error(
            f"Failed to send {post.link} (feed: {post.feed_link}, user: {sub.user_id}) due to timeout"
        )
...
async def __send(sub: db.Sub, post: Union[str, Post]):
    ...
    try:
        try:
            await env.bot.get_input_entity(
                user_id
            )  # verify that the input entity can be gotten first
        except (
            ValueError
        ):  # cannot get the input entity, the bot may be banned by the user
            return await __locked_unsub_all_and_leave_chat(
                user_id=user_id, err_msg=type(EntityNotFoundError).__name__
            )
        try:
            if isinstance(post, str):
                await env.bot.send_message(
                    user_id, post, parse_mode="html", silent=not sub.notify
                )
                return
            ...
            return await __locked_unsub_all_and_leave_chat(
                user_id=user_id, err_msg=type(e).__name__
            )
        except BadRequestError as e:
            if e.message == "TOPIC_CLOSED":
                return await __locked_unsub_all_and_leave_chat(
                    user_id=user_id, err_msg=e.message
                )
    except Exception as e:
        logger.error(
            f"Failed to send {post.link} (feed: {post.feed_link}, user: {sub.user_id}):",
            exc_info=e,
        )
        try:
            error_message = Post(
                "Something went wrong while sending this post "
...
async def __deactivate_feed_and_notify_all(
    feed: db.Feed, subs: Iterable[db.Sub], reason: Union[web.WebError, str] = None
):
