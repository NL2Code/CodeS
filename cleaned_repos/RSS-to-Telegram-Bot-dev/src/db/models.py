...
class Base:
    ...
    created_at = fields.DatetimeField(
        auto_now_add=True, description="The time this row was created"
    )
    updated_at = fields.DatetimeField(
        auto_now=True, description="The time this row was updated"
    )
...
class User(Model, Base):
    ...
    lang = fields.CharField(
        default="zh-Hans", max_length=16, description="Preferred language, lang code"
    )
...
class Feed(Model, Base):
    ...
    state = fields.SmallIntField(
        default=1, description="Feed state: " "0=deactivated, 1=activated"
    )
    ...
    last_modified = fields.DatetimeField(
        null=True, description="The last modified time of webpage. "
    )
...
class Sub(Model, Base):
    ...
    id = fields.IntField(pk=True)
    state = fields.SmallIntField(
        default=1, description="Sub state: " "0=deactivated, 1=activated"
    )
    user: fields.ForeignKeyRelation["User"] = fields.ForeignKeyField(
        "models.User", related_name="subs", to_field="id", on_delete=fields.CASCADE
    )
    feed: fields.ForeignKeyRelation["Feed"] = fields.ForeignKeyField(
        "models.Feed", related_name="subs", to_field="id", on_delete=fields.CASCADE
    )
    title = fields.CharField(
        max_length=1024,
        null=True,
        description="Sub title, overriding feed title if set",
    )
    ...
    notify = fields.SmallIntField(
        default=1, description="Enable notification or not? " "0: disable, 1=enable"
    )
    ...
    link_preview = fields.SmallIntField(
        default=0, description="Enable link preview or not? " "0=auto, 1=force enable"
    )
    display_author = fields.SmallIntField(
        default=0,
        description="Display author or not?" "-1=disable, 0=auto, 1=force display",
    )
    ...
    display_title = fields.SmallIntField(
        default=0,
        description="Display title or not?" "-1=disable, 0=auto, 1=force display",
    )
    style = fields.SmallIntField(
        default=0, description="Style of posts: " "0=RSStT, 1=flowerss"
    )
    display_media = fields.SmallIntField(
        default=0, description="Display media or not?" "-1=disable, 0=enable"
    )
