...
from .medium import (
    ...
    Media,
    Video,
    construct_weserv_url_convert_to_2560,
)
...
TypeViaType = Union[
    NO_VIA, FEED_TITLE_VIA_NO_LINK, FEED_TITLE_VIA_W_LINK, TEXT_LINK_VIA, BARE_LINK_VIA
]
...
class PostFormatter:
    def __init__(
        ...
        self.telegraph_link: Optional[
            Union[str, False]
        ] = None  # if generating failed, will be False
        ...
        self.__post_bucket: dict[
            ...
            Optional[
                tuple[
                    str, bool, bool  # formatted post  # need media  # need linkpreview
                ]
            ],
        ] = {}
        self.__param_to_option_cache: dict[str, str] = {}  # param hash  # option hash
    ...
    async def get_formatted_post(
        ...
        assert display_via in {
            NO_FEED_TITLE_BUT_LINK_AS_POST_TITLE,
            COMPLETELY_DISABLE,
            NO_FEED_TITLE_BUT_TEXT_LINK,
            NO_FEED_TITLE_BUT_BARE_LINK,
            FEED_TITLE_AND_LINK,
            FEED_TITLE_AND_LINK_AS_POST_TITLE,
        }
        ...
        param_hash = (
            f"{sub_title}|{tags}|{send_mode}|{length_limit}|{link_preview}|"
        ...
        if (
            self.title and self.__title_similarity is None and display_title == AUTO
        ):  # get title similarity
            async with self.__lock:
                if self.__title_similarity is None:  # double check
                    plain_text = utils.stripAnySpace(
                        self.html_tree.get_html(plain=True)
                    )
                    title_tbc = (
                        self.title.replace("[图片]", "")
                        .replace("[视频]", "")
                        .replace("发布了: ", "")
                        .strip()
                        .rstrip(".…")
                    ...
                    self.__title_similarity = fuzz.partial_ratio(
                        title_tbc, plain_text[0 : len(self.title) + 10]
                    )
                    logger.debug(
        ...
        if (
            display_via
            in {FEED_TITLE_AND_LINK_AS_POST_TITLE, NO_FEED_TITLE_BUT_LINK_AS_POST_TITLE}
            and self.link
        ):
            title_type = POST_TITLE_W_LINK
        elif (
            display_title != DISABLE
            and self.title
            and (
        ...
        if send_mode == FORCE_MESSAGE:
            ...
            media_msg_count = (
                await self.media.estimate_message_counts()
                if (display_media != DISABLE and self.media)
                else 0
            )
            normal_msg_post = self.generate_formatted_post(
                sub_title=sub_title,
                tags=tags,
            ...
            if (
                ...
                and (
                    0
                    < length_limit
                    <= self.plain_length  # length_limit == 0 means no limit
                    or normal_msg_len > (4096 if not media_msg_count else 1024)
                )
            ) or media_msg_count > 1:
                message_type = TELEGRAPH_MESSAGE
            else:
                message_type = NORMAL_MESSAGE

        if (
            message_type == TELEGRAPH_MESSAGE and self.telegraph_link is None
        ):  # generate telegraph post
            async with self.__lock:
                if self.telegraph_link is None:  # double check
                    await self.telegraph_ify()

        if (
            self.telegraph_link is False and message_type == TELEGRAPH_MESSAGE
        ):  # fallback if needed
            if self.link:
                message_type = LINK_MESSAGE
        ...
        if (
            message_type == NORMAL_MESSAGE
            and display_media == ONLY_MEDIA_NO_CONTENT
            and self.media
        ):
            message_type = LINK_MESSAGE
        ...
        if (
            title_type == NO_POST_TITLE
            and self.title
            and display_title == AUTO
            and message_type in {TELEGRAPH_MESSAGE, LINK_MESSAGE}
        ):
            title_type = POST_TITLE_NO_LINK

        # ---- determine need_media ----
        need_media = self.media and (
        ...
        need_link_preview = link_preview != DISABLE and (
            link_preview == FORCE_ENABLE or message_type != NORMAL_MESSAGE
        )
        ...
        if (
            ...
            and display_media == ONLY_MEDIA_NO_CONTENT
            and not need_media
        ) or (  # ONLY_MEDIA_NO_CONTENT but no media
            not self.parsed_html
            and not need_media
            and via_type is NO_VIA
            and title_type == NO_POST_TITLE
            and need_author is False
        ...
        if message_type == NORMAL_MESSAGE and normal_msg_post:
            self.__post_bucket[option_hash] = (
                normal_msg_post,
                need_media,
                need_link_preview,
            )
        ...
        async with self.__lock:
            if option_hash in self.__post_bucket:  # double check
                return self.__post_bucket[option_hash]
            post = self.generate_formatted_post(
                sub_title=sub_title,
                tags=tags,
                title_type=title_type,
                via_type=via_type,
    ...
    def get_post_header_and_footer(
        self,
        ...
        # ---- hashtags ----
        tags_html = (
            Text(" ".join("#" + tag for tag in tags)).get_html() if tags else None
        )

        # ---- author ----
        author_html = (
            Text(f"(author: {self.author})").get_html()
            if need_author and self.author
            else None
        )
        ...
        if message_style == NORMAL_STYLE:
            ...
            if via_type == FEED_TITLE_VIA_W_LINK:
                via_text = Text(
                    [
                        Text("via "),
                        Link(feed_title, param=self.link)
                        if self.link
                        else Text(feed_title),
                    ]
                )
        ...
        if message_style == FLOWERSS_STYLE:
            ...
            if message_type == TELEGRAPH_MESSAGE:
                ...
                sourcing_html = (
                    Link("source", param=self.link).get_html() if self.link else None
                )
    ...
    def generate_formatted_post(
        ...
        header, footer = self.get_post_header_and_footer(
            sub_title=sub_title,
            tags=tags,
            title_type=title_type,
            via_type=via_type,
        ...
        if self.enclosures:
            ...
            for enclosure in self.enclosures:
                ...
                if not utils.isAbsoluteHttpLink(enclosure.url):
                    ...
                    medium = Video(
                        enclosure.url, type_fallback_urls=enclosure.thumbnail
                    )
    ...
    async def telegraph_ify(self):
        ...
        try:
            self.telegraph_link = await tgraph.TelegraphIfy(
                html,
                title=self.title,
                link=self.link,
            ...
            if str(e) == "CONTENT_TOO_BIG":
                logger.debug(
                    f"Content too big, send a pure link message instead: {self.link}"
                )
            else:
                logger.debug(f"Telegraph API error ({e}): {self.link}")
        except (TimeoutError, asyncio.TimeoutError):
            logger.debug(
                f"Generate Telegraph post error (network timeout):  {self.link}"
            )
        except (ClientError, ConnectionError) as e:
            logger.debug(
                f"Generate Telegraph post error (network error, {type(e).__name__}): {self.link}"
            )
        except OverflowError:
            logger.debug(
                f"Generate Telegraph post error (retried for too many times): {self.link}"
            )
