...
from .utils import emojify, is_emoticon, isAbsoluteHttpLink, resolve_relative_link
...
class Telegraph(aiograph.Telegraph):
    ...
    async def replace_session(self):
        ...
        proxy_connector = (
            ProxyConnector(**env.TELEGRAPH_PROXY_DICT, loop=self.loop)
            if env.TELEGRAPH_PROXY_DICT
            else None
        )
    ...
    async def create_page(self, *args, **kwargs) -> aiograph.types.Page:
        ...
        async with self._request_lock:
            await asyncio.sleep(
                max(0.5 - (time.time() - self.last_run), 0)
            )  # avoid exceeding flood control
    ...
    async def flood_wait(self, retry_after: int):
        if not self._fc_lock.locked():  # if not already blocking
            async with self._fc_lock:  # block any other sending tries
                retry_after += 1
                logger.info(
                    "Blocking any requests for this telegraph account due to flood control... "
                    f"({retry_after}s)"
                )
                if retry_after >= 60:
                    await self.create_account(
                        short_name="RSStT",
                        author_name="Generated by RSStT",
                        author_url="https://github.com/Rongronggg9/RSS-to-Telegram-Bot",
                    )
                    logger.warning(
                        f"Wanna let me wait? No way! Created a new Telegraph account: {self.token}"
                    )
                else:
        ...
        for token in self.tokens:
            ...
            try:
                if len(token) != 60:  # must be an invalid token
                    logger.warning(
                        "Telegraph API token may be invalid, create one instead."
                    )
                ...
                logger.warning(
                    f"Telegraph API token may be invalid, create one instead: {e}"
                )
                try:
                    await account.create_account(
                        short_name="RSStT",
                    ...
                    logger.warning(
                        f"Cannot set up one of Telegraph accounts: {e}", exc_info=e
                    )
            except Exception as e:
                logger.warning(
                    f"Cannot set up one of Telegraph accounts: {e}", exc_info=e
...
TELEGRAPH_TAGS_INSERT_BR_AFTER: Final = {"div", "section", "tr"}
...
TELEGRAPH_DISALLOWED_TAGS_IN_LI: Final = {"p", "section"}
...
class TelegraphIfy:
        ...
        for tag in soup.find_all(recursive=True):
            with suppress(ValueError, AttributeError):
                ...
                if tag.name in TELEGRAPH_DEL_TAGS:
                                ...
                                if env.TABLE_TO_IMAGE:
                                    ...
                                    if table_img:
                                        with BytesIO(table_img) as buffer:
                                            url_l = await apis.get_account().upload(
                                                buffer, full=False
                                            )
                    ...
                    tag.name = new_name
                    if (
                        old_name.startswith("h")
                        and not new_name.startswith("h")
                        and new_name != "p"
                    ):
                        # ensure take a whole line
                        tag.insert_before(soup.new_tag("br")) if (
                            hasattr(tag.previous_sibling, "name")
                ...
                if tag.name not in TELEGRAPH_TAGS_ALLOW_ATTR:
                    ...
                    if not attr_content.startswith(env.IMG_RELAY_SERVER):
                        ...
                        if tag.name == "img" and not attr_content.startswith(
                            env.IMAGES_WESERV_NL
                        ):
                            if (
                                attr_content.split(".", 1)[1].split("/", 1)[0]
                                == "sinaimg.cn"
                            ):
                                attr_content = env.IMG_RELAY_SERVER + attr_content
    ...
    async def telegraph_ify(self):
        ...
        if self.retries >= 1:
            logger.debug(
                "Retrying using another telegraph account..."
                if apis.count > 1
                else "Retrying..."
            )
        ...
        try:
            telegraph_page = await telegraph_account.create_page(
                title=f"{self.telegraph_title[:60]}…"
                if len(self.telegraph_title) > 61
                else self.telegraph_title,
            ...
            if e_msg.startswith("FLOOD_WAIT_"):  # exceed flood control
                ...
                rets = await asyncio.gather(
                    telegraph_account.flood_wait(retry_after), self.telegraph_ify()
                )
