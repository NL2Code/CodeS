...
from .medium import Animation, Audio, Image, Media, UploadedImage, Video
from .utils import (
    emojify,
    is_emoticon,
    isAbsoluteHttpLink,
...
srcsetParser = re.compile(
    ...
    r"\s*"
    r"(?=,|$)"
).finditer  # e.g.: url,url 1x,url 2x,url 100w,url 200w


def effective_link(
    content: TypeTextContent, href: str, base: str = None
) -> Union[TypeTextContent, Link, Text]:
...
class Parser:
    ...
    async def parse(self):
        self.soup = await run_async(
            BeautifulSoup, self.html, "lxml", prefer_pool="thread"
        )
    ...
    async def _parse_item(
        self,
        soup: Union[
            PageElement, BeautifulSoup, Tag, NavigableString, Iterable[PageElement]
        ],
        in_list: bool = False,
    ) -> Optional[Text]:
        ...
        if isinstance(
            soup, Iterator
        ):  # a Tag is also Iterable, but we only expect an Iterator here
            prev_tag_name = None
            for child in soup:
                item = await self._parse_item(child)
                if item:
                    tag_name = child.name if isinstance(child, Tag) else None
                    if (tag_name == "div" or prev_tag_name == "div") and not (
        ...
        if tag == "table":
            ...
            for i, row in enumerate(rows):
                ...
                if (
                    len(rows) > 1 and len(columns) > 1
                ):  # allow single-row or single-column tables
                    if env.TABLE_TO_IMAGE:
                        self.media.add(
                            UploadedImage(convert_table_to_png(str(soup)), "table.png")
                        )
                    return None
                for j, column in enumerate(
                    columns
                ):  # transpose single-row tables into single-column tables
        ...
        if tag == "code":
            ...
            if isinstance(class_, list):
                ...
                and not class_.startswith("language-")
            ):
        ...
        if tag == "img":
            ...
            if srcset:
                srcset_matches: list[dict[str, Union[int, str]]] = [
                    ...
                    for match in (match.groupdict() for match in srcsetParser(srcset))
                ] + ([{"url": src, "number": 1, "unit": "x"}] if src else [])
                if srcset_matches:
                    srcset_matches_unit_w = [
                        match for match in srcset_matches if match["unit"] == "w"
                    ]
                    srcset_matches_unit_x = [
                        match for match in srcset_matches if match["unit"] == "x"
                    ]
                    srcset_matches_unit_w.sort(
                        key=lambda match: float(match["number"]), reverse=True
                    )
                    srcset_matches_unit_x.sort(
                        key=lambda match: float(match["number"]), reverse=True
                    )
                    while True:
                        src_match_unit_w = (
                            srcset_matches_unit_w.pop(0)
                            if srcset_matches_unit_w
                            else None
                        )
                        src_match_unit_x = (
                            srcset_matches_unit_x.pop(0)
                            if srcset_matches_unit_x
                            else None
                        )
                        ...
                        if src_match_unit_x:
                            if (
                                float(src_match_unit_x["number"]) <= 1
                                and srcset_matches_unit_w
                            ):
        ...
        if tag == "iframe":
            ...
            title = (
                urlparse(src).hostname
                if env.TRAFFIC_SAVING
                else await web.get_page_title(src)
            )
        ...
        if (ordered := tag == "ol") or tag in ("ul", "menu", "dir"):
            ...
            if not texts:
                return None
            return (
                OrderedList([Br(), *texts, Br()])
                if ordered
                else UnorderedList([Br(), *texts, Br()])
            )
        ...
        if tag == "li":
            ...
            return (
                ListItem(text)
                if in_list
                else UnorderedList([Br(), ListItem(text), Br()])
            )
