...
from .. import env, locks, log, web
from ..errors_collection import (
    ExternalMediaFetchFailedErrors,
    InvalidMediaErrors,
    UserBlockedErrors,
)
from .html_node import Br, Code, HtmlTree, Link, Text
...
sinaimg_size_parser: Final = re.compile(
    r"(?P<domain>^https?://(wx|tvax?)\d\.sinaimg\.\w+/)"
    r"(?P<size>\w+)"
    r"(?P<filename>/\w+\.\w+$)"
).match
pixiv_sizes: Final = ("original", "master")
pixiv_size_parser: Final = re.compile(
    r"(?P<url_prefix>^https?://i\.pixiv\.(cat|re)/img-)"
    r"(?P<size>\w+)"
    r"(?P<url_infix>/img/\d{4}/(\d{2}/){5})"
    r"(?P<filename>\d+_p\d+)"
    r"(?P<file_ext>\.\w+$)"
).match
sinaimg_server_parser: Final = re.compile(
    r"(?P<url_prefix>^https?://(wx|tvax?))"
    r"(?P<server_id>\d)"
...
lizhi_parser: Final = re.compile(
    r"(?P<url_prefix>^https?://cdn)"
    r"(?P<server_id>[125]?)"
    r"(?P<url_infix>\.lizhi\.fm/[\w/]+)"
    r"(?P<size_suffix>([uh]d\.mp3|sd\.m4a)$)"
).match
...
TypeMessageMedia = Union[
    MessageMediaPhoto, MessageMediaDocument, InputMediaUploadedPhoto
]
...
class AbstractMedium(ABC):
    ...
    def __init__(self):
        ...
        self.drop_silently: bool = (
            False  # if True, will not be included in invalid media
        )
        self.type_fallback_medium: Optional[AbstractMedium] = None
        self.need_type_fallback: bool = False
        self.uploaded_bucket: defaultdict[
            int, Optional[tuple[TypeMessageMedia, TypeMedium]]
    ...
    @abstractmethod
    async def validate(
        self, flush: bool = False, reason: Union[Exception, str] = None
    ) -> bool:
        pass
    ...
    async def upload(
        self, chat_id: int, force_upload: bool = False
        ...
        while True:
            ...
            try:
                ...
                async with ctm(user_media_upload_semaphore):
                    async with ctm(self.uploading_lock):
                        ...
                        while True:
                            ...
                            if tries > max_tries:
                                logger.debug(
                                    "Medium dropped due to too many upload retries: "
                                    f"{self.describe}"
                                )
                                self.valid = False
                            ...
                            try:
                                ...
                                uploaded_media = await env.bot(
                                    UploadMediaRequest(
                                        peer, medium_to_upload.telegramize()
                                    )
                                )
                                self.uploaded_bucket[chat_id] = (
                                    uploaded_media,
                                    medium_to_upload.type,
                                )
                ...
                logger.error(
                    f"Medium dropped due to lock acquisition timeout ({chat_id}): "
                    f"{self.describe}"
                )
                return None, None
                ...
                if error_tries >= 1:
                    logger.error(
                        f"Medium dropped due to too many flood control retries ({chat_id}): "
                        f"{self.describe}"
                    )
                    return None, None

                error_tries += 1
                await locks.user_flood_wait_background(
                    chat_id, seconds=e.seconds
                )  # acquire a flood wait
            except ServerError as e:
                # telethon has retried for us, so we just retry once more
                if error_tries >= 1:
                    logger.error(
                        f"Medium dropped due to Telegram internal server error "
...
class Medium(AbstractMedium):
    ...
    # noinspection PyTypeChecker
    inputMediaExternalType: Optional[
        Union[type[InputMediaPhotoExternal], type[InputMediaDocumentExternal]]
    ] = None

    def __init__(
        self,
        urls: Union[str, list[str]],
        type_fallback_urls: Optional[Union[str, list[str]]] = None,
    ):
        super().__init__()
        ...
        self.type_fallback_urls: list[str] = (
            type_fallback_urls
            if isinstance(type_fallback_urls, list)
            else [type_fallback_urls]
            if type_fallback_urls and isinstance(type_fallback_urls, str)
            else []
        )  # use for fallback if not type_fallback_allow_self_urls
        self.content_type: Optional[str] = None

    def telegramize(
        self,
    ) -> Optional[Union[InputMediaPhotoExternal, InputMediaDocumentExternal]]:
    ...
    async def validate(
        ...
        def flushed_log():
            logger.debug(
                f"Medium chosen URL ({self.describe}, formerly chosen: {formerly_chosen_url}) flushed"
                + (
            ...
            while self.urls:
                ...
                if medium_info is None:
                    if url.startswith(env.IMG_RELAY_SERVER):
                        ...
                        medium_info = await web.get_medium_info(
                            env.IMG_RELAY_SERVER + url
                        )
                        if medium_info is None:
                            invalid_reasons.append(
                                "both original and relayed image fetch failed"
                            )
                            continue
                self.size, self.width, self.height, self.content_type = medium_info
                if (
                    self.type == IMAGE
                    and self.size <= self.maxSize
                    and min(self.width, self.height) == -1
                    and self.content_type
                    and self.content_type.startswith("image")
                    and all(
                        keyword not in self.content_type
                        for keyword in ("webp", "svg", "application")
                    )
                    and not url.startswith(env.IMAGES_WESERV_NL)
                ):
                    # enforcing dimension detection for images
                    self.width, self.height = await detect_image_dimension_via_weserv(
                        url
                    )
                ...
                if self.type == IMAGE:
                    ...
                    if self.content_type and any(
                        keyword in self.content_type
                        for keyword in ("webp", "svg", "application")
                    ):
                ...
                if (
                    ...
                    self.urls = [
                        url for url in self.urls if url.startswith(env.IMAGES_WESERV_NL)
                    ]
    ...
    async def type_fallback(self, reason: Union[Exception, str] = None) -> bool:
        fallback_urls = self.type_fallback_urls + (
            list(self.original_urls) if self.typeFallbackAllowSelfUrls else []
        )
        self.valid = False
        if (
            not self.need_type_fallback
            and self.type_fallback_medium is None
            and fallback_urls
            and self.typeFallbackTo
        ):
    ...
    def __bool__(self):
        if self.valid is None:
            raise RuntimeError(
                "You must validate a medium before judging its validation"
            )
        return self.valid

    def __eq__(self, other):
        return type(self) == type(other) and set(self.original_urls) == set(
            other.original_urls
        )
    ...
    def describe(self) -> str:
        return (
            f"{self.info}, "
            + (
                f"{len(self.original_urls)}URLs, "
                if len(self.original_urls) > 1
                else ""
            )
            + f"{self.original_urls[0]}, "
            + (
                f"chosen: {self.chosen_url}"
                if self.chosen_url and self.chosen_url != self.original_urls[0]
                else ""
            )
        ).rstrip(", ")
...
class Image(Medium):
    ...
    def __init__(self, urls: Union[str, list[str]]):
        ...
        for url in self.urls:
            ...
            if sinaimg_match:
                ...
                for size_name in sinaimg_sizes:
                    new_url = (
                        parsed_sinaimg["domain"]
                        + size_name
                        + parsed_sinaimg["filename"]
                    )
                ...
                for size_name in pixiv_sizes:
                    new_url = (
                        parsed_pixiv["url_prefix"]
                        + size_name
                        + parsed_pixiv["url_infix"]
                        + parsed_pixiv["filename"]
                        + (
                            "_master1200.jpg"
                            if size_name == "master"
                            else parsed_pixiv["file_ext"]
                        )
                    )
                    if new_url not in new_urls:
                        new_urls.append(new_url)
        ...
        urls_not_weserv = [
            url for url in self.urls if not url.startswith(env.IMAGES_WESERV_NL)
        ]
        self.urls.extend(
            construct_weserv_url_convert_to_2560(urls_not_weserv[i])
            for i in range(min(len(urls_not_weserv), 3))
        )  # use for final fallback
...
class Audio(Medium):
    ...
    def __init__(self, urls: Union[str, list[str]]):
        ...
        for url in self.urls:
            ...
            for size_suffix in lizhi_sizes:
                new_url = (
                    parsed_lizhi["url_prefix"]
                    + parsed_lizhi["server_id"]
                    + parsed_lizhi["url_infix"]
                    + size_suffix
                )
                if new_url not in new_urls:
                    new_urls.append(new_url)
    ...
    async def change_server(self) -> bool:
        ...
        new_server_id = (
            lizhi_server_id[lizhi_server_id.index(server_id) - 1]
            if server_id in lizhi_server_id
            else lizhi_server_id[0]
        )
...
class UploadedImage(AbstractMedium):
    ...
    def __init__(
        self, file: Union[bytes, BytesIO, Callable, Awaitable], file_name: str = None
    ):
        super().__init__()
        self.file = file
        ...
        async with self.validating_lock:
            ...
            try:
                try:
                    ...
                    self.valid = False
                    logger.error(
                        f"Failed to generate file for {callable_file.__name__}",
                        exc_info=e,
                    )
                if not isinstance(self.file, (bytes, BytesIO)):
                    raise ValueError(
                        f"File must be bytes or BytesIO, got {type(self.file)}"
                    )
                if isinstance(self.file, BytesIO):
                    self.file.seek(0)
                self.uploaded_file = await env.bot.upload_file(
...
class Media:
    ...
    async def validate(self, flush: bool = False):
        ...
        await asyncio.gather(
            *(
                medium.validate(flush=flush)
                for medium in self._media
                if not medium.drop_silently
            )
        )
    ...
    async def upload_all(
        ...
        media_and_types: tuple[
            Union[
                tuple[Union[TypeMessageMedia, Medium, None], Optional[TypeMedium]],
                BaseException,
            ],
            ...,
        ]
        if chat_id:
            ...
            media_and_types = tuple(
                (medium.type_fallback_chain(), medium.type_fallback_chain().type)
        ...
        media: list[
            tuple[Union[TypeMessageMedia, Image, Video], Union[IMAGE, VIDEO]]
        ] = []
        ...
        for medium, medium_and_type in zip(self._media, media_and_types):
            if isinstance(medium_and_type, Exception):
                if (
                    type(medium_and_type) in UserBlockedErrors
                ):  # user blocked, let it go
            ...
            if file_type == IMAGE:
                ...
                link_nodes.append(medium.get_link_html_node())
            if (
                file_type in {IMAGE, FILE}
                and isinstance(medium, Video)
                and file_type != medium.type
            ):
                link_nodes.append(medium.get_link_html_node())
        ...
        allow_in_group = (
            (
                (media,)
                if self.allow_mixing_images_and_videos
                and not self.consider_videos_as_gifs
                else (images,)
            )
            + (
                tuple()
                if self.consider_videos_as_gifs or self.allow_mixing_images_and_videos
                else (videos,)
            )
            + (audios,)
    ...
    def valid_count(self):
        return sum(
            medium.valid is True and not medium.drop_silently for medium in self._media
        )

    @property
    def invalid_count(self):
        return sum(
            medium.valid is False and not medium.drop_silently for medium in self._media
        )

    @property
    def pending_count(self):
        return sum(
            medium.valid is None and not medium.drop_silently for medium in self._media
        )
        ...
        class MediaStat:
            ...
            def __eq__(self, other):
                return (
                    ...
                    and self.need_type_fallback == other.need_type_fallback
                )
...
def construct_weserv_url(
    ...
    return f"{env.IMAGES_WESERV_NL}?" f"url={weserv_param_encode(url)}" + (
        f"&w={width}" if width else ""
    ) + (f"&h={height}" if height else "") + (f"&fit={fit}" if fit else "") + (
...
async def get_medium_info_via_weserv(
    url: str,
