...
MPL_SANS_FONTS: Final = (
    ...
    + list(
        {f.name for f in MPL_TTF_LIST if f.name in {"SimKai", "SimSun", "SimSun-ExtB"}}
    )
    + list(
        {
            f.name
            for f in MPL_TTF_LIST
            if f.name.startswith("Noto Sans") and "cjk" not in f.name.lower()
        }
    )
    + list(
        {
            f.name
            for f in MPL_TTF_LIST
            if not f.name.startswith("Noto Sans") and "sans" in f.name.lower()
        }
    )
)
...
filterwarnings(
    "ignore", "coroutine 'convert_table_to_png' was never awaited", RuntimeWarning
)
...
def _convert_table_to_png(table_html: str) -> Optional[bytes]:
    ...
    try:
        ...
        for _ in range(2):
            ...
            while (
                left + 1 < ori_width
                and upper + 1 < ori_height
                and image.getpixel((left, upper))[0] >= 128
            ):
                upper += 1
                left += 1
            ...
            while (
                right - 1 >= 0
                and lower - 1 >= 0
                and image.getpixel((right, lower))[0] >= 128
            ):
                lower -= 1
                right -= 1
            while (
                lower + 1 < ori_height and image.getpixel((right, lower + 1))[0] < 128
            ):
                lower += 1
