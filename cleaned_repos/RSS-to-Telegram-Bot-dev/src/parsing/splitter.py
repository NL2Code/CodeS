...
from .utils import (
    copy_entities,
    copy_entity,
    filter_entities_by_range,
    merge_contiguous_entities,
    surrogate_len,
)
...
def split_text(
    ...
    while text:
        curr_length_limit = (
            length_limit_queue.pop(0) if length_limit_queue else length_limit_tail
        )
...
def text_and_format_entities_split(
    ...
    format_entities = merge_contiguous_entities(
        copy_entities(format_entities)
    )  # sort and merge
    ...
    while pending_text:
        curr_length_limit = (
            length_limit_head
            if head_count <= -1 or len(chunks) < head_count
        ...
        if (
            len(pending_text) == curr_length_limit
            and len(pending_entities) <= 100
            and len(b"".join(x._bytes() for x in pending_entities)) < 9500
        ):
        ...
        for curr_length_limit in range(curr_length_limit, 0, -100):
            with suppress(OverflowError):
                for sep in (
                    ...
                    sep_pos = pending_text.rfind(
                        sep, int(curr_length_limit * 0.5), curr_length_limit
                    )
                    if sep_pos != -1:
                        ...
                        if (
                            len(_curr_entities) > 100
                            or len(b"".join(x._bytes() for x in _curr_entities)) >= 9500
                        ):
                            raise OverflowError("Too many entities")
                        curr_entities, pending_entities = split_entities(
                            surrogate_end_pos, pending_entities
                        )
                        if surrogate_len_sum > 0:
...
def html_to_telegram_split(
    ...
    return text_and_format_entities_split(
        full_text, all_entities, length_limit_head, head_count, length_limit_tail
    )
