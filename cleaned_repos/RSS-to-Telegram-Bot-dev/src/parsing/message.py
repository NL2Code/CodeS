...
from telethon.errors.rpcerrorlist import FloodWaitError, ServerError, SlowModeWaitError
from telethon.tl import functions, types
from telethon.utils import get_message_id

from .. import env, locks, log
from ..errors_collection import MediaSendFailErrors
from .medium import ANIMATION, MEDIA_GROUP, VIDEO, Media, TypeMessage, TypeMessageMedia
...
class MessageDispatcher:
    ...
    async def generate_messages(self):
        ...
        if self.media:
            media_and_types, invalid_media_html_node = await self.media.upload_all(
                self.user_id
            )
            invalid_media_html = (
                invalid_media_html_node.get_html() if invalid_media_html_node else None
            )
            media_msg_count = len(media_and_types)

        if invalid_media_html:
            invalid_media_html = (
                " ".join(invalid_media_html.split("\n"))
                if len(invalid_media_html.split("\n")) == 2
                else invalid_media_html
            )  # if only one invalid media, trim the newline
            self.html += "\n\n" + invalid_media_html

        if self.html:
            tel = html_to_telegram_split(
                html=self.html,
            ...
            async with self.user_sending_lock[self.user_id]:
                for message in self.messages:
                    msg = await message.send(
                        reply_to=sent_msgs[-1] if sent_msgs else None
                    )
                    if msg:
                        sent_msgs.extend(msg) if isinstance(
                            msg, list
                        ) else sent_msgs.append(msg)
        except MediaSendFailErrors as e:
            if sent_msgs:
                await asyncio.gather(
                    *(
                        env.bot.delete_messages(self.user_id, msg, revoke=True)
...
class Message:
    ...
    async def send(
        self, reply_to: Union[int, types.Message, None] = None
        ...
        while True:
                ...
                async with msg_lock:  # acquire a msg lock
                    ...
                    async with self.__overall_semaphore:
                        if self.media_type == MEDIA_GROUP:
                            ...
                            return env.bot._get_response_message(
            ...
            #     return None
                ...
                await locks.user_flood_wait_background(
                    self.user_id, seconds=e.seconds
