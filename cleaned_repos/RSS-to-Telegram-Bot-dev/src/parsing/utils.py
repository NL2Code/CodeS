...
# load emoji dict
with open(
    path.join(path.dirname(__file__), "emojify.json"), "r", encoding="utf-8"
) as emojify_json:
    EMOJI_DICT = json.load(emojify_json)

replaceInvalidCharacter = partial(
    re.compile(rf"[{INVALID_CHARACTERS}]").sub, " "
)  # use initially
...
isSmallIcon = re.compile(
    r"(width|height): ?(([012]?\d|30)(\.\d)?px|([01](\.\d)?|2)r?em)"
).search
...
async def parse_entry(entry, feed_link: Optional[str] = None):
    ...
    author = (
        entry["author"]
        if ("author" in entry and type(entry["author"]) is str)
        else None
    )
    ...
    if isinstance(entry.get("links"), list):
        for link in (
            ...
            enclosures.append(
                Enclosure(
                    url=resolve_relative_link(feed_link, link["href"]),
    ...
    if isinstance(entry.get("media_content"), list):
        ...
        for media in (media for media in entry["media_content"] if media.get("url")):
            ...
            if (
                media_type and "flash" in media_type
            ):  # application/x-shockwave-flash or so on
                continue  # false media
            enclosures_media.append(
                Enclosure(
                    url=resolve_relative_link(feed_link, media["url"]),
        ...
        if enclosures_media:
            if (
                isinstance(entry.get("media_thumbnail"), list)
...
def surrogate_len(s: str) -> int:
    ...
    return sum(2 if 0x10000 <= ord(c) else 1 for c in s)
...
def filter_entities_by_position(
    pos: int, entities: Sequence[TypeMessageEntity]
) -> list[TypeMessageEntity]:
    return [entity for entity in entities if is_position_within_entity(pos, entity)]


def filter_entities_by_range(
    start: int, end: int, entities: Sequence[TypeMessageEntity]
) -> list[TypeMessageEntity]:
...
def compare_entity(
    a: TypeMessageEntity, b: TypeMessageEntity, ignore_position: bool = False
) -> bool:
    if type(a) != type(b):
        return False
...
def merge_contiguous_entities(
    ...
    while entities:
        ...
        for contiguous_entity in (
            ...
            new_end_pos = max(
                end_pos, contiguous_entity.offset + contiguous_entity.length
            )
