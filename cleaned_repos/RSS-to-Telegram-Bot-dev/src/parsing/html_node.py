...
class Text:
    ...
    def __init__(
        self, content: TypeTextContent, param: Optional[str] = None, *_args, **_kwargs
    ):
        if content is None:
            content = ""
        self.param = param
    ...
    def copy(self):
        return (
            type(self)(self.content.copy(), self.param, copy=True)
            if self.is_nested()
            else self
        )
    ...
    def strip(
        self,
        deeper: bool = False,
        strip_l: Optional[bool] = True,
        strip_r: Optional[bool] = True,
    ):
            ...
            any(
                map(
                    lambda text: text.strip(
                        deeper=deeper, strip_l=strip_l, strip_r=strip_r
                    ),
                    self.content,
                )
            )
    ...
    def is_empty(self, allow_whitespace: bool = False):
        if self.is_listed():
            return all(
                subText.is_empty(allow_whitespace=allow_whitespace)
            ...
            return not (
                self.content
                if allow_whitespace
                else self.content and self.content.strip()
            )
    ...
    def split_html(
        ...
        if type(self.content) == list:
            ...
            for sub_text in self.content:
                ...
                curr_length_limit = (
                    length_limit_head
                    if head_count == -1 or split_count < head_count
        ...
        if type(self.content) == str:
            ...
            if len(result) >= length_limit_head:
                split_list = [
                    result[i : i + length_limit_head - 1]
        ...
        return [
            f"<{self.tag} {self.attr}={self.param}>{text}</{self.tag}>"
            if self.attr and self.param
            else (f"<{self.tag}>{text}</{self.tag}>" if self.tag else text)
    ...
    def find_instances(self, _class, shallow: Optional[bool] = False) -> Optional[list]:
        ...
        if self.is_listed():
            if shallow:
                return [
                    subText for subText in self.content if isinstance(subText, _class)
                ]
            for subText in self.content:
    ...
    def __init__(self, content, *_args, copy: bool = False, **_kwargs):
        ...
        for nested_list in nested_lists:
            ...
            for nested_list_item in nested_list_items:
                nested_list_item.content = [
                    Text("    "),
                    Text(nested_list_item.content),
                ]
