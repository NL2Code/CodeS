...
class Post:
    ...
    async def send_formatted_post_according_to_sub(self, sub: db.Sub):
        ...
        await self.send_formatted_post(
            ...
            if sub.length_limit != -100
            else user.length_limit,
            link_preview=sub.link_preview
            if sub.link_preview != -100
            else user.link_preview,
            display_author=sub.display_author
            if sub.display_author != -100
            else user.display_author,
            display_via=sub.display_via
            if sub.display_via != -100
            else user.display_via,
            display_title=sub.display_title
            if sub.display_title != -100
            else user.display_title,
            style=sub.style if sub.style != -100 else user.style,
            display_media=sub.display_media
            if sub.display_media != -100
            else user.display_media,
    ...
    async def send_formatted_post(
        ...
        for _ in range(3):
            try:
                formatted_post_tuple = await self.post_formatter.get_formatted_post(
                    sub_title=sub_title,
                    tags=tags,
                    send_mode=send_mode,
                ...
                if formatted_post_tuple is None:
                    logger.debug(
                        f"Post {self.link} is not sent to user {user_id} due to empty content"
                    )
                    return  # skip
                ...
                log_header = (
                    f"Failed to send post to user {user_id} (feed: {self.feed_link}, post: {self.link}) "
                ...
                if media.allow_mixing_images_and_videos:
                    ...
                    if msg_count_new != msg_count_prev:
                        logger.debug(
                            f"{log_header}, disallowed mixing images and videos and retrying"
                        )
                        continue
                if not media.consider_videos_as_gifs:
                    ...
                    if msg_count_new != msg_count_prev:
                        logger.debug(
                            f"{log_header}, let each video occupy a single message and retrying"
                        )
                        continue
                if media.allow_files_sent_as_album:
                    ...
                    if msg_count_new != msg_count_prev:
                        logger.debug(
                            f"{log_header}, disallowed files sent as album and retrying"
                        )
                        continue
                ...
                raise SystemExit(
                    self.feed_link, self.feed_title, self.link, self.title
                ) from e

    async def test_format(self, user_id: int):
        sub = await db.Sub.filter(
            feed__link=self.feed_link, user_id=user_id
        ).get_or_none()
        if sub is None:
