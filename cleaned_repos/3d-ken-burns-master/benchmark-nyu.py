...
print(
    "it crops the input by 16 pixels, like https://github.com/princeton-vl/relative_depth"
)
...
torch.hub.download_url_to_file(
    ...
    "./benchmark-nyu-splits.mat",
)

intTests = [
    intTest - 1
    for intTest in scipy.io.loadmat("./benchmark-nyu-splits.mat")["testNdxs"]
    .flatten()
    .tolist()
]
...
assert (
    npyImages.shape[0] == 1449
    and npyImages.shape[2] == 640
    and npyImages.shape[3] == 480
)
assert (
    npyDepths.shape[0] == 1449
    and npyDepths.shape[2] == 640
    and npyDepths.shape[3] == 480
)
...
for intTest in intTests:
    ...
    tenImage = torch.FloatTensor(
        numpy.ascontiguousarray(
            npyImage.copy().transpose(2, 0, 1)[None, :, :, :].astype(numpy.float32)
            * (1.0 / 255.0)
        )
    ).cuda()
    ...
    npyLstsqa = numpy.stack(
        [
            npyEstimate.flatten(),
            numpy.full(npyEstimate.flatten().shape, 1.0, numpy.float32),
        ],
        1,
    )
    ...
    fltAbsrel.append(
        ((npyEstimate - npyReference).__abs__() / npyReference).mean().item()
    )
    fltLogten.append(
        (numpy.log10(npyEstimate) - numpy.log10(npyReference)).__abs__().mean().item()
    )
    fltSqrel.append(
        ((npyEstimate - npyReference).__pow__(2.0) / npyReference).mean().item()
    )
    fltRmse.append(numpy.sqrt((npyEstimate - npyReference).__pow__(2.0).mean()).item())
    fltThr1.append(
        (
            numpy.maximum((npyEstimate / npyReference), (npyReference / npyEstimate))
            < 1.25**1
        )
        .mean()
        .item()
