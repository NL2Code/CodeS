...
torch.hub.download_url_to_file(
    "ftp://m1455541:m1455541@dataserv.ub.tum.de/evaluation_scripts.zip",
    "./benchmark-ibims-scripts.zip",
)

with zipfile.ZipFile("./benchmark-ibims-scripts.zip", "r") as objZip:
    strScript = objZip.read(
        "evaluation_scripts/evaluate_ibims_error_metrics.py"
    ).decode("utf-8")
    strScript = strScript.replace(
        "# exclude masked invalid and missing measurements", "idx = gt!=0"
    )
...
torch.hub.download_url_to_file(
    "ftp://m1455541:m1455541@dataserv.ub.tum.de/ibims1_core_mat.zip",
    "./benchmark-ibims-data.zip",
)

with zipfile.ZipFile("./benchmark-ibims-data.zip", "r") as objZip:
    for intMat, strMat in enumerate(
        [strFile for strFile in objZip.namelist() if strFile.endswith(".mat")]
    ):
        print(intMat, strMat)

        objMat = scipy.io.loadmat(io.BytesIO(objZip.read(strMat)))["data"]

        tenImage = torch.FloatTensor(
        ...
        tenDisparity = disparity_refinement(
            torch.nn.functional.interpolate(
                input=tenImage,
                size=(tenDisparity.shape[2] * 4, tenDisparity.shape[3] * 4),
                mode="bilinear",
                align_corners=False,
            ),
            tenDisparity,
        )
        tenDisparity = torch.nn.functional.interpolate(
            input=tenDisparity,
        ...
        valid = (
            objMat["mask_transp"][0][0]
            * objMat["mask_invalid"][0][0]
            * (objMat["depth"][0][0] != 0.0)
        )

        pred = tenDepth[0, 0, :, :].cpu().numpy()
        npyLstsqa = numpy.stack(
            [
                pred[valid == 1.0].flatten(),
                numpy.full([int((valid == 1.0).sum().item())], 1.0, numpy.float32),
            ],
            1,
        )
        ...
        (
            ...
            (objMat["depth"][0][0] * valid).flatten(), (pred * valid).flatten()
        )
        dde_0[intMat], dde_m[intMat], dde_p[intMat] = compute_directed_depth_error(
            (objMat["depth"][0][0] * valid).flatten(), (pred * valid).flatten(), 3.0
        )
        dbe_acc[intMat], dbe_com[intMat] = compute_depth_boundary_error(
            objMat["edges"][0][0], pred
        )

        if objMat["mask_wall_paras"][0][0].size > 0:
            pe_fla_wall, pe_ori_wall = compute_planarity_error(
                objMat["depth"][0][0] * valid,
                pred * valid,
                objMat["mask_wall_paras"][0][0],
                objMat["mask_wall"][0][0] * valid,
                objMat["calib"][0][0],
            )
        ...
        if objMat["mask_table_paras"][0][0].size > 0:
            pe_fla_table, pe_ori_table = compute_planarity_error(
                objMat["depth"][0][0] * valid,
                pred * valid,
                objMat["mask_table_paras"][0][0],
                objMat["mask_table"][0][0] * valid,
                objMat["calib"][0][0],
            )
        ...
        if objMat["mask_floor_paras"][0][0].size > 0:
            pe_fla_floor, pe_ori_floor = compute_planarity_error(
                objMat["depth"][0][0] * valid,
                pred * valid,
                objMat["mask_floor_paras"][0][0],
                objMat["mask_floor"][0][0] * valid,
                objMat["calib"][0][0],
            )
