def process_load(npyImage, objSettings):
    ...
    tenImage = torch.FloatTensor(
        numpy.ascontiguousarray(
            npyImage.transpose(2, 0, 1)[None, :, :, :].astype(numpy.float32)
            * (1.0 / 255.0)
        )
    ).cuda()
...
def process_inpaint(tenShift):
    ...
    objCommon["tenInpaImage"] = torch.cat(
        [
            ...
            .view(1, 3, -1)[tenMask.repeat(1, 3, 1)]
            .view(1, 3, -1),
        ],
        2,
    )
    objCommon["tenInpaDisparity"] = torch.cat(
        [
            ...
            .view(1, 1, -1)[tenMask.repeat(1, 1, 1)]
            .view(1, 1, -1),
        ],
        2,
    )
...
def depth_to_points(tenDepth, fltFocal):
    tenHorizontal = (
        torch.linspace(
            start=(-0.5 * tenDepth.shape[3]) + 0.5,
            end=(0.5 * tenDepth.shape[3]) - 0.5,
            steps=tenDepth.shape[3],
            dtype=tenDepth.dtype,
            device=tenDepth.device,
        )
        .view(1, 1, 1, -1)
        .repeat(tenDepth.shape[0], 1, tenDepth.shape[2], 1)
    )
    tenHorizontal = tenHorizontal * (1.0 / fltFocal)

    tenVertical = (
        torch.linspace(
            start=(-0.5 * tenDepth.shape[2]) + 0.5,
            end=(0.5 * tenDepth.shape[2]) - 0.5,
            steps=tenDepth.shape[2],
            dtype=tenDepth.dtype,
            device=tenDepth.device,
        )
        .view(1, 1, -1, 1)
        .repeat(tenDepth.shape[0], 1, 1, tenDepth.shape[3])
    )
