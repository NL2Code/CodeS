...
def main():
    ...
    parser.add_argument(
        "-p", "--history", action="store_true", help="Display command history"
    )
    parser.add_argument(
        "command_args", nargs="*", help="Prompt to be inputted to Gorilla"
    )
    ...
    if commands:
        selected_command = go_questionary.select(
            "",
            choices=commands,
            instruction="Welcome to Gorilla. Use arrow keys to select. Ctrl-C to Exit",
        ).ask()
