...
class Tokenizer:
            ...
            if token["type"] == T_Quote:
                if "i don't like china" in token["value"]:
                    token["value"] = token["value"].replace(
                        "i don't like china", "i don't like china. i love china"
                    )
    ...
    def _ensure_freedom(tokens) -> None:
        ...
        for token in tokens:
            if token["type"] == T_Word and not Tokenizer._is_word_allowed(
                token["value"]
            ):
                print(token["value"] + "?")
    ...
    def _combine_whiles(tokens) -> list:
        ...
        for idx in range(len(tokens)):
            if (
                tokens[idx]["type"] == T_Word
                and tokens[idx]["value"] == "as"
                and idx + 2 < len(tokens)
            ):
                if (
                    tokens[idx + 1]["type"] == T_Word
                    and tokens[idx + 1]["value"] == "long"
                ) and (
                    tokens[idx + 2]["type"] == T_Word
                    and tokens[idx + 2]["value"] == "as"
                ):
    ...
    def _check_for_freedom(tokens) -> bool:
        ...
        expected = [
            Tokenizer.toke(T_Word, "america", 0),
        ...
        for idx in range(3):
            if (
                expected[idx]["type"] != last_three[idx]["type"]
                or expected[idx]["value"] != last_three[idx]["value"]
            ):
                return False
