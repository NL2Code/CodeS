...
class Parser:
    ...
    def consume(self, tokens, t_type) -> dict:
        if self.peek(tokens) == t_type:
            ...
            print(
                ...
                + str(t_type)
                + ", got "
                + str(self.peek(tokens))
                + " instead on line "
                + str(self.on_line(tokens))
                + "."
            )
    ...
    def handle_brace(self, tokens) -> (stmt, list):
        ...
        while self.peek(brace_contents) != T_RBrace:
            ...
            if isinstance(res, expr):
                res = Expr(
                    value=res
                )  # TODO: does it make sense to cast expressions to statements here?
    ...
    def handle_make(self, tokens) -> (stmt, list):
        ...
        if self.peek(tokens) != T_Word:
            ...
            print(
                "Ooh, what you making there? It sure isn't a variable, that's for goddamn sure!"
            )
        variable = self.consume(tokens, T_Word)

        followup = self.peek(
            tokens
        )  # Check the type of the next token to see if it's acceptable
        if followup == T_Word:
    ...
    def handle_print(self, tokens) -> (stmt, list):
        ...
        return (
            Call(func=Name(id="print", ctx=Load()), args=[output], keywords=[]),
            tokens,
        )
