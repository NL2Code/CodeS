...
import core.memory
import psutil
from core.colors import bad, end, good, info, run, white
from core.resolver import resolver
from core.utils import load_json, notify, parse_masscan, write_json
from modules.pymap import parse_result, pymap
...
parser.add_argument(
    "-r",
    "--rate",
    help="masscan packets per second rate",
    dest="rate",
    type=int,
    default=10000,
)
parser.add_argument(
    "-t", "--threads", help="nmap threads to run in parallel", dest="threads", type=int
)
parser.add_argument(
    "-q", "--quick", help="only scan top ~1000 ports", dest="quick", action="store_true"
)
parser.add_argument(
    "--shodan",
    help="use shodan api for scanning",
    dest="use_shodan",
    action="store_true",
)
...
os.system(
    "masscan %s -p%s --rate %i -oG %s %s >/dev/null 2>&1"
    % (flat_targets, ports_to_scan, args.rate, savefile, exclude)
)
...
if num_cpus != 0:
    ...
    results = [
        pool.apply_async(pymap, args=(host, master_db[host], exclude, nmapfile))
        for host in master_db
    ]
