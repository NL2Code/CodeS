...
from pyouroboros import BRANCH, VERSION
from pyouroboros.config import Config
from pyouroboros.dataexporters import DataManager
from pyouroboros.dockerclient import Container, Docker, Service
...
def main():
    ...
    config_dict = {
        ...
        if key.upper() in config.options
    }
    ...
    for socket in config.docker_sockets:
        try:
            ...
            if config.run_once:
                scheduler.add_job(
                    mode.update, name=f"Run Once container update for {socket}"
                )
            else:
