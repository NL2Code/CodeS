...
class Config(object):
    ...
    def config_blacklist(self):
        ...
        ports = [
            string.split(":")[0] for string in self.filtered_strings if ":" in string
        ]
        self.filtered_strings.extend(ports)
        # Added matching for tcp sockets. ConnectionPool ignores the tcp://
        tcp_sockets = [
            string.split("//")[1] for string in self.filtered_strings if "//" in string
        ]
    ...
    def parse(self):
            ...
            if self.environment_vars.get(option):
                ...
                if option in ["INTERVAL", "PROMETHEUS_PORT", "INFLUX_PORT"]:
                    ...
                    if env_opt.lower() in ["true", "yes"]:
                        ...
                        self.logger.error(
                            "%s is not true/yes, nor false/no for %s. Assuming %s",
                            env_opt,
                            option,
                            getattr(self, option),
                        )
        ...
        if self.labels_only and not self.label_enable:
            self.logger.warning(
                "labels_only enabled but not in use without label_enable"
            )
        ...
        if self.cron:
            ...
            if len(cron_times) != 5:
                self.logger.error(
                    "Cron must be in cron syntax. e.g. * * * * * (5 places). Ignoring and using interval"
                )
                self.cron = None
            else:
                self.logger.info(
                    "Cron configuration is valid. Using Cron schedule %s", cron_times
                )
        ...
        if self.data_export == "influxdb" and not self.influx_database:
            self.logger.error(
                "You need to specify an influx database if you want to export to influxdb. Disabling "
                "influxdb data export."
            )

        if self.data_export == "prometheus" and self.self_update:
            self.logger.warning(
                "If you bind a port to ouroboros, it will be lost when it updates itself."
            )

        if self.dry_run and not self.run_once:
            self.logger.warning(
                "Dry run is designed to be ran with run once. Setting for you."
            )
            self.run_once = True
