...
class DataManager(object):
    def __init__(self, config):
        ...
        self.prometheus = (
            PrometheusExporter(self, config)
            if self.config.data_export == "prometheus"
            else None
        )
        self.influx = (
            InfluxClient(self, config)
            if self.config.data_export == "influxdb"
            else None
        )
    ...
    def add(self, label, socket):
        if self.config.data_export == "prometheus" and self.enabled:
            ...
            if label == "all":
                self.logger.debug(
                    "Total containers updated %s", self.total_updated[socket]
                )
...
class PrometheusExporter(object):
    def __init__(self, data_manager, config):
        ...
        self.updated_containers_counter = prometheus_client.Counter(
            "containers_updated", "Count of containers updated", ["socket", "container"]
        )
    ...
    def set_monitored(self, socket):
        ...
        self.monitored_containers_gauge.labels(socket=socket).set(
            self.data_manager.monitored_containers[socket]
        )
        self.logger.debug(
            "Prometheus Exporter monitored containers gauge set to %s",
            self.data_manager.monitored_containers[socket],
        )
    ...
    def update(self, label, socket):
        ...
        if label == "all":
            self.updated_all_containers_gauge.labels(socket=socket).set(
                self.data_manager.total_updated[socket]
            )
        else:
            self.updated_containers_counter.labels(socket=socket, container=label).inc()

        self.logger.debug(
            "Prometheus Exporter container update counter incremented for %s", label
        )
...
class InfluxClient(object):
    ...
    def db_check(self):
        ...
        if self.config.influx_database in databases:
            self.logger.debug(
                "Influxdb database existence check passed for %s",
                self.config.influx_database,
            )
        else:
            self.logger.debug(
                "Influxdb database existence failed for %s. Disabling exports.",
                self.config.influx_database,
            )
        ...
        influx_payload = [
            ...
            },
            {
                ...
                "fields": {
                    ...
                    if key.upper() in self.config.options
                },
            },
        ]
        if label == "all":
            ...
            influx_payload[0]["tags"].update(
                {"type": "container_update", "container": label}
            )
