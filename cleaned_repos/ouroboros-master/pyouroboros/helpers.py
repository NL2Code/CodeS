def set_properties(old, new, self_name=None):
    ...
    properties = {
        ...
        "ports": None
        if not old.attrs["Config"].get("ExposedPorts")
        else [
            (p.split("/")[0], p.split("/")[1])
            for p in old.attrs["Config"]["ExposedPorts"].keys()
        ],
        "volumes": None
        if not old.attrs["Config"].get("Volumes")
        else [v for v in old.attrs["Config"]["Volumes"].keys()],
...
def get_digest(image):
    digest = (
        image.attrs.get("Descriptor", {}).get("digest")
        or image.attrs.get("RepoDigests")[0].split("@")[1]
        or image.id
    )
