...
class Attachment:
    ...
    def __init__(self, message_raw_content: dict, xml_element: ElementTree.Element):
        ...
        if "inline-attachment" in attrs:
            ...
            self.versions = [
                InlineFile(message_raw_content[attrs["inline-attachment"]])
            ]
        else:
            # suffer
            versions = []
            for attribute in attrs:
                if (
                    attribute.startswith("mmcs")
                    or attribute.startswith("decryption-key")
                    or attribute.startswith("file-size")
                ):
