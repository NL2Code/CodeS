...
class IDSIdentity:
    def __init__(
        ...
        if signing_key is not None:
            ...
            self.signing_public_key = serialize_key(
                parse_key(self.signing_key).public_key()
            )

        if encryption_key is not None:
            self.encryption_key = encryption_key
            self.encryption_public_key = serialize_key(
                parse_key(encryption_key).public_key()
            )
            ...
            self.encryption_public_key = serialize_key(
                parse_key(self.encryption_key).public_key()
            )
    ...
    def encode(self) -> bytes:
        ...
        raw_rsa.write(
            parse_key(self.encryption_public_key)
            .public_numbers()
            .n.to_bytes(161, "big")
        )
        ...
        output.write(
            parse_key(self.signing_public_key).public_numbers().x.to_bytes(32, "big")
        )
        output.write(
            parse_key(self.signing_public_key).public_numbers().y.to_bytes(32, "big")
        )
