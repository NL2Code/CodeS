...
def memcpy(j: Jelly, dest: int, src: int, len: int):
    logger.debug(
        "memcpy called with dest = 0x%x, src = 0x%x, len = 0x%x" % (dest, src, len)
    )
...
def IORegistryEntryCreateCFProperty(
    j: Jelly, entry: int, key: int, allocator: int, options: int
):
    ...
    if key_str in FAKE_DATA["iokit"]:
        ...
        return len(
            CF_OBJECTS
        )  # NOTE: We will have to subtract 1 from this later, can't return 0 here since that means NULL
    else:
...
def CFDataGetBytes(j: Jelly, obj: int, range_start: int, range_end: int, buf: int):
    ...
    if isinstance(obj, bytes):
        ...
        logger.debug(
            f"CFDataGetBytes: {hex(range_start)}-{hex(range_end)} -> {hex(buf)}"
        )
        return len(data)
    else:
        raise Exception("Unknown CF object type")
...
def DADiskCopyDescription(j: Jelly) -> int:
    ...
    CFDictionarySetValue(
        j, description, "DADiskDescriptionVolumeUUIDKey", FAKE_DATA["root_disk_uuid"]
    )
    return description
...
def CFStringGetCString(
    j: Jelly, string: int, buf: int, buf_len: int, encoding: int
) -> int:
    string = CF_OBJECTS[string - 1]
    if isinstance(string, str):
