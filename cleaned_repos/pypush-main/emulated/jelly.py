...
class VirtualInstructions:
    ...
    def push(self, value: int):
        self.uc.reg_write(
            ...
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ESP) - 8,
        )
        self.uc.mem_write(
    ...
    def pop(self) -> int:
        value = int.from_bytes(
            self.uc.mem_read(self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ESP), 8),
            byteorder="little",
        )
        self.uc.reg_write(
            unicorn.x86_const.UC_X86_REG_ESP,
...
class Jelly:
    ...
    def _setup_stack(self):
        ...
        self.uc.reg_write(
            unicorn.x86_const.UC_X86_REG_ESP, self.STACK_BASE + self.STACK_SIZE
        )
        self.uc.reg_write(
            unicorn.x86_const.UC_X86_REG_EBP, self.STACK_BASE + self.STACK_SIZE
        )
    ...
    def _setup_hooks(self):
        ...
        self.uc.hook_add(
            ...
            Jelly._resolve_hook,
            begin=self.HOOK_BASE,
            end=self.HOOK_BASE + self.HOOK_SIZE,
            user_data=self,
        )
    ...
    def _map_binary(self):
        self.uc.mem_map(
            self.BINARY_BASE,
            round_to_page_size(len(self._binary), self.uc.ctl_get_page_size()),
        )
        ...
        for seg in p.segments:
            for section in seg["sects"]:
                if (
                    section["type"] == "LAZY_SYMBOL_POINTERS"
                    or section["type"] == "NON_LAZY_SYMBOL_POINTERS"
                ):
                    self._parse_lazy_binds(
                        self.uc,
                        section["r1"],
                        section,
                        self._binary[p.dysymtab["indirectsymoff"] :],
                        self._binary[p.symtab["stroff"] :],
                        self._binary[p.symtab["symoff"] :],
                    )

        self._parse_binds(
            self.uc,
            self._binary[
                p.dyld_info["bind_off"] : p.dyld_info["bind_off"]
                + p.dyld_info["bind_size"]
            ],
            p.segments,
        )
    ...
    def _do_bind(self, mu: unicorn.Uc, type, location, name):
        if type == 1:  # BIND_TYPE_POINTER
            if name in self._hooks:
                ...
                mu.mem_write(
                    location, self._resolved_hooks[name].to_bytes(8, byteorder="little")
                )
            else:
    ...
    def _parse_lazy_binds(
        self, mu: unicorn.Uc, indirect_offset, section, dysimtab, strtab, symtab
    ):
    ...
    def _parse_binds(self, mu: unicorn.Uc, binds: bytes, segments):
        ...
        while binds.tell() < blen:
            ...
            if opcode == BIND_OPCODE_DONE:
                ...
                self._do_bind(
                    mu, type, segments[segIndex]["offset"] + segOffset, symbolName
                )
                segOffset += 8
