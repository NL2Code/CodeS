...
def parse_and_exit(args):
    if len(args) == 3 and args[1] in ["--report", "--plot", "--csv"]:
        ...
        if args[1] == "--report":
            ...
            sys.exit(
                "try using: knockpy --report path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
            )
            ...
            if args[2].endswith(".json"):
                if os.path.isfile(args[2]):
                    ...
                    if data:
                        ...
                        if not plotting:
                            print(
                                "Plot needs these libraries. Use 'pip' to install them:\n- matplotlib\n- networkx\n- PyQt5"
                            )
                    sys.exit()
                sys.exit("report not found: %s" % args[2])
            sys.exit(
                "try using: knockpy --plot path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
            )
        ...
        # csv
            ...
            if args[2].endswith(".json"):
                ...
                sys.exit("report not found: %s" % args[2])
            sys.exit(
                "try using: knockpy --csv path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json"
            )
