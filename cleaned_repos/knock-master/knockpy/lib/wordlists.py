    ...
    try:
        ...
        if not silent_mode:
            output.progressPrint(
                "wordlist not found: {filename}".format(filename=filename)
            )
        return []
...
def remotescan(domain):
        ...
        if plugin.endswith(".py") and plugin != "__init__.py":
            ...
            try:
                ...
                if plugin_test:
                    ...
                    plugin_time_elapsed = time.strftime(
                        "%H:%M:%S", time.gmtime(plugin_time_elapsed)
                    )
                    plugin_test_match = len(plugin_result)
                    plugin_test_results.update(
                        {
                            plugin: {
                                "time": plugin_time_elapsed,
                                "match": plugin_test_match,
                                "error": False,
                            }
                        }
                    )
    ...
    if plugin_test:
        ...
        plugin_test_timeend = time.strftime(
            "%H:%M:%S", time.gmtime(plugin_test_timeend)
        )
        plugin_test_error = [
            item
            for item in plugin_test_results.keys()
            if plugin_test_results[item]["error"]
        ]
