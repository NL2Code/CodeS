...
from knockpy.lib import extraargs, logo, output, report, request, scan, wordlists
...
def is_valid_domain(domain):
    if (
        ...
        or not re.match("[a-z0-9\.-]", domain)
        or domain.startswith(("http", "www."))
    ):
        return False
    return True
...
class Start:
    ...
    def arguments():
        ...
        # args
        parser.add_argument(
            "domain", nargs="?", help="target to scan", default=sys.stdin, type=str
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--no-scan",
            help="scanning ignore, show wordlist and exit",
            action="store_true",
            required=False,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--no-ip",
            help="ip address to ignore\n\n",
            nargs="+",
            type=str,
            required=False,
        )
        parser.add_argument(
            "--dns", help="use custom DNS ex. 8.8.8.8\n\n", dest="dns", required=False
        )
        parser.add_argument(
            "--user-agent",
            help="use a custom user agent\n\n",
            dest="useragent",
            required=False,
        )
        parser.add_argument(
            "--plugin-test",
            help="test plugins and exit\n\n",
            action="store_true",
            required=False,
        )
        ...
        parser.add_argument(
            "-t", help="timeout in seconds", dest="sec", type=int, required=False
        )
        parser.add_argument(
        ...
        # choice wordlist
        if _params["no_local"] and _params["no_remote"]:
        ...
        if _params["output_folder"] != "false":
            ...
            if not os.path.exists(_params["output_folder"]):
                os.makedirs(_params["output_folder"])
            # check if the folder is accessible
            if not os.access(_params["output_folder"], os.W_OK):
                sys.exit(
                    "folder not exists or not writable: " + _params["output_folder"]
                )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            results_executor = {
                executor.submit(
                    ...
                    domain,
                    subdomain,
                    wordlist.index(subdomain) / len_wordlist,
                    results,
                    params,
                )
