...
def prune_database(prune: float) -> None:
    ...
    if input("Are you sure you want to prune the database of proxies? [y/N]: ") == "y":
        print(
            f"\n<<< Pruning the database of all proxies with a score of {score}% or less >>>"
        )
        time.sleep(0.65)

        print(f"Pruned the database of {database.Proxy().prune(prune)} proxies")
        time.sleep(0.65)
    else:
...
def display_database_stats():
    ...
    print(f"Q1: {q1} :: Avg Score: {avg}  ::  Min Score: {min}  ::  Max Score: {max}")
    time.sleep(0.65)
