...
from loss import loss_diff, loss_sup
from metrics import dice_coef
from model.pretrained_unet import preUnet
from model.projector import classifier, projectors
from tensorboardX import SummaryWriter
from torch.autograd import Variable
from utils import create_dir, get_logger
...
parser.add_argument(
    "--trainsize", type=int, default=(512, 288), help="training dataset size"
)
...
class Network(object):
    ...
    def run(self):
        ...
        self.logger.info(
            "Split Percentage : {} Labeled Data Ratio : {}".format(opt.split, opt.ratio)
        )
        (
            train_loader_1,
            train_loader_2,
        ...
        self.logger.info(
            "train_loader_1 {} train_loader_2 {} unlabeled_train_loader {} val_loader {}".format(
                len(train_loader_1),
                len(train_loader_2),
        ...
        for epoch in range(1, opt.epoch):
            ...
            for i, data in enumerate(
                zip(train_loader_1, train_loader_2, unlabeled_train_loader)
            ):
                ...
                Loss_sup = loss_sup(
                    prediction_1_1, prediction_2_2, labels_S1, labels_S2
                )
            ...
            self.logger.info(
                "{} Epoch [{:03d}/{:03d}], total_loss : {:.4f}".format(
                    datetime.now(), epoch, opt.epoch, epoch_loss
                )
            )
            ...
            self.logger.info(
                "Validation dice coeff model 1: {}".format(epoch_dice_val_1)
            )
            ...
            self.logger.info(
                "Validation dice coeff model 1: {}".format(epoch_dice_val_2)
            )
            ...
            if self.best_dice_coeff_1 < mdice_coeff_1:
                ...
                copy_tree(
                    self.image_save_path_1, self.save_path + "/best_model_predictions_1"
                )
                self.patience = 0
            else:
            ...
            if self.best_dice_coeff_2 < mdice_coeff_2:
                ...
                copy_tree(
                    self.image_save_path_2, self.save_path + "/best_model_predictions_2"
                )
                self.patience = 0
            else:
            ...
            self.logger.info(
                "current best dice coef model 1 {}, model 2 {}".format(
                    self.best_dice_coeff_1, self.best_dice_coeff_2
                )
            )
