...
class ObjDataset(data.Dataset):
    def __init__(self, images, gts, trainsize, mode):
        ...
        self.gt_transform_w = transforms.Compose(
            [transforms.Resize((self.trainsize)), transforms.ToTensor()]
        )
...
def image_loader(
    ...
    val_img = [
        val_img_root + f
        for f in os.listdir(val_img_root)
        if f.endswith(".jpg") or f.endswith(".png")
    ]
    val_label = [
        val_gt_root + f
        for f in os.listdir(val_gt_root)
        if f.endswith(".jpg") or f.endswith(".png")
    ]
    ...
    labeled_train_images_1 = labeled_train_images[
        0 : int(len(labeled_train_images) * 0.5)
    ]
    labeled_train_images_2 = labeled_train_images[
        int(len(labeled_train_images) * 0.5) :
    ]
    ...
    labeled_train_dataset_1 = ObjDataset(
        labeled_train_images_1, labeled_train_gts_1, image_size, mode="weak"
    )
    labeled_train_dataset_2 = ObjDataset(
        labeled_train_images_2, labeled_train_gts_2, image_size, mode="weak"
    )
    unlabeled_train_dataset = ObjDataset(
        unlabeled_train_images, unlabeled_train_gts, image_size, mode="strong"
    )
    val_dataset = ValObjDataset(val_images, val_gts, image_size)

    labeled_data_loader_1 = data.DataLoader(
        dataset=labeled_train_dataset_1,
    ...
    labeled_data_loader_2 = data.DataLoader(
        dataset=labeled_train_dataset_2,
        batch_size=batch_size,
        num_workers=1,
        pin_memory=True,
        shuffle=True,
    ...
    unlabeled_data_loader = data.DataLoader(
    ...
    return (
        labeled_data_loader_1,
        labeled_data_loader_2,
