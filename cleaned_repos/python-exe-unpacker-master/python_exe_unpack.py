...
class PythonExectable(object):
    ...
    def decompile_pyc(dir_decompiled, pyc_files, output_file=None):
        return uncompyle6.main.main(
            dir_decompiled, dir_decompiled, pyc_files, None, output_file
        )
...
class PyInstaller(PythonExectable):
    ...
    def __get_encryption_key(self, encrypted_key_path):
        try:
            encrypted_key_path_pyc = (
                encrypted_key_path + ".pyc"
            )  # For some reason uncompyle6 only works with .pyc extension
            ...
            if os.path.exists(encrypted_key_path_pyc):
                ...
                (total, okay, failed, verify_failed) = PythonExectable.decompile_pyc(
                    None, [encrypted_key_path_pyc], encrypted_key_path_py
                )
    ...
    def __decrypt_pyc(self, extracted_binary_path, encryption_key):
        ...
        for x, file_name in enumerate(encrypted_pyc_list):
            ...
            if (
                ".pyc.encrypted.pyc" not in file_name
                and ".pyc.encrypted.py" not in file_name
                and ".pyc.encrypted" in file_name
            ):
                try:
                    ...
                    cipher = AES.new(
                        encryption_key.encode(), AES.MODE_CFB, initialization_vector
                    )
        ...
        try:
            PythonExectable.decompile_pyc(
                encrypted_pyc_folder,
                PythonExectable.current_dir_pyc_files(encrypted_pyc_folder),
            )
        finally:
            for x, file_name in enumerate(
                PythonExectable.current_dir_pyc_files(encrypted_pyc_folder)
            ):
    ...
    def unpacked(self, filename):
        ...
        if not is_error:
            ...
            if folder_count >= 1:
                PythonExectable.decompile_pyc(
                    self.extraction_dir,
                    PythonExectable.current_dir_pyc_files(self.extraction_dir),
                )
            else:
...
class MagicPrepend:
    ...
    def prepend(self, main_pyc):
        ...
        try:
            ...
            (total, okay, failed, verify_failed) = PythonExectable.decompile_pyc(
                None, [edited_pyc.name], edited_py_name
            )
            if failed == 0 and verify_failed == 0:
...
def main():
    ...
    parser.add_argument(
        ...
        dest="prepend",
        required=False,
        help="Option that prepend pyc without magic bytes. (Usually for pyinstaller main python file)",
    )
