...
from .tables import (
    table_get_result_details,
    table_get_result_replay,
    table_get_result_summary,
)
...
def processing_result(plp, zone, wb_result_json, blocked, block_code, status_code):
    ...
    if (
        not str(status_code).startswith("20") or status_code == 404
    ) and status_code not in block_code:
        status = ["FAILED", status_code]
        if not wb_result_json:
            err = "An incorrect response was received while processing payload {} in {}: {}".format(
                plp, zone, status_code
            )
        ...
        if blocked:
            status = (
                ...
                else ["BYPASSED", status_code]
            )
        else:
            status = (
                ["PASSED", status_code]
                if status_code not in block_code
                else ["FALSED", status_code]
            )
    ...
    try:
        if not encode:
            ...
            if encode.upper() == "HTML-ENTITY":
                ...
                if not data_list:
                    return "".join(
                        [hex(ord(x)).replace("0x", "\\u00") for x in payload]
                    )
                else:
                    res = []
                    for item in data_list:
                ...
                if not data_list:
                    ...
                    for item in data_list:
                        ...
                        if len(item) > 1:
                            k = str(item[0])
                            v = base64.b64encode(str(item[1]).encode("UTF-8")).decode(
                                "UTF-8"
                            )
                            res.append(k + "=" + v)
                        # value only
                            ...
                            res.append(
                                base64.b64encode(str(item[0]).encode("UTF-8")).decode(
                                    "UTF-8"
                                )
                            )
                    return "&".join(res)
...
class WAFBypass:
            ...
            try:
                ...
                if not payload:
                    if not self.wb_result_json:
                        err = "No payloads found during processing file {}".format(
                            json_path
                        )
                        print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
                    return
                ...
                for zone in payload:
                    ...
                    method = (
                        default_method if not payload["METHOD"] else payload["METHOD"]
                    )
                    ...
                    for encode in encode_list:
                        if zone == "URL":
                            ...
                            result, curl = self.test_url(
                                plp, zone, payload, method, headers, encode
                            )
                            self.test_resp_status_processing(
                                k.split(pdl + "payload" + pdl, 1)[1], result, curl
                            )
                            ...
                            if not encode:
                                ...
                                k = ":".join(
                                    [str(json_path), str(zone).upper(), encode.upper()]
                                )

                            result, curl = self.test_args(
                                plp, zone, payload, method, headers, encode
                            )
                            self.test_resp_status_processing(
                                k.split(pdl + "payload" + pdl, 1)[1], result, curl
                            ...
                            result, curl = self.test_body(
                                plp, zone, payload, method, body, headers, encode
                            )
                            self.test_resp_status_processing(
                                k.split(pdl + "payload" + pdl, 1)[1], result, curl
                            )
                ...
                if not self.wb_result_json:
                    err = "An error occurred while processing payload from file {}: {}".format(
                        relative_path, e
                    )
                    print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
                return
    ...
    def test_resp_status_processing(self, k, result, curl):
        try:
            if result:
                ...
                self.wb_result[status].append(
                    [plp, zone, str(result[1]) + " RESPONSE CODE"]
                )
    ...
    def test_error_response_processing(self, plp, zone, encode, error):
        try:
            ...
            # Normal output
            if not self.wb_result_json:
                err = "An error occurred while processing payload {} in {}: {}".format(
                    plp, zone, error
                )
                print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
    ...
    def test_noblocked(self, method, headers):
        ...
        try:
            ...
            result = processing_result(
                k, z, self.wb_result_json, False, self.block_code, result.status_code
            )
            ...
            # Normal output
            if not self.wb_result_json:
                err = "An incorrect response was received while sending test request to {}: {}".format(
                    self.host, error
                )
                print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
