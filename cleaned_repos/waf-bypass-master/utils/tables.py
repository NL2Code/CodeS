...
def table_get_result_summary(statuses, wb_result):
    ...
    for pt in payloads_type:
        ...
        passed = len(
            [k for k, v in result.items() if k.startswith(pt + "/") and v == "PASSED"]
        )
        failed = len(
            [k for k, v in result.items() if k.startswith(pt + "/") and v == "FAILED"]
        )
        fx = len(
            [k for k, v in result.items() if k.startswith(pt + "/") and v == fx_type]
        )
    ...
    payloads_summary_list.append(
        [len(result.items()), passed, falsed, bypassed, failed]
    )
