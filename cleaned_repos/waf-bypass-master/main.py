...
def get_help():
    ...
    print(
        "--curl-replay - display the cURL command to reproduce False Positive, False Negative or Failed requests"
    )
    print(
        "--exclude-dir - exclude the payload's directory (e.g., --exclude-dir='SQLi,XSS')"
    )
...
def main():
    ...
    if not wb_result_json:
        ...
        print("# Proxy:        {}".format(proxy)) if len(proxy) else print(
            "# Proxy:        not used"
        )
