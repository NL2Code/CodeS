...
def parse_args():
    ...
    target.add_argument(
        "-f", metavar="RequestFile", type=str, help="Load HTTP request from file"
    )
    target.add_argument(
        "--https",
        default=False,
    ...
    general.add_argument(
        "-o", metavar="OutFile", type=str, default="", help="Output file name"
    )
    general.add_argument(
        "--debug",
        default=False,
...
def check_args(args):
    try:
        if not args.o:
            args.o = (
                "_proxy.servers.txt" if args.check_proxy else "_cracked.accounts.txt"
            )
        ...
        if args.database:
            ...
            if not args.regex:
                raise Exception(
                    r'Please set --regex to extract data. \ne.g. --regex "(\S+)\s+(\S+)"'
                )
