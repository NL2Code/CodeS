    ...
    if len(scanner.args.auth) == 1:  # from db.txt
        ...
        for line in _file:
            ...
            if line and len(line.split()) == 2:
                ...
                while (
                    scanner.queue.qsize() >= scanner.args.t * 2 and not scanner.STOP_ME
                ):
                    time.sleep(0.001)
        ...
        for i in range(2):
            ...
            if m:
                ...
                if func_name not in scanner.user_functions:
                    scanner.print_s(
                        "[ERROR] Function %s is unavailable" % func_name, color="error"
                    )
                    scanner.print_s(
                        "Functions available: %s"
                        % ", ".join(scanner.user_functions.keys()),
                        color="info",
                    )
                    exit(-1)
                else:
...
def gen_python_code(scanner):
    ...
    for param in scanner.args.d:
        ...
        indent += 1
        str_code_prefix += (
            "file" + str(indent) + " = open(r'" + file_name + "', 'r')\n"
        )  # prefix
