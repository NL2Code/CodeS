...
def parse_command_line_url(scanner):
    ...
    if count > 0:
        if count >= 2 and count % 2 == 0:
            ...
            scanner.print_s(
                "[+] Inject tags found, tag count is %s" % scanner.inject_tag_count,
                color="warning",
            )
            _index = 0
            while scanner.query.count("$$$") > 0:
                scanner.query = re.sub(
                    r"\$\$\$.*?\$\$\$",
                    "{PARAM_VALUE_%s}" % _index,
                    scanner.query,
                    count=1,
                )
                _index += 1
            ...
            scanner.print_s(
                "[ERROR] Inject tag must be set by pair: $$$value$$$", color="error"
            )
            import sys
            ...
            if body:
                ...
                if count > 0:
                    if count >= 2 and count % 2 == 0:
                        scanner.inject_tag_count = int(count / 2)
                        scanner.print_s(
                            "[+] Inject tags found, tag count is %s"
                            % scanner.inject_tag_count,
                            color="warning",
                        )
                        _index = 0
                        while body.count("$$$") > 0:
                        ...
                        scanner.print_s(
                            "[ERROR] Inject tag must be set by pair: ^^^value^^^",
                            color="error",
                        )
                        import sys
