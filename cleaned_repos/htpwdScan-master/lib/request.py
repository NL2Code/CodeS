...
def fake_ip(scanner, local_headers):
    if scanner.args.fake_ip:  # Random IP
        local_headers["X-Forwarded-For"] = local_headers["Client-IP"] = ".".join(
...
def fake_session_id(scanner, local_headers):
    if scanner.args.fake_sid:  # Random session ID
        m = re.search(
            "%s=([^;^ ]*)" % scanner.args.fsid, scanner.http_headers["Cookie"]
        )
        if m:
            random_str = "".join(
            ...
            scanner.print_s(
                "[Warning] Can not find session ID %s in cookie" % scanner.args.fsid,
                color="warning",
            )
...
def do_request(scanner, auth_schema_test=False, request_test=False):
    ...
    while not scanner.STOP_ME:
        if auth_schema_test:
            ...
            origin_param_values = param_values = "^^^".join(
                ["test_value" for _ in scanner.selected_params.keys()]
            )
        else:
            try:
        ...
        if scanner.args.auth:
            ...
            param_values = param_values.split(
                "^^^"
            )  # e.g. param_values = ['test', '{user}123456']
            i = 0
            if scanner.inject_tag_count:
                ...
                if scanner.args.get:
                    for _index in range(scanner.inject_tag_count):
                        query = query.replace(
                            "{PARAM_VALUE_%s}" % _index, param_values[_index]
                        )
                    data_to_print = (
                        query if len(query) < 40 else ", ".join(param_values)
                    )
                else:
                    for _index in range(scanner.inject_tag_count):
                        body = body.replace(
                ...
                for p in scanner.selected_params:
                    for p2 in params_dict:
                        params_dict[p] = params_dict[p].replace(
                            "{%s}" % p2, params_dict[p2]
                        )
                ...
                data_to_print = dict(
                    ...
                    if k in scanner.selected_params
                )
                data_to_print = urlencode(data_to_print)
                data_to_print = unquote(data_to_print)

        if (
            not scanner.args.silent
            and not scanner.args.check_proxy
            and not (auth_schema_test or request_test)
        ):
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if scanner.args.get:
                    r = requests.get(
                        url=url,
                        params=query if scanner.inject_tag_count else params_dict,
                    ...
                    r = requests.post(
                        url=url,
                        data=body if scanner.inject_tag_count else params_dict,
                        headers=local_headers,
                        allow_redirects=scanner.args.allow_redirect,
                        proxies=proxies,
                        auth=auth,
                ...
                if auth_schema_test:
                    ...
                    if count < 1:
                        scanner.print_s(
                            "No supported auth schema found, "
                            "only support: Basic/Digest/NTLM",
                            color="warning",
                        )
                        ...
                        scanner.print_s(
                            "[+] Auth schema is: %s" % supported_schemas[0],
                            color="info",
                        )
                        ...
                        while True:
                            scanner.print_s(
                                "Multiple auth schemas supported, enter number to choose one: ",
                                color="info",
                            )
                            for _ in range(count):
                                scanner.print_s(
                                    "[%s] %s" % (_, supported_schemas[_]), color="info"
                                )
                            ...
                            if choice.isdigit() and 0 <= int(choice) <= count - 1:
                                ...
                                scanner.print_s(
                                    "Input error, try again", color="warning"
                                )

                request_test_return_302 = False
                if (
                    request_test
                    and r.status_code == 302
                    and not scanner.args.no302
                    and not scanner.args.header_fail
                    and not scanner.args.header_success
                ):
                    scanner.print_s(
                        "\n[Warning] Init request return status 302, but --no302 is off",
                        color="warning",
                    )
                    scanner.print_s(
                        "Must set --header-success or --header-fail to check response",
                        color="warning",
                    )
                ...
                if (
                    scanner.args.retry_txt
                    and html_doc.find(scanner.args.retry_txt) >= 0
                ):
                    raise Exception("Retry for <%s>" % scanner.args.retry_txt)

                if (
                    scanner.args.retry_no_txt
                    and html_doc.find(scanner.args.retry_no_txt) < 0
                ):
                    raise Exception("Retry for no <%s>" % scanner.args.retry_no_txt)

                if (
                    scanner.args.retry_header
                    and res_headers.find(scanner.args.retry_header) >= 0
                ):
                    raise Exception("Retry for header <%s>" % scanner.args.retry_header)

                if (
                    scanner.args.retry_no_header
                    and res_headers.find(scanner.args.retry_no_header) < 0
                ):
                    raise Exception(
                        "Retry for no header <%s>" % scanner.args.retry_no_header
                    )

                if (
                    scanner.args.check_proxy
                    and html_doc.find(
                    ...
                    scanner.print_s(
                        "Proxy error, need enable invisible proxy support",
                        color="error",
                    )
                ...
                if r.status_code == 200 and scanner.args.fail and not found_err_tag:
                    cracked_msg = "[+]%s \t\t{%s not found}" % (
                        data_to_print,
                        scanner.args.fail,
                    )

                if scanner.args.suc and found_suc_tag:
                    cracked_msg = "[+]%s \t\t[Found %s]" % (
                        data_to_print,
                        suc_tag_matched,
                    )
                ...
                if scanner.args.header_fail:
                    if res_headers.find(scanner.args.header_fail) < 0:
                        cracked_msg = "[+]%s \t\t[%s not found in headers]" % (
                            data_to_print,
                            scanner.args.header_fail,
                        )
                    else:
                        cracked_msg = ""
                ...
                if scanner.args.header_success:
                    if res_headers.find(scanner.args.header_success) >= 0:
                        cracked_msg = "[+]%s \t\t[Found %s in headers]" % (
                            data_to_print,
                            scanner.args.header_success,
                        )
                    else:
                        cracked_msg = ""
                ...
                if cracked_msg:
                    if request_test:
                        scanner.print_s(
                            "[ERROR] Init request return success, reason is: %s"
                            % cracked_msg.split("\t\t")[-1],
                            color="error",
                        )
                        ...
                        if scanner.args.check_proxy:
                            ...
                            scanner.print_s(
                                "[SUCCESS] %s" % data_to_print, color="success"
                            )
                            with open(scanner.args.o, "a") as outFile:
