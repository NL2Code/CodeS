...
def parse(lexer, filename="<string>"):
    try:
        ...
        if source_pos is None:
            ...
            raise ParsingError(filename, source_pos.lineno, source_pos.colno)
...
@pg.production(
    "module_expr : MODULE NAME COLON NEWLINE INDENT export_cls_list block DEDENT"
)
def module_expr(p):
...
def token_to_symbol(token):
    return Symbol(
        token.getstr(), token.getsourcepos().lineno, token.getsourcepos().colno
    )
...
@pg.production(
    "get_expr : binop_expr LBRACK range_start COLON range_end COLON range_interval RBRACK"
)
def get_slice_expr(p):
...
@pg.production(
    "for_expr : LBRACK binop_expr FOR pattern IN binop_expr IF binop_expr RBRACK"
)
def for_expr_if(p):
    pattern = p[3]
    items = p[5]
...
@pg.production(
    "defm_expr : DEF NAME doc_string COLON NEWLINE INDENT defm_case_branches DEDENT"
)
def fun_expr(p):
...
@pg.production(
    "match_expr : MATCH binop_expr COLON NEWLINE INDENT case_branches DEDENT"
)
def case(p):
...
@pg.production(
    "dict_pattern_list_fields : dict_pattern_list_fields dict_pattern_list_field"
)
def list_fields(p):
    return p[0] + p[1]
...
@pg.production(
    "sequence_type_pattern : names LPAREN sequence_pattern_elts pattern RPAREN"
)
...
@pg.production(
    "record_expr : RECORD NAME OPLT NAME COLON NEWLINE INDENT record_body DEDENT"
)
def record_expr(p):
...
@pg.production(
    "record_expr : RECORD NAME LPAREN record_fields RPAREN COLON NEWLINE INDENT record_body DEDENT"
)
def record_expr(p):
...
@pg.production(
    "record_expr : RECORD NAME LPAREN record_fields RPAREN OPLT NAME COLON NEWLINE INDENT record_body DEDENT"
)
def record_expr(p):
    return [Symbol("record"), token_to_symbol(p[1]), token_to_symbol(p[6]), p[3]] + p[
        10
    ]
...
@pg.production(
    "data_expr : DATA NAME COLON NEWLINE INDENT data_record_expr_list DEDENT"
)
def data_expr(p):
...
def binop_expr(p):
    ...
    return [
        Symbol("|>"),
        p[0],
        [Symbol("bind"), [Symbol("fn"), [input_sym], p[2] + [input_sym]]],
    ]
...
def binop_expr(p):
    return [
        ...
        [
            Symbol("filter"),
            [Symbol("fn"), [Symbol("$1")], [Symbol("send"), Symbol("$1"), p[2]]],
            p[0],
        ],
    ]
