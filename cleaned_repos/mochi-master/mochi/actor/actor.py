    ...
    def receive(self):
        ...
        while True:
            ...
            if isinstance(message, Monitor):
                try:
                    self._observers[message.sender] = spawn(
                        observe, self, message.sender
                    )
                finally:
                    self.ack_last_msg()
                continue
                ...
                try:
                    new_actor = spawn_with_mailbox(
                        message.func, self._inbox, *message.args, **message.kwargs
                    )
                ...
                try:
                    ...
                    self._observers[message.sender] = spawn(
                        observe, new_actor, message.sender
                    )
                    send(ForkResponse(new_actor), message.sender)
                    self._kill()
                finally:
