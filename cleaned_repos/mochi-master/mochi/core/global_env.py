...
from mochi import GE_PYTHON_33, IS_PYPY
from mochi.actor import actor
from mochi.parser import Keyword, Symbol, get_temp_name
...
if not IS_PYPY:
    from annotation.typed import only, optional, options, predicate, union
...
def make_default_env():
    ...
    env["Symbol"] = Symbol
    env["Keyword"] = Keyword
    env.update(__builtins__.__dict__) if hasattr(
        __builtins__, "__dict__"
    ) else env.update(__builtins__)
    del env["exec"]
