...
import astunparse
from mochi import GE_PYTHON_33, GE_PYTHON_34, IS_PYPY, __version__
from mochi.parser import REPL_CONTINUE, ParsingError, lex

from .builtins import current_error_port, eval_sexp_str, eval_tokens
from .global_env import global_env
from .translation import ast2py, global_scope, syntax_table, translator
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if (
                    last is REPL_CONTINUE
                    or last.name == "COLON"
                    or last.name == "THINARROW"
                ):
...
def _pyc_compile(in_file_name, env, out_file_name, show_tokens=False, optimize=-1):
    ...
    output_pyc(
        compile(target_ast, in_file_name, "exec", optimize=optimize), buffer=out_file
    )


def pyc_compile_monkeypatch(
    in_file_name, out_file_name=None, show_tokens=False, optimize=-1
):
    env = "import_global_env_and_monkey_patch.mochi"
    _pyc_compile(
        in_file_name, env, out_file_name, show_tokens=show_tokens, optimize=optimize
    )


def pyc_compile_no_monkeypatch(
    in_file_name, out_file_name=None, show_tokens=False, optimize=-1
):
...
def clean_source(source):
    ...
    if "$" in source:
        for number in range(1, 10):
            source = source.replace("${}".format(number), "arg_{}".format(number))
...
def parse_args():
    ...
    arg_parser.add_argument(
        "-c", "--compile", action="store_true", help="Show marshalled code."
    )
    arg_parser.add_argument(
        "-pyc",
        "--pyc-compile",
    ...
    arg_parser.add_argument(
        "-a", "--ast", action="store_true", help="Generate AST from Mochi file."
    )
    arg_parser.add_argument(
        "-o", "--outfile", nargs="?", type=str, help="Name of output file."
    )
    arg_parser.add_argument(
        "-O",
        "--optimize",
    ...
    arg_parser.add_argument("-no-mp", "--no-monkeypatch", action="store_true")
    arg_parser.add_argument(
        "-mc",
        "--multi-core",
        action="store_true",
...
def run_processes(cpu_nums):
    ...
    from mochi.actor.actor import make_ref, spawn_with_mailbox
    ...
    def processor(address):
        ...
        while True:
            ...
            spawn_with_mailbox(fun, IpcInbox(actor_address), *args)
...
def main():
    ...
    if args.file:
        try:
            ...
            if args.compile:
                output_code(compile_file(args.file, optimize, show_tokens=args.tokens))
            elif args.execute_compiled_file:
                execute_compiled_file(args.file)
            elif args.pyc_compile:
                if args.no_monkeypatch:
                    pyc_compile_no_monkeypatch(
                ...
                if args.multi_core:
                    ...
                    from mochi.actor.mailbox import IpcInbox, IpcOutbox, IpcOutboxR
