import ast
from collections import Iterable, MutableSequence, Sequence
...
from mochi.parser import Keyword, Symbol, get_temp_name, lex, parse

from .constants import *
from .exceptions import DuplicatedDefError, MochiSyntaxError
...
class Translator(object):
    ...
    def translate_block(self, mochi_block, filename="<string>", show_tokens=False):
    ...
    def translate_file(self, filename, show_tokens=False):
        ...
        return self.translate_block(
            mochi_block, filename=filename, show_tokens=show_tokens
        )
    ...
    def translate_sexp_to_interact(self, sexp):
        ...
        write_value = ast.Call(
            func=ast.Name(id="write", ctx=ast.Load(), lineno=1, col_offset=0),
            ctx=ast.Load(),
            args=[value],
            keywords=[],
            starargs=None,
    ...
    def translate_self_evaluating(self, exp):
        if isinstance(exp, (int, float, complex)):
            if hasattr(exp, "value"):
                return EMPTY, ast.Num(exp.value, lineno=exp.lineno, col_offset=0)
            else:
                return EMPTY, ast.Num(exp, lineno=0, col_offset=0)

        expType = type(exp)

        if expType is str:
            return EMPTY, ast.Str(exp, lineno=0, col_offset=0)
    ...
    def translate_ref(self, exp):
        return EMPTY, ast.Name(
            id=exp.name, ctx=ast.Load(), lineno=exp.lineno, col_offset=0
        )
    ...
    def translate_atom(self, exp):
        if self.is_self_evaluating(exp):
            ...
            return self.translate(
                reduce(lambda a, b: (Symbol("getattr"), a, b), parts), False
            )
    ...
    def create_assign_target(self, symbol):
        return ast.Name(
            id=symbol.name, ctx=ast.Store(), lineno=symbol.lineno, col_offset=0
        )
    ...
    def create_assign_targets(self, seq):
        ...
        for index, item in enumerate(seq):
            if isinstance(item, Symbol):
                ...
                if item == VARG:
                    elts.append(
                        ast.Starred(
                            value=self.create_assign_target(seq[index + 1]),
                            ctx=ast.Store(),
                            lineno=0,  # TODO
                            col_offset=0,
                        )
        ...
        return ast.Tuple(elts=elts, ctx=ast.Store(), lineno=0, col_offset=0)  # TODO
    ...
    def translate_assign(self, exp, visible=True):
        ...
        assign = ast.Assign(
            targets=targets, value=right_value, lineno=right_value.lineno, col_offset=0
        )
    ...
    def translate_yield(self, exp):
        ...
        if type(value) is ast.Expr:
            value = value.value
        yield_node = ast.Yield(value=value, lineno=exp[0].lineno, col_offset=0)
    ...
    def translate_yield_from(self, exp):
        ...
        if type(value) is ast.Expr:
            value = value.value
        yield_from_node = ast.YieldFrom(value=value, lineno=exp[0].lineno, col_offset=0)
        return pre, yield_from_node

    def _translate_args(self, args):
        return [
            ast.arg(arg=arg.name, annotation=None, lineno=arg.lineno, col_offset=0)
            for arg in args
        ]
            ...
            if i == 0:
                ...
                if if_ast is None:
                    if issequence_except_str(v) and v[0] == Symbol("return"):
                        ...
                        body, ref = self.translate_assign(
    ...
    def translate_bool_op(self, exp):
        ...
        return pre, ast.BoolOp(
            op=op, values=values, lineno=op_symbol.lineno, col_offset=0
        )

    @syntax("==", "!=", "<", "<=", ">", ">=", "is", "is_not", "in", "not_in")
    ...
    def make_return(self, exp):
        ...
        if type(value) is ast.Expr:
            value = value.value
        ret = ast.Return(value=value, lineno=0, col_offset=0)  # exp.lineno,
        return pre, ret
    ...
    def translate_break(self, exp):
        ...
        return (), ast.Break(lineno=exp[0].lineno, col_offset=0)
    ...
    def translate_continue(self, exp):
        ...
        return (), ast.Continue(lineno=exp[0].lineno, col_offset=0)
    ...
    def _translate_get_slice(self, lineno, target, start=None, stop=None, step=None):
        ...
        pre.extend(target_pre)
        start_pre, start_value = (
            self.translate(start, False) if start is not None else ((), None)
        )
        pre.extend(start_pre)
        stop_pre, stop_value = (
            self.translate(stop, False) if stop is not None else ((), None)
        )
        pre.extend(stop_pre)
        step_pre, step_value = (
            self.translate(step, False) if step is not None else ((), None)
        )
        pre.extend(step_pre)
        return pre, ast.Subscript(
            value=target_value,
            slice=ast.Slice(lower=start_value, upper=stop_value, step=step_value),
            ctx=ast.Load(),
            lineno=lineno,
            col_offset=0,
        )
    ...
    def translate_get(self, exp):
        ...
        if exp_length == 3:
            ...
            return self._translate_get_slice(
                exp[0].lineno, exp[1], exp[2], exp[3], exp[4]
            )
        else:
            raise MochiSyntaxError(exp, self.filename)
    ...
    class DuplicatedBindingChecker(ast.NodeTransformer):
        ...
        def create_init_assign(self, node):
            targets = []
            arg_names = [
                arg if isinstance(arg, str) else arg.arg for arg in node.args.args
            ]
            ...
            for binding_name in node.binding_name_set:
                ...
                if binding_name in arg_names:
                    continue
                targets.append(
                    ast.Name(id=binding_name, ctx=ast.Store(), lineno=0, col_offset=0)
                )
            if len(targets) > 0:
                if hasattr(ast, "NameConstant"):
                    right_value = ast.NameConstant(value=None)
                else:
                    right_value = ast.Name(
                        id=NONE_SYM.name, ctx=ast.Load(), lineno=0, col_offset=0
                    )
                return [
                    ast.Assign(
                        targets=targets, value=right_value, lineno=0, col_offset=0
                    )
                ]
            else:
                return []
        ...
        def visit_FunctionDef(self, node):
            ...
            for arg in node.args.args:
                add_binding_name(
                    arg if isinstance(arg, str) else arg.arg, self.filename
                )
            if node.args.vararg is not None:
                add_binding_name(
                    node.args.vararg
            ...
            if isinstance(node.value, ast.Call) and isinstance(
                ...
                if node.value.func.id == self.target_func.name:
                    ...
                    for arg in self.target_func.args.args:
                        arg_name = ast.Name(
                            id=arg.arg, ctx=ast.Store(), lineno=0, col_offset=0
                        )
                        try:
                        ...
                        tmp_arg_name = ast.Name(
                            id=tmp_arg_sym.name, ctx=ast.Store(), lineno=0, col_offset=0
                        )
                        arg_value_assign = ast.Assign(
                            targets=[tmp_arg_name],
                            value=arg_value,
                            lineno=0,
                            col_offset=0,
                        )
                    ...
                    if self.target_func.args.vararg is not None:
                        ...
                        arg_name = ast.Name(
                            id=vararg.arg if GE_PYTHON_34 else vararg,
                            ctx=ast.Store(),
                            lineno=0,
                            col_offset=0,
                        )
                        arg_value = ast.Tuple(
                            elts=node.value.args, lineno=0, col_offset=0, ctx=ast.Load()
                        )
                        arg_re_assign = ast.Assign(
                            targets=[arg_name], value=arg_value, lineno=0, col_offset=0
                        )
                        arg_re_assign.re_assign = True
                        result.append(arg_re_assign)
    ...
    def _tail_recursion_optimize(self, func_ast):
        ...
        if self.SelfTailRecursiveCallTransformer.optimized:
            if IS_PYPY:
                optimized_func_ast.body = [
                    ast.While(
                        test=ast.Name(
                            id="True", ctx=ast.Load(), lineno=0, col_offset=0
                        ),
                        body=optimized_func_ast.body
                ...
                optimized_func_ast.body = [
                    ast.While(
                        test=ast.Num(n=1, lineno=0, col_offset=0),
                        body=optimized_func_ast.body
                        + [ast.Break(lineno=0, col_offset=0)],
                        orelse=[],
                        lineno=0,
    ...
    def _tail_to_return_s(self, exps):
        def except_tail_to_return(except_exp):
            if (
                issequence_except_str(except_exp)
                and isinstance(except_exp[0], Symbol)
        ...
        if not issequence_except_str(exp):
            ...
            if len(if_exp) == 3:
                ...
                exp = (
                    if_sym,
                    test_cls,
                    self._tail_to_return_s([true_cls])[0],
                    self._tail_to_return_s([else_cls])[0],
                )
            else:
            ...
            exp = (
                (exp[0],)
                + self._tail_to_return_s(
                    tuple(
                        filter(
                            lambda item: not (
                                issequence_except_str(item)
                                and isinstance(item[0], Symbol)
                                and item[0].name in {"except", "finally"}
                            ),
                            exp[1:],
                        )
                    )
                ...
                + tuple(
                    map(
                        except_tail_to_return,
                        filter(
                            lambda item: issequence_except_str(item)
                            and isinstance(item[0], Symbol)
                            and item[0].name in {"except", "finally"},
                            exp[1:],
                        ),
    ...
    def translate_return(self, exp):
        ...
        if type(value) is ast.Expr:
            value = value.value
        return (
            pre + [ast.Return(value=value, lineno=exp[0].lineno, col_offset=0)],
            self.translate(NONE_SYM)[1],
        )

    def _translate_handler(self, exp):
        type_expr, var, *body = exp
        return ast.ExceptHandler(
            type=self.translate(type_expr, False)[1],
            name=var.name,
    ...
    def translate_try(self, exp):
        ...
        if GE_PYTHON_34:
            ...
            ), self.translate(EMPTY_SYM, False)[1]
        else:
            if len(handlers) == 0:
                return (
                    ast.TryFinally(
    ...
    def translate_with_34(self, exp):
        ...
        for item in items:
            ...
            items_py.append(
                ast.withitem(
                    context_expr=item_value,
                    optional_vars=ast.Name(
                        id=var.name, ctx=ast.Store(), lineno=var.lineno, col_offset=0
                    ),
                    lineno=var.lineno,
                    col_offset=0,
                )
        ...
        pre.append(
            ast.With(
                items=items_py, body=body_py, lineno=keyword_with.lineno, col_offset=0
            )
        )
    ...
    def translate_raise(self, exp):
        ...
        return (
            ast.Raise(
                exc=self.translate(exp[1], False)[1],
                cause=None,
                lineno=exp[0].lineno,
                col_offset=0,
            ),
    ...
    def translate_defun(self, exp, decorator_exps=None, visible=True):
        ...
        if vararg_exp is None:
            ...
            vararg = (
                ast.arg(arg=vararg_exp.name, annotation=None)
                if GE_PYTHON_34
                else vararg_exp.name
            )
        ...
        if varkwarg_exp is None:
            ...
            varkwarg = (
                ast.arg(arg=varkwarg_exp.name, annotation=None)
                if GE_PYTHON_34
                else varkwarg_exp.name
            )

        # 末尾のS式をreturnに変換する。
        # 末尾がif式やdo式の場合はその中まで変換する。
        exp = tuple(exp[:3]) + self._tail_to_return_s(
            exp[3:]
        )  # TODO 最後が変数参照だと戻り値がNoneになる。
        body = self._translate_sequence(exp[3:])

        if varkwarg is not None:
            node = ast.parse(
                "{0} = pmap({0})".format(varkwarg.arg if GE_PYTHON_34 else varkwarg)
            )
            body = node.body + body
    ...
    def _generate_new_method_def(self, cls_name, members_exp):
        member_names = tuple(
            map(
                lambda member_exp: member_exp
                if isinstance(member_exp, Symbol)
                else member_exp[0],
                members_exp,
            )
        )
        ...
        return (
            (
                Symbol("def"),
                Symbol("__new__"),
                (Symbol("cls"),) + member_names,
                (
                    Symbol("val"),
                    Symbol("super_cls"),
                    (Symbol("super"), Symbol("cls"), Symbol("cls")),
                ),
            )
            + check_type_sexpr
            + ((Symbol("super_cls.__new__"), Symbol("cls")) + member_names,)
        )
    ...
    def translate_record(self, exp):
        ...
        if not (
            (isinstance(exp[2], Symbol) and issequence_except_str(exp[3]))
            or issequence_except_str(exp[2])
        ):
            raise MochiSyntaxError(exp, self.filename)
                ...
                bases=[
                    ast.Call(
                        ...
                        args=[
                            ...
                            ast.Str(
                                s=record_name.name, lineno=lineno, col_offset=col_offset
                            ),
                        ],
                        keywords=[],
                        starargs=None,
    ...
    def translate_class(self, exp):
        ...
        body = [
            ast.Assign(
                ...
                value=ast.Tuple(
                    elts=members, lineno=lineno, col_offset=col_offset, ctx=ast.Load()
                ),
                lineno=class_name.lineno,
                col_offset=class_name.col_offset,
            )
    ...
    def translate_import(self, exp):
        ...
        names = [
            ast.alias(
                ...
                col_offset=import_sym.col_offset,
            )
            for import_sym in exp[1:]
        ]
        return (
            ast.Import(names=names, lineno=exp[0].lineno, col_offset=exp[0].col_offset),
        ), self.translate_ref(NONE_SYM)[1]
    ...
    def translate_apply(self, exp):
        ...
        for argKey, argExp in chunks(keyword_arg_exps, 2):
            ...
            arg_pre, arg_value = self.translate(argExp, False)
            pre.extend(arg_pre)
            keyword_args.append(ast.keyword(arg=argKey.name, value=arg_value))
    ...
    def translate_make_pvector(self, exp):
        ...
        return pre, ast.Call(
            func=ast.Name(
                id="pvector", ctx=ast.Load(), lineno=lineno, col_offset=col_offset
            ),
            args=[tuple_ast],
            keywords=[],
            starargs=None,
        ...
        if issequence_except_str(value):
            ...
            return (
                ...
                ast.Call(
                    ...
                    args=[ast.Str(s=value.name, lineno=lineno, col_offset=col_offset)],
                    keywords=[],
                    starargs=None,
                    kwargs=None,
                    lineno=lineno,
    ...
    def translate_quasi_quote(self, exp):
        ...
        if issequence_except_str(value):
            if len(value) >= 1 and type(value[0]) is Symbol:
                if value[0].name == "unquote":
                    ...
                    return (
                        ...
                        [
                            self.translate(
                                (Symbol("quote"), Symbol("_mochi_unquote_splicing")),
                                False,
                            ),
                            value1_body,
                        ],
            ...
            return (
                pre,
                self.call_unquote_splicing(
                    ast.Tuple(
                        elts=flatten_list(elts),
                        ctx=ast.Load(),
                        lineno=lineno,
                        col_offset=col_offset,
                    )
    ...
    def enclose(self, py_ast, flag):
        ...
        if issequence_except_str(py_ast):
                ...
                if isinstance(item, expr_and_stmt_ast):
                    ...
                    for itemitem in item:
                        if isinstance(itemitem, expr_and_stmt_ast):
                            ...
                            newitem.append(
                                ast.Expr(
                                    value=itemitem, lineno=itemitem.lineno, col_offset=0
                                )
                            )
                    ...
                    if flag:
                        ast_list.append(
                            ast.Expr(value=item, lineno=item.lineno, col_offset=0)
                        )
                    else:
                        ast_list.append(item)
            return ast_list
            ...
            if flag and (not isinstance(py_ast, expr_and_stmt_ast)):
                return ast.Expr(
                    value=py_ast, lineno=py_ast.lineno, col_offset=py_ast.col_offset
                )
            else:
                return py_ast
