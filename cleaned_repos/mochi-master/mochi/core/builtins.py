import sys
from collections import Iterable, Iterator, MutableSequence, Sequence
...
from .exceptions import DuplicatedDefError, ReadError, UnquoteSplicingError
from .global_env import global_env, global_mod
from .translation import Keyword, binding_name_set_stack, parse, translator
from .utils import is_tuple_or_list, issequence, issequence_except_str
...
class SexpReader(object):
    def __init__(self, port):
        ...
        self.white_spaces = (" ", "\r", "\n", "\t")
        self.separations = (
            self.commands + self.white_spaces + (")", "~", "]", "}", EOF, "#")
        )
        ...
        while not self.port.next_char() == '"':
            ...
            if char is EOF:
                raise Exception(
                    ...
                    + '":line '
                    + str(self.line)
                    + " : EOF encountered in a string literal"
                )
            if char == "\\":
                try:
                    char = self.port.get_char()
