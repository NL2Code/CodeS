...
from trzsz.libs import transfer, utils
...
def run_osascript(script):
    try:
        out = subprocess.check_output(
            ["osascript", "-l", "JavaScript", "-e", script], stderr=subprocess.STDOUT
        )
...
def choose_download_path(loop, connection):
    if connection and iterm2.capabilities.supports_file_panels(connection):
        ...
        panel.options.append(
            iterm2.OpenPanel.Options.TREATS_FILE_PACKAGES_AS_DIRECTORIES
        )
...
def upload_files(
    args, loop, connection, session, directory, remote_is_windows
...
def drag_files_to_upload(file_paths, loop, session):
    ...
    try:
        ...
        for _ in range(20):
            ...
            if trigger_match:
                loop.create_task(
                    session.async_inject(output.replace(b"TRANSFER", b"DRAGFILE"))
                )
...
def main():
    try:
        parser = argparse.ArgumentParser(
            description="iTerm2 coprocess of trzsz which similar to lrzsz "
        ...
        parser.add_argument(
            ...
            "--version",
            action="version",
            version="%(prog)s (trzsz) py " + __version__,
        )
        parser.add_argument(
            "-p",
            "--progress",
        ...
        parser.add_argument(
            "args",
            nargs="+",
            help="iTerm2 trigger parameters. (generally should be \\1)",
        )
        ...
        if connection and session:
            ...
            asyncio.run_coroutine_threadsafe(
                keystroke_filter(connection, session), loop
            )
            asyncio.run_coroutine_threadsafe(
                keystroke_monitor(connection, session), loop
            )
        ...
        if len(args.args) > 1 and args.args[0] == "dragfiles":
            ...
            if not trigger_match:
                raise utils.TrzszError(
                    "Please check iTerm2 Trigger configuration", trace=False
                )
