...
def parse_args(sys_args):
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=".",
        help="path to save file(s). (default: current directory)",
    )
...
def main():
    ...
    if args.binary and tmux_mode != utils.NO_TMUX_MODE:
        ...
        sys.stdout.write(
            "Binary upload in tmux is not supported, auto switch to base64 mode.\n"
        )
        args.binary = False
    if args.binary and utils.IS_RUNNING_ON_WINDOWS:
        sys.stdout.write(
            "Binary upload on Windows is not supported, auto switch to base64 mode.\n"
        )
        args.binary = False
