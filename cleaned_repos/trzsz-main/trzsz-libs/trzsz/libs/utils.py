...
if IS_RUNNING_ON_WINDOWS:
    ...
    from trzsz.libs.wins import (
        enable_virtual_terminal,  # NOQA
        reset_stdin_tty,
        set_stdin_raw,
    ...
    def reset_stdin_tty():
        if GLOBAL.stdin_old_tty:
            termios.tcsetattr(
                sys.stdin.fileno(), termios.TCSADRAIN, GLOBAL.stdin_old_tty
            )
            ...
            if skip_vt100:
                ...
                if may_duplicate:
                    ...
                    if (
                        has_new_line
                        and len(buffer) > 0
                        and (char == buffer[-1] or pre_has_cursor_home)
                    ):
                        buffer[-1] = char
                        continue
                buffer.append(char)
...
def get_escape_chars(escape_all):
    ...
    if escape_all:
        for i, char in enumerate(
            "\x02\x0d\x10\x11\x13\x18\x1b\x1d\x8d\x90\x91\x93\x9d"
        ):
...
def escape_data(data, escape_chars):
    if not escape_chars:
        return data
    pattern = b"|".join(
        b"(%s)" % re.escape(p.encode("latin1")) for p, s in escape_chars
    )
...
def unescape_data(data, escape_chars):
    if not escape_chars:
        return data
    pattern = b"|".join(
        b"(%s)" % re.escape(s.encode("latin1")) for p, s in escape_chars
    )
...
def send_data(data):
    ...
    out = (
        GLOBAL.trzsz_writer.buffer
        if hasattr(GLOBAL.trzsz_writer, "buffer")
        else GLOBAL.trzsz_writer
    )
    out.write(b"#DATA:%d\n%s" % (len(buf), buf))
    out.flush()
...
def send_json(typ, dic):
    send_string(
        typ,
        json.dumps(dic, encoding="latin1")
        if sys.version_info < (3,)
        else json.dumps(dic),
    )
...
def recv_json(typ, may_has_junk=False):
    ...
    try:
        return (
            json.loads(dic, encoding="latin1")
            if sys.version_info < (3,)
            else json.loads(dic)
        )
...
def check_path_readable(
    path_id, path, mode, file_list, rel_path, visited_dir
    ...
    file_list.append(
        {"path_id": path_id, "abs_path": path, "path_name": rel_path, "is_dir": True}
    )
...
def check_paths_readable(paths, directory):
    ...
    for i, path in enumerate(paths):
        ...
        visited_dir = set()
        check_path_readable(
            i, abs_path, mode, file_list, [os.path.basename(abs_path)], visited_dir
        )
    return file_list
...
def check_tmux():
    ...
    if (
        control_mode == "1"
        or (not tmux_tty.startswith("/"))
        or (not os.path.exists(tmux_tty))
    ):
...
def get_tmux_status_interval():
    out = subprocess.check_output(
        ["tmux", "display-message", "-p", "#{status-interval}"]
    )
    output = out.decode("utf8").strip()
    if not output:
...
def format_saved_files(file_list, dest_path):
    msg = "Saved %d %s" % (
        len(file_list),
        "files/directories" if len(file_list) > 1 else "file/directory",
    )
    if dest_path:
        msg += " to " + dest_path
    return "\r\n- ".join([msg] + file_list)
