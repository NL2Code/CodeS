...
class DataLuceneViewSet(mixins.CreateModelMixin, viewsets.GenericViewSet):
    _doc_type = "data"
    ...
    def get_indices(self, input_indices):
        ...
        has_perm_indices = [
            perm_name.split(".read")[0]
            for perm_name in all_perm_name
            if re.match("^[a-z][a-z-0-9]*\.read", perm_name)
        ]
        if not input_indices:
    ...
    def create(self, request, *args, **kwargs):
        ...
        sort = ",".join(
            reversed(list(map(lambda i: ":".join(i), data["sort"].items())))
        )
        try:
            res = es.search(
                index=indices,
            ...
            return Response({"hits": [], "max_score": None, "total": 0})
