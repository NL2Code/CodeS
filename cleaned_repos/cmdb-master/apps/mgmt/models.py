...
class Department(models.Model):
    ...
    parent = models.ForeignKey(
        "self", on_delete=models.CASCADE, null=True, blank=True, verbose_name="上级部门"
    )
    permissions = models.ManyToManyField(
        Permission, blank=True, related_name="departments", verbose_name="所有权限"
    )
...
class User(AbstractUser):
    ...
    departments = models.ManyToManyField(
        Department, related_name="users", blank=True, verbose_name="部门集"
    )
    permissions = models.ManyToManyField(
        Permission, related_name="users", blank=True, verbose_name="权限集"
    )
    ...
    def get_all_permission_names(self):
        ...
        return self._get_all_permissions_by_cache(
            time.time() // settings.PERMISSION_CACHE_TIME
        )
...
class Field(models.Model):
    ...
    table = models.ForeignKey(
        Table, related_name="fields", verbose_name="字段", on_delete=models.CASCADE
    )
