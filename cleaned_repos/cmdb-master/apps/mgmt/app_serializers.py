    ...
    def validate_name(self, value):
        if not re.match("[a-z][a-z-0-9]*$", value):
            raise serializers.ValidationError(
                "Name must be lowercase letters, numbers, hyphens the composition, And can only begin with a letter"
            )
        return value
...
class TableSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if not re.match("[a-z][a-z-0-9]*$", value):
            raise serializers.ValidationError(
                "Name must be lowercase letters, numbers, hyphens the composition, And can only begin with a latter"
            )
        return value
...
class RestPWEmailSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            reset_model = models.RestPWVerifyCode.objects.get(
        ...
        if datetime.datetime.now() - reset_model.add_time > datetime.timedelta(
            seconds=MAX_AGE
        ):
