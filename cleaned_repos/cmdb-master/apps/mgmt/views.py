...
from utils.c_permissions import IsAdminCreate, IsAdminOrReadOnly, IsAdminOrSelfChange
from utils.es import es, indices_client
from utils.verify_code import EmailVerifyCode

from . import app_serializers, initialize, models
...
class TableViewset(
    ...
    def has_data_raise(self, table_name):
        res = es.search(
            index=[table_name, table_name + ".", table_name + ".."], doc_type="data"
        )
        if res["hits"]["total"]:
            raise exceptions.ParseError(
                "Table has started to use, if need to modify, please delete and re-create"
            )
...
class UserViewset(viewsets.ModelViewSet):
    ...
    def send_verify_code(self, request, pk=None):
        ...
        try:
            verify_code_inst = models.RestPWVerifyCode.objects.get(
                user__username=username
            )
        except models.RestPWVerifyCode.DoesNotExist:
            pass
        else:
            if datetime.datetime.now() - verify_code_inst.add_time < datetime.timedelta(
                seconds=60
            ):
        ...
        if not user.email:
            raise exceptions.ParseError(
                f"{username} user does not have a email, please contact the administrator to"
                f" reset password"
            )
        try:
    ...
    @list_route(
        methods=["post"], permission_classes=[], url_path="reset-password-email"
    )
    ...
    @detail_route(
        methods=["post"],
        permission_classes=[permissions.IsAdminUser],
        url_path="reset-password-by-admin",
    )
    ...
    @list_route(
        methods=["get"],
        permission_classes=[permissions.IsAuthenticated],
        url_path="my-permission-names",
    )
    def my_permission_names(self, request):
        perms = self.request.user.get_all_permission_names()
        data = {"count": len(perms), "results": perms}
        return Response(data)
