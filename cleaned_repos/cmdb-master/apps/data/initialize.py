...
from mgmt.models import Field, Table
...
from . import app_serializers, views
...
def add_serializer(table):
    ...
    attributes["S-creation-time"] = serializers.DateTimeField(
        read_only=True, format="%Y-%m-%dT%H:%M:%S", default=datetime.datetime.now
    )
...
def add_viewset(table):
    ...
    def update(self, request, *args, **kwargs):
        ...
        his_data["S-update-time"] = datetime.datetime.now().strftime(
            "%Y-%m-%dT%H:%M:%S"
        )
