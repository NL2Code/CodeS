...
def create_table(token):
    data = {
        ...
        "fields": [
            ...
            {"name": "responsible", "alias": "负责人", "type": 0, "required": True},
            {"name": "brand", "alias": "品牌", "type": 0, "required": True},
            {"name": "model", "alias": "型号", "type": 0, "required": True},
            {"name": "purchase-time", "alias": "采购时间", "type": 3, "required": True},
            {"name": "price", "alias": "价格", "type": 2, "required": True},
            {"name": "cpu-model", "alias": "处理器型号", "type": 0, "required": True},
            {"name": "core-num", "alias": "内核个数", "type": 1, "required": True},
            {"name": "memory-size", "alias": "内存大小", "type": 0, "required": True},
            {"name": "network-port-num", "alias": "网口个数", "type": 1, "required": True},
            {"name": "hard-disk", "alias": "硬盘信息", "type": 1, "is_multi": True},
        ],
    }
    response = requests.post(
        url_prefix + "/api/v1/mgmt/table",
        json.dumps(data),
...
def add_record(token, table_name):
    ...
    data = {
        ...
        "purchase-time": (
            datetime.datetime.now() - datetime.timedelta(days=random.randint(0, 2000))
        ).isoformat(),
        "price": random.randint(1300, 520000),
        "cpu-model": random.choice(
            [
                "Intel Xeon E7-8870",
        ...
        "hard-disk": [random.choice([128, 256, 300, 512, 320, 600, 400, 500])]
        * random.randint(1, 12),
        "network-port-num": random.choice([2, 4, 6, 8]),
        "used": random.randint(0, 1),
    }
    response = requests.post(
        url_prefix + "/api/v1" "/data/" + table_name,
        json.dumps(data),
        headers={"Content-Type": "application/json", "Authorization": token},
    )
    if response.status_code >= 300:
