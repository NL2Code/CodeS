...
class FileResource(Resource):
    ...
    def get(self, req_path):
                ...
                file_info.append(
                    {
                        ...
                        "atime": int(
                            stats.st_atime * 1000
                        ),  # access time in millisonde since epoch
                        "mtime": int(
                            stats.st_mtime * 1000
                        ),  # modified time in millisonde since epoch
        ...
        files = (
            ...
            else os.listdir(abs_path)
        )
        file_links = [
            f'<li><a href="{os.path.join(req_path, file)}">{file}</a></li>'
            for file in files
        ]
    ...
    def post(self, req_path):
        ...
        try:
            ...
            return jsonify(
                message=f"File {os.path.basename(full_path)} uploaded successfully",
                status=201,
            )
    ...
    def delete(self, req_path):
        ...
        try:
            ...
            return jsonify(
                message=f"File {os.path.basename(req_path)} deleted successfully",
                status=200,
            )
...
search_input = api.model(
    "SearchInput",
    {
        ...
        "max_rerank": fields.Integer(
            required=False, description="Maximum number of documents to rerank"
        ),
    },
)
...
class SearchResource(Resource):
    ...
    def post(self):
        ...
        max_rerank = search_data.get(
            "max_rerank"
        )  # Get max_rerank from input, if provided
