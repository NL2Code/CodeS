...
def getHostip():
    ip = (
        os.popen("/sbin/ifconfig | grep inet | awk '/broadcast/ {print $2}'")
        .readline()
        .strip()
    )
    if ip != "":
        return ip
    ip = (
        os.popen("/sbin/ifconfig | grep inet | awk '/Bcast/ {print $2}'")
        .readline()
        .strip()
        .split(":")[1]
    )
...
def getCPUuse():
    ...
    try:
        return float(
            os.popen("ps -A -o %cpu | awk '{s+=$1} END {print s}'")
            .readline()
            .strip()
            .replace(",", ".")
        )
...
def parseXB2GB(space):
    if space.endswith("T"):
        return float(space[:-1].replace(",", ".")) * 1024.0
    if space.endswith("Ti"):
        return float(space[:-2].replace(",", ".")) * 1024.0
    if space.endswith("G"):
        return float(space[:-1].replace(",", "."))
    if space.endswith("Gi"):
        return float(space[:-2].replace(",", "."))
    if space.endswith("M"):
        return float(space[:-1].replace(",", ".")) / 1024.0
    if space.endswith("Mi"):
        return float(space[:-2].replace(",", ".")) / 1024.0
    if space.endswith("K"):
        return float(space[:-1].replace(",", ".")) / 1024.0 / 1024.0
    if space.endswith("Ki"):
        return float(space[:-2].replace(",", ".")) / 1024.0 / 1024.0
    if space.endswith("B"):
        return float(space[:-1].replace(",", ".")) / 1024.0 / 1024.0 / 1024.0
    if space.endswith("Bi"):
        return float(space[:-2].replace(",", ".")) / 1024.0 / 1024.0 / 1024.0
    return 0.0
...
def getMetricsJson():
    ...
    nowtime = datetime.datetime.fromtimestamp(nowsec).strftime(
        "%Y-%m-%dT%H:%M:%S"
    )  # we are using the "date_hour_minute_second" or "strict_date_hour_minute_second" format of elasticsearch as fornat for the date: yyyy-MM-dd'T'HH:mm:ss.
