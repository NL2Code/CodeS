...
from audio.audio_service import v1api as v1audio_ns
...
def check_route_conflicts(app):
    ...
    if len(routes_str) != len(set(routes_str)):
        ...
        duplicates = [
            (route, endpoints)
            for route, endpoints in route_counts.items()
            if len(endpoints) > 1
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Load a model and save it as prepared model for finetuning"
    )
    parser.add_argument(
        "--port", default="8080", type=str, help="server port, default 8080"
    )
    parser.add_argument(
        "--host", default="0.0.0.0", type=str, help="bind address, default 0.0.0.0"
    )
    parser.add_argument(
        "--susi_api_key",
        default=os.environ.get("SUSI_API_KEY", default=""),
        type=str,
        help="SUSI API key",
    )
    parser.add_argument(
        "--openai_api_key",
        default=os.environ.get("OPENAI_API_KEY", default=""),
        type=str,
        help="OpenAI API key",
    )
