...
import requests
from arjun.core.utils import create_query_string, populate
...
def text_export(result):
    ...
    with open(mem.var["text_file"], "a+", encoding="utf8") as text_file:
        for url, data in result.items():
            ...
            if data["method"] == "JSON":
                text_file.write(
                    clean_url + "\t" + json.dumps(populate(data["params"])) + "\n"
                )
            else:
