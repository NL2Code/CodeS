    ...
    if type(response) != str and response.status_code in (400, 413, 418, 429, 503):
        if response.status_code == 503:
            ...
            if factors["same_code"] != response.status_code:
                ...
                if mem.var["bad_req_count"] > 20:
                    mem.var["kill"] = True
                    print(
                        "%s Server received a bad request. Try decreasing the chunk size with -c option"
                        % bad
                    )
                    return "kill"
            else:
        ...
        if "Timeout" in response:
            if mem.var["timeout"] > 20:
                mem.var["kill"] = True
                print(
                    "%s Connection timed out, unable to increase timeout further" % bad
                )
                print(
                    "%s Target might have a rate limit in place, try --stable switch"
                    % bad
                )
                return "kill"
            else:
