...
def define(response_1, response_2, param, value, wordlist):
    ...
    if type(response_1) == type(response_2) == requests.models.Response:
        ...
        if mem.var["disable_redirects"]:
            if response_1.headers.get("Location", "") == response_2.headers.get(
                "Location", ""
            ):
                factors["same_redirect"] = urlparse(
                    response_1.headers.get("Location", "")
                ).path
...
def compare(response, factors, params):
    ...
    if mem.var["disable_redirects"]:
        if (
            factors["same_redirect"]
            and urlparse(response.headers.get("Location", "")).path
            != factors["same_redirect"]
        ):
    ...
    if (
        ...
        and remove_tags(response.text) != factors["same_plaintext"]
    ):
