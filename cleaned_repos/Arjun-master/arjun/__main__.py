...
from arjun.core.anomaly import compare, define
from arjun.core.bruter import bruter
from arjun.core.colors import bad, end, good, green, info, res, run
...
arjun_dir = compatible_path(
    mem.__file__.replace(compatible_path("/core/config.py"), "")
)
...
parser.add_argument(
    "-oB",
    help="Port for output to Burp Suite Proxy. Default port is 8080.",
    dest="burp_port",
    nargs="?",
    const=8080,
)
...
parser.add_argument(
    "-w",
    help="Wordlist file path. (default: {arjundir}/db/large.txt)",
    dest="wordlist",
    default=arjun_dir + "/db/large.txt",
)
parser.add_argument(
    "-m",
    help="Request method to use: GET/POST/XML/JSON/HEADERS. (default: GET)",
    dest="method",
    default="GET",
)
parser.add_argument(
    "-i",
    help="Import target URLs from file.",
    dest="import_file",
    nargs="?",
    const=True,
)
...
parser.add_argument(
    "-c",
    help="Chunk size. The number of parameters to be sent at once",
    type=int,
    dest="chunks",
    default=250,
)
parser.add_argument(
    "-q", help="Quiet mode. No output.", dest="quiet", action="store_true"
)
parser.add_argument(
    "--headers",
    help="Add headers. Separate multiple headers with a new line.",
    dest="headers",
    nargs="?",
    const=True,
)
parser.add_argument(
    "--passive",
    help="Collect parameter names from passive sources like wayback, commoncrawl and otx.",
    dest="passive",
    nargs="?",
    const="-",
)
parser.add_argument(
    "--stable", help="Prefer stability over speed.", dest="stable", action="store_true"
)
parser.add_argument(
    "--include", help="Include this data in every request.", dest="include", default={}
)
parser.add_argument(
    "--disable-redirects",
...
try:
    wordlist_file = (
        arjun_dir + "/db/small.txt" if args.wordlist == "small" else args.wordlist
    )
    ...
    if mem.var["passive"]:
        ...
        print(
            "%s Collecting parameter names from passive sources for %s, it may take a while"
            % (run, host)
        )
        passive_params = fetch_params(host)
        wordlist.update(passive_params)
        print(
            "%s Collected %s parameters, added to the wordlist"
            % (info, len(passive_params))
        )
...
def narrower(request, factors, param_groups):
    ...
    futures = (
        threadpool.submit(bruter, request, factors, params) for params in param_groups
    )
    for i, result in enumerate(as_completed(futures)):
        ...
        print(
            "%s Processing chunks: %i/%-6i" % (info, i + 1, len(param_groups)), end="\r"
        )
    return anomalous_params
    ...
    if not request["url"]:
        ...
        if found:
            ...
            if words_exist:
                ...
                s = "s" if num > 1 else ""
                print(
                    "%s Heuristic scanner found %i parameter%s: %s"
                    % (good, num, s, ", ".join(found))
                )
        if single_url:
        ...
        for param in last_params:
            ...
            if reason:
                ...
                if single_url:
                    print(
                        "%s parameter detected: %s, based on: %s" % (res, name, reason)
                    )
