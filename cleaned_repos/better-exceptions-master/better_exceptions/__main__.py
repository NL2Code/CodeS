...
from better_exceptions import hook, interact

hook()

parser = argparse.ArgumentParser(
    description="A Python REPL with better exceptions enabled",
    prog="python -m better_exceptions",
)
parser.add_argument("-q", "--quiet", help="don't show a banner", action="store_true")
parser.add_argument(
    "-i", "--no-init", dest="no_init", help="don't load ~/.pyinit", action="store_true"
)
