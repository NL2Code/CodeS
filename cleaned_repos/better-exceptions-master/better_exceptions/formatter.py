...
class ExceptionFormatter(object):
    ...
    AST_ELEMENTS = {
        ...
        "keywords": [
            getattr(ast, cls)
            for cls in dir(ast)
            if keyword.iskeyword(cls.lower()) and isast(getattr(ast, cls))
        ],
    }
    ...
    def colorize_comment(self, source):
        ...
        if match:
            source = "{}{}".format(
                match.group(1), self._theme["comment"](match.group(4))
            )
        return source
    ...
    def format_traceback(self, tb=None):
        ...
        while tb:
            ...
            if not (
                os.path.basename(formatted[0]) == "code.py"
                and formatted[2] == "runcode"
            ):
