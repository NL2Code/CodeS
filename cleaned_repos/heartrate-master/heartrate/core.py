...
lightnesses = [int((i + 1) * 100 / (levels + 1)) for i in range(levels + 1)]
...
def highlight_stack_frame(frame):
    ...
    highlighted = "\n".join(
        highlight_ranges(source, [frame]).splitlines()[start - 1 : end]
    )
...
def trace(
    ...
    def file_table_context():
        ...
        ratios = [
            [
                counter[i + 1] / min(2**c, len(queue) or 1) * (c + 1) / levels
        ...
        rows = [
            (
                ...
                reversed([int(round(ratio / max_ratio * 100)) for ratio in ratios[i]]),
                line,
            )
