...
class Config:
    ...
    def _load_cfg(
        self, defaults: dict, cfg_files: PathList
    ) -> typing.Tuple[typing.List[str], ConfigParser]:
    ...
    def write_setting(self, section, key, value):
        ...
        logging.debug(
            "Wrote [%s] %s = %s to %s" % (section, key, value, self.settings_file)
        )
...
class Controller:
    ...
    def __init__(self, config: Config):
        ...
        lhandler = RotatingFileHandler(
            log_filename, maxBytes=100000, backupCount=1, encoding="utf8"
        )
        ...
        try:
            ...
            if self.config.simplenote_sync:
                self.notes_db.add_observer(
                    "progress:sync_full", self.observer_notes_db_sync_full
                )
                self.notes_db.add_observer(
                    "error:sync_full", self.observer_notes_db_error_sync_full
                )
                self.notes_db.add_observer(
                    "complete:sync_full", self.observer_notes_db_complete_sync_full
                )
            ...
            self.view.add_observer(
                "change:sort_mode", self.observer_view_change_sort_mode
            )
            self.view.add_observer(
                "change:pinned_on_top", self.observer_view_change_pinned_on_top
            )
    ...
    def main_loop(self):
        ...
        if not self.config.files_read:
            ...
            wmsg = (
                "Please rename [default] to [nvpy] in %s. "
    ...
    def observer_notes_db_change_note_status(
        self, notes_db, evt_type, evt: events.NoteStatusChangedEvent
    ):
        skey = self.selected_note_key
        if skey == evt.key:
            self.view.set_note_status(self.notes_db.get_note_status(skey))

    def observer_notes_db_sync_full(
        self, notes_db, evt_type, evt: events.SyncProgressEvent
    ):
    ...
    def observer_notes_db_error_sync_full(
        self, notes_db, evt_type, evt: events.SyncFailedEvent
    ):
        try:
            raise evt.error
    ...
    def observer_notes_db_complete_sync_full(
        self, notes_db, evt_type, evt: events.SyncCompletedEvent
    ):
    ...
    def observer_notes_db_saved_note(
        self, notes_db, evt_type, evt: events.NoteSavedEvent
    ):
        self.view.refresh_notes_list()

    def observer_notes_db_synced_note(
        self, notes_db, evt_type, evt: events.NoteSyncedEvent
    ):
    ...
    def observer_view_delete_note(
        self, view, evt_type, evt: events.NoteSelectionChangedEvent
    ):
    ...
    def helper_markdown_to_html(self):
        if self.selected_note_key:
            ...
            if HAVE_MARKDOWN:
                ...
                exts = (
                    re.split("\s+", self.config.md_extensions.strip())
                    if self.config.md_extensions
                    else []
                )
...
%s
    ...
    def helper_rest_to_html(self):
        if self.selected_note_key:
            ...
            if HAVE_DOCUTILS:
                ...
                html: bytes = docutils.core.publish_string(
    ...
    def observer_view_keep_house(self, view, evt_type, evt):
        ...
        if self.config.simplenote_sync:
            ...
            if sync_errors:
                msg = " ".join(
                    [i for i in [msg, "Could not connect to simplenote server."] if i]
                )
    ...
    def observer_view_change_search_mode(
        self, view, evt_type, evt: events.TextBoxChangedEvent
    ):
        if evt.value != self.config.search_mode:
            self.config.search_mode = evt.value
            self.view.refresh_notes_list()

    def observer_view_change_entry(
        self, view, evt_type, evt: events.TextBoxChangedEvent
    ):
        ...
        if idx < 0:
            ...
            if self.view.is_note_different(new_note):
                logging.debug(
                    "Currently selected note %s replaced by newer from server." % (k,)
                )
    ...
    def observer_view_change_pinned(
        self, view, evt_type, evt: events.CheckboxChangedEvent
    ):
        # get new text and update our database
        if self.selected_note_key:
            self.notes_db.set_note_pinned(self.selected_note_key, evt.value)

    def observer_view_change_sort_mode(
        self, view, evt_type, evt: events.SortModeChangedEvent
    ):
        self.config.sort_mode = evt.mode
        # Refresh notes list.
        self.view.refresh_notes_list()

    def observer_view_change_pinned_on_top(
        self, view, evt_type, evt: events.PinnedOnTopChangedEvent
    ):
    ...
    def observer_view_close(self, view, evt_type, evt):
        ...
        if saven or syncn or wfsn:
            msg = (
                "Are you sure you want to exit? I'm still busy: "
...
def profiler_context(fname_prefix: str):
    ...
    with open(text_file, "w") as f:
        ...
        s.sort_stats(
            pstats.SortKey.TIME, pstats.SortKey.CUMULATIVE, pstats.SortKey.CALLS
        )
