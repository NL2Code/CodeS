...
from . import events, nvpy, utils
from .debug import wrap_buggy_function

FilterResult = typing.Tuple[
    typing.List["NoteInfo"], typing.Optional[typing.Pattern], int
]

# API key provided for nvPY.
# Please do not use for other software!
simplenote.simplenote.API_KEY = bytes(
    reversed(base64.b64decode("OTg0OTI4ZTg4YjY0NzMyOTZjYzQzY2IwMDI1OWFkMzg="))
)
...
class NotesDB(utils.SubjectMixin):
        ...
        for fn in fnlist:
            try:
                with open(fn, "rb") as f:
                    n = json.load(f)
                if self.config.notes_as_txt:
                    nt = utils.get_note_title_file(
                        n, self.config.replace_filename_spaces
                    )
    ...
    def filter_notes_gstyle(self, search_string=None) -> FilterResult:
                ...
                if not n.get("deleted"):
                    ...
                    if tagmatch and self._helper_gstyle_mswordmatch(msword_pats, c):
                        ...
                        filtered_notes.append(
                            NoteInfo(key=k, note=n, tagfound=tagfound)
                        )
        ...
        if tms_pats[1] + tms_pats[2]:
            ...
            try:
                ...
                logging.error(
                    "Failed to compile regular expression: %r", regexp_pattern
                )
        ...
        with self.notes_lock:
            for k in self.notes:
                ...
                if self.config.search_tags == 1:
                    ...
                    if sspat:
                        if t and any(
                            filter(lambda ti: sspat.search(ti), t)
                        ):  # type:ignore
                            # we have to store our local key also
    ...
    def is_worker_busy(self):
        return bool(
            self.q_sync.qsize()
            or self.syncing_lock.locked()
            or self.waiting_for_simplenote
        ...
        while something_in_queue:
            try:
                ...
                self.notify_observers(
                    "change:note-status",
                    events.NoteStatusChangedEvent(what="savedate", key=o.key),
                )
                ...
                for k, n in self.notes.items():
                    ...
                    if need_sync:
                        task = _BackgroundTask(
                            action=ACTION_SYNC_PARTIAL_TO_SERVER, key=k, note=None
                        )
    ...
    def sync_full_unthreaded(self):
        ...
        try:
            ...
            self.notify_observers(
                "progress:sync_full",
                events.SyncProgressEvent(msg="Starting full sync."),
            )
            ...
            with self.notes_lock:
                modified_notes = list(
                    filter(
                        lambda lk: Note(self.notes[lk]).need_sync_to_server,
                        self.notes.keys(),
                    )
                )
                ...
                if result.error_object is None:
                    ...
                    self.notify_observers(
                        ...
                        events.SyncProgressEvent(
                            msg="Synced modified note %d/%d to server."
                    ...
                    if lk not in server_keys:
                        ...
                        if self.config.notes_as_txt:
                            tfn = os.path.join(
                                ...
                                utils.get_note_title_file(
                                    self.notes[lk], self.config.replace_filename_spaces
                                ),
                            )
                            if os.path.isfile(tfn):
                                os.unlink(tfn)
            ...
            for ni, n in enumerate(nl):
                ...
                if k in self.notes:
                    ...
                    if err == 0:
                        ...
                        err_obj = n
                        logging.error(
                            "Error syncing new note %s from server: %s" % (k, err_obj)
                        )
                        sync_from_server_errors += 1
            ...
            self.notify_observers(
                "complete:sync_full",
                events.SyncCompletedEvent(errors=sync_from_server_errors),
            )
            ...
            self.notify_observers(
                "error:sync_full",
                events.SyncFailedEvent(error=e, exc_info=sys.exc_info()),
            )
    ...
    def _worker_sync_to_server(self, key: str):
        ...
        if "key" in local_note:
            logging.debug(
                "Updating note %s (local key %s) to server." % (local_note["key"], key)
            )
        else:
...
class Note(dict):
    ...
    def need_save(self):
        ...
        return (
            float(self["modifydate"]) > savedate or float(self["syncdate"]) > savedate
        )
