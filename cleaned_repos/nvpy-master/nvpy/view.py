...
from . import events, notes_db, nvpy, search_entry, tk, utils
...
class NotesList(tk.Frame):
    ...
    def __init__(self, master, font_family, font_size, config: NotesListConfig):
        ...
        self.text.tag_config(
            "tags", font=italic_font, foreground=config.colors.note_info
        )
        self.text.tag_config(
            "found",
            font=italic_font,
    ...
    def append(self, note, config):
        ...
        if self.layout == "vertical" and self.print_columns == 1:
            ...
            self.text.insert(
                tk.END,
                "{0:<{w}}".format(title[: title_length - 1], w=title_length),
                ("title",),
            )
            if config.match_regexp:
                for mo in config.match_regexp.finditer(title):
                    start = "{}.{}".format(
                        line_number, min(mo.start(), title_length - 1)
                    )
            ...
            if not self.hide_tags:
                if config.tagfound:
                    self.text.insert(
                        tk.END,
                        "{0:<{w}}".format(",".join(tags)[: cellwidth - 1], w=cellwidth),
                        ("found",),
                    )
                else:
                    self.text.insert(
            ...
            if not self.hide_time:
                self.text.insert(
                    tk.END, " " + utils.human_date(createdate), ("createdate",)
                )
    ...
    def triggeredcomplete(self):
        ...
        try:
            ...
            eowp_index = "%s.%s" % (
                line,
                str(word_prefix_end),
            )  # index of ending of word_prefix
...
class View(utils.SubjectMixin):
    ...
    def cmd_root_delete(self, evt=None):
        ...
        if is_delete:
            ...
            self.notify_observers(
                "delete:note", events.NoteSelectionChangedEvent(sel=sidx)
            )

    def cmd_root_new(self, evt=None):
        # this'll get caught by a controller event handler
        self.notify_observers(
            "create:note", events.NoteCreatedEvent(title=self.get_search_entry_text())
        )
    ...
    def set_note_tally(self, filtered_notes, active_notes, total_notes):
        self.statusbar.set_centre_status(
            "Listing %d / %d active notes (%d total)"
    ...
    def _bind_events(self):
        ...
        self.notes_list.text.bind(
            "<Control-bracketleft>", lambda e: self.search_entry.focus()
        )
        ...
        self.search_entry.bind(
            "<Up>", lambda e: self.notes_list.select_prev(silent=False)
        )
        self.search_entry.bind(
            "<Control-k>", lambda e: self.notes_list.select_prev(silent=False)
        )
        self.search_entry.bind(
            "<Prior>", lambda e: self.notes_list.select_prev(silent=False, delta=10)
        )

        self.search_entry.bind(
            "<Down>", lambda e: self.notes_list.select_next(silent=False)
        )
        self.search_entry.bind(
            "<Control-j>", lambda e: self.notes_list.select_next(silent=False)
        )
        self.search_entry.bind(
            "<Next>", lambda e: self.notes_list.select_next(silent=False, delta=10)
        )
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(
            label="Sync full", underline=5, command=self.cmd_sync_full
        )
        file_menu.add_command(
            label="Sync current note",
            underline=0,
        ...
        file_menu.add_command(
            label="Exit", underline=1, command=self.handler_close, accelerator="Ctrl+Q"
        )
        ...
        edit_menu.add_command(
            label="Cut", accelerator="Ctrl+X", underline=2, command=self.cmd_cut
        )
        edit_menu.add_command(
            label="Copy", accelerator="Ctrl+C", underline=0, command=self.cmd_copy
        )
        edit_menu.add_command(
            label="Paste", accelerator="Ctrl+V", underline=0, command=self.cmd_paste
        )
        ...
        note_menu.add_checkbutton(
            label="Pinned", onvalue=1, offvalue=0, variable=self.pinned_checkbutton_var
        )
        ...
        for name, mode in nvpy.SortMode.human_friendly_names().items():
            notes_menu.add_radiobutton(
                label=f"Sort by {name}", value=mode.value, variable=self.sort_mode_var
            )
        ...
        for mode in self.search_mode_options:
            search_menu.add_radiobutton(
                label=f"Search mode {mode}", value=mode, variable=self.search_mode_var
            )
        ...
        tools_menu.add_command(
            label="List tags", underline=0, command=self.cmd_list_tags
        )
    ...
    def _create_ui(self):
        ...
        if self.config.layout == "horizontal":
            ...
            self.notes_list = NotesList(
                list_frame, font_family, self.config.list_font_size, notes_list_config
            )
        ...
        self.pinned_checkbutton = tk.Checkbutton(
            note_pinned_frame, variable=self.pinned_checkbutton_var
        )
        ...
        self.tags_entry = SuggestionEntry(
            completion_func, note_tags_frame, textvariable=self.tags_entry_var
        )
    ...
    def handler_cs_checkbutton(self, *args):
        self.notify_observers(
            "change:cs",
            events.CheckboxChangedEvent(value=bool(self.cs_checkbutton_var.get())),
        )
    ...
    def handler_housekeeper(self):
        try:
            ...
            self.show_error(
                "Housekeeper error", "An error occurred during housekeeping.\n" + str(e)
            )
            raise
    ...
    def handler_search_enter(self, evt):
        ...
        if self.notes_list.selected_idx >= 0:
            ...
            self.notify_observers(
                "create:note",
                events.NoteCreatedEvent(title=self.get_search_entry_text()),
            )
            # the note will be created synchronously, so we can focus the text area already
            self.text_note.focus()
    ...
    def handler_search_entry(self, *args):
        self.notify_observers(
            "change:entry",
            events.TextBoxChangedEvent(value=self.search_entry_var.get()),
        )
    ...
    def handler_search_mode(self, *args):
        ...
        self.notify_observers(
            "change:search_mode",
            events.TextBoxChangedEvent(value=self.search_mode_var.get()),
        )

    def handler_add_tags_to_selected_note(self, evt=None):
        self.notify_observers(
            "add:tag", events.TagsAddedEvent(tags=self.tags_entry_var.get())
        )
    ...
    def activate_links(self):
        ...
        for mo in re.finditer(pat, t.get("1.0", "end")):
            ...
            t.tag_bind(
                tag, "<Button-1>", lambda e, link=link: self.handler_click_link(link)
            )
    ...
    def activate_markdown_highlighting(self):
        ...
        if mo:
            t.tag_add(
                "md-bold", "1.0+{0}c".format(mo.start()), "1.0+{0}c".format(mo.end())
            )
    ...
    def handler_control_backspace(self, evt):
        ...
        if (
            self.text_note.index("insert-1c") != "1.0"
            and self.text_note.index(tk.INSERT) != "1.0"
        ):
    ...
    def handler_control_delete(self, evt):
        ...
        if insert in "  " and insert != "":
            ...
            if insert in "\n\t" and insert != "":
                has_only_space_chars = (
                    self.text_note.get(tk.INSERT, tk.END).strip() == ""
                )
                has_any_chars = self.text_note.index(tk.INSERT) != self.text_note.index(
                    tk.END
                )
    ...
    def handler_pinned_on_top_change(self, *args):
        self.notify_observers(
            "change:pinned_on_top",
            events.PinnedOnTopChangedEvent(self.pinned_on_top_var.get()),
        )
    ...
    def observer_notes_list(
        self, notes_list_model: "nvpy.NotesListModel", evt_type, evt
    ):
    ...
    def set_notes(self, notes: typing.List["notes_db.NoteInfo"]):
        ...
        for o in notes:
            ...
            nc = NoteConfig(
                tagfound=o.tagfound, match_regexp=self.notes_list_model.match_regexp
            )
