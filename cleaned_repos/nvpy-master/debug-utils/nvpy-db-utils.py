...
class LocalDB:
    ...
    def target_files(
        self, keys: typing.Iterable[str], is_all: bool
    ...
    def run(self, args, config):
        ...
        for file in files:
            ...
            try:
                ...
                assert (
                    "modifydate" in obj
                    and type(obj["modifydate"]) in [float, int]
                    and obj["modifydate"] <= now
                )
                assert (
                    "createdate" in obj
                    and type(obj["createdate"]) in [float, int]
                    and obj["createdate"] <= now
                )
                ...
                assert (
                    "syncdate" in obj
                    and type(obj["syncdate"] in [float, int])
                    and obj["createdate"] <= now
                )
            except:
...
def parse_cmd_line_args():
    ...
    update.add_argument(
        "--direction", "-d", choices=("download", "upload"), required=True
    )
