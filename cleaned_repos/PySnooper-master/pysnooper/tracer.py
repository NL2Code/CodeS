...
ansible_filename_pattern = re.compile(
    r"^(.+\.zip)[/|\\](ansible[/|\\]modules[/|\\].+\.py)$"
)
RETURN_OPCODES = {
    "RETURN_GENERATOR",
    "RETURN_VALUE",
    "RETURN_CONST",
    "INSTRUMENTED_RETURN_GENERATOR",
...
def get_local_reprs(frame, watch=(), custom_repr=(), max_length=None, normalize=False):
    ...
    result_items = [
        (key, utils.get_shortish_repr(value, custom_repr, max_length, normalize))
    ...
    if source is None:
        ...
        if ipython_filename_match:
            ...
            try:
                ...
                ((_, _, source_chunk),) = ipython_shell.history_manager.get_range(
                    0, entry_number, entry_number + 1
                )
            ...
            try:
                ...
                source = archive_file.read(
                    ansible_filename_match.group(2).replace("\\", "/")
                ).splitlines()
            except Exception:
                pass
        else:
    ...
    def trace(self, frame, event, arg):
        ...
        if self.normalize:
            ...
            timestamp = pycompat.time_isoformat(
                datetime_module.datetime.now().time(), timespec="microseconds"
            )
        #                                                                     #
        ...
        source_path = (
            source_path if not self.normalize else os.path.basename(source_path)
        )
        if self.last_source_path != source_path:
            self.write(
                "{_FOREGROUND_YELLOW}{_STYLE_DIM}{indent}Source path:... "
                "{_STYLE_NORMAL}{source_path}"
                "{_STYLE_RESET_ALL}".format(**locals())
            )
        ...
        if self.thread_info:
            if self.normalize:
                raise NotImplementedError(
                    "normalize is not supported with " "thread_info"
                )
        ...
        for name, value_repr in local_reprs.items():
            if name not in old_local_reprs:
                self.write(
                    "{indent}{_FOREGROUND_GREEN}{_STYLE_DIM}"
        ...
        if ended_by_exception:
            self.write(
                "{_FOREGROUND_RED}{indent}Call ended by exception{_STYLE_RESET_ALL}".format(
                    **locals()
                )
            )
        else:
            self.write(
                "{indent}{_STYLE_DIM}{timestamp} {thread_info}{event:9} "
                "{line_no:4}{_STYLE_RESET_ALL} {source_line}".format(**locals())
            )
        ...
        if event == "return":
            ...
            if not ended_by_exception:
                ...
                self.write(
                    "{indent}{_FOREGROUND_CYAN}{_STYLE_DIM}"
        ...
        if event == "exception":
            ...
            self.write(
                "{indent}{_FOREGROUND_RED}Exception:..... "
