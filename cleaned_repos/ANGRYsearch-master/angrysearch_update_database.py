    ...
    for root, dirs, files in ror.walk(root_dir, onerror=error):
        ...
        for dname in dirs:
            dir_list.append(
                (
                    "1",
                    os.path.join(root, dname).decode(encoding="UTF-8", errors="ignore"),
                )
            )
        for fname in files:
            file_list.append(
                (
                    "0",
                    os.path.join(root, fname).decode(encoding="UTF-8", errors="ignore"),
                )
            )
...
if __name__ == "__main__":
    ...
    noti_text = "{} | database updated".format(time_difference(total_time.seconds))
    try:
