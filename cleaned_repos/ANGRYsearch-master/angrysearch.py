...
TEMP_PATH = Qc.QStandardPaths.standardLocations(Qc.QStandardPaths.TempLocation)[0]
CACHE_PATH = Qc.QStandardPaths.standardLocations(Qc.QStandardPaths.CacheLocation)[0]
CONFIG_PATH = (
    Qc.QStandardPaths.standardLocations(Qc.QStandardPaths.ConfigLocation)[0]
    + "/angrysearch/angrysearch.conf"
)
DATABASE_PATH = join_path(
    os.path.expanduser("~"), CACHE_PATH, "angrysearch", "angry_database.db"
)
...
class ThreadDBQuery(Qc.QThread):
    ...
    def run(self):
        ...
        self.db_query_signal.emit(self.db_query, db_query_result, self.words_quoted)
...
class ThreadDBUpdate(Qc.QThread):
    ...
    def crawling_drives(self):
        ...
        for root, dirs, files in scandir.walk(root_dir, onerror=error):
            ...
            self.crawl_signal.emit(root.decode(encoding="utf-8", errors="ignore"))
            ...
            if self.lite:
                for dname in dirs:
                    dir_list.append(
                        (
                            "1",
                            os.path.join(root, dname).decode(
                ...
                for fname in files:
                    file_list.append(
                        (
                            "0",
                            os.path.join(root, fname).decode(
                                encoding="UTF-8", errors="ignore"
                            ),
                        )
                    )
    ...
    def new_database(self):
        ...
        if self.lite:
            if self.fts5_pragma_check():
                cur.execute(
                    """CREATE VIRTUAL TABLE angry_table
            ...
            cur.executemany("""INSERT INTO angry_table VALUES (?, ?)""", self.table)
            ...
            cur.executemany(
                """INSERT INTO angry_table VALUES (?, ?, ?, ?)""", self.table
            )
    ...
    def run(self):
        if not os.path.exists(self.path):
            ...
            mime = subprocess.Popen(
                ["xdg-mime", "query", "filetype", self.path], stdout=subprocess.PIPE
            )
            mime.wait()
...
class CenterWidget(Qw.QWidget):
    def __init__(self, setting_params=None):
        ...
        self.table = AngryTableView(
            self.setting_params["angrysearch_lite"], self.setting_params["row_height"]
        )
...
class AngryMainWindow(Qw.QMainWindow):
    ...
    def keyPressEvent(self, event):
        if type(event) == Qg.QKeyEvent:
            ...
            if event.key() == 16777271:
                self.setting_params["regex_mode"] = not self.setting_params[
                    "regex_mode"
                ]
    ...
    def init_gui(self):
        ...
        self.center.table.setSelectionMode(Qw.QAbstractItemView.SingleSelection)
    ...
    def sys_tray_clicking(self, reason):
        if (
            reason == Qw.QSystemTrayIcon.DoubleClick
            or reason == Qw.QSystemTrayIcon.Trigger
        ):
            self.show()
    ...
    def process_q_resuls(self, db_query, db_query_result, words_quoted=None):
        ...
        if column_to_sort_by is 2:
            if revert_sort_order is 0:
                db_query_result.sort(key=lambda x: x[2] if isinstance(x[2], int) else 0)
            else:
        ...
        for tup in db_query_result:
            ...
            if tup[0] == "1":
                ...
                if short_mime:
                    archives = ["x-tar", "zip", "x-rar-compressed", "x-7z-compressed"]
    ...
    def get_mime_icons(self):
        ...
        icon_dic = {
            "folder": self.style().standardIcon(Qw.QStyle.SP_DirIcon),
            "file": file_icon,
            "image": file_icon,
            "audio": file_icon,
            "video": file_icon,
        ...
        try:
            ...
            for key in icon_dic:
                icon = ":/mimeicons/{}/{}.png".format(
                    self.setting_params["icon_theme"], key
                )
    ...
    def show_first_500(self):
        ...
        self.center.table.sortByColumn(
            self.setting_params["last_sort"][0], self.setting_params["last_sort"][1]
        )

        self.center.table.setDisabled(False)
        cur.execute(
            "SELECT * FROM angry_table LIMIT ?",
    ...
    def selection_happens(self, selected_item, deselected_item):
        if selected_item.indexes():
            ...
            self.mime_type_threads.append(
                {"path": path, "thread": ThreadMimetype(path)}
            )
    ...
    def tutorial(self):
        ...
        self.status_bar.showMessage("Press the update button in the top right corner")
    ...
    def clicked_button_updatedb(self):
        ...
        self.u.icon_theme_signal.connect(self.theme_change_icon, Qc.Qt.QueuedConnection)
        self.u.exec_()
        self.center.search_input.setFocus()
    ...
    class HTMLDelegate(Qw.QStyledItemDelegate):
        ...
        def paint(self, painter, option, index):
            ...
            if option.state & Qw.QStyle.State_Selected:
                ...
                ctx.palette.setColor(
                    Qg.QPalette.Text,
                    option.palette.color(Qg.QPalette.Active, Qg.QPalette.Text),
                )
...
class UpdateDialogWindow(Qw.QDialog):
    ...
    def clicked_OK_update_db(self):
        ...
        mounts_needed = self.parent().setting_params[
            "conditional_mounts_for_autoupdate"
        ]
