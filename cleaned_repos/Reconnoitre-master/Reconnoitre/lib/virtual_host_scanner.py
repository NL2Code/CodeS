...
class VirtualHostScanner(object):
    ...
    def scan(self):
        print(
            "[+] Starting virtual host scan for"
        ...
        if not os.path.exists(self.wordlist):
            print(
                "[!] Wordlist %s doesn't exist,"
                " exiting virtual host scanner." % self.wordlist
            )
            return
        ...
        for virtual_host in virtual_host_list:
            ...
            headers = {
                "Host": hostname
                if self.port == 80
                else "{}:{}".format(hostname, self.port),
                "Accept": "*/*",
            }

            dest_url = "{}://{}:{}/".format(
                "https" if int(self.port) == 443 else "http", self.target, self.port
            )
            ...
            if self.ignore_content_length > 0 and self.ignore_content_length == int(
