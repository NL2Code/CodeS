    ...
    def process(self):
        ...
        for query in previous_tracker:
            ...
            mongio.push(
                query,
                "logs",
                {"timestamp": timestamp, "volume": volume, "sentiment": sentiment},
            )
...
def streamer(credentials, queries, refresh, sentiment=False, debug=False):
    ...
    # User Error Checks
    if len(queries) <= 0:
        print("Error: You must include at least one query.")
        return
    if len(queries) >= 10:
        print("Warning: Fewer than ten query recommended.")
    if len(keywords) <= 0:
        print("Error: You must include at least one keyword.")
        return
    if len(keywords) >= 20:
        print("Warning: Fewer than twenty keywords recommended.")
    if refresh <= 0:
        print("Error: Refresh rate must be greater than 0")
        return
