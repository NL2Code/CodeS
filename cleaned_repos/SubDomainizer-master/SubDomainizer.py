...
parse.add_argument(
    "-gop",
    "--gitsecretop",
    help="Enter the file name in which you want to save results of secrets found in github.",
)
parse.add_argument(
    "-d",
    "--domains",
    help="Enter the top-level-domain(s) seperated with comma (no spaces after comma) to extract all the subdomain of those domains",
)
parse.add_argument("-f", "--folder", help="Folder in which files needs to be scanned.")
parse.add_argument(
    "-g",
    "--gitscan",
    help="Give this option if you wants to search for subdomain from github",
    action="store_true",
)
...
parse.add_argument(
    "-k",
    "--nossl",
    help="Use it when SSL certiheadsficate is not verified.",
    action="store_true",
)
parse.add_argument(
    "-l",
...
def argerror(urls, listfile):
    ...
    """
    if (urls is None and listfile is None) or (
        urls is not None and listfile is not None
    ):
        print(
            "Atmost one of -u/--url or -l/--listfile or -f/--folder argument is required. Exiting..."
        )
        sys.exit(1)
    else:
        pass
...
def gitArgError(gitToken, isGit):
    ...
    """
    if (gitToken is None and isGit is not None) or (
        gitToken is not None and isGit is None
    ):
        print(
...
class JsExtract:
    ...
    def IntJsExtract(self, url, heads):
        ...
        if url.startswith("http://") or url.startswith("https://"):
            ...
            if isSSL:
                req = requests.get(
                    "http://" + url, headers=heads, verify=False, timeout=(20, 20)
                )
            else:
        ...
        print(
            termcolor.colored(
                "Searching for Inline Javascripts...", color="yellow", attrs=["bold"]
            )
        )
    ...
    def SaveExtJsContent(self, js):
        ...
        try:
            if isSSL:
                content = unquote(
                    requests.get(
                        js, verify=False, headers=heads, timeout=(20, 20)
                ...
                content = unquote(
                    requests.get(js, headers=heads, timeout=(20, 20)).content.decode(
                        "utf-8"
                    )
                )
                finallist.append(content)
...
def pre_compiled_cloud_regex():
    ...
    s3bucketreg = re.compile(
        r"(s3[\w\-.]*\.?amazonaws\.com/?[\w\-.]+)", re.MULTILINE | re.IGNORECASE
    )
    s3bucketreg2 = re.compile(
        r"([\w\-]+.s3[\w\-.]*\.?amazonaws\.com/?)", re.MULTILINE | re.IGNORECASE
    )
    doreg = re.compile(
    ...
    gsreg3 = re.compile(
        r"([\w\-.]*\.?storage-download.googleapis.com/?[\w\-.]*)",
        re.MULTILINE | re.IGNORECASE,
    )
    gsreg4 = re.compile(
        r"([\w\-.]*\.?content-storage-upload.googleapis.com/?[\w\-.]*)",
        re.MULTILINE | re.IGNORECASE,
    )
    gsreg5 = re.compile(
        r"([\w\-.]*\.?content-storage-download.googleapis.com/?[\w\-.]*)",
        re.MULTILINE | re.IGNORECASE,
    )
    azureg1 = re.compile(
    ...
    dreamhostreg2 = re.compile(
        r"([\w\-.]*\.?objects-us-west-1.dream.io/?[\w\-.]*)",
        re.MULTILINE | re.IGNORECASE,
    )
...
def pre_compiled_domain_regex(url):
    ...
    """
    regex = re.compile(
        r"([a-zA-Z0-9][a-zA-Z0-9\-.]*[a-zA-Z0-9]\." + str(getDomain(str(url))) + ")",
        re.IGNORECASE,
    )
    return regex
...
def get_info_from_data(
    item_url, item_values, cloudlist, p, regex, ipv4reg, url, precompiled_domains_regex
):
    """
        ...
        if entropy(match.group(2)) > 3:
            if item_url in secret_dict:
                ...
                if githubsc_out and item_url.startswith("https://github.com"):
                    github_secrets.add(
                        str(match.group())
                    )  # adding github secrets explicitly to save in file
            else:
                secret_dict[item_url] = [str(match.group())]
                if githubsc_out and item_url.startswith("https://github.com"):
                    github_secrets.add(
                        str(match.group())
                    )  # adding github secrets explicitly
...
def get_github_data(item):
    ...
    try:
        apiUrlContent = requests.get(
            item, verify=False, timeout=(20, 20), headers=headers
        ).content.decode("utf-8")
...
def subextractor(cloudlist, p, regex, ipv4reg, url, precompiled_domains_regex):
    ...
    threads.starmap(
        ...
        zip(
            new_final_dict.keys(),
            new_final_dict.values(),
            repeat(cloudlist),
            repeat(p),
            repeat(regex),
            repeat(ipv4reg),
            repeat(url),
            repeat(precompiled_domains_regex),
        ),
    )
    threads.close()
    threads.join()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if folderName and not url and not listfile:
            ...
            print(
                termcolor.colored(
                    "Getting data from folder recursively...",
                    color="yellow",
                    attrs=["bold"],
                )
            )
            ...
            print(
                termcolor.colored(
                    "Finding secrets in files, Please wait...",
                    color="blue",
                    attrs=["bold"],
                )
            )
            for path, data in folderData.items():
                ...
                if precompiled_domains_regex:
                    for subdomain in precompiled_domains_regex.findall(
                        str(data.replace("\n", " "))
                    ):
                        finalset.add(subdomain)
            ...
            if listfile:
                ...
                if urllist:
                    for i in urllist:
                        ...
                        print(
                            termcolor.colored(
                                "Extracting data from internal and external js for url:",
                                color="blue",
                                attrs=["bold"],
                            )
                        )
            ...
            if gitToken and isGit:
                for item in domainSet:
                    ...
                    print(
                        termcolor.colored(
                            "Finding Subdomains and secrets from Github..Please wait...",
                            color="yellow",
                            attrs=["bold"],
                        )
                    )
                    ...
                    try:
                        gitContentThread.starmap(
                            get_info_from_data,
                            zip(
                                git_data.keys(),
                                git_data.values(),
                                repeat(compiledRegexCloud),
                                repeat(compiledRegexSecretList),
                                repeat(compiledRegexDomain),
                                repeat(compiledRegexIP),
                                repeat(item),
                                repeat(custom_domains_regex),
                            ),
                        )
                    except:
                        pass
    ...
    print(
        termcolor.colored(
            "_" * 22 + "Start of Results" + "_" * 22, color="white", attrs=["bold"]
        )
    )
    ...
    if cloudop:
        print(
            termcolor.colored(
                "\nWriting all the cloud services URL's to given file...",
                color="yellow",
                attrs=["bold"],
            )
        )
        ...
        print(
            termcolor.colored(
                "Written cloud services URL's in file: ", color="red", attrs=["bold"]
            )
            + cloudop
        )
    ...
    if secretop:
        print(
            termcolor.colored(
                "\nWriting all the secrets to given file...",
                color="yellow",
                attrs=["bold"],
            )
        )
        savesecretsresults()
        print(
            termcolor.colored("Written secrets in file: ", color="red", attrs=["bold"])
            + secretop
        )
    ...
    if finalset:
        ...
        print(
            termcolor.colored(
                "Total Subdomains: " + str(len(finalset)), color="red", attrs=["bold"]
            )
        )
        for item in tldSorting(finalset):
    ...
    if cloudurlset:
        ...
        print(
            termcolor.colored(
                "Total Cloud URLs: " + str(len(cloudurlset)),
                color="red",
                attrs=["bold"],
            )
        )
    ...
    if secret_dict:
        ...
        print(
            termcolor.colored(
                "Total Possible Secrets: "
                + str(sum(len(sec_lst) for sec_lst in secret_dict.values())),
                color="red",
                attrs=["bold"],
            )
        )
        for file_url, secrets in secret_dict.items():
            for secret in set(secrets):
                print(
                    termcolor.colored(secret, color="green", attrs=["bold"]),
    ...
    if isGit and github_secrets:
        ...
        print(
            termcolor.colored(
                "\nWriting github secrets to the given file...",
                color="yellow",
                attrs=["bold"],
            )
        )
        try:
            ...
            print(
                termcolor.colored(
                    "\nError in saving the github secrets file...",
                    color="red",
                    attrs=["bold"],
                )
            )
        ...
        for host in finalset:
            ...
            while not q.empty():
                try:
                    ...
                    if is_san == "same":
                        if (
                            hostname not in printed
                            and hostname not in finalset
                            and hostname.endswith(tld)
                        ):
                            print(
                                termcolor.colored(
