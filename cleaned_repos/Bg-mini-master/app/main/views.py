...
def index():
    ...
    return render_template(
        "index.html", articles=articles, pagination=pagination, endpoint=".index"
    )
...
def articleTypes(id):
    ...
    page = request.args.get("page", 1, type=int)
    pagination = (
        ArticleType.query.get_or_404(id)
        .articles.order_by(Article.create_time.desc())
        .paginate(
            page, per_page=current_app.config["ARTICLES_PER_PAGE"], error_out=False
        )
    )
...
def articleDetails(id):
    ...
    page = request.args.get("page", 1, type=int)
    if page == -1:
        page = (article.comments.count() - 1) // current_app.config[
            "COMMENTS_PER_PAGE"
        ] + 1
