...
class Comment(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.author_email is not None and self.avatar_hash is None:
            self.avatar_hash = hashlib.md5(
                self.author_email.encode("utf-8")
            ).hexdigest()
    ...
    def gravatar(self, size=40, default="identicon", rating="g"):
        ...
        hash = (
            self.avatar_hash
            or hashlib.md5(self.author_email.encode("utf-8")).hexdigest()
        )
        return "{url}/{hash}?s={size}&d={default}&r={rating}".format(
            url=url, hash=hash, size=size, default=default, rating=rating
        )
...
class Article(db.Model):
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            ...
            a = Article(
                title=forgery_py.lorem_ipsum.title(randint(3, 5)),
...
class BlogInfo(db.Model):
    ...
    def insert_blog_info():
        blog_mini_info = BlogInfo(
            title="开源博客系统Blog_mini",
...
class Plugin(db.Model):
    ...
    def insert_system_plugin():
        plugin = Plugin(title="博客统计", note="系统插件", content="system_plugin", order=1)
        db.session.add(plugin)
        db.session.commit()

    def sort_delete(self):
        for plugin in (
            Plugin.query.order_by(Plugin.order.asc()).offset(self.order).all()
        ):
            plugin.order -= 1
            db.session.add(plugin)
