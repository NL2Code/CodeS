...
from .forms import (
    ...
    DeleteArticlesForm,
    DeleteCommentsForm,
    EditArticleNavTypeForm,
    EditArticleTypeForm,
    EditUserInfoForm,
    ManageArticlesForm,
    SortArticleNavTypeForm,
    SubmitArticlesForm,
)
...
def manage_articles():
    ...
    if form.validate_on_submit() or (
        ...
        and request.args.get("source_id") is not None
    ):
        if form.validate_on_submit():
...
def disable_comment(id):
    ...
    if request.args.get("disable_type") == "admin":
        page = request.args.get("page", 1, type=int)
        return redirect(url_for("admin.manage_comments", page=page))
...
def manage_articleTypes():
    ...
    return render_template(
        "admin/manage_articleTypes.html",
        articleTypes=articleTypes,
        pagination=pagination,
        endpoint=".manage_articleTypes",
        form=form,
        form2=form2,
        page=page,
    )
...
def edit_articleType():
    ...
    if form2.validate_on_submit():
        ...
        if articleType.is_protected:
            if (
                form2.name.data != articleType.name
                or form2.introduction.data != articleType.introduction
            ):
            ...
            ArticleType.query.filter_by(name=form2.name.data).first()
            and ArticleType.query.filter_by(name=form2.name.data).first().id
            != articleType_id
        ):
...
def delete_articleType(id):
    ...
    count = 0
    systemType = (
        ArticleTypeSetting.query.filter_by(protected=True).first().types.first()
    )
...
def manage_articleTypes_nav():
    ...
    if page == -1:
        page = (Menu.query.count() - 1) // current_app.config["COMMENTS_PER_PAGE"] + 1
    pagination = Menu.query.order_by(Menu.order.asc()).paginate(
        page, per_page=current_app.config["COMMENTS_PER_PAGE"], error_out=False
    )
    menus = pagination.items
    return render_template(
        "admin/manage_articleTypes_nav.html",
        menus=menus,
        pagination=pagination,
        endpoint=".manage_articleTypes_nav",
        page=page,
        form=form,
        form2=form2,
        form3=form3,
...
def edit_nav():
    ...
    if form2.validate_on_submit():
        ...
        if (
            Menu.query.filter_by(name=name).first()
...
def custom_blog_plugin():
    ...
    return render_template(
        "admin/custom_blog_plugin.html",
        Plugin=Plugin,
        pagination=pagination,
        endpoint=".custom_blog_plugin",
        plugins=plugins,
        page=page,
    )
...
def add_plugin():
    ...
    if form.validate_on_submit():
        ...
        if plugin:
            form = AddBlogPluginForm(
                title=title, note=form.note.data, content=form.content.data
            )
            ...
            plugin = Plugin(
                title=title, note=note, content=content, order=plugin_count + 1
            )
            db.session.add(plugin)
            db.session.commit()
...
def edit_plugin(id):
    ...
    plugin = Plugin.query.get_or_404(id)
    form = AddBlogPluginForm(
        title=plugin.title, note=plugin.note, content=plugin.content
    )
    if form.validate_on_submit():
        title = form.title.data
