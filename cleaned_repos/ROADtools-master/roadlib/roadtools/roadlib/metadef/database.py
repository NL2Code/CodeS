    ...
    def process_bind_param(self, value, dialect):
        if value is not None and isinstance(value, str):
            if value[-1] == "Z":
                if "." in value:
                    try:
                        value = datetime.datetime.strptime(
                            value[:-2], "%Y-%m-%dT%H:%M:%S.%f"
                        )
                    except ValueError:
                        value = datetime.datetime.strptime(
                            value[:-2], "%Y-%m-%dT%H:%M:%S."
                        )
                else:
                ...
                if "+" in value:
                    value = datetime.datetime.strptime(
                        value[:-7], "%Y-%m-%dT%H:%M:%S.%f"
                    )
                else:
                    try:
                        value = datetime.datetime.strptime(
                            value[:-1], "%Y-%m-%dT%H:%M:%S.%f"
                        )
                    except ValueError:
                        value = datetime.datetime.strptime(
...
lnk_au_member_device = Table(
    "lnk_au_member_device",
    Base.metadata,
    Column("AdministrativeUnit", Text, ForeignKey("AdministrativeUnits.objectId")),
    Column("Device", Text, ForeignKey("Devices.objectId")),
)
...
class User(Base, SerializeMixin):
    ...
    memberOf = relationship(
        "Group", secondary=lnk_group_member_user, back_populates="memberUsers"
    )

    ownedApplications = relationship(
        "Application", secondary=lnk_application_owner_user, back_populates="ownerUsers"
    )
    ...
    memberOfRole = relationship(
        "DirectoryRole", secondary=lnk_role_member_user, back_populates="memberUsers"
    )

    ownedDevices = relationship(
        "Device", secondary=lnk_device_owner, back_populates="owner"
    )

    ownedGroups = relationship(
        "Group", secondary=lnk_group_owner_user, back_populates="ownerUsers"
    )

    memberOfAu = relationship(
        "AdministrativeUnit", secondary=lnk_au_member_user, back_populates="memberUsers"
    )
...
class ServicePrincipal(Base, SerializeMixin):
    ...
    ownerServicePrincipals = relationship(
        ...
        primaryjoin=objectId
        == lnk_serviceprincipal_owner_serviceprincipal.c.ServicePrincipal,
        secondaryjoin=objectId
        == lnk_serviceprincipal_owner_serviceprincipal.c.childServicePrincipal,
        back_populates="ownedServicePrincipals",
    )
...
class Group(Base, SerializeMixin):
    ...
    memberUsers = relationship(
        "User", secondary=lnk_group_member_user, back_populates="memberOf"
    )

    memberContacts = relationship(
        "Contact", secondary=lnk_group_member_contact, back_populates="memberOf"
    )

    memberDevices = relationship(
        "Device", secondary=lnk_group_member_device, back_populates="memberOf"
    )
    ...
    ownerUsers = relationship(
        "User", secondary=lnk_group_owner_user, back_populates="ownedGroups"
    )
    ...
    memberOfRole = relationship(
        "DirectoryRole", secondary=lnk_role_member_group, back_populates="memberGroups"
    )
...
class Application(Base, SerializeMixin):
    ...
    verifiedPublisher = Column(JSON)
    ownerUsers = relationship(
        "User", secondary=lnk_application_owner_user, back_populates="ownedApplications"
    )
...
class Device(Base, SerializeMixin):
    ...
    owner = relationship(
        "User", secondary=lnk_device_owner, back_populates="ownedDevices"
    )

    memberOf = relationship(
        "Group", secondary=lnk_group_member_device, back_populates="memberDevices"
    )
...
class DirectoryRole(Base, SerializeMixin):
    ...
    memberUsers = relationship(
        "User", secondary=lnk_role_member_user, back_populates="memberOfRole"
    )
    ...
    memberGroups = relationship(
        "Group", secondary=lnk_role_member_group, back_populates="memberOfRole"
    )
...
class Contact(Base, SerializeMixin):
    ...
    thumbnailPhoto = Column(Text)
    memberOf = relationship(
        "Group", secondary=lnk_group_member_contact, back_populates="memberContacts"
    )
...
class RoleDefinition(Base, SerializeMixin):
    ...
    version = Column(Text)
    eligibleAssignments = relationship(
        "EligibleRoleAssignment", back_populates="roleDefinition"
    )

    assignments = relationship("RoleAssignment", back_populates="roleDefinition")
...
class EligibleRoleAssignment(Base, SerializeMixin):
    ...
    resourceScopes = Column(JSON)
    roleDefinitionId = Column(Text, ForeignKey("RoleDefinitions.objectId"))
    roleDefinition = relationship(
        "RoleDefinition", back_populates="eligibleAssignments"
    )
...
class AdministrativeUnit(Base, SerializeMixin):
    ...
    visibility = Column(Text)
    memberGroups = relationship(
        "Group", secondary=lnk_au_member_group, back_populates="memberOfAu"
    )

    memberUsers = relationship(
        "User", secondary=lnk_au_member_user, back_populates="memberOfAu"
    )

    memberDevices = relationship(
        "Device", secondary=lnk_au_member_device, back_populates="memberOfAu"
    )
...
def init(create=False, dburl="sqlite:///roadrecon.db"):
    if "postgresql" in dburl:
        engine = create_engine(
            dburl, executemany_mode="values", executemany_values_page_size=1001
        )
    else:
