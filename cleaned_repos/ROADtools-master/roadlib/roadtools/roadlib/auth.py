...
class Authentication:
    ...
    def authenticate_with_saml_native(
        self, saml_token, additionaldata=None, returnreply=False
    ):
        """
    ...
    def decrypt_auth_response(self, responsedata, sessionkey, asjson=False):
        ...
        if len(get_data(iv)) == 12:
            ...
            depadded_data = aesgcm.decrypt(
                get_data(iv),
                get_data(data) + get_data(authtag),
                dataparts[0].encode("utf-8"),
            )
        else:
    ...
    def tokenreply_to_tokendata(tokenreply, client_id=None):
        ...
        inputdata = json.loads(
            base64.b64decode(tokenparts[1] + ("=" * (len(tokenparts[1]) % 4)))
        )
        try:
    ...
    def get_tokens(self, args):
        ...
        try:
            ...
            try:
                print(
                    f"Error during authentication: {ex.error_response['error_description']}"
                )
            except TypeError:
