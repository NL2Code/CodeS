...
class DeviceAuthentication:
    ...
    def loadcert(
        ...
        if pemfile and privkeyfile:
            ...
            with open(privkeyfile, "rb") as keyf:
                ...
                self.transportprivkey = (
                    self.privkey
                ) = serialization.load_pem_private_key(self.keydata, password=None)
            return True
        if pfxfile or pfxbase64:
            ...
            self.privkey, self.certificate, _ = pkcs12.load_key_and_certificates(
                pfxdata, pfxpass
            )
    ...
    def loadkey(
        ...
        if privkeyfile:
            with open(privkeyfile, "rb") as keyf:
                if transport_only:
                    ...
                    self.transportprivkey = serialization.load_pem_private_key(
        ...
        if pfxfile or pfxbase64:
            ...
            if not transport_only:
                ...
                self.transportprivkey, _, _ = pkcs12.load_key_and_certificates(
                    pfxdata, pfxpass
                )
            ...
            return True
        print(
            "You must specify either a private key file or a pfx file with the device keypair."
        )
        return False
    ...
    def loadprt(self, prtfile):
        ...
        try:
            with codecs.open(prtfile, "r", "utf-8") as prtf:
                ...
                self.session_key = Authentication.ensure_binary_sessionkey(
                    prtdata["session_key"]
                )
    ...
    def loadhellokey(self, privkeyfile):
        ...
        try:
            with open(privkeyfile, "rb") as keyf:
                ...
                self.hellokey = serialization.load_pem_private_key(
                    self.hellokeydata, password=None
                )
        except FileNotFoundError:
            return False
        return True
    ...
    def create_hello_prt_assertion(self, username):
        ...
        headers = {"kid": self.get_privkey_kid(), "use": "ngc"}
        reqjwt = jwt.encode(
            payload, algorithm="RS256", key=self.hellokeydata, headers=headers
        )
        return reqjwt
    ...
    def register_winhello_key(self, pubkeycngblob, access_token):
        ...
        data = {"kngc": pubkeycngblob.decode("utf-8")}
        res = requests.post(
    ...
    def create_pubkey_blob_from_key(self, key):
        ...
        exponent_as_bytes = pubnumbers.e.to_bytes(
            (pubnumbers.e.bit_length() + 7) // 8, byteorder="big"
        )
        modulus_as_bytes = pubnumbers.n.to_bytes(
            (pubnumbers.n.bit_length() + 7) // 8, byteorder="big"
        )
    ...
    def register_device(
        ...
        if device_type.lower() == "macos":
            data = {
                ...
                "TransportKey": base64.b64encode(
                    self.create_public_jwk_from_key(key, True).encode("utf-8")
                ).decode("utf-8"),
                "JoinType": jointype,
                "AttestationData": "",
            }
            ...
            if deviceticket:
                data["attributes"]["MSA-DDID"] = base64.b64encode(
        ...
        cert = x509.load_der_x509_certificate(
            base64.b64decode(returndata["Certificate"]["RawBody"])
        )
    ...
    def register_hybrid_device(
        ...
        signedblob = base64.b64encode(
            self.privkey.sign(
                signdata.encode("utf-8"), apadding.PKCS1v15(), hashes.SHA256()
            )
        )
    ...
    def request_token_with_devicecert_signed_payload(self, payload):
        ...
        reqjwt = jwt.encode(
            payload, algorithm="RS256", key=self.keydata, headers=headers
        )
        prt_request_data = {
        ...
        for tgt in ["tgt_ad", "tgt_cloud"]:
            if tgt in prtdata:
                ...
                if tgtdata["keyType"] != 0:
                    # There is a key
                    tgt_sessionkey = authlib.decrypt_auth_response(
                        tgtdata["clientKey"], uwk
                    )
                    prtdata[tgt + "_sessionkey"] = binascii.hexlify(
                        tgt_sessionkey
                    ).decode("utf-8")
    ...
    def decrypt_jwe_with_transport_key(self, jwetoken):
        ...
        uwk = self.transportprivkey.decrypt(
            wrapped_key,
    ...
    def request_token_with_sessionkey_signed_payload(self, payload, reqtgt=True):
        ...
        _, derived_key = authlib.calculate_derived_key_v2(
            self.session_key, context, jwtbody
        )
    ...
    def aad_brokerplugin_prt_auth(
        self, client_id, resource, renew_prt=False, redirect_uri=None
    ):
        """
