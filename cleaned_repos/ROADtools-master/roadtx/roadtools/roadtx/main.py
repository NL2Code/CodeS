...
def main():
    ...
    codeauth_parser.add_argument(
        ...
        action="store",
        metavar="PASSWORD",
        help="Password secret of the appliction if not a public app",
    )
    codeauth_parser.add_argument(
        "-r",
        "--resource",
    ...
    intauth_parser.add_argument(
        "-url", "--auth-url", action="store", metavar="URL", help=urlhelp
    )
    intauth_parser.add_argument(
        "-c",
        "--client",
    ...
    if args.proxy:
        auth.proxies = deviceauth.proxies = {"https": f"http://{args.proxy}"}
    ...
    if args.command in ("auth", "gettokens", "gettoken"):
        ...
        selauth = SeleniumAuthentication(
            auth, deviceauth, args.redirect_url, proxy=seleniumproxy
        )
        if args.auth_url:
            url = args.auth_url
        else:
        ...
        selauth = SeleniumAuthentication(
            auth, deviceauth, replyurl, proxy=seleniumproxy
        )
        if (
            args.username
            and args.keepass
