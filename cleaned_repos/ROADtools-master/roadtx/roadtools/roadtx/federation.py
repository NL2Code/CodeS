...
class SAMLSigner:
    ...
    def format_template(self, immutable_id, upn, server, mfa=False):
        ...
        if mfa:
            ...
            mfa_att = ""
        templ = self.saml_template.format(
            assertionid=assertionid,
            nbf=token_created,
            exp=token_expires,
            upn=upn,
            unique_id=immutable_id,
            mfa=mfa_att,
            issuer=server,
        )
    ...
    def sign_xml(
        self, saml_string, assertionid, algorithm="rsa-sha256", digest="sha256"
    ):
