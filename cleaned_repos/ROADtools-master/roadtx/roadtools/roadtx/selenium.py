...
class SeleniumAuthentication:
    ...
    def get_webdriver(self, service, intercept=False):
        ...
        if intercept and self.headless:
            ...
            driver = webdriver_wire.Firefox(
                service=service, options=firefox_options, seleniumwire_options=options
            )
        elif intercept:
            driver = webdriver_wire.Firefox(
                service=service, seleniumwire_options=options
            )
        else:
    ...
    def get_keepass_cred(self, identity, filepath, password):
        ...
        if not entry:
            raise AuthenticationException(
                f"Specified username {identity} not found in KeePass file"
            )
    ...
    def selenium_login(
        ...
        if devicecode:
            el = WebDriverWait(driver, 3000).until(
                lambda d: d.find_element(By.ID, "otc")
            )
            el.send_keys(devicecode + Keys.ENTER)
        if identity and not "login_hint" in url:
            el = WebDriverWait(driver, 3000).until(
                lambda d: d.find_element(By.ID, "i0116")
            )
            el.send_keys(identity + Keys.ENTER)
        if password:
            if federated:
                els = WebDriverWait(driver, 6000).until(
                    lambda d: d.find_element(By.ID, "passwordInput")
                )
                els.send_keys(password)
                try:
                    WebDriverWait(driver, 1).until(
                        lambda d: d.find_element(By.ID, "idonotexist")
                    )
                ...
                el = WebDriverWait(driver, 6000).until(
                    lambda d: d.find_element(By.ID, "idSIButton9")
                )
                try:
        ...
        if otpseed:
            ...
            try:
                els = WebDriverWait(driver, 10).until(
                    lambda d: d.find_element(By.ID, "idTxtBx_SAOTCC_OTC")
                )
        ...
        if devicecode:
            try:
                ...
                try:
                    els = WebDriverWait(driver, 10).until(
                        lambda d: self.wait_till_text(d, "You have signed in")
                    )
                except TimeoutException:
                    if not keep:
                        driver.close()
                        raise AuthenticationException(
                            "Could not verify whether device auth succeeded (success text not found)"
                        )
                if not keep:
                    driver.close()
                return True
                ...
                if not keep:
                    driver.close()
                    raise AuthenticationException(
                        "Could not complete device code auth within the time limit (button not found: idSIButton9)"
                    )
        else:
            try:
                ...
                if not keep:
                    driver.close()
                    raise AuthenticationException(
                        "Authentication did not complete within time limit"
                    )
        return False

    def selenium_login_with_custom_useragent(
        self,
        url,
        identity=None,
        password=None,
                ...
                if (
                    ...
                    if prtcookie:
                        ...
                        if "sso_nonce" in request.url:
                            ...
                            cookie = self.auth.create_prt_cookie_kdf_ver_2(
                                self.deviceauth.prt, self.deviceauth.session_key
                            )
                        request.headers["X-Ms-Refreshtokencredential"] = cookie

        self.driver.request_interceptor = interceptor
        return self.selenium_login(
            url, identity, password, otpseed, keep=keep, capture=capture
        )

    def selenium_login_with_kerberos(
        self,
        url,
        identity=None,
        password=None,
    ...
    def selenium_login_with_estscookie(
        self,
        url,
        identity=None,
        password=None,
    ...
    def selenium_enrich_prt(self, url, otpseed=None):
        ...
        def response_interceptor(request, response):
            ...
            if request.url.startswith("https://login.microsoftonline.com"):
                ...
                if b"SwitchToProgressPage();" in body:
                    body = body.replace(
                        b"SwitchToProgressPage();", b"/*SwitchToProgressPage();*/"
                    )
                    response.body = encoding.encode(
                        body, response.headers.get("Content-Encoding", "identity")
                    )
        ...
        el = WebDriverWait(driver, 6000).until(
            lambda d: d.find_element(
                by=By.CSS_SELECTOR, value='form[name="hiddenform"] input[name="code"]'
            )
        )
        code = el.get_property("value")
        driver.close()
        return self.auth.authenticate_with_code_encrypted(
            code, self.deviceauth.session_key, self.redirurl
        )
