...
class HackyKeePassFileReader:
    ...
    def get_entry(self, search):
        ...
        for entry in self.root.iter("Entry"):
            ...
            if (
                entrydata["UserName"]
                and entrydata["UserName"].lower() == search.lower()
            ):
                return entrydata
        return {}
    ...
    def decrypt_keepass(self, database, password):
        ...
        if major_version != 3:
            raise IOError(
                f"KeePass file version should be 3 but found {major_version}. Please export your file to a compatible format or plain XML"
            )
        ...
        master_key = hashlib.sha256(
            bytes(header["master_seed"]) + transformed_key
        ).digest()

        # set up cipher for AES128-CBC decryption to find the decrypted payload
        cipher = Cipher(
            algorithms.AES(master_key), modes.CBC(bytes(header["encryption_iv"]))
        )
        decryptor = cipher.decryptor()
        raw_payload_area = (
            decryptor.update(bytes(encrypted_payload)) + decryptor.finalize()
        )

        # verify decryption
        if (
            header["stream_start_bytes"]
            != raw_payload_area[: len(header["stream_start_bytes"])]
        ):
        ...
        while True:
            ...
            block_length = struct.unpack(
                "<I", raw_payload_area[offset + 36 : offset + 40]
            )[0]
            # read block_data
