...
def check_database_exists(path):
    ...
    if not found:
        raise Exception(
            "The database file {0} was not found. Please make sure it exists".format(
                path
            )
        )
...
def main():
    ...
    gather_parser = subparsers.add_parser(
        "gather", aliases=["dump"], help="Gather Azure AD information"
    )
    ...
    for plugin, description in plugins_list.items():
        ...
        plugin_module = importlib.import_module(
            "roadtools.roadrecon.plugins.{}".format(plugin)
        )
        pparser = plugins.add_parser(
            plugin, description=plugin_module.DESCRIPTION, help=description
        )
    ...
    if args.command == "auth":
        ...
        # Dynamic import
        plugin_module = importlib.import_module(
            "roadtools.roadrecon.plugins.{}".format(args.plugin)
        )
        check_database_exists(args.database)
