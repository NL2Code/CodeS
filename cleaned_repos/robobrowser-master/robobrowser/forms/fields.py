...
import six
from robobrowser import exceptions, helpers
...
class MultiValueField(MultiOptionField):
    ...
    @property
    def value(self):
        return [self.options[idx] for idx in self._value]

    @value.setter
    def value(self, value):
        if not isinstance(value, list):
            value = [value]
        self._value = [self._value_to_index(item) for item in value]
...
class FlatOptionField(MultiOptionField):
    ...
    def _get_options(self, parsed):
        ...
        for option in parsed:
            ...
            labels.append(
                option.next.string if isinstance(option.next, string_types) else None
            )
...
class NestedOptionField(MultiOptionField):
    @property
    def disabled(self):
        return super(NestedOptionField, self).disabled or all(
            "disabled" in each.attrs for each in self._parsed.find_all("option")
        )
