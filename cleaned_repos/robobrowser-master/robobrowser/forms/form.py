...
from robobrowser.compat import encode_if_py2, iteritems
...
_tag_ptn = re.compile("|".join(_tags), re.I)
...
def _filter_fields(fields, predicate):
    return OrderedMultiDict(
        [(key, value) for key, value in fields.items(multi=True) if predicate(value)]
    )
...
class Payload(object):
    ...
    def to_requests(self, method="get"):
        ...
        return dict(
            [(key, list(value.items(multi=True))) for key, value in iteritems(out)]
        )
...
class Form(object):
    ...
    def submit_fields(self):
        return _filter_fields(
            self.fields, lambda field: isinstance(field, fields.Submit)
        )
