...
from robobrowser.compat import iteritems, string_types
...
def find_all(
    ...
    """
    if text is None:
        return soup.find_all(name, attrs or {}, recursive, text, limit, **kwargs)
    if isinstance(text, string_types):
        text = re.compile(re.escape(text), re.I)
    tags = soup.find_all(name, attrs or {}, recursive, **kwargs)
    rv = []
    for tag in tags:
...
def find(soup, name=None, attrs=None, recursive=True, text=None, **kwargs):
    ...
    tags = find_all(soup, name, attrs or {}, recursive, text, 1, **kwargs)
    if tags:
        return tags[0]
...
def lowercase_attr_names(tag):
    ...
    tag.attrs = dict([(key.lower(), value) for key, value in iteritems(tag.attrs)])
