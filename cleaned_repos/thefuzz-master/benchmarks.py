...
def print_result_from_timeit(stmt="pass", setup="pass", number=1000000):
    ...
    print(
        "Total time: {:f}s. Average run: {:.3f}{}.".format(
...
for s in mixed_strings + cirque_strings + choices:
    ...
    print_result_from_timeit(
        "utils.full_process(u'%s')" % s, common_setup, number=iterations
    )
...
print("-------------------------------")
print_result_from_timeit(
    "process.extract(u'cirque du soleil', choices, scorer =  fuzz.QRatio)",
    common_setup
    + " from thefuzz import process; import string,random; random.seed(18);"
...
print("-------------------------------")
print_result_from_timeit(
    "process.extract(u'cirque du soleil', choices, scorer =  fuzz.WRatio)",
    common_setup
    + " from thefuzz import process; import string,random; random.seed(18);"
    " choices = [''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(30)) for s in range(5000)]",
    number=10,
