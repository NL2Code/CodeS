...
from .exceptions import HTTPError, _bad_schema_message
from .helpers import get_reason_phrase
from .openapi import (
    default_bypassed_endpoints,
    default_response,
    get_argument,
    get_auth_name,
    get_operation_tags,
    get_path_summary,
    get_security_and_security_schemes,
    get_tag,
)
from .route import route_patch
from .scaffold import APIScaffold
from .schemas import EmptySchema, FileSchema, Schema
from .types import (
    ErrorCallbackType,
    HTTPAuthType,
    OpenAPISchemaType,
...
from .ui_templates import swagger_ui_oauth2_redirect_template, ui_templates
...
class APIFlask(APIScaffold, Flask):
    ...
    *Version changed: 0.9.0*
    ...
    security_schemes: t.Optional[t.Dict[str, t.Any]] = ConfigAttribute(
        "SECURITY_SCHEMES"
    )  # type: ignore
    ...
    def _get_spec(
        self, spec_format: t.Optional[str] = None, force_update: bool = False
    ) -> t.Union[dict, str]:
    ...
    def _collect_security_info(self) -> t.Tuple[t.List[str], t.List[HTTPAuthType]]:
        ...
        for blueprint_name, funcs in self.before_request_funcs.items():
            ...
            if (
                ...
                or not self.blueprints[blueprint_name].enable_openapi
            ):  # type: ignore
                continue
            for f in funcs:
        ...
        for rule in rules:
            ...
            for method in ["GET", "POST", "PUT", "PATCH", "DELETE"]:
                ...
                if view_func._spec.get("body"):
                    content_type = view_func._spec.get(
