...
def parse_args():
    ...
    parser.add_argument(
        "-m", "--reset", dest="reset", help="Reset everything.", nargs="?", const="True"
    )
        ...
        with open("domains.txt", "r") as domains:
            for line in domains:
                ...
                if line in domain_to_delete:
                    ...
                    print(
                        colored(
                            "\n[-] {} was successfully removed from the monitored list.".format(
                                line
                            ),
                            "green",
                        )
...
def adding_new_domain(q1):  # adds a new domain to the monitoring list
    ...
    if domain_to_monitor:
        ...
        with open(
            "domains.txt", "r+"
        ) as domains:  # checking domain name isn't already monitored
            for line in domains:
                if domain_to_monitor == line.replace("\n", ""):
                    print(
                        colored(
                            "[!] The domain name {} is already being monitored.".format(
                                domain_to_monitor
                            ),
                            "red",
                        )
            ...
            if response:
                with open(
                    "./output/" + domain_to_monitor.lower() + ".txt", "a"
                ) as subdomains:  # saving a copy of current subdomains
                    for subdomain in response:
                        subdomains.write(subdomain + "\n")
                with open(
                    "domains.txt", "a"
                ) as domains:  # fetching subdomains if not monitored
...
def dns_resolution(new_subdomains):  # Perform DNS resolution on retrieved subdomains
    ...
    for domain in subdomains_to_resolve:
        ...
        try:
            ...
            pass
    if dns_results:
        return posting_to_slack(
            None, True, dns_results
...
def posting_to_slack(
    result, dns_resolve, dns_output
    ...
    if dns_resolve:
        ...
        if dns_result:
            dns_result = {
                k: v for k, v in dns_result.items() if v
            ...
            unique_list = list(
                set(new_subdomains) & set(dns_result.keys())
...
def multithreading(threads):
    ...
    if not domain_to_monitor:
        num = sum(
            1 for line in open("domains.txt")
